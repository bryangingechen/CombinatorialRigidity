...
| `show X = Y from rfl` as a `rw` / `simp only` argument | `grep -nE "show .* from rfl" CombinatorialRigidity/*.lean` | Almost always either (a) a `let`-binding the rewrite chain should be reducing on its own — reorder so the binder lives at the elaboration boundary, or (b) a bundled-vs-unbundled morphism gap with a named `_apply` lemma upstream (`RingHom.mapMatrix_apply`, `LinearMap.coe_mk`, `DFunLike.coe_fn_eq`, …). Phase 8-cleanup B4 found one of each at a single Rank.lean site. |
