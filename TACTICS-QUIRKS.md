...
- `Type mismatch: Fin.snoc (fun b ↦ ?m.34) ?m.45 (index) has type ?m.20 (index) but is expected to have type α`, from a `Fin.snoc f x` (or the sibling `Fin.cons x₀ p`) applied to a further index inside `Matrix.of` (or any nested elaboration context — *including a plain `def` body*) even though `f`/`x` are both plainly non-dependent → § 96 (the dependent motive `α : Fin (n+1) → Sort*` doesn't get resolved to the constant type before the application is checked; ascribe the whole `Fin.snoc f x` / `Fin.cons x₀ p` term's type explicitly, `(… : Fin (n+1) → α) (index)`, before applying it)
...
**Sibling `Fin.cons`, and the plain-`def`-body trigger.** `Fin.cons (x₀ : α 0) (p : ∀ i, α i.succ)`
has the identical dependent motive and identical failure — and it is *not* limited to `Matrix.of`: a
plain `def` body that applies it to an index hits it too. Phase 39 `PencilSeed.toCoord`
(`Molecule/Pencil/Steer.lean`), `Fin.cons (seed.hubNormal …) (fun j => seed.fillHub … j …) p.2.1`
with return type `K`, failed with `Fin.cons ?m.17 (fun j ↦ ?m.28) p.2.1 has type ?m.7 p.2.1 but is
expected to have type K`; the same whole-term ascription `(Fin.cons … : Fin 4 → K) p.2.1` fixes it.
