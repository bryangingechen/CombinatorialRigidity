...
| 458 | route-A A4 — the (6.61) column-op bridge `Matrix.rank_ge_of_isUnit_mul_reindex_fromBlocks` (93770ac) | 2/2/1 | opus | normal | clean (scope-to-fit; crux deferred to A5) | ✓✓✓—✓✓ | 129k tok / 49 tools / 21.8 min | Scope-to-fit shrink to the BRIDGE half. Landed carrier-agnostic A4→A3 bridge `rank_ge_of_isUnit_mul_reindex_fromBlocks` (`Rank.lean`) + specialization `rigidityMatrix_mul_rank`: KT (6.61) = rank-invariant unit-det right-multiply, never span membership. DEFERRED-CRUX-AS-HYP: bridge takes `hblock:(M*U).reindex=fromBlocks A B 0 D` as a hyp → the genuinely-new explicit-`U` build + `hblock` discharge (§(4.21) 'WHOLE content') relocates to A5. Coordinator: sound reusable slice; the crux is HONESTLY re-flagged as A5's cost-outlier (NOT re-buried) → rate A5 by the `U`-build; warning-clean rebuild, axiom-clean. NEXT=A5. |
