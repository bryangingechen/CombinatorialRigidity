...
| 12 | G4c-ii CORRECTIVE (`ofNormals_relabel` + `rigidityRows_ofNormals_relabel` + existential corollary) | n/a | opus | corrective (coordinator-direct) | clean | ✓✓✓✓✓✓ | — | not a dispatch grade: the coordinator landed the §1.49(3) correction of #11 directly. Restated G4c-ii at the fixed-seed `ofNormals` level in the producer direction (v-split→a-split at seed `q₀∘ρ`), added the `(funLeft ρ).dualMap`-image rigidityRows correspondence G4d-ii consumes, and derived the existential as a producer-direction corollary (deleting the reversed one — no two parallel transports). Reused G4c-i `splitOff_isLink_relabel` (.mp + .mpr). The salvage #11 anticipated held: the conjunct-transport construction was reusable, only the statement shape + direction + the row-space deliverable changed. Build + lint + verify.sh + lint.sh green, axiom-clean |
