...
*Commit 3 — `Candidate.lean` dead cert forks + the two membership/acolumn leaves (CONTIGUOUS dead tail).* Block from the doc-comment of `hingeRow_mem_caseIIICandidate_rigidityRows_reproduced` (just after the LIVE `linearIndependent_mkQ_corner_of_gate` body, ~`:2280`) through EOF (`:2836`). Removes 8 decls:
