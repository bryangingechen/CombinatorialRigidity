...
### (4.47) A4 DE-RISK SPIKE — CORRECTS §(4.46): A3/A4/cert/arm are all LANDED, but the literal-`Matrix` route does NOT escape the bottom-deficiency wall. The project's `columnOp` gives 0 in the LOWER-left (not KT's upper-right); the operated `e_b` `ab`-fill strands in the discarded upper-right block, so the cert's bottom is the deficient pure-`Gv` `R(Gv)` (hD unsatisfiable = §(4.36)). KT's full-rank `R(Gab)` bottom is NOT a submatrix of `R(G,p)` (G lacks the `a—b` edge); the matrix-equality form is BLOCKED. VERDICT: the deficiency wall is intrinsic across ALL FOUR bottom representations; the genuinely-new core is an `R(Gab)`-reproduction bottom. STOP — strategic user decision (read-only compiler-checked spike, tree clean, session #36, row 506).

**Inventory (corrects §(4.46)'s "~9–14 new leaves" — A3/A4 are DONE).** Landed + axiom-clean: A3 `Matrix.rank_fromBlocks_zero₂₁_ge_of_linearIndependent_rows` (`Rank.lean:315`); A4 `Matrix.rank_ge_of_isUnit_mul_submatrix_fromBlocks` (`Rank.lean:418`) on mathlib's `Matrix.rank_mul_eq_left_of_isUnit_det` (note: `_left_`, NOT the `_right_` §(4.46) cited); A5a column-op-as-right-mult (`prodColumnOpEquiv` + unit-det); A5c composition cores (`finrank_span_rigidityRows_ge_of_edge_submatrix_fromBlocks`, `Concrete.lean:820`); A6 0-block kernel (`Concrete.lean:1303`); the operated-entry facts; the literal cert `case_III_rank_certification_matrix` (`Candidate.lean:2276`); the literal arm `case_III_arm_realization_matrix` (`ForkedArm.lean:130`, builds `U`/`en`/`hblock` + fires the cert + SHARED tail). The literal spine is ~80% built.

**The orientation finding (kernel-grounded; corrects §(4.46)).** §(4.46) claimed KT's column op makes the UPPER-right block 0 (block-LOWER-triangular `[Mᵢ 0; * R]`), dissolving the §(4.41) `e_b`-pin wall. The project's actual `columnOp hva` (`Basic.lean:1087`, `S ↦ update S v (S v + S a)`) produces the OPPOSITE: `fromBlocks A B 0 D` = 0 in the LOWER-left (`…_apply_pin_zero`). The operated `e_a` corner rows go pure-`v` (0 off-`v`), but the operated `e_b` corner row reads `ρ((S v + S a) − S b)`, **nonzero at the off-`v` `(a,·)`/`(b,·)` columns** (the `ab`-fill) — it strands in the upper-right `B`. A3 discards `B` (needs only the lower-left 0), so the bottom `D = toBlocks₂₂` is the **un-operated pure-`Gv` `R(Gv)`** submatrix (`…_apply_off_pin`). For the interior, `Gv = G − vᵢ` is deficient (`k' ∈ [1, D−2]`), so `rank R(Gv) = D(|Vᵥ|−1) − k' < target` ⟹ the arm's `hD` (literal `G`-edge bottom rows, `re ∘ Sum.inr`) is **UNSATISFIABLE** — exactly §(4.36). This reconciles with §(4.41)/(4.42) ("`B` nonzero only in the `e_b` row"), which §(4.46) glossed. KT's "0 upper-right" would need a DIFFERENT op / the `R(Gab)`-relabel, NOT the project's `columnOp`.

**Why no literal-`Matrix` re-orientation escapes.** KT's full-rank bottom is `R(Gab)` (`Gab = splitOff`, deficiency 0), reached via the (6.62) relabel row-correspondence. But `R(Gab)` has the fresh `e₀=(a,b)` edge that `G` lacks, so **`R(Gab)` is NOT a submatrix of `R(G,p)`** — the candidate's `toBlocks₂₂` is `R(Gv)` (no `e₀` row), and equating them is `submatrix_columnOp_toBlocks₂₂_eq_Gab`, recorded BLOCKED (§(4.40), "un-provable equal *chosen* basis vectors"). The 23d L-span/L-rank/L-hD "hD RANK route" worked at the rank level around that equality, but it lands a MIXED bottom whose `e₀`-rows face the same membership escape (§(4.44)). So **all four bottom mechanisms wall on the one obstruction**: `R(G,p)`'s genuine rows (or `caseIIICandidate F₀`'s span) do not contain a full-rank `v`-blind `D(|Vᵥ|−1)` bottom when `Gv` is deficient. The `e₀`-direction (KT's `ab`-fill) is only ever reproduced as the SINGLE redundancy `ρ`, never a full `k'`-block.

**Meta-lesson.** §(4.46) was a PROSE source recon — right about KT's math (the column-op/relabel/block-additivity argument is real), wrong about project-side feasibility, because it characterized the project's `columnOp` orientation from §(4.30) prose instead of compiler-checking the actual def. The A4 de-risk spike (compiler-checked, the user's chosen first step) caught it pre-build. Confirms the "prose mischaracterizes the types on a route-composition question; use a compiler-checked spike" principle (DESIGN.md *Compiler-checked spike, not prose recon*).

**VERDICT — strategic user decision owed (the bottom-deficiency wall is intrinsic; KT's faithful construction is genuinely-new).** KT certifies via a bottom block that IS `R(Gab)` (the full-rank split-off matrix WITH its `e₀` edge), reproduced from `R(G,pᵢ)` by the column op + (6.62) relabel — the candidate's fork `e_a`/`e_b` hinges are meant to ENCODE the `e₀=(a,b)` constraint so the operated matrix reproduces `R(Gab)`. The project never established that reproduction (the matrix-equality is BLOCKED; the rank-route lands a mixed bottom that escapes). The genuinely-new core is **the `R(Gab)`-reproduction bottom**: show the operated candidate matrix's bottom block has the rank of `R(Gab)` (full), via a rank-preserving construction that does NOT require `R(Gab)` to be a literal submatrix of `R(G,p)`. This is genuinely-new math-adjacent matrix engineering, high-risk, no landed precedent. Directions: **(a)** invest in the `R(Gab)`-reproduction bottom (the genuine open core); **(b)** reconsider the fallback (C) declined at 23d-open (route A's bottom is now confirmed genuinely-hard across all four representations); **(c)** re-scope. No honest commit estimate; the literal spine (A1–A6, cert, arm) is reusable for whichever bottom construction lands.

### (4.48) R(Gab)-REPRODUCTION FEASIBILITY SPIKE + USER DECISION + PLAN — VERDICT: kernel-grounded NO-GO on the literal-`Matrix` reproduction (sharpens §(4.47) to a DUAL-ORIENTATION single-row impossibility); the obstruction is a FORMALIZATION representation-mismatch, NOT open math (KT's proof is complete/correct). USER DECISION (2026-06-26, session #37): **pursue the genuinely-new certificate** — the goal is a complete formalization of KT, so fallback (C) / freeze-at-`d=3` are declined. 23e RE-SCOPED to the KT-faithful rank certificate. (Compiler-checked construct-or-concede spike, scratch reverted, tree clean.)

**The spike (kernel-grounded, scratch `Spike48.lean`, reverted; `#print axioms` = the 3 standard only).** Probed the §(4.47) genuinely-new core directly: can ANY unit-det column op land a full-rank `e₀`-carrying bottom? Three sorry-free obstruction lemmas: (L1) `obstruction_bottom_rank_eq_unoperated` — for any unit-det column op + any pin-`v`-avoiding bottom-row injection, the operated bottom `toBlocks₂₂.rank` EQUALS the un-operated `R(Gv)` row-subset rank (one `rw` of the landed `submatrix_columnOp_toBlocks₂₂_eq`, `Concrete.lean:1426`; realizes "column ops preserve the rank of any fixed row-subset"); (L2) `≤ rigidityMatrixEdge.rank` (`rank_submatrix_le`); (L3, capstone) `obstruction_hD_unsatisfiable_of_deficient` — given the deficient `R(Gv)` rank `< #m₂`, the cert's `hD : LinearIndependent D.row` is FALSE (`linearIndependent_rows_iff_rank_eq_card` + L1 + omega). The deficient-rank input rests on the LANDED, `U`-INDEPENDENT `splitOff_removeVertex_minimalKDof` (`Reduction.lean:1492`) + `removeVertex_deficiency_gt_of_noRigid` (`Reduction.lean:1687`) via `rank_add_deficiency_eq` — NOT the §(4.22) smuggled-crux trap (the crux "can any `U` escape?" is exactly what L1 answers: no).

**The sharper obstruction (DUAL-ORIENTATION single-row impossibility).** The operated `e_b` row — the ONLY row carrying KT's `ab`-fill — has BOTH a nonzero PIN entry (`rigidityMatrixEdge_mul_columnOp_apply_corner`, `Concrete.lean:1239`) AND nonzero off-`v` `ab`-fill (`…_apply_eB_off_pin`, `Concrete.lean:1395`), so it fits NEITHER block of ANY block-triangular `fromBlocks`: (i) **0 lower-left** (the only landed cert, `case_III_rank_certification_matrix`) forces bottom rows pin-`v`-free ⟹ `e_b ∉ m₂` ⟹ deficient pure-`Gv` bottom [L1–L3]; (ii) **0 upper-right** (KT's block-LOWER-triangular orientation, the spike's PRIMARY) forces corner rows pure-`v` ⟹ `e_b ∉ m₁` ⟹ corner is only `e_a`'s `D−1` rows, rank `D−1 < D`. KT (6.65–6.67) shares `e_b`'s single `(D−1)`-dim block between the corner `Mᵢ`'s `±r` row AND the `e₀`-fill via a **union-dimension argument** — a NON-block-triangular use the literal `fromBlocks` model cannot express (one matrix row → exactly one of `m₁`/`m₂`).

**NOT open math — formalization representation-mismatch.** KT 2011 is a complete, refereed proof (Conjecture 1.2 is a theorem); the classical LA (rank-additivity / union-dimension, Crapo–Whiteley) is settled. The wall is that the project formalized the cert as block-triangular `fromBlocks` over `G`'s own edges, while KT's argument is non-block-triangular (the `e_b` row feeds both blocks) and is block-triangular only AFTER the (6.62) relabel row-correspondence identifying the operated bottom with the full-rank base `R(G₁∖row)`. Faithfully realizing that correspondence at the rank level is the unbuilt piece — known-true, not in mathlib/the project, and the current cert/arm architecture is shaped wrong for it.

**The genuinely-new requirement (two equivalent views).** (a) a NON-block-triangular rank lower bound `rank (fromBlocks A B C D) ≥ #m₁ + #m₂` tolerating a nonzero lower-left `C` when the corner `A = Mᵢ` is invertible; equivalently (b) **the Schur-complement full-row-rank fact** `rank (D − C·A⁻¹·B) = D(|V|−2)` (with `A` invertible the identity `rank (fromBlocks A B C D) = #m₁ + rank(Schur)` is standard LA), where the Schur complement IS KT's row-op-(6.52) bottom `R(G₁∖row)`. This is §(4.42)'s **option 1**, DECLINED there (in favor of option 2, which then walled at `hbotmem`) — so it is UN-ATTEMPTED, not refuted.

**Efficiency lever — the stranded `mixedBottom` family (LIVE, sound, zero callers).** `Concrete.lean` already holds `submatrix_columnOp_toBlocks₂₂_eq_mixedBottom` (`:1460`), `rank_…_eq_finrank_span_mixedBottom` (`:1518`), `linearIndependent_toBlocks₂₂_row_mixedBottom_of_finrank_eq` (`:1610`): they DO place `e_b ∈ m₂` and reduce its row-LI to a `hrank` span-count DISCHARGEABLE from the def-0 IH (via the reproduction `hCeq` in `exists_candidate_row_eq612`, `Candidate.lean:736`). They cannot wire into the current cert (`e_b ∈ m₂` ⟹ `toBlocks₂₁ ≠ 0` ⟹ `hblock = fromBlocks A B 0 D` unsatisfiable) — i.e. they are most of the bottom-side work for the new cert, stranded behind the block-triangularity gate. The new cert's job is precisely to RELAX `toBlocks₂₁ = 0`. (Do NOT misread their existence as "the bottom is solved.")

**THE PLAN (23e re-scoped; codes-until-open for the rest).** Cert-before-dispatch, because the dispatch consumes the cert: **23e (re-scoped)** = the KT-faithful rank certificate (the non-block-triangular / Schur-complement rank lower bound + the full-rank `R(G₁∖row)` bottom, reshaping `case_III_rank_certification` / `_arm_realization`, `d=3` zero-regression); **23f** = the general-`k` dispatch + CHAIN-5 (the old 23e scope; the landed `chainData_arm_realization_sep` wrapper parks here); **23g** = ENTRY; **23h** = ASSEMBLY (general-`d` Thm 5.5 → Thm 5.6 → Conjecture 1.2). 23e step sequence: (1) cert-shape design recon — validate the Schur-complement-with-invertible-corner route vs KT (6.52)–(6.67) + the `mixedBottom` family + §(4.42); (2) construct-or-concede de-risk spike on the chosen rank lemma (the GATE — no reshape until green); (3) build the rank infra (the LA lemma, likely upstream-eligible under `Mathlib/LinearAlgebra/Matrix/Rank.lean` + wire `mixedBottom` + the Schur = `R(G₁∖row)` identity); (4) reshape cert + arm. The literal spine (A1–A6, cert, arm) + option-2 leaves are reusable parallel facts.

**Pointer note for this section.** §(4.47)/§(4.48) cite landed decls by short file name per the doc's convention; the `CaseIII/…` files are under `CombinatorialRigidity/Molecular/AlgebraicInduction/CaseIII/` (cert `Candidate.lean:2276`/`_sep :2355`; arm `ForkedArm.lean:130`/`_sep :234`). A3 = `rank_fromBlocks_zero₂₁_ge_of_linearIndependent_rows` (`Rank.lean:315`); A4 = `rank_ge_of_isUnit_mul_submatrix_fromBlocks` (`:418`) + `_reindex_` (`:376`), both on mathlib `rank_mul_eq_left_of_isUnit_det`.

### (4.49) CERT-SHAPE DESIGN RECON (23e item 1) — VERDICT: GO. A THIRD, un-examined cert shape `fromBlocks A 0 C D` (zero UPPER-right, A3-TRANSPOSED) dodges all four walls AND §(4.42)'s Schur concern: the bottom is the LANDED full-rank `mixedBottom` block (UNCHANGED), the row op zeros the corner's off-`v` `B` (NOT the bottom's `C`), and the only genuinely-new content localizes to the corner `Mᵢ`-invertibility (KT (6.66)/(6.67) union-dimension, on the GREEN Lemma 2.1 + the landed `d=3` discriminator). Recommend the step-2 construct-or-concede spike. (Read-only recon, session #37; `det_fromBlocks_zero₁₂` confirmed in mathlib `Determinant/Basic.lean:723`.)

**The decisive realization — the bottom is ALREADY solved.** The stranded `mixedBottom` family
(`Concrete.lean:1460/1518/1610`) proves sorry-free, axiom-clean, NO span membership / NO `ScrewSpace`
unfold: with `e_b ∈ m₂` the operated bottom block reproduces `R(Gab)`'s rows entrywise (L-eq) and has
**full rank `#m₂ = D(|V|−2)`** (L-rank/L-hD), conditional ONLY on the def-0 IH count `hrank`. The
membership wall (§(4.44)) never forms — this is a rank/coordinate computation, not a span membership. The
only blocker is that `e_b ∈ m₂` forces a nonzero LOWER-left, which the current cert's A3
(`fromBlocks A B 0 D`) forbids.

**The new shape = `fromBlocks A 0 C D` (zero UPPER-right), via A3-transposed.** `rank (fromBlocks A 0 C D)
≥ #m₁ + #m₂` given `hA`/`hD` is the exact TRANSPOSE of A3 — a trivial mirror swapping
`det_fromBlocks_zero₂₁ → det_fromBlocks_zero₁₂` (both mathlib). The zero upper-right is produced by a row
op zeroing `B` (the corner's off-`v` content) by subtracting the `e_b` bottom row (carrying the SAME
`ab`-fill) from the `±r` corner row. **Why this dodges §(4.42):** §(4.42) declined the row op because it
zeros `C` (lower-left) → mutates the BOTTOM into the Schur complement `D − C·A⁻¹·B` (full-rank-ness
genuinely-new). This op zeros `B` (upper-right) → mutates the CORNER (`A → A' = A − L₀C`), leaving the
bottom `[C D]` UNTOUCHED (stays the `mixedBottom` full-rank block). The project was anchored on the
`e_b ∈ m₁` placement and never examined the `e_b ∈ m₂` + zero-`B` orientation. The row op exists as a
unit-det left-multiply `L = [I −L₀; 0 I]` with `L₀ D = B` solvable (`B`'s only nonzero row = the `±r`
row's `ab`-fill = one bottom row's `D`-part, so `B ∈ rowspan D`).

**The genuinely-new content localizes to ONE piece — the corner `hA'`.** Leaf list (23e): (1)
`rank_fromBlocks_zero₁₂_ge_of_linearIndependent_rows` (A3-transposed, trivial mirror, `Rank.lean`); (2)
A4-transposed = column op (existing) + row op (new unit-det left-mult) → `fromBlocks A' 0 C D`; (3) **the
corner `hA' : LinearIndependent A'.row`** — the row-op'd `Mᵢ` (the `e_a` pure-`v` panel rows + the
adjusted `±r` row) stays invertible = KT (6.66) `±r` carry + (6.67) union-dimension, bottoming on the
GREEN Lemma 2.1 (`omitTwoExtensor_linearIndependent`) + the landed `d=3` discriminator
`exists_complementIso_ne_zero_of_homogeneousIncidence_gen` (`Claim612.lean:1462`), generalized to general
`d`; (4) reshape `case_III_rank_certification` to consume (1)–(3) + the `mixedBottom` `hD`. So NOT
"no-estimate open research" — trivial-mirror + mechanical + landed-bottom, with the one hard piece (3)
having a green foundation and a `d=3` precedent.

**`d=3` zero-regression: FORK the cert.** `d=3` keeps the current `_matrix`/M₃ path (green); only the
general-`d` arm routes through the new `_zero₁₂` cert. Clean separation, zero `d=3` risk.

**Step-2 spike make-or-break (construct-or-concede, NOT carry-as-hypothesis):** (i) the row op composes as
a unit-det left-multiply yielding `fromBlocks A' 0 C D` from the operated candidate matrix (`B` nonzero
only in the `±r` row via the landed operated-entry facts; `mixedBottom` for the bottom); (ii) the corner
...
### (4.53) STEP-3b MATRIX-ASSEMBLY SPIKE — VERDICT: WALL. The cert `case_III_rank_certification_zero₁₂` is NOT instantiable from the landed geometry: it demands an UPPER-right-zero `hblock`, but the only landed `U` (the column op) produces a LOWER-left-zero shape `fromBlocks A B 0 D` with the upper-right block `B` (the `±r`/`e_b` corner row's off-`v` `ab`-fill) GENERICALLY NONZERO (kernel-checked residual). The §(4.49)–(4.52) "row op zeros the upper-right `B`" plan needs a LEFT row-op `L` that (a) is NOT in tracked source (only the reverted `Spike49`) and (b) cannot be expressed inside the cert's `hblock : (M * U).submatrix re en = …` form (a RIGHT multiply only). Two genuinely-new leaves owed before the cert is consumable. The §(4.52) "remaining = ASSEMBLY, no new math" framing is REFUTED. STOP — adjudication owed (re-cert-shape vs. build the row-op A4 bridge). (Compiler-checked spike `Spike53.lean`, reverted, tree clean; session #38.)

**The kernel residual (sorry-free `rw` chain, one residual goal).** Probed the cert's `hblock` requirement directly. The cert's `hblock` is `(rigidityMatrixEdge ends hgp * U).submatrix re en = fromBlocks A 0 C D` — zero UPPER-right (`toBlocks₁₂ = 0`), `m₁`-corner pinned by `hm₁ : card m₁ = screwDim k`. The landed geometry under the column op `U = (toMatrix' (prodColumnOpEquiv (columnOp hva).symm))ᵀ` gives the OPPOSITE block: `rigidityMatrixEdge_mul_columnOp_submatrix_toBlocks₂₁_eq_zero` proves `toBlocks₂₁ = 0` (LOWER-left), i.e. the shape is `fromBlocks A B 0 D`. The upper-right block is NOT zero: for a corner row `p` with FIRST endpoint `v`, SECOND endpoint `≠ v` (the `±r`/`e_b` row), at any off-`v` column `(body, c)`, `rigidityMatrixEdge_mul_columnOp_apply_eB_off_pin` rewrites the entry to `hingeRow a (ends p).2 (blockBasisOn hgp p.2) (Pi.single body (finScrewBasis k c))` — the `ab`-fill, nonzero at `body = a` (`= ρ(s) ≠ 0`). The spike's `example` (corner-row upper-right-zero claim) reduces by ONE `rw` to exactly that residual `hingeRow a (ends p).2 (…) (Pi.single body …) = 0`, which is FALSE in general. So `toBlocks₁₂ ≠ 0`: the column op produces lower-left-zero, never the cert's upper-right-zero, with `m₁` = the `D`-row corner.

**Why no reindex `(re, en)` rescues it.** `submatrix re en` is a free row/column reordering, so one might hope to relabel `fromBlocks A B 0 D` into `fromBlocks A' 0 C' D'`. The only zero block in the column-op grid (rows: corner `m_c` / bottom `m_b`; cols: pin-`v` `n_p` / off-`v` `n_o`) is (bottom, pin) = `0`. Mapping it to the upper-right slot forces `m₁` = bottom (the `D·(|Vᵥ|−1)`-row block), `n₂` = pin cols — but the cert's `hm₁ : card m₁ = screwDim k` PINS `m₁` to the small `D`-row corner. With `m₁` = corner, the upper-right block (corner × n₂) is zero only if the corner vanishes on ALL `n₂` columns; but the corner is nonzero at pin-`v` cols (the `Mᵢ` block) AND at `a`/`b` cols (the `±r` `ab`-fill), so no `n₂` makes it zero. The lower-left-zero shape is structurally locked under the cardinality contract; the upper-right-zero genuinely needs to MUTATE the corner (a row op), not reindex.

**The §(4.52) `corner_hA'_of_gate` → cert-`hA` composition is ALSO refuted (the coordinator's SHAPE CONCERN, kernel-confirmed).** The ONLY landed corner→`A.row` bridge is `linearIndependent_toBlocks₁₁_row_of_corner_gate` (`Concrete.lean:1775`); its `hLI` is `LinearIndependent ℝ (fun i : m₁ ↦ blockBasisOn hgp (re (Sum.inl i)).1.2 (re (Sum.inl i)).2)` — a PLAIN `m₁`-indexed `blockBasisOn` family, every corner row a basis vector (because `rigidityMatrixEdge_mul_columnOp_apply_corner` reads EVERY corner pin entry as `blockBasisOn hgp p.2`, including the `±r` row). `corner_hA'_of_gate` (`Concrete.lean:620`) instead produces `LinearIndependent ℝ (Sum.elim (fun j : Fin (D−1) ↦ blockBasisOn(e_a) j) (fun _ : Unit ↦ ρ₀))` — a `(D−1) ⊕ 1` family whose `D`-th member is ρ₀ (the redundancy vector `= Σⱼ λⱼ rab j`, KT 6.66), NOT a `blockBasisOn`. So `corner_hA'_of_gate` does NOT feed the cert's `hA`: under the bare column op, the corner's `D`-th row is `blockBasisOn(e_b, j₀)` (a genuine `e_b`-block functional, shared with the bottom — exactly the §(4.50) collapse), so the REAL `hA` the bridge produces is LI of `[blockBasisOn(e_a) ⊕ blockBasisOn(e_b, j₀)]`, which is NOT the discriminator's `[blockBasisOn(e_a) ; ρ₀]`. The §(4.52) "the row op turns the `±r` row into ρ₀, dissolving the §(4.50) concede" claim RESTS on the same absent row op `L`: ρ₀ only appears as the corner row AFTER the left row op subtracts the `Gv`-pin-zero widening; without `L`, the corner row is `blockBasisOn(e_b, j₀)` and `corner_hA'_of_gate` is the wrong shape. (The pin-zero fact `rigidityMatrixEdge_mul_columnOp_apply_pin_zero` is real and landed, but it zeros the BOTTOM rows' pin entries — the lower-left — not the corner's off-`v` upper-right.)

**`hD` and the cardinalities DO compose (the good news, kept honest).** `hD` reads off `mixedBottom` exactly as claimed: `linearIndependent_toBlocks₂₂_row_mixedBottom_of_finrank_eq` (`Concrete.lean:1677`) proves LI of `toBlocks₂₂.row` for the SAME column-op `submatrix re (columnSplit v).symm`, conditional on the IH rank count `hrank` (the def-0 `R(Gab)` full-rank fact, dispatch-supplied) + `hbot2`/`hbot1` (bottom second endpoint `≠ v`; first `≠ v` or `= v` the `e_b` split row). Cardinalities ground out: corner block = `screwDim k` columns (`columnSplit_corner_card`, the pin-`v` block) = `card m₁` (`screwDim k`, `hm₁`) ⟹ square `D×D` corner ✓; `screwSpace_finrank : finrank ℝ (ScrewSpace k) = screwDim k` so the matrix column index `α × Fin (finrank …)` matches `Fin (screwDim k)` per body ✓; `re : m₁ ⊕ m₂ → ({e // …} × Fin (screwDim k − 1))` is an injection drawing `D−1` corner rows from `e_a` + 1 from `e_b` (the `±r`), dropping the `D−2` surplus `v`-rows (§(4.33)(3)) — count-consistent. But these compose only for the LOWER-left-zero `fromBlocks A B 0 D` shape (the cert `case_III_rank_certification_matrix`, `Candidate.lean:2276`), NOT the `_zero₁₂` upper-right shape, because the corner `hA` and the upper-right-zero `hblock` both require the absent row op.

**The two genuinely-new leaves owed (exact residual signatures).** The A3-transposed cert is consumable iff a LEFT row op `L = fromBlocks 1 (−L₀) 0 1` is introduced and threaded. Both leaves were sorry-free in the reverted `Spike49`/`Spike49c`; re-create them as tracked Lean:
- **LEAF-RowOp-1 (`rowOp_isUnit_det`, upstream-eligible, `Mathlib/LinearAlgebra/Matrix/Rank.lean`):** `theorem (L₀ : Matrix m₁ m₂ K) : IsUnit (Matrix.fromBlocks (1 : Matrix m₁ m₁ K) (−L₀) 0 (1 : Matrix m₂ m₂ K)).det` (via `det_fromBlocks_zero₂₁` + `det_one`). TRIVIAL.
- **LEAF-RowOp-2 (`rowOp_zeroes_upperRight`, upstream-eligible, same file):** `theorem (hB : B = L₀ * D) : (fromBlocks 1 (−L₀) 0 1) * (fromBlocks A B C D) = fromBlocks (A − L₀ * C) 0 C D`. Pure `Matrix.fromBlocks_multiply` + `hB`. TRIVIAL. (These were the §(4.50) sorry-free facts.)
- **LEAF-RowOp-3 (the genuinely-new bridge, `Mathlib/LinearAlgebra/Matrix/Rank.lean` or `Concrete.lean`):** the cert/A4 lemma `rank_ge_of_isUnit_mul_submatrix_fromBlocks_zero₁₂` (`Rank.lean:507`) and the cert `_zero₁₂` must be RESHAPED to consume a row op too — `hblock : (Lrow * (M * U)).submatrix re en = fromBlocks A 0 C D` with `Lrow` unit-det (so `rank (Lrow * M * U) = rank M` by BOTH `rank_mul_eq_left` and `rank_mul_eq_right_of_isUnit_det`). This is a SIGNATURE change to the cert (not a new fact), but the cert is axiom-clean LANDED, so it is a re-statement + a one-`rw` body change. **OR** the cleaner route: keep the cert as-is, and build the geometry lemma `rigidityMatrixEdge_mul_columnOp_rowOp_submatrix_toBlocks₁₂_eq_zero` proving the COMBINED (column op THEN row op) submatrix has `toBlocks₁₂ = 0` — but the row op must left-multiply `rigidityMatrixEdge` (it is NOT a `* U` right op), so the cert's `hblock` `(M * U)` form must absorb the `Lrow` either way. The genuinely-new content is the `L₀`-construction `L₀C = B` from LEAF-3's `cGv` widening (the §(4.52) dissolution) — `L₀` weights ONLY the `Gv` rows (`hingeRow a b ρ = Σⱼ cGv j • hingeRow (uvGv j) (vvGv j) (rvGv j)`, all endpoints `≠ v`), so `L₀C` mutates the corner's off-`v` block to `0` while the pin (`Gv`-rows pin-zero) is untouched — THIS is real and needs a tracked lemma, NOT covered by any landed fact.

**Reconciliation with §(4.50)–(4.52) (what was stale).** §(4.50) correctly identified the corner `hA'` as the relocated crux and that a row op is needed. §(4.51)/§(4.52) then declared the row op + `hA'` "all landed/sorry-free, remaining = ASSEMBLY" — but the row-op facts were in the REVERTED `Spike49`/`Spike49c`, NEVER landed as tracked Lean, and the only tracked corner-`hA` bridge (`linearIndependent_toBlocks₁₁_row_of_corner_gate`) does NOT take the ρ₀-augmented family. So §(4.52)'s `corner_hA'_of_gate` (which DID land, `Concrete.lean:620`) is a dual-space fact with NO consumer — it proves `[blockBasisOn(e_a); ρ₀]` LI, but nothing wires that to the cert's matrix `A.row` without the row op. This is the §(4.46) lesson firing a SECOND time: §(4.52)'s "wiring kernel-confirmed" spike (`Spike49c`) confirmed the dual-space `corner_hA'_of_gate` fact and the pin-zero fact in ISOLATION, but did not instantiate the cert's `hblock`/`hA` against them — the route-composition the §(4.46) principle warns to compiler-check end-to-end. The matrix-assembly step is where the gap was hiding.

**Re-pointed hand-off — the EXACT next buildable commit (a leaf with its signature).** The cert is NOT consumable as landed; the next commit is **LEAF-RowOp-1 + LEAF-RowOp-2** (the two trivial row-op LA facts, re-created from the reverted `Spike49`), landed together under `Mathlib/LinearAlgebra/Matrix/Rank.lean`:
```
...
**Route (A) cert-layer LANDED 2026-06-26 (axiom-clean):** the `Lrow`-reshape is done — all three decls
(`rank_ge_of_isUnit_mul_submatrix_fromBlocks_zero₁₂`, `finrank_span_rigidityRows_ge_of_edge_submatrix_fromBlocks_zero₁₂`,
`case_III_rank_certification_zero₁₂`) now consume `(Lrow, hLrow, …, hblock : (Lrow * M * U).submatrix re en = fromBlocks A 0 C D)`,
rank-invariant via the mathlib `rank_mul_eq_right_of_isUnit_det`. Two Lean-mechanics gotchas (→ TACTICS-QUIRKS § 69): an explicit
`[Fintype {e // e ∈ F.graph.edgeSet}]` binder, and `Lrow` typed at the *candidate-graph* edgeSet (`*`/`HMul` matches indices
syntactically, not up to `caseIIICandidate_graph = G` rfl) with `set F₀` dropped (it split the `Fintype` instance off `hLrow`).
The remaining of (3b″) is the geometry arm below.

**`B = L₀ * D` matrix-algebra half LANDED 2026-06-26 (axiom-clean):** `Matrix.of_eq_mul_of_row_comb` (`Rank.lean`, after
`rowOp_zeroes_upperRight`): `(∀ i j, B i j = ∑ i', w i i' * D i' j) → B = Matrix.of w * D`, one `ext` + `Matrix.mul_apply` + `rfl`.
This is the separable matrix-algebra step of the `L₀C = B` leaf — it converts the *per-row* `cGv` combination into the `B = L₀ * D`
form `rowOp_zeroes_upperRight` consumes (`L₀ := Matrix.of w`). The `L₀C = B` leaf thus splits into (a) THIS arm-independent
per-row→matrix-product conversion [DONE] + (b) the arm-coupled geometry half (exhibit the operated corner off-`v` reads AS a
`cGv`-weighted combination of the `mixedBottom` rows, needing the dispatch `re`/`m₂` split → lands with 23f). Only (b) + the
corner-`hA` wire remain of the geometry arm.

THEN the adjudication owed before the cert reshape: **(A)** reshape `case_III_rank_certification_zero₁₂` + `rank_ge_of_isUnit_mul_submatrix_fromBlocks_zero₁₂` to thread `Lrow` (a unit-det LEFT factor: `hblock : (Lrow * (M * U)).submatrix re en = fromBlocks A 0 C D`, `rank` invariance via `rank_mul_eq_right_of_isUnit_det`) [DONE, see above], then build the geometry leaf for `L₀C = B` from LEAF-3's `cGv` widening + the corner-`hA` leaf for `A − L₀C = [blockBasisOn(e_a); ρ₀]` (where `corner_hA'_of_gate` FINALLY plugs in); **OR (B)** re-examine whether a DIFFERENT cert shape avoids the left row op entirely (none found in §(4.49)–(4.53); the lower-left-zero `_matrix` cert needs the full-rank bottom in `m₂` which puts `e_b` off-pin nonzero, breaking ITS `toBlocks₂₁ = 0` — the §(4.41)/(4.44) wall that started this; the disjoint-block `_sep` needs `hbotmem` — the §(4.44) wall). Recommend (A): the row op is real KT (6.63) content, the two LA leaves are trivial, and `corner_hA'_of_gate`/`mixedBottom`/`cGv` are all landed — the gap is the `Lrow`-threading reshape + the one `L₀C = B` geometry leaf, NOT open math. Estimate: LEAF-RowOp-1/2 one commit; the cert/A4 reshape [DONE] + `L₀C=B` geometry leaf + corner-`hA` wire ≈ 3–4 leaves.
...
### (4.54) STEP-2c′ END-TO-END COMPOSITION SPIKE (the 3b‴ "arm-coupled, lands with 23f" deferral, tested) — VERDICT: GO on the cert, **but the deferral is sound AND under-specified.** The reshaped `case_III_rank_certification_zero₁₂` *is* invokable at the abstract 23e-framework level (kernel-confirmed: `refine … (m₁ := Fin (screwDim k)) (m₂ := …) (n₁ := …) (n₂ := …) ?…` elaborates, all block data left as goals — no shape/index/cardinality wall in the cert's *consume* contract). The geometry that discharges `hblock`/`hA` is **genuinely 23f arm-coupled** — not because of the `re`/`m₂` index (that is framework-determined, §below), but because the row-op factor `L₀` is the `cGv` widening, and `cGv` is a *conclusion of the IH-fed W6b producer* (`exists_candidateRow_bottomRows_of_rigidOn`, `Candidate.lean:401`, takes `hrig : IsInfinitesimallyRigidOn Gab` + `h622lb`), i.e. NOT derivable from `caseIIICandidate`'s abstract `G`/`ends`/`e_a`/`e_b`/`v` alone. So 23e's cert scope is COMPLETE (route (A) discharged at the cert layer); the geometry arm is 23f. **NO fourth wall** (the composition closes in principle), **but TWO leaves the §(4.53) plan elided are now visible** — both genuinely-new, both 23f-arm-coupled. (Read-only spike + `lean_goal`/`lean_diagnostic_messages` against the LANDED signatures; scratch `Spike54.lean` deleted, tree clean; session #39.)

**The cert consumes; no fourth wall in the contract (kernel-checked).** Instantiated `case_III_rank_certification_zero₁₂ G Gv ends hVone hVcard hgp hends (m₁ := Fin (screwDim k)) (m₂ := Fin (screwDim k * (V(Gv).ncard − 1))) (n₁ := …) (n₂ := …) ?hm₁ ?hm₂ ?Lrow ?hLrow ?U ?hU ?re ?en ?hblock ?hA ?hD` — elaborates cleanly (only `A`/`C`/`D` stay metavars, expected: they are determined *by* `hblock`). Cardinalities ground out as §(4.53) said. So the cert's *consume* side has no hidden obstruction; the whole question is the *produce* side (`hblock`/`hA`).

**The `re`/`m₂` split is FRAMEWORK-determined, not arm-determined (corrects the 3b‴ rationale).** §(4.53)/item-3b‴ said the geometry half "needs the dispatch's `re`/`m₂` split." Traced to ground: the corner/bottom classification is a predicate on the *full edge index* `p := {e // e ∈ E(G)} × Fin (screwDim k − 1)` — corner `m₁` = the `D−1` `e_a`-panel slots + the 1 `e_b`-`±r` slot (`hc1 : (ends (re (inl i)).1).1 = v`, `hc2 : .2 ≠ v`); bottom `m₂` = the `e_b` `ab`-fill + all `Gv` rows (`hbot2 : .2 ≠ v`, `hbot1 : .1 ≠ v ∨ .1 = v`, the mixed bottom of `linearIndependent_toBlocks₂₂_row_mixedBottom_of_finrank_eq`, `Concrete.lean:1685` — which DOES include the `.1 = v` `e_b` row). Both predicates reference only `ends`/`v`/`a`/`e_a`/`e_b` — the cert's own parameters — so the `re` *construction recipe* is framework-level (the arm supplies concrete `ends₁`/`e_a`/`e_b`, but no IH). The genuinely arm-coupled object is **only `L₀` (= the `cGv` weights)**.

**The single arm-coupling, traced to ground.** `hblock`'s upper-right-zero needs `B = L₀ · D` with `B i = hingeRow a b (blockBasisOn(e_b) …)` (the `±r` corner row's `ab`-fill, `rigidityMatrixEdge_mul_columnOp_apply_eB_off_pin`, re-kernel-confirmed this session) and `D` = the `mixedBottom` `Gv`-rows. The weights come from `cGv`: `hingeRow a b ρ = ∑ⱼ cGv j • hingeRow (uvGv j) (vvGv j) (rvGv j)` (W6b producer, `Realization.lean:825`–831), each summand a `Gv`-row. `of_eq_mul_of_row_comb` (LANDED, item 3b‴) turns this per-row form into `B = Matrix.of w · D` once `w` is built from `cGv` against the `m₂` row index. But `cGv` exists only via the IH (`hsplitGP : HasGenericFullRankRealization k n (G.splitOff v a b e₀)` → `exists_candidateRow_bottomRows_of_rigidOn`'s `hrig`/`h622lb`). **This is the irreducible 23f dependency.**

**TWO leaves the §(4.53) plan elided (genuinely-new, NOT covered by the landed facts).**
- **LEAF-RowOp-`Lrow`-on-`p` (a unit-det bridge `rowOp_isUnit_det` does NOT supply).** The cert's `Lrow : Matrix p p ℝ` is typed on the *full edge index* `p`, but the landed `rowOp_isUnit_det`/`rowOp_zeroes_upperRight` are stated for `fromBlocks 1 (−L₀) 0 1` on `m₁ ⊕ m₂`. `p ≠ m₁ ⊕ m₂` (the `re` injection DROPS the `D−2` surplus `v`-rows, §(4.33)(3)), so the row op must be carried to `p` by a reindex `e : p ≃ m₁' ⊕ m₂'` (with `m₁'` the FULL set of `v`-incident-`.1` rows incl. the surplus, `m₂'` the rest): `Lrow := (Matrix.reindex e e) (fromBlocks 1 (−L₀') 0 1)`, unit-det via `Matrix.det_reindex_self` + `rowOp_isUnit_det` (both LANDED — `det_reindex_self` confirmed in mathlib this session). This is one new leaf, NOT open math, but it is the `p`-level threading the §(4.53) "thread `Lrow`" framing left implicit. The `submatrix_mul` factorization caveat (`(Lrow*M*U).submatrix re en` does NOT split through the *injection* `re` — `Matrix.submatrix_mul` needs a *bijective* middle index) is why the row op lives on `p` and is selected by `re`, rather than acting on the post-`re` block.
- **LEAF-corner-`hA`-post-rowop (the bridge `corner_hA'_of_gate` needs but the LANDED `linearIndependent_toBlocks₁₁_row_of_corner_gate` is NOT).** After the row op, the cert's `A = A' = A − L₀C` (`rowOp_zeroes_upperRight`). `C` (bottom × pin block) is NOT zero: the `e_b` `ab`-fill bottom row has `.1 = v`, so its pin entry is `blockBasisOn(e_b)` (`rigidityMatrixEdge_mul_columnOp_apply_corner`, NOT pin-zero); the `Gv` bottom rows ARE pin-zero (`…_apply_pin_zero`). So `A' = A − L₀C` subtracts the `cGv`-weighted `e_b`-pin row from the corner's `±r` pin row — turning `blockBasisOn(e_b, j₀)` into `ρ₀ = ∑ λⱼ rab j` (KT 6.66's redundancy vector). THIS is where `corner_hA'_of_gate`'s `[blockBasisOn(e_a); ρ₀]` family is the right shape — vindicating §(4.52)'s claim, but ONLY after the row op mutates the pin block, exactly as §(4.53) warned. The LANDED `linearIndependent_toBlocks₁₁_row_of_corner_gate` (`Concrete.lean:1783`) reads the *un-row-op'd* corner via `…_apply_corner` (giving the `[blockBasisOn(e_a); blockBasisOn(e_b,j₀)]` family — which is what `exists_corner_blockBasisOn_linearIndependent`, `Concrete.lean:566`, handles via the escape argument). It does NOT see `A' = A − L₀C`. So the post-row-op corner-`hA` needs a NEW bridge reading `A'` (= the row-op'd `toBlocks₁₁`) as the `[blockBasisOn(e_a); ρ₀]` coordinate matrix, then `corner_hA'_of_gate` closes its LI. Genuinely-new, arm-coupled (the `ρ₀`/`cGv` enter).

**Reconciliation with the prior sections (what each got right/wrong).** §(4.51)/(4.52) "remaining = ASSEMBLY" — still wrong (these two leaves are new). §(4.53) route (A) — right, and the cert-layer reshape it landed is confirmed satisfiable here. §(4.53)'s "the gap is the `Lrow`-threading reshape [DONE] + the one `L₀C = B` geometry leaf" — UNDERCOUNTED: the `B = L₀ D` matrix-algebra half (3b‴) is one piece, but the *geometry* half splits into (i) the `cGv`→`w` instantiation (arm), (ii) the `Lrow`-on-`p` reindex bridge (new), and (iii) the post-row-op corner-`hA` bridge (new), beyond the corner-`hA` *gate* (`corner_hA'_of_gate`, landed). The item-3b‴ "exhibit the corner off-`v` `B` as a `cGv`-weighted combination" is (i); (ii)/(iii) were not separately tracked. NOTE the genuine BOTH-block coupling: zeroing `B` (off-`v`) via `L₀` SIMULTANEOUSLY mutates `A`-pin into `A'` — so the upper-right-zeroing and the corner-`hA` are ONE row op, not two independent leaves; the `hA` shape is *forced* by the same `L₀` that kills `B`. This is why `corner_hA'_of_gate`'s `ρ₀` is not a free choice — it is `A`-pin minus `L₀`·`C`-pin, computed from `cGv`.

**The index families / cardinalities to ground (the 23c `d = n` discipline).** `m₁` card `= screwDim k = D` (`hm₁`; `D−1` `e_a`-slots `+ 1` `e_b`-`±r`); `m₂` card `= D · (|V(Gv)| − 1)` (`hm₂`; the `mixedBottom` = `e_b`-fill `+ Gv`-rows, the full-rank `R(Gab)` block). `n₁` = pin-`v` cols card `D` (`columnSplit_corner_card`); `n₂` = off-`v` cols card `D·(|V(Gv)|)`-ish (the `(columnSplit v).symm ∘ Sum.inr` block). The `cGv` summands are indexed by `Fin nGv` (the `Gv`-links of the W6b producer), and map to the `m₂` bottom rows **by a stated contract fact, not `Fin`-arithmetic**: each `cGv` summand `hingeRow (uvGv j) (vvGv j) (rvGv j)` with `Gv.IsLink (evGv j) (uvGv j) (vvGv j)` is a `Gv`-row, hence a member of the `mixedBottom` `m₂` family — the `w : m₁ → m₂ → ℝ` weight is `cGv` re-keyed onto the `m₂` index of the matching bottom row (and `0` on the `e_b`-fill `m₂` row + the rows the corner doesn't use). The re-keying is the (i) leaf above; it needs the producer's `evGv`/`uvGv`/`vvGv` to land in the `re`-image of `m₂`, which is a `Gv.IsLink`→`re` membership fact (arm-supplied, like the §(4.53) `hbotmem` analogue but a RANK-route weight, not a span membership — so the §(4.44) `hbotmem` wall does NOT reform: `w` lands `L₀` for an algebraic `B = L₀ D`, never a span-`∈`).

**Re-pointed hand-off — 23e's cert scope is COMPLETE; the geometry arm is 23f.** No adjudication owed, no wall. The cert chain (`rank_ge_of_isUnit_mul_submatrix_fromBlocks_zero₁₂` / `finrank_span_rigidityRows_ge_of_edge_submatrix_fromBlocks_zero₁₂` / `case_III_rank_certification_zero₁₂`) consumes `(Lrow, hLrow, U, hU, re, en, hblock, hA, hD)` and is end-to-end SATISFIABLE for the real interior arm — confirmed by instantiation + the ground-traced producers. **The 23e close looks like:** flip the 23e ROADMAP row to ✓ with the scope clause "the A3-transposed cert + its LA scaffolding (cert chain + `rowOp_*`/`of_eq_mul_of_row_comb` + `mixedBottom`/`corner_hA'_of_gate` gates) all landed axiom-clean; the geometry arm that *constructs* the cert's block data from the IH-fed `cGv` widening is 23f"; the remaining `[ ]` items (3c gate bridge, 4 dispatch) move to **23f's** checklist. **23f's first buildable commit** = the geometry arm `hblock`/`hA` construction, in dependency order: (i) `cGv`→`w` re-key leaf (the `Gv.IsLink`→`m₂` membership + `of_eq_mul_of_row_comb`), (ii) the `Lrow`-on-`p` reindex unit-det bridge (`reindex e e (fromBlocks 1 (−L₀') 0 1)` + `det_reindex_self`), (iii) the post-row-op corner-`hA` bridge (read `A' = toBlocks₁₁` of `Lrow*M*U` as the `[blockBasisOn(e_a); ρ₀]` coordinate matrix, close via `corner_hA'_of_gate`), then assemble `hblock` from (i)+(ii)+`rowOp_zeroes_upperRight`-on-`p` + the `submatrix_columnOp_toBlocks₂₂_eq_mixedBottom` bottom + `…_submatrix_toBlocks₂₁` lower-left (the `_zero₁₂` shape's `C`/`D`). These are ≈ 3 new leaves + the assembly, all against the IH-supplied W6b outputs — NOT open math, but NOT "assembly, no new math" either (the (ii)/(iii) bridges are genuinely-new tracked content). The `cGv` widening, `mixedBottom`, `corner_hA'_of_gate`, `of_eq_mul_of_row_comb`, `rowOp_*` are all LANDED; the gap is the three bridges + the `p`-level assembly that wires them into the cert's `hblock`/`hA`.
