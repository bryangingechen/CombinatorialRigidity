...
## (4.105) Phase 23f remaining-work GAP-MAP + plan (the `chainData_dispatch` router) — fan-out architectural recon, 6 gaps, all adversarially CONFIRMED

**Provenance.** Fan-out workflow (2026-06-29): 6 read-only deep recons (one per gap) → 6 adversarial verifiers (all `verdict=confirmed`) → synthesis. The load-bearing claim — the router composes — was **compiler-verified twice**: by the `router` recon (lean_run_code, sorry-free against the real imported lemmas) and **re-confirmed by the coordinator on acceptance** (lean_run_code against current HEAD: `success:true`, zero errors, no `sorry`, only cosmetic style warnings). The architecture is SOUND — no hidden 23f blocker.

**State.** The three landed pieces compose into the missing router as PURE ROUTING. No `chainData_dispatch` decl exists (grep: only doc-comment forward refs). The firing producer returns an 8-binder + 14-conjunct existential bundle at the base split `(vtx 1, vtx 0, vtx 2) e₀`; the interior branch consumes the {hgp, hρ₀e₀, hw, hwmem', hedgeGv, hLI, hgate, hρ₀Gv, hrecGab}+hdef subset under `2 ≤ (i:ℕ)`, the floor branch the {hgp, hends', hρ₀ne, hρ₀e₀, hw, hwmem', hLI, hgate, hρ₀Gv}+hdef subset under `(i:ℕ) ≤ 1`. Both subsets are literally named conjuncts; the dichotomy is exhaustive; interior's `3 ≤ cd.d` derives from `i.isLt`+`hint` by omega.

**Gap-map (each: gap → verified verdict).**
- **`router`** (clean-build, confirmed): pure routing; the spike compiled the exact body sorry-free. Residual (cosmetic, resolved): drop the unused `[DecidableEq α]` (declare `[DecidableEq β] [Finite α] [Finite β]` only, matching the landed siblings); wrap the `obtain` line ≤100 chars; no `_`-prefix needed (no unusedVariables warning fired).
- **`thm55`** (route-composition, confirmed): two interfaces. (A) the inner router is 23f-buildable pure routing — RAN GREEN. (B) the outer ASSEMBLY is a REAL shape mismatch — the `hcand`/`hdispatch` field is the d=3 8-tuple, the router is `cd`-based; they do not compose without the C.0-trio reshape (23g/23h). No motive change crosses the interface.
- **`c3hih`** (design-decision, confirmed): the C.3 `hIH` add is a clean ADDITIVE bundle — the general (k':ℤ) IH is already in scope at the spine (`theorem_55_minimalKDof_k_all_k.hdispatch`, supplied at the live call) and at `case_III_realization_all_k`; the add is purely "stop down-restricting to k'=0" (drop the `fun G' … => hIH 0 G' …` adapter at Realization.lean:3367-3368). GAP-6 (k'≠0 at `removeVertex v`) FORCES the general form but it is already present — no IH-strength change. `interior_hsplitGP` (Realization.lean:814) is DEAD/orphaned (its only refs are its decl + a doc comment); §(4.88.4)'s D1 description is stale but the additive verdict stands.
- **`cdsupply`** (clean-build, confirmed): the 23f/23g boundary is already cleanly cut — the dispatch pieces consume `cd` purely via accessors; ENTRY's `exists_chain_data_of_noRigid` (Reduction.lean:383) still returns the d=3 4-tuple and is never asked to build a `ChainData` (zero `ChainData` value constructors anywhere). `2/3 ≤ cd.d` are below-contract (`d_eq_kAdd`) / derived in-branch from `i`.
- **`hyps`** (route-composition, confirmed): `hsplitGP`/`hdef_Gab` are router INPUTS threaded verbatim into `chainData_fire_discriminator`, NOT proved in the router. Their proof lives one layer up in the 23g assembly via `splitOff_isMinimalKDof` then `(hIH _ … ).1 hGvSimple` — exactly the d=3 `case_III_hsplit_producer_all_k` (Arms.lean:894-913), where `hdef_Gab = hGv.1`. The prompt's `interior_hsplitGP`+`splitOff_swap_ab` route would type-match at i=1 but is NOT the architecture's route (and is orphaned). 23g detail: align the `(v,a,b)` extraction order with the contract's `(vtx 1,vtx 0,vtx 2)` via `splitOff_swap_ab` (C.4).
- **`floor`** (clean-build, confirmed): the M₁/M₂ floor re-route (`chainData_dispatch_floor_of_discriminator`, Realization.lean:3126) is axiom-clean + complete (i=0→M₁, i=1→M₂; no §(4.102) hwmem trap — exact `rigidityRows_ofNormals_congr_ends`). The "DEAD `chainData_split_realization` floor" claim HOLDS (interior-only `0<i`, re-fires W6b with its own seed, universal `htrans` can't take the matched gate) — the prior a/b-swap floor plan is correctly retired.

**Plan (sequenced).** (1) [done — coordinator spike re-confirmed against HEAD] the router-composition spike. (2) BUILD `chainData_dispatch` after Realization.lean:3322 with the `[DecidableEq β] [Finite α] [Finite β]` signature + general (k':ℤ) hIH (the C.3 add, user-approved 2026-06-26) + separate `hdef`/`hdef_Gab`/`hsplitGP` inputs; body = `obtain` the 22-name bundle, `by_cases 2 ≤ (i:ℕ)`, feed the matching branch. Zero blast radius. (3) gates (build/lint/warning-clean/axiom-clean). (4) USER sequencing call: land-unused vs pull CHAIN-5+C.2 forward. (5, 23g) the C.0-trio reshape + ENTRY length-d `ChainData` extractor (KT Lemma 4.6/4.8) that gives the router a live consumer.

**The one open USER adjudication (does NOT block the router build).** The router lands with NO live consumer because the C.0-trio `hcand`/`hdispatch` field is still the d=3 8-tuple shape and no `ChainData` value constructor exists. (a) land the router unused this phase + defer CHAIN-5 (8-tuple→`cd` field reshape) + ENTRY C.2 (the new general-`d` `ChainData` extractor, KT Lemma 4.6/4.8) to 23g — the design-pinned default per §C.2/§C.3; OR (b) pull CHAIN-5 + C.2 forward into 23f so the router has a live consumer now. (b) is strictly more work gated on the un-built ENTRY extractor. No motive/IH-strength change either way.

**Verdict.** 23f's remaining build is ONE clean-build commit (the router), de-risked by the green spike; the only open adjudication is the wire-now-vs-defer sequencing.

## (4.106) The `_aug`/override/(D-substitution) DEAD-ISLAND closure + the deletion sequencing (the 23f-CLOSE recon, read-only on Lean) — VERDICT: **the island is FULLY CLOSED and deletable; the prompt's discard list mis-listed `caseIIICandidate` (it is LIVE — shared with the honest engine), and the ForkedArm geometry leaves the hand-off and §(4.98) name (`baseRedundancy_perp_interior_reproduced_panel`/`interior_hρe₀_of_*`/`reproduced_panel_eq_splice_panel`/`ofNormals_supportExtensor_eq_panel_of_ends`) are LIVE. The deletable closure is 24 CaseIII decls across `Candidate.lean`/`Realization.lean`/`Relabel/ForkedArm.lean` (every call site of every member is itself a member — verified by `lean_references` per decl); the matrix backbone `rigidityMatrixEdgeAug` + its ~40-lemma `Concrete.lean` sub-API + `finrank_span_rigidityRows_ge_of_aug_submatrix_fromBlocks_zero₁₂` is consumed ONLY by the dead cert forks but is RETAINED until the CaseIII island goes (deleted in a follow-up Concrete commit, NOT this pass — its exact intra-`Concrete` closure needs a per-decl trace and over-deletion would break the live edge-path). Three coherent gating-green deletion commits.**

**Method (computed a reference graph; did not trust prose).** Seeded the candidate-dead set from the hand-off step-7 + LANDED-INVENTORY "DISCARDED" bullet + §(4.105)(i). For EVERY candidate, ran `lean_references` (lean-lsp, authoritative) and cross-checked with `grep` for code call-sites (`refine`/`exact`/`apply`/`:=`). A decl is DELETABLE iff every caller is itself in the dead set; if ANY caller is on a live path it is RETAINED. The honest engine `case_III_rank_certification` (`Candidate.lean:1662`) + `case_III_realization_of_rank` (`Arms.lean:63`) + `case_III_arm_realization{,_M2}` (`Arms.lean:310/374`) region was grep-confirmed to reference NO aug/`_matrix`/`_sep`/`_chain`/`zero₁₂`/`EdgeAug` machinery — the live path is genuinely disjoint from the fork.

**THE `ForkedArm.lean:499` CRUX (the "still-called" warning) — RESOLVED: the caller is DEAD.** `case_III_rank_certification_aug` (`Candidate.lean:2694`) is called ONLY at `ForkedArm.lean:499`, which is inside `case_III_arm_realization_aug` (`ForkedArm.lean:426`); that arm is called ONLY at `Realization.lean:2025`, inside `chainData_arm_realization_aug_zero₁₂` (`Realization.lean:1915`), which has NO callers (`lean_references` = self only). The chain `_rank_certification_aug ← _arm_realization_aug ← chainData_arm_realization_aug_zero₁₂ ← ∅` is a closed dead branch. The island is NOT blocked.

**(a) THE DELETABLE SET — 24 CaseIII decls, grouped into 3 gating-green commits.** Every member's full call-site set is intra-island (verified per decl). Line numbers at current HEAD; delete each decl WITH its preceding `/-- … -/` doc block.

*Commit 1 — `Realization.lean` dead interior-arm wrappers + corner-gate leaves (the island's top; no in-file caller depends on them).* CONTIGUOUS block `Realization.lean:1611–2338` (doc-comment of `chainData_arm_realization_sep` at 1611 through the end of `chainData_arm_corner_blockBasis_linearIndependent_of_triLI`'s body at 2338, immediately before the LIVE `exists_chainData_discriminator_pick` doc at 2340). Removes 7 decls:
- `chainData_arm_realization_sep` (`:1634`) — call sites: NONE (the now-superseded interior wrapper; calls dead `case_III_arm_realization_matrix_sep`).
- `chainData_arm_realization_zero₁₂` (`:1771`) — NONE (D-CAN-3b; calls dead `case_III_arm_realization_rowOp`).
- `chainData_arm_realization_aug_zero₁₂` (`:1915`) — NONE (the SOLE caller of `case_III_arm_realization_aug`, `:2025`).
- `chainData_arm_realization_ofNormals` (`:2059`) — NONE (calls dead `case_III_arm_realization_aug_ofNormals`).
- `chainData_arm_corner_hA_of_discriminator_gate` (`:2141`) — NONE.
- `chainData_arm_corner_hA_ofNormals_of_gate` (`:2220`) — NONE.
- `chainData_arm_corner_blockBasis_linearIndependent_of_triLI` (`:2292`) — NONE.

*Commit 2 — `Relabel/ForkedArm.lean` dead arms + corner-assembly + (D-subst) tail + chain-edge/bottom-relabel leaves (SELECTIVE — the widening geometry leaves are interleaved and LIVE; delete by name, keep the live ones).* Removes 11 decls; KEEP `reproduced_panel_eq_splice_panel` (`:547`), `ofNormals_supportExtensor_eq_panel_of_ends` (`:596`, shared), `baseRedundancy_perp_interior_reproduced_panel` (`:669`), `interior_hρe₀_of_splice_perp` (`:741`), `interior_hρe₀_of_widening` (`:779`), `interior_hρe₀_of_baseWidening` (`:826`):
- `case_III_arm_realization_chain` (`:59`) — sole call site `ForkedArm.lean:1144`, inside dead `case_III_arm_corner_assembly`.
- `case_III_arm_realization_matrix` (`:130`) — NONE.
- `case_III_arm_realization_matrix_sep` (`:234`) — sole call site `Realization.lean:1735` (dead `chainData_arm_realization_sep`, deleted in commit 1).
- `case_III_arm_realization_rowOp` (`:315`) — sole call site `Realization.lean:1876` (dead `chainData_arm_realization_zero₁₂`, commit 1).
- `case_III_arm_realization_aug` (`:426`) — sole call site `Realization.lean:2025` (dead `chainData_arm_realization_aug_zero₁₂`, commit 1).
- `hingeRow_mem_ofNormals_rigidityRows_chainEdge` (`:621`) — NONE (S1 leaf; the colliding-membership `hr`-filler).
- `bottomRelabel_image_mem_span_caseIIICandidate` (`:915`) — sole call site `ForkedArm.lean:1026`, inside dead `bottomRelabel_rigidityRows_mem_span_caseIIICandidate`.
- `bottomRelabel_rigidityRows_mem_span_caseIIICandidate` (`:998`) — NONE.
- `case_III_arm_corner_assembly` (`:1064`) — sole call site `ForkedArm.lean:1217`, inside dead `_via_leafB2`.
- `case_III_arm_corner_assembly_via_leafB2` (`:1173`) — NONE.
- `case_III_realization_of_rank_ofNormals` (`:1251`) — sole call site `ForkedArm.lean:1395`, inside dead `case_III_arm_realization_aug_ofNormals`.
- `case_III_arm_realization_aug_ofNormals` (`:1322`) — sole call site `Realization.lean:2116` (dead `chainData_arm_realization_ofNormals`, commit 1).

  (That is 12 decls — `case_III_arm_realization_aug_ofNormals` counted; the kept LIVE leaves are 547/596/669/741/779/826. The `/-! ## The (D-substitution) realization tail` section header at `:1221` is left no-op or dropped if it becomes empty after the two D-subst tail decls go.)

*Commit 3 — `Candidate.lean` dead cert forks + the two membership/acolumn leaves (CONTIGUOUS dead tail).* **✓ LANDED (session #52, 2026-06-29).** Deleted `:2255`–EOF (the `/-! ### The ±r corner row` section header at `:2255` through the last dead body at `:2834`; file 2836→2255 lines, 581 deleted), keeping the `end CombinatorialRigidity.Molecular` closer. Self-verified caller-less first (grep `*.lean`: 5 decls ZERO external refs; `_chain`/`_matrix`/`_zero₁₂` had only doc-comment prose in `Basic`/`Arms`/`Concrete`/`Relabel.Chain`, no code call-site). Full build (2830 jobs) + lint green + warning-clean, `d=3` untouched. Block from the doc-comment of `hingeRow_mem_caseIIICandidate_rigidityRows_reproduced` (just after the LIVE `linearIndependent_mkQ_corner_of_gate` body, at `:2255` the section header / `:2286` the decl) through EOF. Removes 8 decls:
- `hingeRow_mem_caseIIICandidate_rigidityRows_reproduced` (`:2286`) — call sites `ForkedArm.lean:962` (dead `bottomRelabel_image_mem_span_caseIIICandidate`) + `:1128` (dead `case_III_arm_corner_assembly`).
- `reproducedSlot_pmR_acolumn_eq` (`:2314`) — sole call site `ForkedArm.lean:1137` (dead `case_III_arm_corner_assembly`).
- `case_III_rank_certification_chain` (`:2350`) — sole call site `ForkedArm.lean:95` (dead `case_III_arm_realization_chain`).
- `case_III_rank_certification_matrix` (`:2429`) — sole call site `ForkedArm.lean:200` (dead `case_III_arm_realization_matrix`).
- `case_III_rank_certification_matrix_sep` (`:2508`) — sole call site `ForkedArm.lean:279` (dead `case_III_arm_realization_matrix_sep`).
- `case_III_rank_certification_zero₁₂` (`:2599`) — sole call site `ForkedArm.lean:383` (dead `case_III_arm_realization_rowOp`).
- `case_III_rank_certification_aug` (`:2694`) — sole call site `ForkedArm.lean:499` (dead `case_III_arm_realization_aug`).
- `case_III_rank_certification_aug_ofNormals` (`:2783`) — sole call site `ForkedArm.lean:1381` (dead `case_III_arm_realization_aug_ofNormals`).

**(b) THE RETAINED SET — `_aug`-adjacent decls that MUST stay, with the live caller that forces it.**
- `caseIIICandidate` (`Candidate.lean:940`, the candidate-framework `def`) + its ENTIRE API (`caseIIICandidate_graph`/`_supportExtensor_{candidate,reproduced,of_ne,reproduced_eq_ofNormals,…}`/`_hsupp_of_rowClassifier`/`_panelRow_eq_add_smul`/`_exists_good_shear`/`_supportExtensor_eq_ofNormals_of_ends_eq`/`hingeRow_mem_caseIIICandidate_rigidityRows_of_ofNormals_link`, `Candidate.lean:940–1297`) — **the prompt's discard list mis-lists this as "the `caseIIICandidate` override device".** It is LIVE: the honest arm `case_III_realization_of_rank` (`Arms.lean:63`, fed `caseIIICandidate`-shaped `hrank` at `:80/:92/:151`) is called by the LIVE `case_III_arm_realization` (`Arms.lean:310`, at `:352`), and the honest engine `case_III_rank_certification` (`Arms.lean:350`) consumes a `caseIIICandidate`-rank bound. Deleting `caseIIICandidate` would break the live `d=3` `k=2`-spine engine.
- `ofNormals_supportExtensor_eq_panel_of_ends` (`ForkedArm.lean:596`) — LIVE-SHARED: used by the dead `hingeRow_mem_ofNormals_rigidityRows_chainEdge` (`:630`) AND by the LIVE `baseRedundancy_perp_interior_reproduced_panel` (`:717/:722`). Keep.
- The widening chain `reproduced_panel_eq_splice_panel`/`baseRedundancy_perp_interior_reproduced_panel`/`interior_hρe₀_of_splice_perp`/`interior_hρe₀_of_widening`/`interior_hρe₀_of_baseWidening` (`ForkedArm.lean:547/669/741/779/826`) — LIVE: `interior_hρe₀_of_baseWidening` is consumed at `Realization.lean:2725` inside the LIVE router branch `chainData_dispatch_interior` (`:2653`); the chain feeds the `hρe₀` slot of the LIVE interior arm `chainData_interior_realization_hρGv` (`Realization.lean:1420`, called at `:2864`). The hand-off explicitly (correctly) flags `baseRedundancy_perp_interior_reproduced_panel` LIVE — the whole chain rides with it.
- The `Concrete.lean` matrix backbone `rigidityMatrixEdgeAug` (`:1045`) + `finrank_span_rigidityRows_ge_of_aug_submatrix_fromBlocks_zero₁₂` (`:1258`) + the `*aug*`/`*Aug*` lemmas + the A5d corner-index split (`finScrewDimSplitCorner`/`cornerRowInjectionAug`/`reAug`). **✓ DELETED (4th commit, session 2026-06-29).** After commits 1–3 these became fully unreferenced (external refs only from the now-deleted dead cert forks). §(4.106)(b) warned this is NOT a contiguous delete (the aug lemmas interleave with the LIVE non-aug `rigidityMatrixEdge_mul_columnOp_*` edge-path helpers), so the 4th commit ran a **per-decl closure trace**: computed every in-file occurrence of each `aug` name, verified every CODE reference (doc-comment prose ignored) sits inside another `aug` decl ⇒ deleting the aug cluster (19 decls) strands no kept decl; KEPT all non-aug helpers. `finScrewDimSplitCorner` (non-aug-named) rode along — sole callers were the deleted aug arms. 917 lines, file 3798→2881; full build (2830 jobs) + lint green + warning-clean, `d=3` untouched. Plus a stale-`rigidityMatrixEdgeAug` doc-pointer reword in `CaseIII/Realization.lean`.

**(c) Build-coherence per commit.** Commit 1 (Realization wrappers): removes 7 top-of-island decls with no callers anywhere; the live `chainData_split_realization`/`chainData_interior_realization_hρGv`/`exists_chainData_discriminator_pick`/`chainData_dispatch*` all compile (none reference the removed wrappers). Commit 2 (ForkedArm arms/leaves): each removed arm's only caller was a Realization wrapper deleted in commit 1 (or another ForkedArm member deleted in the same commit); the kept LIVE widening leaves + `ofNormals_supportExtensor_eq_panel_of_ends` still compile (their callers are the LIVE router/interior-arm). Commit 3 (Candidate certs): each removed cert's only caller was a ForkedArm arm deleted in commit 2; the live honest engine `case_III_rank_certification` (`:1662`) + `caseIIICandidate` API (all < `:2286`) are untouched. After all three, `Concrete.lean`'s aug-block is dead-but-present (compiles, unreferenced) until the deferred commit 4. `d=3` stays green throughout via the untouched `k=2`-spine honest engine. ORDER (top-down: leaf-most callers first) is forced — commit 1 must precede 2 (2's arms are called by 1's wrappers), 2 must precede 3 (3's certs are called by 2's arms); a single-commit whole-island delete is also coherent (no external dep) but the 3-way split keeps each diff reviewable.

**(d) §(4.105)/step-7 reconciliation (hand-off discards: confirmed deletable vs reclassified).**
- CONFIRMED DELETABLE (matches step-7): `case_III_rank_certification_aug{,_ofNormals}`/`_matrix{,_sep}`/`_zero₁₂`/`_chain` (6 certs); `case_III_arm_realization_aug_ofNormals` + `hingeRow_mem_ofNormals_rigidityRows_chainEdge` (S1 leaf); `chainData_arm_realization_sep` + the `_zero₁₂` D-CAN-3b wrapper (`chainData_arm_realization_zero₁₂`); plus the step-7-implied arms (`_chain`/`_matrix`/`_matrix_sep`/`_rowOp`/`_aug`), the override spine/corner (`chainData_arm_realization_{aug_zero₁₂,ofNormals}` + the three `chainData_arm_corner_*`), the corner assemblies (`case_III_arm_corner_assembly{,_via_leafB2}`), the bottom-relabel leaves, `reproducedSlot_pmR_acolumn_eq`, `hingeRow_mem_caseIIICandidate_rigidityRows_reproduced`, `case_III_realization_of_rank_ofNormals`.
- RECLASSIFIED LIVE (step-7 OVER-LISTED): **`caseIIICandidate` + its API** (step-7 "the `caseIIICandidate` override device" — WRONG, it is the shared candidate framework the honest engine uses). The "(D-subst) `_ofNormals` siblings" of step-7 resolve to exactly `case_III_rank_certification_aug_ofNormals` + `case_III_arm_realization_aug_ofNormals` + `case_III_realization_of_rank_ofNormals` (all dead, in commits 2/3) — there is no further live-`_ofNormals` sibling to delete; the `caseIIICandidate_*_eq_ofNormals` API lemmas are RETAINED (caseIIICandidate API).
- §(4.105) `c3hih` flagged `interior_hsplitGP` (`Realization.lean:814`) DEAD/orphaned — `lean_references` CONFIRMS (self only). It is NOT `_aug`-fork (it is the C.3-route orphan), so OUT of the prompt's named scope; can ride in commit 1 (same file) or be left to 23g. Two further OUT-OF-SCOPE d=3-era orphans noted, not folded in: `case_III_realization_of_line` (`Arms.lean:568`, self-only) — leave to a later sweep.

(opus, 2026-06-29 session #51 — read-only recon; `lean_references` per decl + grep cross-check; no Lean touched. Docs-only.)
