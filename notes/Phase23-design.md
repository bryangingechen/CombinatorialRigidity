...
- The `Concrete.lean` matrix backbone `rigidityMatrixEdgeAug` (`:1045`) + `finrank_span_rigidityRows_ge_of_aug_submatrix_fromBlocks_zero₁₂` (`:1258`) + the ~40 `rigidityMatrixEdge{,Aug}_mul_columnOp_*`/`submatrix_columnOp_toBlocks*_aug*`/`corner_hA_aug_zero₁₂_of_gate`/`*_Gab_aug*`/`exists_aug_bottom_blockData_of_Gab` lemmas (`:1045–3741`) — consumed ONLY by the dead CaseIII cert forks (external refs at `Candidate.lean:2724/2748/2808/2827` + `Realization.lean:1972/2090` + `ForkedArm.lean:468/1350`, all dead members) and by each other within `Concrete.lean`. After commits 1–3 they become fully unreferenced. **RETAINED for THIS pass** (NOT in the three commits): a clean `Concrete.lean` deletion needs a per-decl intra-file closure trace to separate the aug-only lemmas from the *edge*-path (non-aug) helpers in the same region (e.g. `rigidityMatrixEdge_mul_columnOp_*` without the `Aug`), or over-deletion breaks the live edge-rank API. Defer to a fourth `Concrete.lean` deletion commit (post-island), gated green — out of the prompt's named CaseIII scope but the natural follow-up.
