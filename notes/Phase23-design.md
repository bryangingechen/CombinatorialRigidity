...
### (4.73.4) BUILD ORDER. (1) ✓ DONE — placement-reconciliation (§(4.73.2)): NO seam; the `hA` leaf `chainData_arm_corner_hA_of_discriminator_gate` is LANDED (modulo `hAeq`); (2) the `hAeq` leaf — the operated-corner identity (KT 6.66) feeding the LANDED `chainData_arm_corner_hA_of_discriminator_gate`; (3) `hB`/`L₀` leaf — `corner_row_eq_cGv_comb_of_baseWidening` (the base-widening transport) feeding `matrix_eq_mul_of_dual_row_comb`; (4) `re`/`hre` builder (`Sum.elim cornerRe reInr` + injectivity, bookkeeping); (5) the dispatch shell (`Fin cd.d` router: base/`d=3` → `chainData_split_realization`, interior `2 ≤ i` → the spine) + CHAIN-5 + the C.3 `hIH` add.
