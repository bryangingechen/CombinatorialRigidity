...
### (4.73.2) THE LOAD-BEARING SEAM — placement reconciliation (the next dispatch, a micro-spike).
The spine (`Realization.lean:1481`) states ALL its gates against a **direct** `q` (`hLn`/`hgab` use `q (cd.vtx i.succ, j)`; `hgp`/`re`/`hM'eq` use `caseIIICandidate G ends q … (fun j => q (cd.vtx i.succ, j)) n' … 0`, no relabel). The corner `hA` enters `hρe₀` via `corner_hA_zero₁₂_of_gate`, but the only perp-gate producer `interior_hρe₀_of_baseWidening` (`ForkedArm.lean:785`) concludes against `panelSupportExtensor (fun j => q (cd.shiftPerm i.castSucc (cd.vtx i.succ), j)) …` — i.e. `q ∘ shiftPerm`. §(4.72.2)'s placement-consistency claim covered the `_sep` (span-membership) arm, NOT this `_zero₁₂` corner `hρe₀` route, so it does NOT discharge this seam. **The dispatch must set `q := (base seed) ∘ shiftPerm` so the spine's direct-`q` corner gate and the relabel-`q` redundancy coincide — OR the spine's `q`/gate hypotheses are at the wrong placement.** This is below the C.0–C.6 contract (a `q`-choice in the arm, NOT a motive change) and so is the coordinator's to settle by a kernel-checked micro-spike before building `hA`; the design prose §(4.72.2) asserted `q := Q.normal` WITHOUT the `shiftPerm` factor the redundancy leaf needs. The d=3 dispatch (`:302`) sidesteps it (its W6b/discriminator/realization all sit at the same base split); the general-`d` interior arm at a matched candidate `i ≠ base` does not.
...
### (4.73.4) BUILD ORDER (post-placement-spike). (1) placement-reconciliation micro-spike (§(4.73.2)); (2) `hA` leaf — the operated-corner `hAeq` (KT 6.66) feeding `corner_hA_zero₁₂_of_gate`; (3) `hB`/`L₀` leaf — `corner_row_eq_cGv_comb_of_baseWidening` (the base-widening transport) feeding `matrix_eq_mul_of_dual_row_comb`; (4) `re`/`hre` builder (`Sum.elim cornerRe reInr` + injectivity, bookkeeping); (5) the dispatch shell (`Fin cd.d` router: base/`d=3` → `chainData_split_realization`, interior `2 ≤ i` → the spine) + CHAIN-5 + the C.3 `hIH` add.
