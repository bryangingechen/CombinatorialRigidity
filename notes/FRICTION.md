...
### [idiom] `unusedDecidableInType` can false-positive on a `[DecidableEq β]` that's genuinely load-bearing, with no `classical` in sight — verify by deleting it, don't trust the warning
- **Where it bit:** Phase 39 (PENCIL) W3-L6a (`Molecular/Induction/Contraction.lean`, `Graph.rigidContract_deficiency_eq`).
- **Friction:** the theorem's only uses of `[DecidableEq β]` are as an *instance-implicit argument threaded to callees* (`contract_matroidMG_deficiency_eq`, `matroidMG_rigidContract_eq_contract`, `Graph.rank_add_deficiency_eq`, each of which pins its own `[DecidableEq β]`) — never named directly in the tactic script. `lake build` reports the hypothesis as unused (exit 0, warning only). Trusting the warning and dropping it (as the CLAUDE.md fix-precedence order says to for a *genuinely* unused instance — exactly what worked for the sibling W3-L2a commit the same session) breaks the build outright: `failed to synthesize instance of type class DecidableEq β` at every one of those call sites. This is a **different root cause** from the already-documented W3-L1/W3-L2 `classical`-shadowing false positive (`notes/Phase39.md`) — no `classical` tactic appears anywhere in this proof — so "the same warning has a documented cause" doesn't transfer; the linter's usage-detection apparently just doesn't count an instance used only as a threaded callee argument.
- **Fix:** empirically check before deleting — touch the file, drop the hypothesis, rebuild; a hard failure (not just the warning going away) proves it's load-bearing. Restore it and suppress with `set_option linter.unusedDecidableInType false in` (before the docstring, per TACTICS-QUIRKS §51) plus a one-line comment recording the empirical check, per CLAUDE.md's suppression discipline.
- **Alternative fix (preferred when the callees don't need a *specific* instance):** if the `[DecidableEq β]` is threaded only to callees that would accept *any* `DecidableEq β` (matroid/deficiency lemmas whose statements are instance-irrelevant up to `Subsingleton`), add `classical` at the top of the proof and **drop the binder** — the threaded-callee flavor becomes the classical-shadowing flavor and the deletion that broke the build now succeeds warning-clean. This is strictly cleaner than a `set_option` suppression (no linter opt-out). Used in W5-L6a-safe-exists (`exists_adjacent_degree_two_pair_of_{edgeBound,noRigid_of_deficiency_pos}`, `ReducibleVertex.lean`): both generalize a lemma off `IsMinimalKDof` — whose *type* carried `matroidMG` (hence `DecidableEq β`), so dropping it stranded the instance as unused-in-type — and both close cleanly with `classical`, no binder.
- **Status:** idiom (recurs whenever this linter's report contradicts what a quick deletion-rebuild shows; the `classical`-conversion fix applies when generalizing off a hypothesis that carried the typeclass in its type).
...
### [idiom] `Submodule.eq_of_le_of_finrank_eq (hle : S₁ ≤ S₂) (h : finrank S₁ = finrank S₂) : S₁ = S₂` — the `≤` direction must match which side is `S₁`, easy to get backwards when the goal reads "known span = derived perp"
- **Where it bit:** Phase 39 (PENCIL) W5-L4 (`Molecular/Molecule/Pencil.lean`, `exists_smul_cross₃_eq_of_linearIndependent`), proving a `1`-dimensional common perp equals `Submodule.span K {q}` from `q ∈ perp` (giving `span {q} ≤ perp`, the *only* containment directly available) plus equal `finrank`.
- **Friction:** stated the goal as `perp = Submodule.span K {q}` (the "natural" reading order, perp-first) and passed the available containment `span {q} ≤ perp` as `hle` — type mismatch (`Submodule.eq_of_le_of_finrank_eq` unifies `S₁ := perp`, `S₂ := span {q}` from the goal's shape, so it wants `perp ≤ span {q}`, the direction *without* a direct proof). The already-landed sibling `range_cross₃L_eq_perp` (W5-L1) has the same lemma call but happened to state its goal `range = perp` with `range ≤ perp` as the directly-available containment, so the pattern looked identical at a glance while the roles were swapped.
- **Fix:** state the equality with the *provable* containment's LHS first (`Submodule.span K {q} = perp`), then `.symm`/rewrite with `←` at use sites if the other orientation reads better downstream. General: before calling `Submodule.eq_of_le_of_finrank_eq`, name which submodule the *available* `≤` proof points *into*, and put that submodule first in the goal — don't default to whichever variable name looks "more like the answer".
- **Status:** resolved in-proof (usage note).

### [idiom] `omega` failed to combine a directly-in-context `Set.ncard` fact with a syntactically-identical goal term — another instance of the omega-atom family (TACTICS-QUIRKS §58/§63/§98)
- **Where it bit:** Phase 39 (PENCIL) W5-L4 continued (`Molecular/Molecule/Pencil.lean`, `ncard_closedNbhd_le_three_of_not_pencilHub`), the final step combining `Set.ncard_insert_le v (N(G, v)) : (insert v (N(G,v))).ncard ≤ (N(G,v)).ncard + 1` with `hNcard : (N(G, v)).ncard ≤ 2` to conclude `≤ 3`.
- **Friction:** `lean_goal` at the failing `omega` call confirmed `hNcard : N(G, v).ncard ≤ 2` sits in context and the reduced goal is literally `N(G, v).ncard + 1 ≤ 2 + 1` — a one-line consequence of `hNcard` alone — yet `omega` reported a counterexample allowing the LHS atom to be `≥ 3`, i.e. it did not use `hNcard` at all. No `Set`/`Fintype`/`Decidable` instance mismatch was visible in the pretty-printed goal.
- **Fix:** replace the `omega` step with the direct term `Nat.add_le_add_right hNcard 1` (term-mode, sidestepping omega's atomization entirely) — this closed immediately. General: when `omega` inexplicably ignores a hypothesis whose printed type matches the goal's atom, don't debug the mismatch — substitute a direct `Nat.add_le_add_*`/`le_trans`-style term proof for that one step first; it's usually faster than diagnosing which internal representation diverged.
- **Status:** resolved in-proof (usage note; adds a `Set.ncard`-flavored data point to the existing omega-atom family, not yet worth its own TACTICS-QUIRKS §).
- **Recurred:** Phase 39 W5-L6b-ii (`Molecule/Pencil/Witness.lean`, `exists_coord_linearIndepOn_pencilChartPoint_perBody`), the non-hub case-split `(N(G, v)).ncard = 0 ∨ = 1 ∨ = 2` from `hNcard : (N(G, v)).ncard ≤ 2` (`hNcard` extracted via `Set.encard_le_coe_iff_finite_ncard_le` off the `eDegree` chain). Same signature: `omega`'s counterexample allowed the `ncard` atom `≥ 3`, ignoring `hNcard` — because the `encard`-chain `N(G, v)` is a distinct-but-defeq elaboration from the freshly-written goal `N(G, v)`. Worked around by *not* re-deriving the equality with `omega`: split with `Nat.lt_or_ge (N(G, v)).ncard 1`/`… 2` (fresh `<`-hyps about the *goal's* term, which `omega` does read) and close the last leg with `le_antisymm hNcard h'` (term-mode, defeq-bridged), no `omega` on `hNcard`.
...
### [idiom] `by decide` on a `∀…∃…`-quantified statement over `Fin 4` blows the elaborator's whnf heartbeat budget — and the half-elaborated decl poisons *downstream* proofs in the file with opaque tactic timeouts
- **Where it bit:** Phase 39 (PENCIL) W5-L5 L5-cut-v-b (`Molecular/Molecule/Pencil/Witness.lean`), the padding-slot pick facts (`∃ x : Fin 4` avoiding up to three given values, stated with a `∀ a b c : Fin 4` prefix for `decide`).
- **Friction:** `lake build` failed with *"(deterministic) timeout at `whnf`"* pointing at the declaration, plus `simp`/tactic heartbeat timeouts in *later* proofs of the same file (initially misread as § 99 context-weight problems in those proofs). The quantifier prefix chains `Fintype.decidableForall/ExistsFintype` instances the elaborator cannot whnf within budget, even though the underlying case count is tiny.
- **Fix:** ordinary binders + a `Finset.card` counting argument (`by_contra`/`push Not`, `univ ⊆ {a, b, c}`, `card_insert_le`, `omega`), with the two larger pick facts derived by iterating the base pick instead of re-proving. `fin_cases`-to-literals-then-`decide` also works.
- **Status:** resolved in-proof. **Lifted to:** TACTICS-QUIRKS § 101.
...
### [idiom] A fused `rw` lemma whose target endpoints `(x, y)` are *implicit* collapses only concrete-endpoint sites — a self-referential `rfl`/`Prod.mk.eta` `hf` breaks HO unification for the function-valued implicit
- **Where it bit:** Phase 38 (FACTOR) T1a, collapsing the ~16 `ofNormals` support-extensor hand-chains in `CaseII.case_II_realization_all_k` (+ `case_II_placement_eq612`) to the hoisted `PanelHingeFramework.ofNormals_supportExtensor_eq_panel_of_ends _ f hf` (`hf : ends f = (x, y)`, `{x y}` implicit).
- **Friction:** the 14 sites with a *concrete* endpoint record (`hf : ends e = (v, a)` etc.) collapse cleanly — the literal pair pins `x`, `y`, and the graph/seed unify against the goal. The **4 general-edge sites** read the extensor at `(Q.ends e).1 / .2` with no concrete pair; passing `rfl`, `(Prod.mk.eta).symm`, or even `(x := (ends e).1) (y := (ends e).2) … rfl` all leave the `rw` a no-op with error `Did not find … (ofNormals ?m (Prod.mk ?m) ?m)…` — the higher-order unification for the **implicit function-valued `endsσρ`** mis-solves to `Prod.mk ?x` (a partially-applied constructor) instead of `ends`, so the rewrite pattern never matches. Fully pinning every implicit (`(endsσρ := …) (Gv := …) (qρ := …)`) would fire but is longer than the 5-lemma hand-chain it replaces.
...
- **Proposed fix:** reuse `euclidean_inner_eq_dotProduct` + the `ofLp`/`toLp` boundary lemmas in W7 / Phase 26 whenever the bar-joint `RigidityMap` meets the screw-velocity algebra.
- **Status:** idiom.

### [idiom] A `simp only` through `WithLp.linearEquiv …|>.symm.toLinearMap` leaves a `(addEquiv …).invFun` residual — close with a trailing `rfl` (it is `toLp` only by defeq)
- **Where it bit:** Phase 25 W4 `Molecule/Dictionary.lean` — `molecularVel_apply` (`molecularVel c S v = toLp 2 (screwVel (S v) (ofLp (c v)))`).
- **Friction:** `LinearMap.pi_apply`/`comp_apply`/`proj_apply` reduce the pi-of-comp cleanly, but the outer `(WithLp.linearEquiv 2 ℝ (Fin 3 → ℝ)).symm.toLinearMap z` does not `simp`-rewrite to `toLp 2 z` — the simp-normal form of `.symm` is `(WithLp.addEquiv …).invFun`, which is `toLp` only definitionally. `WithLp.linearEquiv_symm_apply` / `coe_symm_linearEquiv` produce the `invFun` form, not `toLp`. Fix: drop those from the `simp only` and end the proof with `rfl` (the residual `(addEquiv …).invFun z = toLp 2 z` is `rfl`).
- **Status:** idiom.

### [idiom] Computing a lifted alternating-`k`-form's value on a `ScrewSpace.mk (extensor v)` — bridge `equivExteriorPower` to `ιMulti`, then `alternatingMapLinearEquiv_apply_ιMulti`
- **Where it bit:** Phase 25 W1 `ScrewVelocity.lean` — `screwOmega_lineExtensor`, `screwTau_lineExtensor`, `screw{Omega,Tau}_stdBiv` (evaluating the graded Plücker coordinate maps, defined as `alternatingMapLinearEquiv omegaForm ∘ₗ equivExteriorPower 2`, on a concrete support extensor).
- **Friction:** the coordinate map is `(alternatingMapLinearEquiv f) ∘ₗ (equivExteriorPower 2).toLinearMap`; to fire `exteriorPower.alternatingMapLinearEquiv_apply_ιMulti` (which reads `alternatingMapLinearEquiv f (ιMulti ℝ n a) = f a`) the argument must first be turned from `ScrewSpace.mk (extensor v) …` into `ιMulti ℝ n v`. The bridge `equivExteriorPower_mk_extensor` proves exactly `equivExteriorPower n (mk (extensor v) …) = ιMulti ℝ n v` by `Subtype.ext; simp only [ScrewSpace.mk, ScrewSpace_def]; rfl` (both sides are `ExteriorAlgebra.ιMulti ℝ n v` under the coe). Then `f a` is the raw alternating-form formula, reachable by `change`/`rfl` because the `AlternatingMap` coe reduces definitionally.
- **Proposed fix:** reuse the two-step route (`equivExteriorPower_mk_extensor` then `alternatingMapLinearEquiv_apply_ιMulti`) whenever W4/W7 need the velocity field / dictionary value on a molecular support extensor `mk (affineSubspaceExtensor …)`.
- **Status:** idiom.

### [idiom] `map_smul` over-fires on a bilinear `crossProduct` reach-in — `crossProduct (c • u) x` becomes `(c • crossProduct u) x`, needing `LinearMap.smul_apply` to re-associate
- **Where it bit:** Phase 25 W1 `ScrewVelocity.lean` — `screwVel_smul` (`vel_{c•S} x = c • vel_S x`).
- **Friction:** `crossProduct : (Fin 3 → ℝ) →ₗ[ℝ] (Fin 3 → ℝ) →ₗ[ℝ] Fin 3 → ℝ` is *linear in its first argument as an outer map*, so a `map_smul`/`simp` reach-in rewrites `crossProduct (c • u)` to `c • crossProduct u` (a smul of a `LinearMap`), leaving `(c • crossProduct u) x` — not the expected `c • (crossProduct u x)`. `rw [crossProduct_smul_left]` (my `(c•u) ⨯₃ v = c • (u ⨯₃ v)` helper, itself `map_smul` + `LinearMap.smul_apply`) then can't find its LHS. Fix: add `LinearMap.smul_apply` to the `simp only` set so the `(c • f) x = c • f x` re-association fires; `simp only [screwVel_apply, map_smul, smul_add, LinearMap.smul_apply]` closes it.
- **Status:** idiom.

### [idiom] Viewing an ∃-bound framework `Q0` as `ofNormals G Q0.ends (fun p => Q0.normal p.1 p.2)` — align the graph field with `rw [← hQ0g]` first, then close by structure/`fun`-eta `rfl` (`rw`'s auto-`rfl` won't fire)
- **Where it bit:** Phase 25 W6 `Molecule/Theorem56.lean` — `hself` in `exists_rankHypothesis_isGeneralPosition4_of_two_le` (the "view the genuine producer's output at its own normals" step the hand-off anticipated as a tiny `ofNormals_self` bridge). Needed to transfer `Q0`'s `RankHypothesis`/`hC`/rank to the `ofNormals` framework the genericity device (`exists_rankPolynomial_of_le_finrank_linking`) varies over.
- **Friction:** `ofNormals G Q0.ends (fun p => Q0.normal p.1 p.2) = Q0` is *not* `rfl` when `Q0` is an opaque `obtain`-bound fvar (its `hQ0g : Q0.graph = G` was `rfl` inside the producer but is an abstract hypothesis after unpacking, so `Q0.graph` does not reduce to `G`). `rw [← hQ0g]` rewrites the goal's `G` to `Q0.graph`, leaving `ofNormals Q0.graph Q0.ends (fun p => Q0.normal p.1 p.2) = Q0` — now defeq by structure-eta + `fun`-eta + beta (`(fun p => Q0.normal p.1 p.2) (a,i) ≡ Q0.normal a i`), but `rw`'s post-rewrite `rfl` does *not* discharge it (its transparency misses the eta); a standalone `rfl` tactic does.
- **Resolution:** `by rw [← hQ0g]; rfl`. Reusable for the W7 transport and any "read a framework as `ofNormals` at its own normals". The mirror facts (`(ofNormals … ).toBodyHinge` = `Q0.toBodyHinge`, rows/supportExtensor/motions agree) then come by `congrArg`/`rw [hself]`.
- **Status:** idiom.

### [idiom] `BodyHingeFramework` has no `@[ext]` — prove framework equality with `congrArg (BodyHingeFramework.mk (k := …) G)` on the `supportExtensor` equality, letting structure-eta absorb the (defeq) `graph` field
- **Where it bit:** Phase 25 W7 `Molecule/Duality.lean` — `molecularOfCentres_mapExtensor_screwComplementIso`, the dual-correspondence hub `(molecularOfCentres G ends c).mapExtensor screwComplementIso = (ofNormals G ends (homogenized centres)).toBodyHinge`. Both sides are `BodyHingeFramework 2 V β` on graph `G` with edgewise-equal supporting extensors.
- **Friction:** `BodyHingeFramework` (`RigidityMatrix/Basic.lean`) carries no `@[ext]`, so the auto-`ext` name `BodyHingeFramework.ext` does **not** exist (Lean 4 core does not synthesize `.ext`; only the `@[ext]`/mathlib `ext` attribute does) — `refine BodyHingeFramework.ext …` fails with *"Unknown constant"*. Both frameworks reduce to `mk G supportExtensor` with the *same* `graph` field only up to defeq (`(mapExtensor F Λ).graph ≡ F.graph ≡ G`, `(P.toBodyHinge).graph ≡ P.graph ≡ G`), not syntactically.
- **Resolution:** prove the field equality `hsupp : LHS.supportExtensor = RHS.supportExtensor` (`funext e; rw […]`), then `exact congrArg (BodyHingeFramework.mk (k := 2) G) hsupp`. `congrArg (mk G) hsupp : mk G LHS.supp = mk G RHS.supp`, which the `exact` checks against `LHS = RHS` up to structure-eta (`X ≡ mk X.graph X.supp`) plus `G ≡ X.graph` defeq — so the `graph` field never needs an explicit equality proof. Pin `k` (`(k := 2)`) since `mk G` alone can't infer it. Reusable for any equality of two frameworks that share a (defeq) graph and differ only in the supporting extensors.
- **Status:** idiom.

### [process] A design doc's pinned lemma "home" can be unbuildable across a `module`/non-`module` boundary — the spike that validated it likely didn't exercise the real import graph
- **Where it bit:** `Graph.bodyBarDim_eq_screwDim_sub_one` (Phase 23-cleanup E2, the consumer-surface reshape) — `notes/FreshEdgeSupply-design.md` §E.3 pinned its home as `Molecular/RigidityMatrix/Basic.lean` (the `screwDim` numeric-arithmetic kit, topically the right home), and the standalone proof was compile-spiked there successfully.
- **Friction:** `RigidityMatrix/Basic.lean` is a `module` file, and `Graph.bodyBarDim` lives in `BodyBar/Framework.lean`, a non-`module` file (it transitively depends on the non-`module` `Matroid.Graph.Matrix`). Per `LEAN-OPS.md` *Module-system conversion*, a `module` file can only `import`/`public import` other `module` files — adding `public import CombinatorialRigidity.BodyBar.Framework` failed with *"cannot import non-`module` … from `module`"*. The design doc's spike (evidently run as an ad hoc snippet, not as an in-tree file build) never exercised this import-graph constraint, so the pinned home looked validated but wasn't.
- **Resolution:** moved the lemma to `PanelLayer.lean` (non-`module`, already importing `RigidityMatrix.Basic` and transitively `BodyBar.Framework`), right next to its already-landed sibling `Graph.eq_add_one_of_bodyBarDim_eq_screwDim` — same name/signature/proof, different file. General rule: before trusting a design doc's pinned file location for a new lemma that crosses a definition boundary, check whether the target file is a `module` file and, if so, whether every definition the new lemma needs is reachable through the `module`-only import graph — a green isolated `lean_run_code`/scratch compile does not exercise this.
- **Status:** resolved.
...
### [idiom] A carried-hypothesis field / `∃`-bundle that indexes `cd.vtx ⟨2,_⟩` needs `2 ≤ cd.d` *in scope for the `by omega`* — use an explicit `hd2` binder (Pi) or nest the exists (`∃ (cd) (hd2 : 2 ≤ cd.d), …`), NOT a flat `∃ cd, 2 ≤ cd.d ∧ …`; and the OfNat literal `cd.vtx 1` is NOT defeq to `⟨1,_⟩`
- **Where it bit:** the CHAIN-5 C.3 reshape (Phase 23g) — the reshaped `hcand`/`hextract` field types on `case_III_hsplit_producer_all_k` (`CaseIII/Arms.lean`) and the `d = 3` discharge `chainData_extract_d3` (`ForestSurgery/Reduction.lean`). Both index the `v₁`-split `splitOff (cd.vtx ⟨1,_⟩) (cd.vtx ⟨0,_⟩) (cd.vtx ⟨2,_⟩) cd.e₀`.
...
### [idiom] `span_singleton_eq_span_singleton.mpr ⟨-1, _⟩`'s residual `(-1 : ℝˣ) • x = y` does NOT respond to `neg_one_smul` — unfold `Units.smul_def` first
...
- **Friction:** `Submodule.span_singleton_eq_span_singleton {x y} : (R ∙ x) = (R ∙ y) ↔ ∃ z : Rˣ, z • x = y` leaves `(-1 : ℝˣ) • x = y` — a `Units`-action smul, not the scalar `(-1 : ℝ) • x`. So `neg_one_smul` / `simp [neg_one_smul]` does NOT fire (no `(-1 : R) • _` pattern; the `simp` args report as unused).
- **Resolution:** `rw [Units.smul_def, Units.val_neg, Units.val_one, neg_one_smul, …]` — `Units.smul_def` turns `(z : Rˣ) • x` into `(↑z : R) • x`, after which `Units.val_neg`/`Units.val_one` expose `(-1 : R)` and `neg_one_smul` applies. A one-off.
...
### [idiom] A producer demanding a *specific* `ends`-orientation (`ends e = (x, y)`) whose conclusion is orientation-INVARIANT should take the recording DISJUNCTION (`(x,y) ∨ (y,x)`) instead — an IH selector only records each link up to a free swap
- **Where it bit:** the §(4.96) `hends_i` residual of the `chainData_dispatch` interior branch (Phase 23f, §(4.98)): `baseRedundancy_perp_interior_reproduced_panel` (`CaseIII/Relabel/ForkedArm.lean`) demanded `hends_i : ends (cd.edge i) = (vtx i.succ, vtx i.castSucc)` (a *specific* `Prod`-orientation of the matched chain edge), but the dispatch's base selector `ends₀ = Q.ends` is the IH realization's selector, which records each `removeVertex`-link only up to a free disjunction (`hends'`/`Q.ends e ∈ {(u,w),(w,u)}`). The specific orientation is *unprovable* from the discriminator.
- **Friction:** the producer's conclusion `ρ₀ (panelSupportExtensor (q (vtx i+1)) (q (vtx i))) = 0` is *orientation-invariant* — it factors through `ofNormals_supportExtensor_eq_panel_of_ends`, and `panelSupportExtensor` is antisymmetric in its two normals, so the swapped recording only flips a sign. The lemma was over-specified.
