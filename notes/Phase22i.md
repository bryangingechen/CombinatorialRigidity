...
**L1a and L1b are complete.** Next: any of L1c/L1d/L1f/L1g (all independent; see Hand-off).
...
**L0 fully complete. L1a and L1b complete.** Next independent L1 slices
(pick any one): **L1c** (the V3/V4/G0 in-place all-`k` restates in
`ReducibleVertex.lean` gaining `TwoEdgeConnected` hypothesis),
**L1d** (KT 3.6 part 1 in `Deficiency.lean`), **L1f** (KT 4.5(ii) in
`ReducibleVertex.lean`), or **L1g** (reverse acyclicity bricks in
`ForestSurgery.lean`) — all independent per §1.58(i). Signatures pinned
in §1.58. **Smallest next forward commit: L1c or L1d (either; both
