...
- **L6a build (2026-06-13, sonnet):** `PanelHingeFramework.case_II_placement_eq612_kdof` (CaseI.lean). Rigid
  body verbatim with two swaps: (i) `_of_rigidOn_linking` → `_le_finrank` with rank input `hNrank`; (ii) count
  `N + (screwDim k − 1) ≤ Nat.card s` (vs `D(|V(G)|−1)−1 ≤ Nat.card s`). Transport: `hZeq`
