...
**L0e M3-first complete.** M3 (`HasGenericFullRankRealization`) gains `n` parameter + rank
conjunct (ℤ-cast row-span equality at deficiency `n`); all GP producer seams converted
(W2 + `hdef` cast in root producers `hasGenericFullRankRealization_of_splice_ofNormals`/
`hasGenericFullRankRealization_of_splice_set_ofNormals`; B1.mpr extraction at every extract
site: ~16 functions across CaseI.lean + rigidContract_rigidity_transport); old forgetful
`hasFullRankRealization_of_generic` deleted (no live consumers); `hasPanelRealization_of_generic`
(M4) drops `hdef` (rank is direct transfer from restated M3); `def:rank-hypothesis` re-prosed
for rank-form substrate + B1 bridge. Build green (2764 jobs, zero warns); `lake lint` clean;
`blueprint/verify.sh` + `lint.sh` pass; structural-edit grep gate run.
Pair unswapped: `theorem_55_generic`/`theorem_55_d3` second slot still `HasFullRankRealization k G`.

**Next concrete step: the L0e pair-swap sub-commit** — swap `theorem_55_generic`'s
conditioned pair second slot from `HasFullRankRealization k G` to `HasPanelRealization k n G`
(plumbing-only body change); re-type `theorem_55_d3`'s three bare carries
(`hbase`/`hsplit`/`hcontract`) to `HasPanelRealization 2 n`-shaped slots; update
`thm:theorem-55-d3-instance` prose to reflect re-typed carries. Structural-edit grep gate
applies. After this, L0e is fully complete.
...
- [ ] **L0** — motives M1–M5 + bridges B1/B2 + the blueprint def-node flips; signatures
  ...
  - [ ] **L0e** (pair-swap) — swap `theorem_55_generic`/`theorem_55_d3` second slot to
...
**Smallest next forward commit: the L0e pair-swap** — swap `theorem_55_generic`'s second
slot from `HasFullRankRealization k G` to `HasPanelRealization k n G` (plumbing-only body;
`minimal_kdof_reduction_full` projects `.2` on the IH); re-type `theorem_55_d3`'s three bare
carries (`hbase`/`hsplit`/`hcontract`) to `HasPanelRealization 2 n`-shaped slots (hypotheses,
no proofs to fix); update `thm:theorem-55-d3-instance` prose re the re-typed carry family.
Grep gate: `grep blueprint/src/ HasPanelRealization` (new in signatures). After this L0 is
complete and L1 combinatorial bricks open per §1.57(h).
At phase close:
