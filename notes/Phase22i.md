...
**Next concrete step: L1 combinatorial bricks** — the `|V| ≤ 2` trichotomy; the cut-edge
decomposition (KT 3.6 `k = k₁ + k₂ + 1` + 3.3 sides; the `TwoEdgeConnected` predicate, V2);
`rigidContract_isMinimalKDof` all-`k` (KT 3.5, V4); `exists_degree_eq_two` 2EC-hypothesis
restate (KT 4.6, V3); KT 4.8 (`G_v^{ab}` minimal `(k−1)`-dof at `k > 0`). Opens per §1.57(h).
