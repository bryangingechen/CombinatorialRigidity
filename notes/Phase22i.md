...
**L0a complete.** `ExtensorInPanel` landed in `RigidityMatrix.lean` (graph-free geometry
section, after the incidence suite) and three new lemmas landed in `PanelLayer.lean`:
`exists_two_perp_of_linearIndependent_normals`, `exists_extensor_eq_panelSupportExtensor`,
and `extensorInPanel_panelSupportExtensor`. All three are purely additive; no blueprint
edits (def-node flip waits for L0d). Build green (2810 jobs, zero warns). **Next concrete
step: the L0b build commit** — the coannihilator complement brick + B1 (additive).
