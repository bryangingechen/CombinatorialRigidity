...
**L0c complete.** Four lemmas landed in `PanelLayer.lean` (§ `Classical_L0c` section, after
the `screwDim_mul_numParts_sub_le_finrank_partitionMotions` block) and one in
`GenericityDevice.lean` (B2, beside the L0b bricks):
- `screwDim_mul_range_card_sub_le_finrank_partitionMotions` (range-exact motion bound)
- `crossingEdges_complement_sep` (complement-separation for `crossingEdges`)
- `range_complement_sep_card` (range count under complement-separation)
- `screwDim_mul_compl_add_deficiency_le_finrank_infinitesimalMotions` (relative hub: `D·(|Vᶜ|+1) + def ≤ finrank Z`)
- `BodyHingeFramework.finrank_span_rigidityRows_add_deficiency_le` (B2 in `GenericityDevice.lean`)

Build green (2810 jobs, zero warns). **Next concrete step: the L0d build commit** —
`HasPanelRealization` (M2) + the forgetful map (M4) + `def:genuine-hinge-realization` pinned
and green (`checkdecls`), per §1.57(d).
...
**Smallest next forward commit: the L0d build commit** — `HasPanelRealization` (M2) +
forgetful map (M4) + `def:genuine-hinge-realization` pinned green (`checkdecls`), per
§1.57(d). Then L0e per the layer plan.
At phase close:
