...
**L1a is complete.** Next: any of L1b/L1c/L1d/L1f/L1g (all independent; see Hand-off).
...
**Smallest next forward commit: the L1b build** (the `|V| ≤ 2` trichotomy + `lem:two-vertex-
trichotomy`; additive to `Deficiency.lean`), or independently **L1c** (the V3/V4/G0 in-place
all-`k` restates in `ReducibleVertex.lean` gaining `TwoEdgeConnected` hypothesis),
**L1d** (KT 3.6 part 1 in `Deficiency.lean`), **L1f** (KT 4.5(ii) in `ReducibleVertex.lean`),
or **L1g** (reverse acyclicity bricks in `ForestSurgery.lean`) — all independent per §1.58(i).
L0 is fully complete; L1a is complete; the remaining L1 signatures are pinned in §1.58.
At phase close:
