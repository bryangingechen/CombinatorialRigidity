...
**Design pins complete (§1.56 motive + §1.57 L0 signatures); no Lean touched yet.** §1.56
pins the motive (the honest bare motive moves to the free-hinge `BodyHingeFramework`
carrier with an `ExtensorInPanel` containment + the ℤ-cast rank-deficiency equality; the
generic motive keeps its `PanelHingeFramework` carrier with only the rank form generalized;
the reduction becomes the four-case all-`k` principle). §1.57 resolves V1 (pointwise
`ExtensorInPanel`, coercion-fixed; the meet-decomposition lemma pinned at `k = 2` over the
landed PanelLayer duality) and V10 (B2 = relative re-derivation via a complement-separating
partition refinement; `reaim` not applicable), fixes the exact M1–M5/B1/B2 Lean statements
and placements, re-times M5 (the weak-motive *deletion* is an L9 phase-close invariant, not
an L0 action), and cuts L0 into five commit-sized slices **L0a–L0e** with the blueprint
def-node dispositions. **Next concrete step: the L0a build commit** (§1.57(e)):
`ExtensorInPanel` in RigidityMatrix.lean + `exists_extensor_eq_panelSupportExtensor` (+ its
perp-pair sub-brick) in PanelLayer.lean — purely additive; the double-annihilator step is
the slice's one mathlib-search item.
...
- [ ] **L0** — motives M1–M5 + bridges B1/B2 + the blueprint def-node flips; signatures
  ...
  - [ ] **L0a** — `ExtensorInPanel` + the `k = 2` meet-decomposition lemma (additive).
  - [ ] **L0b** — the coannihilator complement brick + B1 (additive).
...
**Smallest next forward commit: the L0a build commit** (§1.57(a)/(e)): land
`ExtensorInPanel` in RigidityMatrix.lean's graph-free geometry section and
`exists_extensor_eq_panelSupportExtensor` (with its LI-perp-pair sub-brick and the
`ExtensorInPanel`-pair corollary) in PanelLayer.lean — exact statements in §1.57(a), purely
additive, no blueprint edits (the def-node flip waits for L0d). The proof's one open search
item is the finite-dimensional double-annihilator step (`(∀ r, r C = 0 → r x = 0) → x ∈
span {C}`); if mathlib lacks the exact form, land a small dual-separation mirror brick.
Then L0b–L0e per the layer plan. At phase close:
