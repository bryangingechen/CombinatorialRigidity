...
generic `dim Z > 0`, each with `a = 0` and structured (GR-8) maximum 1, all
on `Λ ≠ ∅` shapes. So the sub-circuit members of the (GR-8) family really
do bite, and (NC1) alone is not the colouring-existence target.
