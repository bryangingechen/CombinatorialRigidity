...
| **(K-grid)** *(new, 2026-08-06, direction T)* | (K-grid) G0–G103 | **The tight-stratum residual of (AC-6) reduces, via a chain of proven results on the contracted multigraph `G°`, to one open geometry-free gap, (GR-15). Several further targets are refuted or reshaped with exact `n_hub` boundaries — class uniformity remains untouched.** **Proven / proven-informally.** (GR-1): each `⋆`-eigen-block is a `C¹`-quadratic spline system on `G°`, exact identity `rank = 3n_c − 3 − dim Z` (688/688). (GR-3)/(GR-8): two counting families bound `dim Z`, unified as (GR-8). (GR-5): a target-rank grid IS `hK`'s conclusion object (`Escape.lean:555`), class-uniform over every infinite char-0 field via (AC-7); 907/907. (GR-9)/(GR-12): the tree-triple certificate theorem, three equivalent forms; (GR-11) an ε-adic fallback (undeveloped). (GR-16): `dim Z` reduces to an exact `3c × 3c` system on `G°` alone. (GR-17): `dim Z = 0` forces `runs(γ) ≥ 3`. (GR-18): `def(G) = 0` forces exactly 6 spanning trees, one both-block length-compatible. (GR-19): the collapse-hierarchy certifies every census-pool separator at `r = 4` (unproven in general). At `Λ = ∅`: the binding-circuit-rich stratum is exactly `D = 0` (GR-21); five sparsity caps hold there (GR-22); an injective flip repair bounds violated colourings (GR-23); an unbounded-`T,Q` repair applies when every binding circuit owns a private even branch (GR-24); a `2^{n_hub}` cut criterion is the canonical `D = 0` oracle (GR-25). (GR-27): `g` is additive over 2-edge-connected blocks. (GR-28)(i)–(iii), at `Λ = ∅`: a closed defect formula; automatic `a = 0`; NC1 `⟹ g = 1` at `k = 2`. (GR-20)'s certificate-3: proven per swept `D = 0` shape (the uniform all-`n_hub` form is not). (GR-32)/(GR-33): the capacity theorem (`cap(S) ≥ 7` at every proper habitat chunk) and the weakness lemma (binding needs `save_A ≥ N − 2` aligned weaknesses). (GR-35): the defect formula is submodular. (GR-36): a structural charge bounding defect below by interior adjacency; its **binding-capable** family strictly contains the capacity-tight one. (GR-37): admissible colourings are exactly (majority-colour, minority-dart) pairs, solvable iff the even-branch vector lies in `Cut(G°)`. (GR-38): a slack identity and attachment lemma classify every chunk crossing; at `n_hub ≤ 6` no two same-block binding chunks cross (0/53 740, 4920 shapes). (GR-39): capacity-tight proper chunks have a proven local structure; three kills close the all-triangle-type cell. (GR-40): the corner charge prunes the fully-capable-hot census 815 → 573. (GR-42): a polynomial habitat-membership criterion and the pentagon necklaces `NK(m)`, `φ(NK(m)) ≥ m`, every member rank-certified fully-good. (GR-43): the odd-cycle-packing shift floor gives `d_par = d_adm = d_fg = m` **exactly** at NK(2)/6/8/10 — the shift-metric layer is UNBOUNDED. (GR-44): the Hall/SDR step is AUTOMATIC and the parity layer has an exact per-matching coset formula `d_par(M) = w_M`, repairing (GR-37)(iii)'s parity half to statement-equals-proof. **Superseded but kept as record** (each proven when landed, none refuted): (GR-41)'s parity floor by (GR-44); (GR-45)–(GR-48)'s legal-move calculus by (GR-49)–(GR-54) and, for move pricing, (GR-68); (GR-57)'s SDR exchange calculus by the z-form. (GR-49)–(GR-53): the z-form absorbs the (c, m)/coset/SDR/matching apparatus into one bit per branch, making balance a degree-constrained orientation decided EXACTLY in polynomial time, its Hall condition a local weight inequality (1/44/4837 maximal constraint structures at `2k = 2/4/6`, exhausted). (GR-54): the balance theorem — every connected cubic loop-free hub multigraph with evenly many `2k ≤ 6` odd branches carries a balanced admissible configuration, discharging input (X) without Petersen. (GR-55): every minority map at distance `d` from a perfect matching `M` is a `(y, Z, φ)` triple; the `M`-avoiding coset space is affine of dimension `n/2 − 1`. (GR-56): the SPLIT identity `defect_A(S) − defect_B(S) = δ_S − σ_S` collapses full-goodness to ONE inequality per chunk — the balance rider its whole-graph instance. (GR-58): `d_fg = d_adm` at ALL 4920 `n_hub ≤ 6` shapes, no cap, and at the first `n = 30` tests. (GR-60): input (Y), (a′)'s named residual — by (GR-56)(iv) inputs (X) and (Y) are the whole-graph and proper-chunk instances of one inequality, and (GR-54) is whole-graph-only, so (Y) stays open. (GR-61): the (GR-56) chunk invariants carry into the z-form exactly. (GR-64): a colouring-free collision bound prices (Y)'s two quantifiers, with a proven `≤ 2` per-chunk ceiling. (GR-65): the fit identity — `dist(m, M)` and `z_mono(S)` are the **same statistic**. (GR-67): the M-anchored z-form gives the deviation count as a 2-factor sign-change count and a **PARITY LAW** (every per-matching layer gap EVEN). (GR-68): every legal move priced in closed form — matching branches and whole 2-factor cycles FREE, a single-path repair at most 2 — proving (b′)'s price half outright. (GR-69): the imbalance ceiling `\|δ\| ≤ 2·min(k, ⌊n_hub/4⌋)`, exactly 2 at `n_hub ≤ 6` and 4 from `n_hub = 8` (a Wagner witness), downgrading GDESC's reading there to forced arithmetic. (GR-73): `slack = 0` iff the pair shares no X-hub, forcing `deg_T ∈ {2,3}` on `T := S ∩ S′` with attachment families `≥ 2`; the (GR-36)(iii) J-charge extends to any branch set. (GR-74): at `n_hub = 8` the AA-glue configuration is pinned to exactly **one** template, EXHAUSTIVE at all 44 premise-satisfying pairs of all 20 classes, explaining (GR-38)'s `n_hub ≤ 6` vacuity. (GR-75): that configuration and its whole kill residual (`slack + defect(T) ≤ 1`) are **NOT realizable** at `n_hub = 8`, certified by an EXHAUSTIVE uncapped scan of the complete stratum (39 689 shapes, 9 617 854 colourings, 0 instances) reproducing (GR-38)'s `n_hub ≤ 6` headline — so the kill is a **THEOREM** there, **non-vacuously**. (GR-76): a general-`n` charge forces `n_hub ≥ 10` and narrows `n_hub = 10` to three templates — **superseded, not refuted**, by (GR-82). (GR-79): what `defect_A(T) = 0` forces on the `T`-frame before any length is chosen — `A(β) = 1` on every `T`-branch, AA interiors with B free darts, no interior-interior `T`-branch ((GR-74)(i) re-derived), and the **corner ledger** `2p = 3n_3 − 2n_2 − 2q_T`. (GR-80): the **corner charge** `n_3 ≥ 2n_2 + 2q_T`, a factor-3 strengthening of (GR-74)(iii) and a **third** proof of (GR-75)(i). (GR-81): the universal dart identity `bend(β) = e − 2c + 1` (2 545 902 instances, complete `n_hub ≤ 6` stratum) pins `#{B-darts at X} = (3\|X\| − n_2 − 2q_T)/2` **exactly** — (GR-76)(i) is its `≥ 0` instance, the per-hub instance the **X charge** `\|X\| ≥ n_2 + 2q_T`. (GR-82): chaining them gives the `n`-free `n_hub ≥ 4(n_2 + q_T) ≥ 16`, so the configuration is impossible at **every `n_hub ≤ 14`** and all three (GR-76)(iv) templates are **NOT realizable**, each killed independently by either charge and again by a charge-free dart enumeration. (GR-83): the bound is **EXACT** — an `n_hub = 16` **WITNESS**, habitat-gated by (GR-25), admissible by the canonical predicate, all three charges tight, rank-certified by `gridwit.subgraph_g` and an exact `dim Z = 3 > 0` at generic labels, at which the **(GR-38) kill FAILS** (proper union, defect 4). **Refuted, and what replaced it.** (GR-2): "(AC-4)(i)–(ii) ⟹ both blocks isostatic" — refuted (`ds-K4`). (GR-4): the (GR-3) bound equals generic `dim Z` — refuted as stated, repaired by **(GR-4′)**, true-modulo-named-gap. **(GR-4′) is off the critical path in one sense, load-bearing in another**: (GR-9) discharges `hK` with no generic-arrangement input, but route (i)/certificate-3 yields (GR-15) only *modulo (GR-4′)*. (GR-10): the min-max form is refuted as posed, **(GR-10) itself staying open** — grouped-packing is NP-complete at exact balance (GR-13). (GR-28)(iv): the `g ≤ 1` cap at `k ≥ 3` — refuted with an exact boundary (a THEOREM at `n_hub ≤ 6` (GR-29), FALSE from `n_hub = 8` on (GR-30)'s four witnesses, per-shape (GR-15) holding at all four (GR-31)). (GR-34): the uncorrelated union-bound mechanism is refuted by a constructed `CL10` witness; a correlated rung-minority rule closes the probed instances. **The bounded-deviation selection form**: **REFUTED as posed** ((GR-41)+(GR-42): `d(NK(m)) ≥ m/2` unbounded while every member stays fully-good). (GR-59): the PER-MATCHING variant of (a′) is REFUTED — 1278 of 24 638 pairs carry a finite `d_adm(M) < d_fg(M)`, gaps exactly `{2, 4}`; `min_M` is LOAD-BEARING. (GR-62): full goodness is **NOT** a function of the (GR-50) degree data (7982/217 468 fibres at 1499/4924 shapes) — no (GR-51)-shaped criterion applies to the chunk system. (GR-63): the coordinator's predicted parity obstruction is **REFUTED as stated** — (GR-52) localizes for free, being a per-hub-subset identity; the (Y)-chain breaks earlier, at (GR-50)→(GR-51). (GR-70): per-matching (b′) reduces to one availability clause, EXHAUSTIVE on the stratum in its T1-only instance — REFUTED from `n_hub = 8` by stuck witnesses, each repaired at price 0 by a named mixed-pair successor. (GR-77): the dispatch's predicted consequence — that (GR-75) makes outright binding laminarity a theorem — is **REFUTED**: 3 774 crossing same-block binding pairs over the complete `n_hub = 8` stratum (against 0 at `n_hub ≤ 6`); what survives is only the uncrossing of the **maximal** family per block. (GR-88): **sub-clause 2 of Clause A′ is REFUTED as posed** — an `n_hub = 8` witness where a doubly-blocked *matching* branch prices **exactly 4** (the ceiling), the case first occurring there by (GR-89)(iii); two landed readings corrected (the `W`-arithmetic needs **one** deviating endpoint, not two; route 1's named input runs the **wrong way**, optimality *capping* `2\|W ∩ S\| ≤ \|W\|`). Clause A′ itself is **not** refuted: three other majority-side branches at the witness price `≤ 2`. (GR-84): and that survivor **does not reach `n_hub = 16`** — at the (GR-83) witness colouring all three maximal binding pairs CROSS and the **whole** kill residual is inhabited, so the sequence is laminar outright at `n_hub ≤ 6`, maximal-laminar at 8, **neither** from 16; an **organizational** tool is lost, no ledger entry's status. (GR-85): at a fixed odd pattern `dist(·, M)` is a **changeover count** `#{v : A(v) = 1}` over even-branch orientations inside the 2-factor — so route 3's *cost* side is a **parity** count, not a flow cost. (GR-86): the **repair-chain theorem** — one odd-branch flip is repaired by a chain of even-branch flips, a chain exists **iff** the flipped pattern is (GR-50)-feasible, and its price **telescopes**, hence is length-independent: `Δdist ≤ 2` in five of six blocking cases, `≤ 4` in the sixth. (GR-87): **sub-clause 1 of Clause A′ is PROVEN**, with no far-end side condition and at any chain length. (GR-89): iterating (GR-86) gives `d_adm(M) − d_par(M) ≤ 4·min(k, ⌊n_hub/4⌋) ≤ 12`, **`n`-free, the arc's first proven bound of (b′)'s own shape — modulo one named clause (R1)** (0 failures at 771 530 configurations, unproven); its counting bound `n_hub ≥ 3k + 5\|δ\|/2` **proves** on the whole `n_hub ≤ 6` stratum that some majority-side branch is not a doubly-blocked matching branch. **Measured, not proved.** (GR-14): a proven certificate, nil habitat applicability. (GR-26): an exhaustive 40 742-shape `D = 0` sweep, zero (GR-15) misses. (GR-31)'s flip-distance-≤-`g` law: sweep-local, breaks at `n_hub = 16`. (GR-39)'s hot-dart census, exhaustive over all 4920 `n_hub ≤ 6` shapes: zero fully-hot hubs on the capacity-tight and realized-binding families; the **binding-capable** family (GR-36) carries 761 on a 184-shape subsample. The deviation decomposes into the parity layer (GR-44) and a balance layer costing `{0, 1, 2}` everywhere probed. (GR-66): GBAL's certificate against (Y) misses the distance optimum at 3514/4924 shapes, full goodness at 651/4924. (GR-71): (b′) at 536 exact shapes at `n_hub = 8/10/12`, cap-free certificates at `n = 30`. (GR-78): every one of the 39 689 `n_hub = 8` shapes carries a fully-good colouring — EXHAUSTIVE, no cap — at 86.9% of 9 833 022 colourings; counting-side only. Ledger entry 5 (per-shape admissibility): **PROVEN in both halves** (GR-54) — a **HIT**. **E3 is ARMED, targeting entry 1's (a′), and does NOT fire.** **(b′) is now a HIT at a DIFFERENT CONSTANT**: price half proven (GR-68), ceiling proven-with-an-exact-boundary (GR-69), and the gap itself bounded `≤ 4·min(k, ⌊n_hub/4⌋) ≤ 12` **modulo (R1)** (GR-89) — while **(b′) at the constant 2 stays OPEN**, its exact residual now the **selection** clause (C2) (a *cheap* majority-side branch exists) rather than a repair statement, 0 failures at 96 930 exhaustive stratum configurations. **Attack (c)'s AA-glue case is SETTLED at an EXACT BOUNDARY** — impossible at every `n_hub ≤ 14` by an `n`-free proof (GR-82), REALIZED at `n_hub = 16` (GR-83) where the (GR-38) kill FAILS — while the residual's other two cases (`slack + defect(T) = 1`) stay **OPEN at `n_hub ≥ 10`**, bounded only at `n_hub = 8` and both realized at 16. **(GR-15) stays OPEN throughout, unchanged in status, no gap-map status move** — the arc's sole remaining open gap; see *what would close it*. | **(GR-15)** *(the re-aimed discharge residual, Step G17, the critical-path form — (GR-10) stays open as the strictly stronger statement, its min-max refuted as posed)* — every tight class shape admits an admissible colouring with generic `dim Z₊ = dim Z₋ = 0` in both blocks. Implied by (GR-10); one-point-decidable per shape ((GR-7) remark (i); τ-side analogue §(K-ann) (ANH-9)), so the census's 907/907 exact hits are already **per-shape proofs** and only the uniformity gap remains. Its per-block min-max question is **(GR-4′)'s equality** — counting-shaped, untouched by (GR-13)'s hardness — a pure statement about `(G°, ℓ, bits)` alone (GR-16), exhaustively swept to 40 742 `D = 0` shapes with no hit (GR-26). Any successor route must discharge `hK` on the tight stratum over **every infinite characteristic-0 field** ((GR-1)/§(K-clos) (AC-4), (GR-5), (AC-7)). **Four live routes, none closed:** (i) a colouring-existence argument over Step G12's branch bits ("some admissible colouring has `a = 0 ∧ max g ≤ 0` in both blocks" + (GR-4′)), free at 98.6% of circuits by (GR-17); (ii) a bound on the collapse order `κ` (measured `≤ 4` on the census pool by (GR-19), unproven in general); (iii) a characterization of which `r`-co-independent groupings certify beyond `r = 3`; (iv) an exchange argument on (GR-18)'s guaranteed 6-tree packing supplying a length-compatible 3+3 split. **Dead or unselected, with homes — do not re-derive, and do not re-run as routes:** the (GR-27)/(GR-28) exact computation and the `g ≤ 1` cap on the (GR-8) family, hence the whole certificate-3-uniformity route ((GR-28)(iv) refuted at an exact boundary, (GR-29)/(GR-30)); the bounded-deviation selection form ((GR-42)); (GR-31)'s three offered candidates; and the bounded {T1, T2} / {T1, T2, K3} descent and (GR-45)–(GR-47) matching-flexibility routes, **superseded not contradicted** — (GR-54) and (GR-68) prove the statement they were chasing. **The growth-law decomposition is the best-understood target** (Steps G48–G91). Parity layer: **exact**, `d_par(M) = w_M` ((GR-44)), unbounded along the necklaces ((GR-43)); whether `w_M` admits a min-max/packing or polynomial certificate is open and deliberately untouched ((GR-13) caution). Balance layer: existence **PROVEN at every shape** ((GR-54)), and its price is `≤ 2` for a single-path repair, any legal move, any shape, no cap ((GR-68)). Fully-good layer: measured **zero everywhere ever probed**, including `d = 10`, rank-certified at the optimum to `n_hub = 50`. **The two sharpest open statements.** **(a′)** the `d_fg = d_adm` law — exhaustive at all 4920 `n_hub ≤ 6` habitat shapes at no cap and true at the first odd-carrying `n = 30` tests ((GR-58)); residual **input (Y)** ((GR-60)), a joint matching-and-representative selection in the (GR-55) coordinates; per-matching variant **REFUTED** ((GR-59): `min_M` load-bearing, no proof may fix its anchor matching). Its route is re-scoped, not replaced: the `z`-form serves the chunk/balance rung ((GR-61)), the coset coordinates the distance rung, bridged by (GR-65)'s `fit`, over a Hall/deficiency condition on the tight-chunk hypergraph of exit selections (GORIENT *Step G43*) — a (GR-51)-shaped degree-constraint criterion is **excluded** ((GR-62)), on a 10-shape exceptional family aside. **(GR-64)(R2)** is the sharpest cheap successor named. **(b′)** `d_adm − d_par ≤ 2` — price half **PROVEN** ((GR-68)); imbalance ceiling **PROVEN at `n_hub ≤ 6`, FALSE from `n_hub = 8`** ((GR-69)); availability reduced to Clause A′ ((GR-70)), whose **sub-clause 1 is now PROVEN** ((GR-87)) and **sub-clause 2 REFUTED as posed** ((GR-88), price exactly 4 at `n_hub = 8`). The **gap is bounded `≤ 12`, `n`-free, modulo (R1)** ((GR-89)) — so what remains at the constant **2** is the **selection** clause **(C2)**: at some parity-optimal configuration with `\|δ\| ≤ 2`, some majority-side odd branch is feasible-flippable and not a doubly-blocked matching branch. (C2) is **proven below `n_hub = 3k + 5\|δ\|/2`** (hence on the whole `n_hub ≤ 6` stratum) with its first possible failure pinned to `n_hub = 8`, `2k = 2`, both odd branches doubly-blocked matching branches — a two-branch condition. **(R1)** is the cheaper sibling and is what upgrades (GR-89) to a theorem. The min-cost-flow route is **corrected, not available**: by (GR-85) the objective is a parity count, so flow machinery does not apply on the cost side; (GR-86)'s repair chain is the working exchange. **Four named dispatchable attacks:** **(a′)** and **(b′)** above; **(c)** the (GR-38) kill residual at `n_hub ≥ 10` — its **AA-glue case is SETTLED at an EXACT BOUNDARY** ((GR-82): impossible at every `n_hub ≤ 14`, `n`-free, all three (GR-76)(iv) templates dead; (GR-83): REALIZED at `n_hub = 16`, where the kill FAILS; (GR-84): even the maximal-family uncrossing is gone from 16) — so what is left is the `slack + defect(T) = 1` residual, bounded **only** at `n_hub = 8` and both cases realized at 16; the named successor is redoing (GR-79)–(GR-82) with a one-unit defect budget; **(d′)** the corner-armed realized-binding fully-hot seed hunt past GDEV's caps (not a flank by itself, E1's clarification). |
