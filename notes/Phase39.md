...
((BE-188)–(BE-203)); **AND (BE-E4′)'s FRONTIER IS EXHAUSTED, the clause itself UNREFUTED** — `f+g
≥ 6` decomposed then **ALL FIVE members DEAD**, `(BE-G_2)` REFUTED in the regime by
(BE-175)(i) itself, **residue EXACTLY `ρ₁+ρ₂ ≤ 7`** ((BE-204)–(BE-217)). So **BOTH halves
hang on that one clause**, open on the 51 long-arc rest. **OPEN** at 0/772 + 0/411; uniformity, the 12
...
**THE TENTH STRATEGY-ONLY PASS'S RANKING LIVES AT `notes/Pencil-strategy.md` §8** and is **not restated here** (it supersedes the ninth pass, the 2026-08-25 list and the 2026-09-02 `hK` table *as rankings*). **The ninth's ranks 1–4 ALL SPENT**: BSCOND 76 ((S1)/(S2)), BARCH 77 (the method class **changes ambient**, 14 → 12 reduced to (E4), (BE-149)–(BE-155)), GLEAF 80 (the reach question SPLIT, no proof there moves (GR-10), (GR-145)–(GR-152)), BINSERT 82. **AND THE TENTH'S RANK 1 IS SPENT THREE TIMES OVER** (BGPROP 85, BRANKV 86, BLONGARC 87), **AND ITS RANK 2 IS SPENT WITH ITS FRONTIER EXHAUSTED** (BEFOURP 88, BGTWOA 89). **§8's bar now reads: `A_sharp` properness BARRED, (E4) REVERSED, `Γ`-properness STRUCK AS A CLOSURE ROUTE, and the whole ARC / PATH-BOUND METHOD CLASS SPENT** — recorded in §8, as §8's own rule requires; **no further properness attempt on that population is authorized**.
**THE NEXT CONCRETE TASK is §8's RANK 2 SUCCESSOR — (BE-E4′) ON THE `ρ₁ + ρ₂ ≤ 7` FIRING ZONE**, which (BE-216) makes the **exact** residue. Cheapest live sub-slice: a firing side at `ρ_i ∈ {4,5}` (so `e_i ∈ {2,3}`) against a non-firing side at `e_j ≤ 6 − ρ_i`. BGTWOA's 90-row sweep never pairs them because `ρ₂ ∈ {0,3,6}` is **quantized by the uniform skeleton profile length** while `ρ₁ ∈ {2,3,4}` is set by the side-1 piece, so the sub-slice needs a **non-uniform** profile or a side-2 family outside `bpeel.subdivided`. **If that shape exists in the regime, (BE-E4′) is REFUTED; if it provably does not, (BE-E4′) is a THEOREM** — either way a DECISION, which is more than rank 2 has ever had. **Why this and not the frontier (BGTWOA 89):** §8's rank 2 is **SPENT and its frontier EXHAUSTED**, not decomposed. `c_j(Π_x) ≤ dim Π_x = 2` gives `e_j ≥ ρ_j − 2` with **no hypothesis at all**, so `(BE-G_2)` is a THEOREM at `ρ_j ≥ 4`, FALSE at `ρ_j ≤ 1`, and its whole content is the two rungs `ρ_j ∈ {2,3}` — **DISJOINT from BSTEER's `ρ_j = 1`**, so the lift was never a re-statement ((BE-210)); `(BE-G_2a)` is **UNSATISFIABLE at `ρ_j ≥ 5`**, the realizable range being exactly `max(0,ρ−4) ≤ c ≤ min(2,ρ)`, CONSTRUCTED ((BE-211)); **and `(BE-G_2)` is FALSE at 4/4 FULLY-GATED peels INSIDE the generic flag regime** — `ρ = (2,6)`, `c = (1,2)`, `e = (1,4)`, `δ = (2,6)`, every `bline.legal_peel` gate asserted — where the refuting mechanism is **(BE-175)(i) ITSELF** on the non-firing side: BSTEER's `rnode_shaped` exclusion and the flag-regime hypothesis are both indexed to **side 2** while `(BE-G_g)`'s subject is the **NON-FIRING** side, which is side 1 whenever the firing side is the skeleton ((BE-214)). Every surviving member carries `g ≥ 2` and so implies `(BE-G_2)`, and `(5,1)`/`(6,0)` were already refuted — **so all five members are dead** ((BE-215)). **TWO PREDECESSOR AUDIT FINDINGS, recorded not repaired:** (BE-206)'s four-rung grading is a **TAUTOLOGY** where `befourp.run_peel` asserts it (only under `c_i = 2`) and **FALSE** off that branch (8/23 constructed shapes), so **`(BE-F_4)` IS the floor `ρ_i ≥ 4`** and `q̂` adds nothing ((BE-212)); and `barch.all_tuples` omits the Grassmann floor `c_i ≥ ρ_i − 4`, making **2 800 of its 6 400 tuples unrealizable** — but every escape is floor-legal and the frontier grid is IDENTICAL, so (BE-205)(iii) is **PROTECTED** ((BE-213)); the priced repair is a coordinator round, not a dispatch's ((BE-213)/harness debt). **WHAT SURVIVES: (BE-E4′) is UNREFUTED and OPEN** — 0 failures over 90 in-regime R-node rows and it HOLDS at all four kill witnesses at margin `+1` — because the same ambient cap supplies `e_i = 4` at `ρ_i = 6` ((BE-215)(ii)/(BE-216)(iii)). **BARS, cumulative:** BLONGARC's four and BEFOURP's three, **plus three new** — no further attempt at `(BE-G_2)`/`(BE-G_2a)`/any `(BE-G_g)` with `g ≥ 2`, none at any `f+g ≥ 6` member (and no re-derivation of its grid), and no reading of (BE-206)'s grading as a handle on `(BE-F_4)` ((BE-217)(iv)). **Alternatives after it, in order:** `(BE-F_4)` as the plain `ρ`-floor; a *less degenerate* (BE-192)/(BE-200) witness; then the `V^k` graph at `k ≥ 3`. **DOC DEBT — the row DID NOT NEED TO PAY.** The `(K-bare)` row is **2,707 / 2,856** words at **278** labels (**9.74** each) against the cap the coordinator bumped at `a5b071ce` — **149 words of headroom**, `gapdiff` **0 DROPPED** — so BGTWOA **appended without compressing, relocating or bumping**, the first landing in six on this row to pay nothing. **But the density floor still climbed 9.23 → 9.74**, so **RELOCATION, not compression, remains the next lever** and a **fourth** cap bump is the trigger to revisit the **ROW SPLIT, still PROPOSED, NOT EXECUTED** (DECLINED FOR NOW at `424f93dc`; costing in `notes/Pencil-fanout.md` §"BRANKV"). **(BE-14) needs both halves of S-mark and both hang on (PENCIL-SATURATES) at `deg ≥ 2`**, whose surviving repair is (BE-E4′) ((BE-177)(iii)).
...
- **BGTWOA** (89, recon-opus, §8's rank 2) — **THE BSTEER LIFT FAILS AND THE FRONTIER IS
  EXHAUSTED.** `c_j(Π_x) ≤ dim Π_x = 2` gives `e_j ≥ ρ_j − 2` **free**, so `(BE-G_2)`'s content
  is only `ρ_j ∈ {2,3}`, DISJOINT from BSTEER's `ρ_j = 1` ((BE-210)/(BE-211)); **it is FALSE at
  4/4 fully-gated IN-REGIME peels, refuted by (BE-175)(i) ITSELF** on the non-firing side —
  both added hypotheses index **side 2** — so **all five members die** ((BE-214)/(BE-215)).
  **`(BE-F_4)` IS `ρ_i ≥ 4`** ((BE-212)); **(BE-E4′) UNREFUTED, residue `ρ₁+ρ₂ ≤ 7`** ((BE-216)). `w4/bgtwoa.py`.
