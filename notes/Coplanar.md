# COPLANAR — multigraph strength for KT Conjecture 1.2 (planning note)

**Status:** planned (2026-07-18); user-scheduled, next in the ROADMAP
queue; phase number minted when it opens (`PHASE-BOUNDARIES.md` *When
this commit opens a phase*). Paired with the PIN placeholder (the 2-d
molecular conjecture; ROADMAP queue entry only, unplanned). Same editing
discipline as phase notes (≤8-line entries, lift cross-cutting lessons).

## What this project is

User-proposed (2026-07-18): recover the **full multigraph strength** of
KT Conjecture 1.2 and Theorem 5.6, retiring the PROSPECT K1 wall
(`notes/Prospect.md`). The formalized `molecular_conjecture` is the
...
KT Lemma 5.3's coincident-panel double-edge realization is inexpressible
(blueprint `fmlnote:molecular-conjecture-multigraph`; the multigraph iff
is **false** in that model, verified 2026-07-04). Target: state the
panel side in KT's own hinge-coplanar model and prove the multigraph iff.

## Survey findings (2026-07-18; verified against decl bodies, not docstrings)

The K1 verdict ("a different hinge encoding, i.e. a re-architecture")
is **partially stale**: the KT-faithful encoding already exists in-tree,
and the hard theorem already holds at multigraph strength in it. What
remains is additive, localized to the 5.6 assembly and the statement layer.
...
## Candidate route (work items; recon-first, R0 gates the builds)
...
- **W5 — blueprint + status-surface reconciliation.** Rescope
  ...
  *stands* as a remark — it is true and load-bearing exposition; the
  ...
  (intro.tex / README / home_page), and the Prospect K1 entry. The
  simple-graph meet-model theorems are **retained** (for simple graphs
  they are the stronger, more concrete statements; no refactor).

## Open recon questions (the opening dispatch settles these)

- **R0 — route verification (gates everything).** This note's route is
  a session derivation, not a compiler-checked one (the *transcribed
  proof vs carrier* discipline, top-level `CLAUDE.md`): verify W2
...
the wall was assessed against re-architecting the *meet-model statement*;
the reclassification is that the alternative encoding and its hard
theorem (multigraph 5.5 bare, incl. the Lemma-5.3 base) landed with
Phase 22i's M2 motive and the 22h/23 base producers — only the last
assembly step ever consumed the Simple gate.
...
- Katoh, Tanigawa, *A proof of the molecular conjecture*, Discrete
  ...
  *References*); the specific pointers used here (Conjecture 1.2 for
  ...
  `fmlnote:molecular-conjecture-multigraph` this session.

## Hand-off / next step

The opening commit mints the phase number, seeds `notes/PhaseN.md` from
the work items above, and dispatches **R0 as the first recon** (with
R1–R3 riding the same dispatch or the next). No Lean work before R0
returns GO on the W2 route.
