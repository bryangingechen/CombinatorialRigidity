...
**L7 complete (L7a + L7b). Next: L8 — the Lemma-6.5 arm.** The `h622` carry is discharged; the
new public `case_III_realization` carries the all-`k` IH and derives `h622lb` internally.
`theorem_55_d3` calls the thin wrapper `case_III_realization_0dof` (Flag F1) until L9 swaps the
spine. `lem:case-III-nested-rank-lower` flipped green; `lem:case-III` / `thm:theorem-55-d3-instance`
restated; seed-transfer citation corrected. `lake build` + `lake lint` + `blueprint/verify.sh` clean.

**Done (L7b, 2026-06-15):** `case_III_realization` restated — drops `h622`, adds `hn`, upgrades
`hIH` to all-`k`; `h622lb` derived inline (`splitOff_removeVertex_minimalKDof` → all-`k` IH →
`exists_rankPolynomial_of_IH_linking` → footnote-6 non-root → arithmetic). Thin wrapper
`case_III_realization_0dof` keeps `theorem_55_d3` building. Blueprint nodes updated; gates clean.
**Done (L7a, prior commit):** `exists_rankPolynomial_of_IH_linking` + `finrank_span_rigidityRows_ofNormals_eq`.
**L8–L10 open.**
...
- [x] **L7** ✓ — the Case-III rewire: `case_III_realization` restated, `h622` *derived* from the
  ...
  discharge + thin wrapper `case_III_realization_0dof` (Flag F1). Blueprint nodes:
  `lem:case-III-nested-rank-lower` (green), `lem:case-III` / `thm:theorem-55-d3-instance` (restated).
...
|---|---|---|---|
| `h622` (KT eq. (6.22), the nested-IH rank lower bound at the `k'`-dof `G_v`) | `lem:case-III-nested-rank-lower` (case-iii.tex) | **DISCHARGED (22k L7b)**: `case_III_realization` restated, `h622lb` derived inline; `theorem_55_d3` calls thin wrapper `case_III_realization_0dof` (Flag F1). `lem:case-III-nested-rank-lower` green. | **L7 complete** (22k): all-`k` IH at `G_v` → `exists_rankPolynomial_of_IH_linking` → footnote-6 non-root → arithmetic. |
...
- **L7b — `case_III_realization` restate + `h622` discharge (2026-06-15, sonnet, clean):** dropped
  `h622` carry; added `hn : bodyBarDim n = screwDim 2`; upgraded `hIH` to all-`k` form. `h622lb`
  derived inline from `splitOff_removeVertex_minimalKDof` + all-`k` IH + `exists_rankPolynomial_of_IH_linking`
  + footnote-6 non-root + arithmetic. Thin wrapper `case_III_realization_0dof` keeps `theorem_55_d3`
  building (Flag F1) until L9. Blueprint: `lem:case-III-nested-rank-lower` green, `lem:case-III` /
  `thm:theorem-55-d3-instance` restated. Gates clean.
