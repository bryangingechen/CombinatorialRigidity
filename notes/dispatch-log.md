...
| 2026-09-03 | Phase39 concurrent round of four (this commit records) | opus (coordinator) | **FOUR distinct reservation defects in ONE round, all mine, all from one root cause** | (i) `git grep -c` piped to `wc -l` counts **files**, reported as **hits**; (ii) the next sampled a range's interior, missing its **opening** tokens; (iii) the third reserved the **wrong family** — `(D)` for §(K-dom), whose family is `(DM-)`; (iv) the fourth was **off by one against a declared tail**, reading a scan's maximum as consumed when it was a declaration. **Root cause: I DERIVED reservations by scanning the corpus instead of READING the tail the registry declares.** Fix: read the declared tail, open there, scan only to confirm. DSAT and GLEAF both deviated correctly, citing the registry. |
