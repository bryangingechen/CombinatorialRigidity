...
| 2026-09-03 | Phase39 direction GELIM (dispatched, corrected mid-flight, KILLED; nothing committed, HEAD unmoved) | opus (coordinator error; recon-opus dispatched) | **the coordinator dispatched on a summary surface's stale recommendation, and the USER caught it twice before the record did** | `Pencil-strategy.md` §5.3 lists item (i) (the `V_bc` elimination question) as its highest-value unrun CAS item. It was dead **the day it was written** — (D4) landed 2026-08-05, same day — and stayed listed four weeks. The coordinator picked it as a reprioritization without opening §(K-ann) or §(K-dom). The user then recalled the M2 layer had been abandoned on size grounds: correct, **(ANH-16)** bracketed its reach 2026-08-06 and the layer has been cold since 08-07. The user then asked whether elimination is even one finite problem: also correct, and §4.6 (R1) already says *"the criterion has no graph quantifier and is discharged; the graph quantifier lives entirely in the realizability of `λ`"*. Killed. The salvage resume returned the full disposition with no driver: item (i) is not frame-expressible AND contentless at `k ≥ 4`, frame-expressible and the only stratum with content at `k = 3` — which is (K-res), barred. **Both fixes landed here.** Lesson: **status objects are gated, recommendation lists are not** — nothing re-checks a forward-looking item when a landing elsewhere kills it. | §5.3 listed the `V_bc` elimination question as its top unrun CAS item. It was dead **the day it was written** ((D4), same day) and stayed listed four weeks — dispatched on without opening §(K-ann) or §(K-dom). The **user** caught it twice: the M2 layer was abandoned on size grounds ((ANH-16) bracketed its reach 08-06; cold since 08-07), and elimination is not one finite problem — §4.6 (R1) puts the graph quantifier in **realizability**. Killed, nothing committed; the salvage resume returned the disposition with no driver. **Status objects are gated; recommendation lists are not.** 
