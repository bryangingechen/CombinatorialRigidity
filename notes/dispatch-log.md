...
| 2026-08-28 | Phase39 58th direction BRULE (`d3f584a2`; this commit repairs it) | opus (coordinator) | **F17 again — and the stale surface was the one the COORDINATOR wrote at the prep** | The landing updated the ROADMAP row and `Phase39.md` to *58 landed, none in flight* but left `Pencil-fanout.md`'s header reading *57 LANDED … BRULE is IN FLIGHT* — contradicting both, and its own write-up 7 000 lines below in the same file. That header was **coordinator-written at the prep**, which is why nobody re-read it; F19's second half applies. **The catch is a cross-surface compare, not a gate:** after every landing, read the arc count off all three surfaces in one call. The write-up heading had also drifted off the `### <CODE> — landing write-up (…)` convention; normalised. |
| 2026-08-29 | Phase39 59th direction BWIN (killed, resumed → `21694d3d`) | fable | **KILLED by the 64k output-token cap — resume-first worked, cause named** | First `max_output_tokens` kill in either repo's log: a **single response** exceeded the 64k output cap, killing the dispatch while it was still orienting. Not a capability or usage limit, so **not an escalation trigger** — nothing landed, HEAD unmoved, tree clean, no draft stranded. Rescue §3's resume-first applied: same-agent `SendMessage`, rung-pinned so it returned at fable, with **one** added instruction — work incrementally, append across passes, never emit one very long response. **The resume landed HIT shape 1.** Naming the cause in the resume is what stops it recurring. |
| 2026-08-29 | Phase39 59th direction BWIN (`21694d3d`; this commit repairs it) | fable | **side conditions reached every surface EXCEPT the phase-note status header** | (BE-57) carries two side conditions (S1)/(S2) — hypotheses on the middle, vacuous at every drawn piece but **not theorems**. The workbook says so in four places and the gap-map row too. The **phase-note `**Status:**` header did not**: *"(β) is PROVED there on the 87-of-91 domain"*, unconditional — the first paragraph a fresh session reads. Third instance in this sub-arc of a **summary outrunning its own table** ((BE-36), (BE-38)(iii), now this); the first two were caught by the NEXT direction, this one at verification. **Check: read the status header against the result's own caveat block.** |
| 2026-09-01 | Phase39 61st direction BDECOR (`ef01c9e8`; this commit repairs it) | opus | **FOURTH summary-outruns-its-caveat instance: the product theorem's proviso `G` dropped on SIX surfaces** | (BE-64)(ii) states `Config(H; ϕ) ≅ (∏_b Ear) ∩ G`, and the section's own definitions block says `G` *"is disclosed every time"*. It was not: the **gap-map row** (the authoritative status object), the section heading, the F12 hunk at originating prose, the fan-out header and **both** phase-note sites all said *"literal product"* — and *literal* is the damage, reading as *nothing further*. Body prose was correct, so no gate could fire. **Check: read the theorem's modulo-clause against every surface summarizing it, and treat a self-certifying phrase as the claim most worth testing.** |
| 2026-09-01 | Phase39 62nd direction BPEEL (this commit) | opus | **A landed workbook clause was FALSE AS STATED, and its refutation sat inside the very step it cited** | (BE-66)(iv) said *"the only landed mechanism forcing `π_x = π_y` is (BE-15)'s triangle propagation, which needs `x ∼ y`"*. (BE-15)(ii) carries its **own inline scope correction** recording a landed *general* rule needing neither — `K_{3,3}`, forced flat and triangle-free. No gate could fire: both are prose, and the second is nested inside the first's own citation. **Check: when a discharge rests on "the only landed mechanism is X", open X's own step and read its scope corrections before quoting it — a caveat one level down is invisible to every summary above it.** |
| 2026-09-01 | Phase39 session, dirs BDECOR + BPEEL (`ef01c9e8`, `94af7fdd`) | opus (substituted for fable) | **playbook deviation: the TOP RUNG went unavailable mid-session, and the substitution held** | The user reported fable absent from the account's model selector after a client update (BRNODE had run at fable minutes earlier). The playbook's substitute — *nearest available rung at or above the mapped one* — put two top-rung-mapped class-statement recons at `recon-opus`; **no dispatch was spent probing**. Both returned HITs, and BPEEL refuted a landed clause that BDECOR and the coordinator had both accepted. **Next session: re-check availability rather than assume either way** — the `*-fable` definitions are unchanged and pin the bare `fable` alias, so restoring is a no-op. |
| 2026-09-01 | Phase39 63rd direction BSPREAD (`6b113c58` prep, `f08c4ea6` landing) | opus (substituted for fable) | **FIFTH summary-outruns-its-caveat instance — and the first caught by the PRE-DISPATCH guard, before a dispatch was spent on it** | (BE-41)(ii) is refuted by (BE-41)(iii) **printed immediately below it**, and the shipped driver `assert`s that escape set non-empty. Three summary surfaces re-stated (ii) without the carve-out and **two later landings inherited it as the thing to prove**. The four prior instances were each caught by the NEXT direction; this one by the coordinator's rating-step guard, so the spec targeted the true residue. **Check: when a hand-off names a statement to prove, read the sibling clause beneath it — and prefer the driver's `assert`s to the prose summarizing them.** |
| 2026-09-01 | Phase39 65th direction BGENUINE (`1bac0f55` prep, `121124ae` landing) | opus (substituted for fable) | **a COORDINATOR-AUTHORED prep handed the dispatch the WRONG YARDSTICK, and the landing agent caught it** | The spec quoted (BE-22)(iii) as *"does it drop below `min(δ₁+δ₂,6)`"*, dropping its **both-pieces-attain** hypothesis — copied from a gap-map row that had dropped it, not from the lemma. At **100 of 392** witnesses one side does not attain, and those rows read as violations against that denominator. The same coordinator quoted it **correctly two preps earlier**, so this is drift inside one session. The direction re-derived the criterion unconditionally (`+ a₁+a₂`) and repaired both surfaces. **Check: quote a criterion WITH its hypotheses, or name the surface it was copied from.** |
| 2026-09-02 | Phase39 66th direction WTRI (`736efc9c`) | opus (substituted for fable) | **SIXTH summary-outruns-its-caveat instance, and a new twist: the caveat was dropped from an INVENTORY, not a summary** | §(SAFE-RES) *Step 4* concluded *"(T) is **not provable** from the landed set"* after enumerating the feasibility **criteria** and omitting the two **transfers** (`PencilNondegFeasible.mono`, `pencilNondegFeasible_induce_of_pendant_deg3`), which compose to prove it. It rode a month as a reason not to attack (T). The defect is not a caveat lost in restatement but a **class of declaration never enumerated**. **Check: on "not provable from the landed set", ask which declarations were enumerated — grep the tree, don't trust the list.** |
| 2026-09-02 | Phase39 67th direction WELOC (`99d7ed64` prep, `1b4cb95e` landing) | opus (substituted for fable) | **a COORDINATOR selection rationale had a WRONG SUPPORTING LEG, and the landing corrected it** | The prep justified picking (E-loc) over (V) on three legs, one of which was *"(E-loc) has TWO consumers — (E) and Step 0's `hfresh` discharge"*, quoted from *Step 0*. The landing found *Step 0*'s claim is true of the landed **proof** and not of the **obligation** (a residual is `Simple`, so the edge count is free), so **(E) has ONE real consumer**. The pick's main leg — (V) is downstream of (E) — held, and the direction confirmed it. **Check: a consumer count copied from a note is a claim about an obligation; verify it against the consumer's hypotheses, not its proof.** |
| 2026-09-02 | Phase39 68th direction WPAIR (`04e9e54c`; this commit corrects it) | opus (substituted for fable) | **an E-RIDER READING drifted, and the corpus carries TWO formulations of the rider it drifted on** | The landing reported *"E3 comes within one step of firing"* because proving (E-pair) empties **W4's** dispatchable list. But E1–E3 are stated *"over the ledger and the target's status"*, and **"the target" is the ARC's, not a direction's local obligation** — the (BE-14) thread stays dispatchable and the hold's release condition is unmet. Contributing cause: the archive holds **two** E3 texts, GEXIST's two-conjunct form and a later *"target proven, full stop"* deviation. **Check: the coordinator re-runs E1/E2/E3; quote the rider's CURRENT text and confirm what "the target" names.** |
| 2026-09-02 | Phase39 coordinator follow-up `61e046a6` (caught at the WGROW prep, this commit) | opus (coordinator) | **an F17 status-surface miss inside a commit whose whole purpose was correcting a status surface** | `61e046a6` corrected WPAIR's E3 reading at *Hand-off* and the write-up, and left `notes/Phase39.md`'s `**Status:**` header still reading *"E3 is ARMED ... and is now ONE LANDING AWAY"* — the exact sentence it had just refuted, on the note's first-read paragraph. Section-scoped editing again: the header is not re-read by an edit aimed elsewhere. **This is RESEARCH-ARC §7's second half** — coordinator artifacts need a subagent's verification tier, and here the corrector's own F17 sweep missed the surface nearest the reader. Repaired at the prep. |
| 2026-09-02 | Phase39 69th direction WGROW (`c40006f7`; this commit repairs it) | opus (substituted for fable) | **SEVENTH summary-outruns-its-caveat instance -- the first inside a freshly-landed lemma STATEMENT, caught by coordinator scrutiny** | (GROW-2)'s size clause said *"for `A` a proper subset of `W`, `3 <= \|U(A)\|` whenever `U(A)` is rigid"*. False at `\|A\| = 1`: `U({z}) = {z}`, one vertex, `partitionDef = 0` so rigid, and (R1) does not bite because `two_le_degree_of_isKDof_zero` carries `2 <= \|V\|`. **Unused at `\|A\| = 1` everywhere** -- (GROW-4) quotes it only at `\|X_i\| >= 2`, GW5 re-derives, and the driver ranges `r` from 2 -- so no figure or verdict moves. The mathematics is verified and stands; the repair adds the missing hypothesis at source. |
| 2026-09-02 | Phase39 72nd direction BDOUBLE (`9780f113`; this commit corrects it) | opus (substituted for fable) | **a MECHANICAL CAP was bumped on a COMPLIANT row -- the F21 coasting shape, inside a procedure that reads as if it authorizes it** | The landing recomputed the `(K-bare)` row honestly (1 472 -> 1 546, scripted set-diff, zero labels dropped), then added a `SPECIAL_CAPS` entry at 1 630. But 1 546 is **under the generic 1 600** -- the row was compliant, so the bump bought headroom for a landing that had not happened. **Every other entry in that table was for a row that had EXCEEDED its cap, each with a density argument**; none was made here. Withdrawn; the gate's docstring now states the missing precondition -- *recompute first, twice, then bump* presupposes an overflow. **No overflow, no bump.** |
| 2026-09-02 | Phase39 73rd direction BSATUR (`e53cb9b4`) + the `hK` scoping recon | opus (substituted for fable) | **an in-driver ASSERTION was true and the claim was false -- the sampler never ranged over the claim's own quantifier, and the coordinator had cited the assertion as REASSURANCE** | `bearcase.py:1452` really carries `assert not (cuv >= 2 and r1 < 6)`, true of every draw that run made; the run varied the CONFIGURATION and never the FLAG, so the witness sat one quantifier away. The BSATUR spec had weighted the clause **higher** because it was asserted rather than reported -- exactly backwards. Promoted to `RESEARCH-ARC.md` Sec.4: **name the sampler's support and ask which of the claim's variables it varies.** Distinct from F11 (there no driver tested the sentence; here one did, correctly, over a blind population). |
| 2026-09-02 | Phase39 74th direction GPACK (`3f046a3c`) | opus (substituted for fable) | **a coordinator caveat written into the option board INVERTED its own source, and the direction it primed caught it** | The board asked whether `hK`'s non-tight remainder is free *"by Sec.2.5's `dim R_a >= 2` => escape automatic"*. Sec.2.5 is a **negative** result: at tight shapes the count FORCES `dim R_a = 1`, so that invariant is exactly the one that cannot be carried, and F5(d) has seeds where the count says 1 and geometry delivers 0. Repaired at source. The substantive answer is also in: the remainder **is owned**, by `Sec.(K-res)/(RS-5)` via the escape route. Shape to watch: a coordinator citing a section by its *implication* rather than its *purpose*. |
| 2026-09-02 | Phase39 74th direction GPACK (`3f046a3c`), self-reported | opus (substituted for fable) | **F6 near-miss, self-caught: a gate auto-backgrounded because only the SHELL `timeout` was used, not the Bash tool's parameter** | The direction's first `--resid` run auto-backgrounded at the harness default; it re-ran in the foreground and every quoted figure comes from a foreground run, so no figure is affected. Logged because it is F6's exact recorded shape recurring after the in-prompt reminder, and because the agent surfaced it unprompted rather than letting the coordinator find it -- the disclosure is the reason this is a near-miss and not a defect. |
| 2026-09-02 | Phase39 concurrent round, dirs GLIST/BPROPER/OWALL (`cadfc1f3`, `815ed946`, `1f424ec2`; this commit reconciles) | opus (coordinator) | **a SHARED COUNTER was double-claimed by concurrent directions, and no gate or single return could see it** | `RESEARCH-ARC.md` section 7's prediction tally is read-modify-write. Three directions landing the same day each read a different baseline: GLIST and BPROPER both landed "twelve" -- the same slot -- while OWALL read section 7's own figure, stale by six, and said "ninth". Correct is fourteen. Caught only by the coordinator's cross-return pass: each return is internally consistent, and `check-phase-note.py` / `check-gapmap-cells.py` / `gapdiff.py` are all blind to it. Lesson: the coordinator reconciles a shared counter after the round, in its own commit; no direction re-derives it. |
| 2026-09-02 | Phase39 concurrent round, dirs GLIST/BPROPER/OWALL (this commit records) | opus (coordinator) | **the read-only rule protects WRITERS from writers; two hazards hit what a concurrent dispatch READS** | (i) BPROPER measured the phase note off a committing sibling's DIRTY tree and reported budget figures that were never a landed state -- it diffed the gap map against `HEAD` correctly in the same pass, so the rule is *diff against HEAD, never the working tree*. (ii) The session scratchpad is SHARED: GLIST had two default-named working files overwritten mid-edit by a sibling, verified by byte count; nothing was corrupted only because the consumer re-derived from an input it had re-verified against `HEAD`. Both landed in `RESEARCH-ARC.md` section 2. |
| 2026-09-03 | Phase39 direction GELIM (dispatched, corrected mid-flight, KILLED; nothing committed, HEAD unmoved) | opus (coordinator error; recon-opus dispatched) | **the coordinator dispatched on a summary surface's stale recommendation, and the USER caught it twice before the record did** | §5.3 listed the `V_bc` elimination question as its top unrun CAS item. It was dead **the day it was written** ((D4), same day) and stayed listed four weeks — dispatched on without opening §(K-ann) or §(K-dom). The **user** caught it twice: the M2 layer was abandoned on size grounds ((ANH-16) bracketed its reach 08-06; cold since 08-07), and elimination is not one finite problem — §4.6 (R1) puts the graph quantifier in **realizability**. Killed, nothing committed; the salvage resume returned the disposition with no driver. **Status objects are gated; recommendation lists are not.** |
| 2026-09-03 | Phase39 82nd direction BDEGTWO (`36bb1af4`) | opus | **a LANDED clause overstated its own obstruction, and the coordinator relayed it -- naming the source precisely is what got it tested** |(BE-134)(i) said *no lemma covers* the `k >= 2` `p_x`-sweep and that item 1 was **two steps short**. BDEGTWO rebuilt the sweep at **411/411**: the hypothesis (BE-124)(i) leans on is not load-bearing -- `n_x` is determined but **rational**. A write-up gap, not an obstruction; one step, not two. The coordinator relayed the clause and said *rate the work by that* -- because the spec **quoted and attributed** it instead of asserting it, the direction tested rather than inherited it. Cost outlier (436k tok / 69 min) scrutinized and **benign**: 933-line driver, six modes, 612 workbook lines. |
| 2026-09-03 | Phase39 83rd direction RPOOL (`81c3e9f0`) | opus | **a per-shape proof was refuted by the SAME GENERATOR one parameter along, from the prover's own pool** | RESGRID proved (RS-5) per-shape on `W19` and varied the shape **family**; RPOOL's `R20` is `family_g(**5**,...)` where `W19` is `family_g(**4**,...)` -- same generator, **core one edge longer** -- and it sat in RESGRID's own recorded 255-pool from 2026-08-02. Second BSATUR-shape instance: the sampler varied every parameter but the one the claim turns on. **§4 sharpening: for a per-shape proof over a GENERATED family, enumerate the generator's parameters and say which were varied.** Relay defect too -- my spec quoted the row's *"resgrid.py is the instrument"*; it has no pool mode. |
| 2026-09-03 | Phase39 ninth strategy pass (this commit; no dispatch) | opus (coordinator) | **a forward-looking entry was defective ONE COMMIT after the round that swept that whole class** | The `hK` carried item read *"Four successors at (GR-144), successor 1 DEMOTED, three live"* and named only 2 and 3. The unnamed one is **4**, leaf-covering on the branches — by the gap map's own words *"the one place the landed Phase-12/13/14 machinery might reach"*, so the only argument-shaped live successor was the invisible one. The count was right and the list short, so nothing contradicted anything: a liveness sweep compares an entry against its **owner**, and this shows only against its own **count**. **Check: a hand-off saying *N live* and listing fewer — open the owner's list.** |
| 2026-09-03 | Phase39 84th direction BSCOND (`60691565`; this commit corrects it) | opus | **verdict RIGHT, cause WRONG — and the wrong cause taught a check that was never the problem** | The landing rightly caught that my reservation's hit counts were off and rightly kept its conclusion (one file, all declarations). Its diagnosis — *the prep's check predated `3c49ce60`* — is false: the counts are byte-identical at `89eb1fdb` and `3c49ce60`. My error was a metric conflation: `git grep -c` piped to `wc -l` counts **files** while I reported **hits**. The lesson it drew (*diff against HEAD*) was already satisfied; the one owed is *say which metric you report*. **Check: when a dispatch diagnoses a COORDINATOR error, verify the cause as hard as the verdict.** |
| 2026-09-03 | Phase39 concurrent round of four (count updated at OBAR's landing) | opus (coordinator) | **FIVE reservation defects in ONE round, all mine, one root cause — and TWO directions fixed the same one** | Shapes: (i) `git grep -c` piped to `wc -l` counts **files**, reported as **hits**; (ii) sampled a range's interior, missing its **opening** tokens; (iii) **wrong family** (`(D)` for §(K-dom), which uses `(DM-)`); (iv)+(v) **off by one, then two, against a DECLARED tail** — a scan saw the declaration *present* and read it as *consumed*. **Root cause: I DERIVED reservations by scanning the corpus instead of READING the tail the registry declares.** GLEAF and OBAR each deviated DOWN citing the registry. Fix: grep the registry's declared tails; scan only to confirm. |
| 2026-09-08 | Phase39 concurrent round of three, dirs BFOUR/BINSERT/BSERIES (`410aa523`, `67124a9c`, `dbd2de51`; this commit reconciles) | opus (coordinator) | **the round CONVERGED on one clause, and both of my surviving defects were caught by the dispatches I primed** | 3/3 landed serially, zero label collisions -- reservations derived by READING the registry's declared tails rather than scanning it, which retired last round's five-defect root cause. Two coordinator defects survived that fix: an **arithmetically impossible** compression target (<= 1,250 words against 219 undroppable labels = 5.8 w/label), withdrawn at BFOUR's landing; and the **sixth reservation-defect shape**, a bare `(E4)` minted INSIDE a correctly-enumerated range, which a range check cannot see -- (L6) now greps the whole repo, `*.lean` doc-comments counting.
| 2026-09-08 | Phase39 91st direction BSERIES (`dbd2de51`), self-reported | opus | **the F6 in-prompt reminder was given and the park shape still happened once** | The prompt carried the validated F6 backstop line verbatim; one `sweep` run was still launched without the Bash `timeout` parameter and auto-backgrounded. Self-caught, re-run in the foreground, **no figure taken from the parked run** -- the cost was a turn, not a defect. Recorded because F6's own remedy is *"the in-prompt line is the cheap backstop"*: this is a backstop with a leak rather than a fix, and an 8-mode numerics direction is where the leak shows. What caught it was the direction's own disclosure, no gate.
| 2026-09-08 | Phase39 93rd direction BGPROP (`04478fcf`; my defect, its catch) | opus (coordinator) | **I posed as an open check a question the corpus had already answered, and named a surface I had not read** | My spec called the proper->generic transport question *"explicitly NOT an assumption"* and sent the direction to (BE-122)/(BE-123)'s proof sites. **(BE-139)(iv) already stated that architecture and recorded inputs 1 and 2 as available at `k >= 2`**, citing (BE-123)(i) and (BE-136)(iii) -- one grep from the clause I was theorising about, found in the dispatch's first pass. Lesson, its words: when a spec names a surface it did not read, grep for a landed clause that reads it. Distinct from F22 -- there a stale summary was quoted; here the summary was right and never consulted.
| 2026-09-08 | Phase39 97th direction BGTWOA (`e585ff29`); two coordinator defects, both mine | opus (coordinator) | **I left a status paragraph stale in my OWN commit, and relayed a measured claim without opening its branch condition** | (i) `a5b071ce` bumped the `(K-bare)` cap and left the phase note's DOC DEBT paragraph reading *2,483 / 2,514, cap NOT bumped*: F17 applied to a coordinator commit, and section 7's *coordinator artifacts need a subagent's tier* measured again. BGTWOA repaired it; 8th instance. (ii) My spec relayed BEFOURP's *the radical method has an object here*, but (BE-206)'s grading is a tautology inside a `c_i = 2` branch, false at 8/23 shapes off it, so `q_hat` adds nothing. Relaying a MEASURED claim without opening its branch condition.
