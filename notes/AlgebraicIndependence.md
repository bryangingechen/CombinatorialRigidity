...
| **Phase 22e Claim 6.12 — N3a** (`lem:case-III-claim612-points-affineIndep`, KT eq. (6.45) point choice) | the four points with the `Π(a)/Π(b)/Π(c)` incidence pattern are **affinely independent** (KT p. 691) | **IS an alg-independence (genericity) site** (re-corrected 2026-06-06 after re-reading KT against `.refs`; the 2026-06-06 "N3 design pass" re-classification to "not a site / direct from `IsGeneralPosition`" was **wrong**). KT p. 691: *"Since `(Gᵥᵃᵇ, q)` is a **generic** nonparallel framework, we can take such four points in such a way that they are affinely independent"*; the general-`d` form is explicit (p. 698, eq. (6.67)): *"the set of coefficients appearing in the equations expressing `Πᵢ` is **algebraically independent over the rational field**. Therefore, for any `j` hyperplanes among them, their intersection forms a `(d−j)`-dimensional affine space."* Pairwise independence of the homogeneous ℝ⁴ normals does **not** suffice (parallel panels are pairwise-independent yet have coplanar/empty triple intersection). So N3a `\uses{lem:genericity-device}`; the affine-independence determinant is nonzero by alg-independence of the panel coefficients. (N3b, the point-join↔panel-meet duality, stays alg-independence-free — pure Grassmann–Cayley.) **Genericity-free halves green** (existence `exists_ne_zero_dotProduct_eq_zero`; closure `exists_affineIndependent_of_det_polynomial_ne_zero` — the latter takes the det-is-a-nonzero-poly as hypothesis and runs `MvPolynomial.exists_eval_ne_zero`). **Sole residual: `P ≠ 0`** (the homogenization determinant, as a poly in the seed, is nonzero). | **candidate: §2 product-route** — like the Phase-22d kernel, the residual is "fold the incidence/affine-indep determinant into the device's non-root product" at one fixed seed; the closure brick (`exists_eval_ne_zero` composition) is already green |
