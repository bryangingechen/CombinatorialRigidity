...
| **Phase 22e Claim 6.12 — N3a** (`lem:case-III-claim612-points-affineIndep`, KT eq. (6.45) point choice) | the four points with the `Π(a)/Π(b)/Π(c)` incidence pattern are **affinely independent** (KT p. 691) | **NOT an alg-independence site** (re-classified 2026-06-06, N3 design pass) — general position comes **directly from `IsGeneralPosition`** (pairwise-independent panel normals `n_a,n_b,n_c`), the same panel-layer GP predicate the framework already carries; three transversal hyperplanes meet in an affine point + lines, and a determinant on the homogenized coords gives affine independence. **No transcendence / `AlgebraicIndependent` needed.** The split's other half N3b (`lem:case-III-claim612-line-in-panel-union`, the point-join↔panel-meet duality) is pure Grassmann–Cayley, also alg-independence-free. | n/a — not a site |
