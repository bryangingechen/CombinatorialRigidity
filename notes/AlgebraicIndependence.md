...
| **Phase 23** — KT Lemma 6.13, general `d` (the length-`d` chain) — **two distinct sites** | (a) the footnote-6 seed-rank transfer along the chain `v₀…v_d` (the general-`d` lift of `case_III_nested_rank_lower`, which *already* consumes `AlgebraicIndependent ℚ q` at `d=3`); (b) the eq. (6.67) **N3a points-in-general-position** step — KT p. 698: *"the set of the coefficients … is algebraically independent over the rational field. Therefore, for any `j` hyperplanes among them, their intersection forms a `(d−j)`-dimensional affine space."* | **open / scoped** (Phase-23 design recon 2026-06-17, `notes/Phase23-design.md` OD-4; **`CHAIN`=23b opened 2026-06-17, OD-4 re-confirmed open — routed to the CHAIN-4 build, not resolved**) | (a) the alg-independence machinery is **live** regardless (the `d=3` kernel already uses it; the 23a-lifted `case_III_nested_rank_lower_all_k` consumes `AlgebraicIndependent ℚ q`) — lifts to general grade in `CARRIER`(done)/`CHAIN`. (b) **uncertain whether a NEW site, CHAIN-open finding: leaning toward forced**: the `d=3` N3a was **AVOIDED** (existence/Zariski route — exhibit one explicit seed where the points are affinely independent, row #106) via an explicit construction (`p₁`=triple-intersection, `pᵢ=p₁+sᵢ·(nⱼ×nₖ)`); the 23b recon checked this does **not** obviously generalize — at `d+1` panels in `ℝ^d` the "intersection of `d−1` is a line" needs the `j`-hyperplanes-meet-in-`(d−j)`-flat fact, which is *exactly* KT's alg-independence consequence. If forced, it is a `Fin (d+1)` generalization of the existence brick (`exists_affineIndependent_of_det_…`) possibly needing `AlgebraicIndependent`-driven non-vanishing of the `d+1` homogenization dets — a CHAIN-4 sub-leaf, not new infra (the seed `q` is the IH-generic base, already alg-indep-carrying). §2 risk (c) (does the touched-subgraph family grow with `d`?) is exactly site (a)'s relaxation question. Decided at the CHAIN-4 detailed-build recon. |
