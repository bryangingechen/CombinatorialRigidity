...
| **Phase 22e Claim 6.12 — N3** (`lem:case-III-claim612-points`, KT eq. (6.45) point choice) | general position of the realization: from a *generic nonparallel* `(Gᵥᵃᵇ,q)`, four points with the `Π(a)/Π(b)/Π(c)` triple-intersection incidence pattern can be taken **affinely independent** (KT p. 691), and every connecting line lies in `Π(a)∪Π(b)∪Π(c)` | future (this sub-phase, **research-shaped**, recon-before-build) | **likely** — this is a *general-position* (configuration non-degeneracy) use, the same `IsGeneralPosition` device Cases I/II already discharge via the existence/Zariski route (§3 precedent rows), **not** a "this seed attains the rank" use; a finite set of non-degeneracy non-root conditions, fold into the candidate product (§2 risk (a)) |
