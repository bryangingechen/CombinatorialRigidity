...
| **Phase 22d kernel** — KT Claim 6.11, eq. (6.22)/(6.18), footnote 6 (`lem:case-III-seed-rank-bridge`) | the inductively-fixed seed `q` attains the IH/matroid-predicted rank of nested subgraphs (`G_v^{ab}`, `G_v`) — so a redundant `ab`-row exists | **first forced site; being built** via the alg-independence route — leaf (i) `AlgebraicIndependent.aeval_ne_zero` ✓ landed (mirror); (ii) **SPLIT** by the opening recon (`Phase22d.md` *Kernel sub-phase (ii) recon*; §1.32) into (ii-a) seed-genericity motive conjunct + (ii-b) a **rationality bridge** (the device `Q : MvPolynomial σ ℝ` exhibited over ℚ so leaf (i) applies — a new obligation: zero `algebraMap ℚ ℝ` scaffolding tree-wide); (iii) the kernel bridge next | **candidate: §2 product-route** (~70% at `d=3`) — finitely many subgraphs, `q` chosen at composition |
...
- The Phase-22d kernel sub-phase builds (i)+(ii)+(iii) directly. Leaf (i)
  ...
  bridge (next build = (ii-b)'s `eval = aeval ∘ descend` descent mirror); (iii) after
