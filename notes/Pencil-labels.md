...
**The FOURTEENTH direction — GDESC — PREPPED 2026-08-18, NOT yet
dispatched** (`notes/Pencil-fanout.md` §"Fourteenth direction"), so
this is a **live reservation**, not a landing record: **(GR-46)+** and
**Steps G63+** — §(K-grid)'s unclaimed tails after GPSA consumed
(GR-44)–(GR-45) / G58–G62 — are **RESERVED** for it, driver
`w4/gdesc.py`, suggested modes
`--stuck`/`--cases`/`--opt`/`--adv`/`--validate` (the direction sets
the final list; the registry row below is updated at *landing*, not
now). A **single direction**, not a fan-out. **No selection was made:
...
the deliverable changed). The direction *extends* §(K-grid), so its
reservation is that section's unclaimed tails and the owning section
stays authoritative. **On outgrowth reuse the already-reserved
§(K-gcap) / `GC-`** — returned unopened by GCAP, GUNIF, GEXIST,
GORIENT, GDEV, GADM and GPSA, available an **eighth** time. **Do not
mint §(K-unif)/`GU-`** — considered twice, deliberately not minted,
and this prep does not revive it. No M2 leaf expected — the target is
