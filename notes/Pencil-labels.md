...
**The THIRTEENTH direction — GPSA — PREPPED 2026-08-18, NOT yet
dispatched** (`notes/Pencil-fanout.md` §"Thirteenth direction"), so
this is a **live reservation**, not a landing record: **(GR-44)+** and
**Steps G58+** — §(K-grid)'s unclaimed tails after GADM consumed
(GR-43) / G53–G57 — are **RESERVED** for it, driver `w4/gpsa.py`,
suggested modes `--sdr`/`--balance`/`--odd`/`--adv`/`--validate` (the
direction sets the final list; the registry row below is updated at
*landing*, not now). A **single direction**, not a fan-out.
...
*extends* §(K-grid), so its reservation is that section's unclaimed
tails and the owning section stays authoritative. **On outgrowth reuse
the already-reserved §(K-gcap) / `GC-`** — returned unopened by GCAP,
GUNIF, GEXIST, GORIENT, GDEV and GADM, available a **seventh** time.
