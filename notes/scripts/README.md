...
| `dz_gadget`, `sample_dz_pencil` | `danger` | `optc`, **`breakhunt`**, **`w4/battain`**, **`w4/bzavoid`** (4 — BATTAIN was the FIRST `w4/` consumer of a `kbare/` *driver*; earlier `w4/` consumers reached only the `kbare_common` model layer, so this is a **cross-stack** edge, recorded 2026-08-26. BZAVOID is the **second**, which settles the question the BATTAIN row left open: the cross-stack edge is a **pattern, not a one-off**, so the move-down is now load-bearing rather than tidy) |
