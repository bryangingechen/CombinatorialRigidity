...
| the 5/6-sparsity **slack** `σ(F) = 6·r(F) − Σ_F(6 − ℓ) = Σ_F ℓ − 6·c(F)` and the graphic rank, over every branch subset of a hub multigraph (the bound in (GR-129)'s signing criterion; `gridcol.nash_williams_ok` is the same inequality as a yes/no) | `sigma_table` | `w4/gpack` |
...
| `python3 notes/scripts/w4/weloc.py --validate` | 1 s | ibid. §"widened kernels (routes 1/3)" *Steps EL1–EL6* (direction WELOC; the (EL-2) identities, the minimal-dependent-set branch enumeration against brute force, the Ear Lemma threshold, (EL-1) on 533 `hcard` graphs) |
| `python3 notes/scripts/w4/weloc.py --witness` | 2 s | ibid. (`T32`, the (E-loc) refutation, every `hnoGood'` clause on three rigidity oracles) |
| `python3 notes/scripts/w4/weloc.py --brick` | 33 s | ibid. (the (EL-4) falsification audit — 374 hub-cycle carriers through *Step EL4*'s chain, 0 assertion failures) |
| `python3 notes/scripts/w4/weloc.py --pool` | 54 s | ibid. (the (E-loc) denominator disclosure, 255 → 160 / 95 / **0**) |
| `python3 notes/scripts/w4/weloc.py --hunt` | 8 s | ibid. (the shape-1 family: 24 refuting residuals, mixed `C₄`/`C₅` cores) |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/wpair.py --validate` | 25 s | ibid. *Steps PR1–PR6* (direction WPAIR; (PAIR-1)'s deficit identity on 33 299 instances, (PAIR-2) on 48 133 rigid sets / 14 521 simple contractions, (PAIR-3)'s monotone invariant over 22 359 chain runs — 0 mismatches) |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/wpair.py --sub` | 98 s | ibid. ((PAIR-4)'s proved stratum: 220 full subdivisions, 173 836 rigid sets, all with `∂_hub = ∅`, no residual) |
...
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/gpack.py --split` | 16 s | `notes/Pencil-informal-grid.md` §(K-grid) *Steps G149–G150* (direction GPACK; (GR-129)/(GR-130): the sparsity slack `σ(F) = Σ_F ℓ − 6c(F)` asserted `≥ 0` with `σ(E) = 0` at 907/907 shapes **exhaustively over all `2^M` branch subsets** and agreeing with `gridcol.nash_williams_ok`; the signing criterion `\|s(F)\| ≤ σ(F)` compared with `saferes.union_rank`'s verdict on BOTH sides at **every** balanced signing of every shape — 7 291/7 291 instances, 0 disagreements; a legal signing asserted to exist at 907/907, with the split asserted equitable, length-legal and BALANCED; and the theorem's own matroid step (`H₀` independent in `N₃`, the `rank_M` closed form, Edmonds' `\|S\| ≤ 2 rank_M(S)`) asserted at 200 sampled `S`) |
...
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/btwocut.py validate` | 117 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE24–BE28* (direction BTWOCUT; reduced tiers of all eight modes — the pinned strengthened statement's checks, the ear fix, the moduli count, the cross-pair battery and the obstruction hunt). **`binduc` imported read-only.** The three exhaustive headlines are the rows below |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/btwocut.py spqr` | 403 s | ibid. *Step BE24*, the **EXHAUSTIVE** leaf-base tier: **19 696** (3-connected `B`, edge `uv`) leaf blocks `B − uv` on `n = 4…6`, **ZERO** with `def₂ > 0` or `def₃ > 0`, bound measured **tight** at `−1`, plus 6 721/6 721 carrying S1. Its own disclosure: this is base 1's argument confirmed, **not** independent evidence about the flexible case |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/btwocut.py crosspair` and `hunt` | 404 s + 190 s | ibid. *Steps BE27–BE28*: the cross-pair welding clause at **13/13** battery graphs (21–91 pairs each) and **1 200/1 200** census instances with zero violations, two deficiency oracles cross-checked at **408 080** (graph, pair) instances; and the obstruction hunt, **13 484/13 484** both-`δ`-positive instances attaining (11 896 exhaustive + 1 588 both-sides-hubbed), zero shortfalls, under an explicit **F27 escalation** (one cheap draw, residuals only multi-seeded). **Run these two separately — together they exceed the 600 s foreground budget** 
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bimage.py validate` | ~125 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE29–BE33* (direction BIMAGE; reduced tiers of all nine modes). **`btwocut` and `binduc` imported read-only.** The two enumerating headlines are the rows below |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bimage.py combi` | 36 s | ibid. *Step BE31*, the **EXHAUSTIVE** combinatorial tier: all connected labelled graphs on `n = 3…6` (27 474 graphs / 408 080 pairs) plus 3 000 seeded masks at each of `n = 7, 8` — **542 893** `(graph, pair)` instances, five predicates, **ZERO** violations, including the necessary condition **`δ_{uv} ≤ dist(u,v)`** for S-all/S-mark. Cap: exhaustive to `n = 6` only, so it reads *not found under the cap*, never *none exists* |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bimage.py probe` | 282 s | ibid. *Step BE33*, the **F27 escalation**: **607/607** distinct trapped `(side 1, m)` rows reach the criterion **and** the whole-graph target under 2 independent ladder seeds × 16 ear draws — every apparent trap a **configuration artifact**. Cap: the census is subsampled to 420 instances (seed printed), so the 607 are that subsample's trapped rows, not the census's ||
...
| invocation | ~time | cited by 
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearcase.py validate` | 117 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE34–BE37* (direction BEARCASE; reduced tiers of all seven modes). **`bimage`, `btwocut` and `binduc` imported read-only** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearcase.py twostep` | 38 s | ibid. *Step BE34*, the **ENUMERATING** tier for the 2-step lemma that closes (α): **2 205** `(W, M)` comparisons of predicted against measured reach over seven structured families, **0 mismatches**, and the `(dim W, dim C, gain)` shape set **enumerated** — 14 feasible triples, 14 realized, **0 missing, 0 unpredicted** (a forbidden triple would `assert`) |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearcase.py greedy` and `corners` | 41 s + 90 s | ibid. *Step BE35*: the reordering-and-slide chain **constructed**, reaching the classification's predicted maximum at **1 432/1 432** with **every restart count 1** and 0 excess (an excess would refute the classification); and **1 067** targeted `m ≤ 2` instances with `A` *built* to hit each named corner, agreeing at 1 067 — **no fourth trapping mechanism found under that cap** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearcase.py betahunt` | 165 s | ibid. *Step BE37*, the falsification arm on a sampler **independent of the BTWOCUT ladder** (a free parametrization of the pencil stratum for subdivisions, so it owes the constructor nothing): 24 pieces × 14 generic draws, including **R-node** pieces (subdivided `K₄` / `K_{3,3}` / prism), **ZERO candidates**. Cap: excludes pieces with **adjacent branch vertices**, and `δ₁` is the measured max, which **over**-reports candidates — the conservative direction ||
|---|---|---|
| `M2 --script notes/scripts/m2/lambda1.m2` | 1 s | workbook §(K-Λ) *Step 2* ((Λ1) as an identity over the function field) 
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearfull.py validate` | 467 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE38–BE42* (direction BEARFULL; reduced tiers of all eight modes). **`bearcase`, `bimage`, `btwocut`, `binduc`, `bzavoid` imported read-only** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearfull.py merge` and `cycles` | 39 s + 24 s | ibid. *Steps BE38–BE39*, the partition-lattice layer: the **quotient sparsity law** at **130 331 ENUMERATED** part-subsets of **28 633** optimal partitions, **0 violations**, **61** tight, girth histogram `forest: 28 572, 6: 61` with **0** below 6; the **join lemma** and `δ_uv = 0 ⟺` same block of `P_max` at **435 072** pairs; and the **SHORT-CYCLE LAW** — **241 771 ENUMERATED** `(cycle ≤ 5, optimal partition)` incidences with **0** splits, **294 500** `≤6`-cycle-class pairs with **0** having `δ ≠ 0`, and **61 770** non-adjacent pairs with `≥ 2` common neighbours (which (BE-32)(iii) does *not* cover) **0** with `δ ≠ 0` |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearfull.py forced` and `chain` | 9 s + 1 231 s | ibid. *Steps BE40*: (BE-32)(+) for the **named** aggressive closure — **401 489 of 401 544** forcing steps and **196 043 of 203 723** forced pairs **PROVED**, **100 % of the exhaustive `n ≤ 6` tier**; the **55** SPREAD steps are the residue. `chain` locates the boundary (a triangle-chain family whose admitting step is **asserted** genuine and whose shortest cycle is 7 or 8, `δ = 0` anyway) and runs a blind hunt in the `def₃ > 0` band at `n = 9…13`, **27 414** forced pairs, **0** escapes. **Run these two separately — `chain` alone exceeds the 600 s foreground budget** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearfull.py eardec` and `chord` | 3 s + 25 s | ibid. *Step BE42*, the routing verdict: **12 620** 2-connected graphs **decided** per graph, **6 968** chord-free and **every one** with a degree-2 vertex, **2 084** 3-connected graphs with **ZERO** chord-free; plus `def₃(G+uv) = max(def₃(G) − δ_uv, def₃(G) − 5)` **EXHAUSTIVE** at **189 587** `(graph, non-edge)` instances, **0** violations, and **128** generic pencil draws **DECIDED** exactly with **0** admitting the chord |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bearfull.py bgrass` | 394 s | ibid. *Step BE41*: the Grassmann bound held as an **exact** inequality at every draw of **19** pieces, all with `dist_{G₁}(u,v) ≥ 5` — the regime (BE-38)(ii) does **not** reach — **TIGHT at 144** draws, **0** failures of (b2). Cap: bearcase's sampler shape guard, and `δ₁` measured (the conservative direction) ||
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bsharp.py validate` | 282 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE43–BE47* (direction BSHARP; reduced tiers of all five modes). **`bearfull`, `bearcase`, `bimage`, `binduc`, `bzavoid` imported read-only** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bsharp.py red` and `split` | 46 s + 61 s | ibid. *Steps BE43–BE44*: the reduction, and the DICHOTOMY. **37** pieces, one guarded draw each; the per-path invariant `dim⟨P⟩ = min(\|P\|,6)` and `dim(⟨P⟩ ∩ Π_u) = 1 ⟺ dim⟨P⟩ ≤ 5 ⟺` the factor **is** `⟨ℓ_first⟩` **ASSERTED** at every path of every piece; the reduction proves `0` at **8** and is loose at **6**. `split` **ASSERTS** the dichotomy — a mechanism fires **iff** `dim(ρ̄₁ ∩ Π_u) ≥ 1` — with **0 mismatches**: (M1) alone at 3, (M2) alone at 17, both at 6, neither at 11 (8 of those proved, 3 measured only) |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bsharp.py gen` | 456 s | ibid. *Step BE45*, the genericity proviso: **37 pieces × 8 independent draws = 296** guarded configurations, **11** exhibiting `ρ̄₁ ∩ Π_u = 0` (each a proof of that shape's stratum, by openness + irreducibility), **0** pieces whose value varies over the attaining draws |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bsharp.py hyp` and `adj` | 66 s + 24 s | ibid. *Steps BE46–BE47*: the coverage classification over **49** pieces — **0** fail (b2), **0** unclassified, **5** in the named residual window, where `ρ̄₁ ∩ Z = ⟨ℓ_u, ℓ_v⟩` is verified as an **identity of spaces** at 5 of 5. `adj` is the **first parametrization past bearcase's shape guard** (adjacent branch vertices): **12** pieces × **6** draws, **0** dichotomy mismatches. Cap: branch subgraph of max degree `≤ 2` |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/brule.py validate` | 330 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE48–BE52* (direction BRULE; reduced tiers of all five modes). **`bsharp`, `bimage` imported read-only, and `bearfull` / `bearcase` / `btwocut` / `binduc` / `bzavoid` / `kbare_common` through them** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/brule.py dom` and `domin` | 1 s + 1 s | ibid. *Steps BE48 / BE50*: (b3)'s honest domain and the (R)/(Z) dominance. `dom` **ASSERTS** the ruling geometry as identities of spaces at 12 flag draws (`Π_u = p_u ∧ L`, `E = M ∧ L`, `M`/`L` skew, all same-/opposite-ruling incidences, the vertex-based shape test, and `other` impossible), plus **278** coplanar-pencil meetings certifying that at `π_u = π_v` the trap is (Z) and (b3) must be **struck**. `domin` builds 3- and 4-dimensional `W ⊆ E` **containing** a `y ∧ L` and checks the LANDED `bimage.bad_predicate` against `reach_measured` at **8/8 + 8/8** — the equality test's under-report of (R) is absorbed by (Z), so no landed figure moves |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/brule.py sep` | 58 s | ibid. *Step BE49*, the SEPARATION THEOREM: **49** pieces, one guarded draw each (47 on the honest domain), the shape of `ρ̄₁ ∩ E` computed by the **Klein-form + vertex** test and the theorem **ASSERTED** per row. **0** (b3) failures; **0** rows where a 2-dimensional `ρ̄₁ ∩ E` is even totally singular; the corrected containment form fires at **11** rows, all `dim(ρ̄₁ ∩ E) = 4` with `δ₁ = 6` (the vacuous corner). The contrapositive guard fires non-vacuously at **33** rows |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/brule.py wit` and `hunt` | 228 s + 415 s | ibid. *Steps BE51–BE52*: **49 pieces × 4 = 196** guarded configurations exhibiting (b3) — each a proof of that shape's stratum by openness + **properness** + irreducibility — with **0** pieces varying over draws; and the falsification arm at 8 seeds, **392** guarded draws, **111** at the no-mechanism pieces the separation theorem leaves open, **3** discarded as cross-incident, **0** failures. Caps: bearcase's shape guard on 37 pieces, (BE-48)(i)'s branch-degree-`≤ 2` guard on 12, `δ₁` measured per draw. **Run these two separately** ||
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bwin.py validate` | 112 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE53–BE57* (direction BWIN; reduced tiers of all five modes). **`bsharp`, `bimage`, `bearcase`, `binduc` imported read-only, and `bearfull` / `btwocut` / `bzavoid` / `kbare_common` through them** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bwin.py cls` and `sweep` | 164 s + 100 s | ibid. *Steps BE54/BE56*: the theorem's construction run literally — the window identity, (b1) and `dim Z = 4` **asserted at 160/160** fresh end draws over fixed middles (each re-gated through both gates), and the per-`L` criterion (identity ⟺ `excess ≤ 1`) asserted at 84/84 resweeps with the three MUST-FAIL `δ₁ ≥ 5` controls failing at every end draw; the end-choice lemma `(ℓ_u ∧ ℓ_v)^{⊥K} = ⟨ℓ_u, ℓ_v⟩` asserted at every skew resweep. **Run these two separately** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/brnode.py validate` | 17 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE58–BE62* (direction BRNODE; reduced tiers of all four modes — `carve`'s `n = 6` tier there is named shapes + a disclosed 300-graph sample). **`bwin` (`dehom`), `bsharp`, `bimage`, `binduc` imported read-only, and the rest of the chain through them** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bspread.py validate` | 54 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE73–BE77* (direction BSPREAD; reduced tiers of all three modes — the block-absorption lemma enumerated over every optimal partition and block, the strong form of (BE-32)(+) under four closure operators with the threshold-2 SHARPNESS control asserted to FAIL, job 0's boundary-family correction re-derived off `bearfull`'s own objects, and the peel factorization + the `δ₁ = δ₂ = 1` confinement). Full tiers `lemma` 67 s, `chain` 3 s, `peel` 33 s. **`bearfull` (incl. `optimal_partitions`), `bpeel`, `btwocut`, `bimage` (`forced_same_plane`), `bzavoid` imported read-only, and the rest of the chain through them** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/boneone.py validate` | 54 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE78–BE82* (direction BONEONE; all three modes — the per-side FACTORIZATION asserted at 400/400 glued pairs against the whole-graph oracle, the two witnesses `WIT11` / `WIT16`, the SIZE bound checked by exhaustive enumeration of 38 580 R-node-shaped sides, the path-side law and the structural zero-chances assertion run over `bpeel.constructed_tier` itself, and the exhaustive forcing census at n = 11, 12). Modes `side` 1 s, `bound` 52 s, `force` 1 s. **`bspread` (`closure_trace`), `bpeel` (`rnode_shaped` / `peel_scan` / `pair_forced` / `constructed_tier` / `subdivided` / `SKELETONS` / `hubs_of`), `bearfull` (`step_shape`), `btwocut`, `binduc` (`split_at_pair`) imported read-only, and the rest of the chain through them** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bunif.py validate` | 66 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE93–BE97* (direction BUNIF; reduced tiers of all five modes — the flag pair's stabilizer with the four-block direct sum, flag fixity and `Λ²g`-equivariance and the ONE-SIDED reglue through both gates all ASSERTED; the block CAP and the degeneration bound bracketing the measured `dim(ρ̄₁+ρ̄₂)` at every peel row of two populations, with `cap == deg` counted as the law proved at that row; the violation-margin histogram ASSERTED `≤ 0`, the per-side non-generic census and job 2's stress test of (BE-38)(iii)'s third clause; the coincident-regime degeneration identities with two-sided (Z) exhibited on ear pairs; and the abstract `max_g dim(V₁+gV₂) = cap` test off the graphs). Full tiers `equiv` 41.6 s, `law` 48.4 s, `bind` 30.4 s, `coin` 0.2 s, `abst` 103.2 s. **`bdecor` (`R_BATTERY` / `nested_piece` / `sample_by_branches` / `assemble` / `d3` / `weld_d3`), `bpeel` (`peel_sides` / `constructed_tier`), `bimage` (`rho_bar_of` / `plane_at` / `plane_meet` / `pencil_space` / `lam2` / `sample_flags` / `sample_ear_span` / `span` / `dim` / `isect` / `same_space` / `contains`), `binduc` (`split_at_pair`), `bwin` (`dehom`) and `kbare_common` imported read-only, and the rest of the chain through them.** The only new primitives are the four-block flag frame, `Λ²g`, and the two bounds. Exact ℚ; the single printed seed is `20260902` |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bsatur.py validate` | 137 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE103–BE107* (direction BSATUR; reduced `scan` tier, all five modes — the PINNED REFUTATION of (PENCIL-SATURATES) at `K4(5,3,3,3,3,3)` peeled at its 5-branch, with the constructed plane ASSERTED equal to `plane_at`'s closed-star plane of `x` in the whole graph, both `bdecor.assemble` gates, the GENERIC flag regime, `Π_x ⊆ ρ̄₁` as SPACES at `ρ₁ = 5`, attainment, and the F13 negative control that the sampler's own random plane gives `c₁ = 1`; the THRESHOLD `dim(ρ̄ ∩ Σ_x) = max(1, ρ − 3)` ASSERTED at every degree-1 terminal of BEARCASE's 24-piece battery and `Π_u = ⟨ℓ_e, ℓ_{e'}⟩` as spaces at every degree-`≥2` one; the bad-flag SCAN over subdivided `K₄`/prism/`K_{3,3}` with `margin ≤ 0` at four blocks and `reach = min(δ₁+δ₂,6)+a₁+a₂` ASSERTED at every row; the enumeration of what the refutation costs (BE-101)(i), with both controls reproducing its own `300/300, 0` and `313`; and job 2's `⟨M⟩` hunt over BUNIF's two populations). Full tiers `witness` 1.6 s, `mech` 77.8 s, `arith` 0.0 s, `mhunt` 26.5 s, `scan` 131.7 s. **`bunif` (`battery_rows` / `tier_rows` / `measure_row` / `blocks_of` / `flag_frame` / `profile` / `SEED`), `bearcase` (`piece_battery` / `sample_piece_config`), `bpeel` (`SKELETONS` / `subdivided` / `delta_pair` / `rnode_shaped`), `bdecor` (`assemble` / `d3` / `draw_branch` / `flag_assignment` / `hubs_and_branches` / `weld_d3`), `bimage` (`rho_bar_of` / `plane_at` / `pencil_space` / `span` / `dim` / `isect` / `contains` / `same_space`), `binduc` (`split_at_pair`) and `exactcore` imported read-only, and the rest of the chain through them.** The only new primitives are `Σ_x`, the wedge inverse and the bad plane. Exact ℚ; the single printed seed is `20260902` |
| `python3 notes/scripts/gapdiff.py [gap-key] [git-ref]` | 1 s | **A GATE, not a figure source.** The scripted label set-diff F21 requires when a `notes/Pencil-informal.md` gap-map row is recomputed: it reads the row at `git-ref` and in the working tree through `check-gapmap-cells.py`'s own `iter_row_cells`, so the two agree on what a row and a cell are, and exits 1 if any label was DROPPED (added labels are reported, never an error). Its label regex is stated in its docstring so the count is reproducible. Shipped 2026-09-02 with BSATUR because BDOUBLE's equivalent probe was **not retained** and the next landing had to write one again |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bdouble.py validate` | 33 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE98–BE102* (direction BDOUBLE; reduced tiers of all four modes — the exhaustive arithmetic behind reading (1)'s equivalence and its `δ₁+δ₂ ≤ 6` threshold, the REDUNDANCY THEOREM (every `Π_x` violation is a `U = Λ²K⁴` violation under (PENCIL-SATURATES), 300/300 caught, 0 escaping) with its F13 negative control at 313 escapes, and the `a = 0` denominator correction to (BE-97)(i)'s triage; the PINNED double-pencil witness at `K4 + th(6,6,6)/ab + ear4/ua` peel `(a,b)` with `ρ̄₂ = Λ²K⁴` asserted as a SPACE and the margin `−1` at both 2-blocks; the double-pencil hunt over BUNIF's two populations with `margin ≤ 0` and `slack` ASSERTED at every hit; and reading (2)'s weak link refuted against `bimage.dist_in`, with (BE-30)'s `ρ_i ≤ min(dist_i, 6)` ASSERTED at 184/184 sides). Full tiers `arith` 0.0 s, `witness` 3.9 s, `hunt` 28.5 s, `price` 26.2 s. **`bunif` (`battery_peels` / `battery_rows` / `tier_rows` / `measure_row` / `SUBS` / `CAP` / `SEED`), `bimage` (`dist_in` / `contains` / `dim` / `same_space`), `bdecor` (`sample_by_branches`), `bpeel` (`peel_sides`) imported read-only, and the rest of the chain through them** — the FIRST external consumer of any `bunif` device. No new primitive: the block frame, the profile `c_i(U)` and both bounds are BUNIF's. Exact ℚ; the single printed seed is `20260902` |
| `python3 notes/scripts/w4/bbase.py validate` | 33 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE88–BE92* (direction BBASE; reduced tiers of all five modes — the `B_real` census over the class tier with job 2's transport identity asserted per vertex, the complete-intersection dimension by exact-ℚ Jacobian rank, the flag tower's constant-dimension fibre `F_Π` with reading (3)'s constructed `p₁ = p₂` coincidence, the short-cycle reducibility certificates with their F13 adversarial pair, and cross-sampler agreement with `bdecor.flag_assignment`). Full tiers `census` 120 s, `dim` 1 s, `forest` <1 s, `tri` <1 s, `cross` <1 s. **`bdecor` (`hubs_and_branches` / `hub_adjacency` / `flag_assignment` / `girth` / `R_BATTERY` / `nested_piece`), `bpeel` (`SKELETONS` / `split_at_pair` / `constructed_tier`), `gridcol` (`class_shape`), `cflank` (`length_tuples`), `bimage` (`v4` / `pt_in`), `kbare_common` (`verts_of`) imported read-only, and the rest of the chain through them** — the first `w4/` driver to import **both** `gridcol`'s class filter and `bpeel`'s peel machinery, which is what makes the class-tier census possible. Exact ℚ; the single printed seed is `20260902`; no `set` is printed unsorted, so no `PYTHONHASHSEED` pin is needed |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bgenuine.py validate` | 145 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE83–BE87* (direction BGENUINE; all four modes — the hinge-pair certificate census over BONEONE's whole 928-member family with the geometric rank re-checked at a guarded draw per forced witness, the flat-`A` chart parametrization with its confinement (3 580/3 580) and freedom (1 100/1 100) controls and the (CH-1) clause census, the shortfall at all 392 forced witnesses with `dim(ρ̄₁+ρ̄₂) = 2+a₁+a₂` / `ρ̄₁ ∩ ρ̄₂ = 0` as spaces / `ρ_i = δ_i+a_i` / `dim M(H) = 6+def₃(H)` all four ASSERTED, and the new `(10,5)` row at `n = 13`). Modes `cert` 1 s, `chart` 4 s, `bite` 106 s, `wider` 35 s. **`boneone` (the factorized generator and `wit11`), `bspread` (`closure_trace`), `bpeel` (`hubs_of` / `rnode_shaped`), `bearfull` (`step_shape`), `bdecor` (`girth`), `bimage` (`rho_bar_of` / `plane_at` / `isect` / `same_space` / `span` / `dim` / `v4` / `pt_in`), `bwin` (`dehom`), `btwocut` (`g_exact`), `binduc` (`assert_generic_star`) and `kbare_common` imported read-only, and the rest of the chain through them.** Exact ℚ; the single printed seed is `20260902` |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bpeel.py validate` | 39 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE68–BE72* (direction BPEEL; reduced tiers of all five modes — the dichotomy's semicontinuity signature, the peel-independence CROSS-MIX test, the 2-cut `δ` law, the forced-coincidence enumeration with its non-vacuity check, and the proviso-`G` control). Full tiers `open` 39 s, `indep` 40 s, `law` 5 s, `force` 12 s, `gate` 2 s. **`bdecor`, `btwocut`, `binduc` (incl. `flat_forcing_closure`), `bimage`, `bzavoid` imported read-only, and the rest of the chain through them** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/bdecor.py validate` | 182 s | `notes/Pencil-informal.md` §(K-bare-ext) *Steps BE63–BE67* (direction BDECOR; reduced tiers of all five modes — the branch-product theorem both ways incl. the MIX converse, the theta child's generic law, the small-branch confinement, the (CH-1) hypothesis check + cross-sampler agreement, and half (B) at R-node peels with theta children). Full tiers `prod` 33 s, `theta` 22 s, `small` 20 s, `chart` 166 s, `attain` 15 s. **`brnode`, `bsharp`, `bimage`, `binduc`, `bwin` (`dehom`), `widened` (`place_pencil_general`), `nogood_subdiv` (`deficiency`) imported read-only, and the rest of the chain through them** |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/brnode.py carve full` | 4 s | ibid. *Step BE60*: the (BE-61)(ii) enumeration, EXHAUSTIVE at `n = 4, 5, 6` — 1 + 26 + 1 768 labelled 3-connected skeletons (cross-checking btwocut's 1 795 census), 198 360 `(B, uv, e)` triples, `δ_{xy}(B − uv − e) = 0` at 20.0 % / 70.9 % / 84.8 % |
| `PYTHONHASHSEED=0 python3 notes/scripts/w4/resgrid.py --audit` | <1 s | `notes/Pencil-informal-grid.md` §(K-res) *Step RS1* (direction RESGRID; the habitat table for `W19`/`S29`/`NT21c3` + the θ(2,3,7) control; (RS-3)(ii) `max f(W) ≤ index + def` asserted per shape; the (GR-18)/(GR-25) sparsity input asserted to FAIL at the `W19`/`S29` cores, `Σ(6−ℓ) = 8 > 6`). **`grid`, `gridwit`, `closure`, `widened`, `saferes`, `dominance` imported read-only** |
...
| `two_core` (the `T32` witness family) | **`w4/weloc`** | **`w4/wpair`** (1 — the **FIRST external consumer of any `weloc` device**, recorded 2026-09-02 at the WPAIR landing, the day after `weloc` landed; below the §2 rule-2 threshold, recorded so the second consumer trips it rather than rediscovering it. `wpair` imports it **read-only**, for the `--pool` and `--vee` regression rows only) |
| `dz_gadget`, `sample_dz_pencil` | `danger` | `optc`, **`breakhunt`**, **`w4/battain`**, **`w4/bzavoid`**, **`w4/binduc`**, **`w4/btwocut`**, **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`** (9 — BATTAIN was the FIRST `w4/` consumer of a `kbare/` *driver*; earlier `w4/` consumers reached only the `kbare_common` model layer, so this is a **cross-stack** edge, recorded 2026-08-26. BZAVOID is the **second**, which settles the question the BATTAIN row left open: the cross-stack edge is a **pattern, not a one-off**, so the move-down is now load-bearing rather than tidy. **BEARFULL, 2026-08-27, is the ninth and the chain is SEVEN deep** — `battain → bzavoid → binduc → btwocut → bimage → bearcase → bearfull`. **BSHARP, 2026-08-28, is the tenth and the chain is EIGHT deep**, `… → bearfull → bsharp`; **BRULE, 2026-08-28, is the eleventh and the chain is NINE deep**, `… → bsharp → brule`; **BWIN, 2026-08-29, is the twelfth and the chain is TEN deep**, `… → brule → bwin`; **BRNODE, 2026-09-01, is the thirteenth and the chain is ELEVEN deep**, `… → bwin → brnode`; **BDECOR, 2026-09-01, is the FOURTEENTH and the chain is TWELVE deep**, `… → brnode → bdecor`; **BPEEL, 2026-09-01, is the FIFTEENTH and the chain is THIRTEEN deep**, `… → bdecor → bpeel`; **BSPREAD, 2026-09-01, is the SIXTEENTH and the chain is FOURTEEN deep**, `… → bpeel → bspread`; **BONEONE, 2026-09-01, is the SEVENTEENTH and the chain is FIFTEEN deep**, `… → bspread → boneone`; **BGENUINE, 2026-09-01, is the EIGHTEENTH and the chain is SIXTEEN deep**, `… → boneone → bgenuine`; **BBASE, 2026-09-02, is the NINETEENTH (SEVENTEEN deep)**, **BUNIF the TWENTIETH (EIGHTEEN deep)**, **BDOUBLE the TWENTY-FIRST (NINETEEN deep)** and **BSATUR the TWENTY-SECOND (TWENTY deep)**, `… → bgenuine → bbase → bunif → bdouble → bsatur`, and BONEONE's job 3 returned the verdict this list had been owed since 2026-08-20 — *fifteen deep is a **de-facto shared layer**, not a device chain, and has not been one since roughly the tenth link*: rule 2 is about a **device** with two consumers, whereas what `w4/` borrows are the arc's **oracles** (the deficiency oracle, the closure, the peel scan, the R-node stand-in, the step classifier). A helper moved down is a refactor; an **oracle** moved down re-baselines every landed figure measured through it, which is why every dispatch has correctly declined. The actionable debt is an **interface decision** (does the layer get a name and an acceptance test?), a coordinator/user call — see `notes/Pencil-informal.md` (BE-83)(iii); still **no move made** — a dispatch may not edit a landed driver another direction may be importing in flight) |
...
| `def3_exact`, `flat_witness`, `two_cut_law` and the `bzavoid` deficiency oracles | **`w4/bzavoid`** | **`w4/binduc`**, **`w4/btwocut`**, **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`** (7 — the FIRST external consumer of any `bzavoid` device, recorded 2026-08-26 at the BINDUC landing; the `battain → bzavoid → binduc → btwocut → bimage` chain is now **five** deep, which is the shape the §2 rule-2 threshold watches) |
| `def2_exact`, `sample_Y`, `solve_schedule`, `closed_star`, `span_dim`, `necklace`, `target_of`, `rows_from_W` | **`w4/battain`** | **`w4/bzavoid`**, **`w4/binduc`**, **`w4/btwocut`**, **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`** (8 — the FIRST external consumer of any `battain` device, recorded 2026-08-26 at the BZAVOID landing. A one-consumer row is **not** yet debt by the §2 rule-2 threshold; it is recorded so the next consumer trips it rather than rediscovering it, and so that if the `kbare/` move-down happens `bzavoid.py` joins the acceptance test alongside `battain.py`) |
| `deltas_at`, `delta_all_pairs`, `extended_construction`, `g_exact`, `split_battery`, `two_cut_census` | **`w4/btwocut`** | **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`**, **`w4/bpeel`**, **`w4/bspread`**, **`w4/boneone`**, **`w4/bgenuine`** (9 — the **FIRST external consumer of any `btwocut` device**, recorded 2026-08-27 at the BIMAGE landing; a second arrived the same day at the BEARCASE landing, which **trips the §2 rule-2 threshold** — recorded, no move made) |
| `a_battery`, the flag-regime builders, `loss_of`, `reach_pred`, **`bad_predicate` / `reach_measured` / `pencil_vertex` / `sample_flags` (BRULE)** | **`w4/bimage`** | **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`**, **`w4/bwin`**, **`w4/brnode`**, **`w4/bdecor`**, **`w4/bpeel`**, **`w4/bgenuine`** (9 — the **FIRST external consumer of any `bimage` device**, recorded 2026-08-27 at the BEARCASE landing. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it) |
| `sample_piece_config`, `subdivide`, `piece_battery` | **`w4/bearcase`** | **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`**, **`w4/bwin`** (`subdivide` only), **`w4/bsatur`** (5 — BSATUR, 2026-09-02, is the fifth and the **first external consumer of `piece_battery`**, which it uses as the population the (BE-105)(ii) threshold is asserted on. The FIRST external consumer of any `bearcase` device was BEARFULL, recorded 2026-08-27; BSHARP is the second, 2026-08-28, which **trips the §2 the **FIRST external consumer of any `bearcase` device** was BEARFULL, recorded 2026-08-27; BSHARP is the second, 2026-08-28, which **trips the §2 rule-2 threshold** — recorded, no move made. `bsharp.sample_piece_config_adj` is a **successor**, not a replacement: the old sampler stays the one every landed figure was measured on) |
| `dist_in_graph`, `far_piece_battery`, **`step_shape` / `optimal_partitions` / `cycle_classes` / `def3_fast` / `forcing_derivation` / `short_cycles` / `tri_chain` (BSPREAD)** | **`w4/bearfull`** | **`w4/bsharp`**, **`w4/brule`**, **`w4/bspread`**, **`w4/boneone`**, **`w4/bgenuine`** (`step_shape` only for the last two) (5 — the **FIRST external consumer of any `bearfull` device**, recorded 2026-08-28 at the BSHARP landing; BRULE the second the same day, which **trips the §2 rule-2 threshold** — recorded, no move made. BSPREAD, 2026-09-01, is the third and reaches the forcing/partition devices; BONEONE the fourth and BGENUINE the fifth, both 2026-09-01, taking only `step_shape` — and BGENUINE reports it beside its own finer hinge-pair test rather than using it) |
| `battery`, `adj_battery`, `sample_piece_config_adj`, `guarded_draw`, `measure`, `simple_paths`, `usable_first_edges` | **`w4/bsharp`** | **`w4/brule`**, **`w4/bwin`**, **`w4/brnode`**, **`w4/bdecor`** (4 — the **FIRST external consumer of any `bsharp` device** was BRULE, recorded 2026-08-28; BWIN, 2026-08-29, is the second, also consuming the shape builders (`barbell`, `_relabel`, `K4`/`K33`) and `lines_of_path`; BRNODE, 2026-09-01, is the third, consuming `guarded_draw`, `sample_piece_config_adj` and `_relabel` only; BDECOR, 2026-09-01, is the fourth, consuming `guarded_draw` and `sample_piece_config_adj`. `brule` consumes `sample_piece_config_adj` unchanged, so BSHARP's successor sampler now has a second measurer and must not move either) |
| `dehom` | **`w4/bwin`** | **`w4/brnode`**, **`w4/bdecor`**, **`w4/bgenuine`** (3 — the **FIRST external consumer of any `bwin` device** was BRNODE, recorded 2026-09-01; BDECOR the second the same day, which **trips the §2 rule-2 threshold**; BGENUINE the third — recorded, no move made) |
| `build_piece`, `K4_named`, `PRISM_named` | **`w4/brnode`** | **`w4/bdecor`** (1 — the **FIRST external consumer of any `brnode` device**, recorded 2026-09-01 at the BDECOR landing. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it; BPEEL reaches these only *through* `bdecor`, so the count is unchanged) |
| `R_BATTERY`, `nested_piece`, `d3`, `weld_d3`, `girth`, `hubs_and_branches`, `sample_by_branches`, `assemble`, `flag_assignment`, `draw_branch` | **`w4/bdecor`** | **`w4/bpeel`**, **`w4/bgenuine`**, **`w4/bbase`**, **`w4/bunif`**, **`w4/bdouble`**, **`w4/bsatur`** (6 — BSATUR, 2026-09-02, is the sixth and the **first external consumer of `flag_assignment` / `draw_branch`**, which it uses to re-place ONE side's branches against a modified hub plane, the (BE-105) construction; BDOUBLE is the fifth, taking `sample_by_branches` only; BBASE, 2026-09-02, is the third and BUNIF the fourth, the latter taking `R_BATTERY` / `nested_piece` / `sample_by_branches` / `assemble` / `d3` / `weld_d3`. The count was 2 — the **FIRST external consumer of any `bdecor` device** was BPEEL, recorded 2026-09-01, the same day `bdecor` itself landed; BGENUINE the second the same day (`girth` only), which **trips the §2 rule-2 threshold** — recorded, no move made. BGENUINE also records a **negative** about this module: `sample_by_branches` **cannot** draw the charts of a forced-coincidence witness, because its flag stage never discovers the forced coplanarity — see (BE-85)(i)) |
| `assert_generic_star` (the `plane_basis`-class guard: no two hinge lines coincide at a body) | **`w4/binduc`** | **`w4/bdecor`**, **`w4/brnode`**, **`w4/bgenuine`**, and every `w4/` sampler through `bsharp.guarded_draw` (recorded 2026-09-01 at the BGENUINE landing, which is the first direction to use it as a **premise** rather than as a gate: (BE-84)(i) reads its body as the assertion that a vertex and any two of its neighbours are affinely independent, and that is what makes the aggressive forcing genuine. A move-down would re-baseline nothing numerically, but every landed figure in the arc is measured on its locus — so it is an **oracle**, not a helper, in (BE-83)(iii)'s sense) |
| `flat_forcing_closure` (the aggressive plane-class closure) | **`w4/binduc`** | **`w4/bpeel`** (1 — the **FIRST consumer of this device outside `binduc` itself**, recorded 2026-09-01 at the BPEEL landing. BPEEL does **not** move or edit it: it re-implements the same growth rule locally as `closure_from` to expose the *admitted hubs* a pair question needs, and **asserts** its own flat verdict equal to this one at 4 000 graphs. A second consumer would trip the §2 rule-2 threshold) |
| `rnode_shaped`, `peel_scan`, `pair_forced`, `constructed_tier`, `subdivided`, `SKELETONS`, `hubs_of`, `closure_from`, `peel_sides`, `delta_pair` | **`w4/bpeel`** | **`w4/bspread`**, **`w4/boneone`**, **`w4/bgenuine`**, **`w4/bbase`**, **`w4/bunif`**, **`w4/bdouble`**, **`w4/bsatur`** (7 — BSATUR, 2026-09-02, is the seventh and the first consumer of **`delta_pair`**, and the first to build its own peel population out of `SKELETONS` + `subdivided` + `rnode_shaped` rather than take `constructed_tier` whole; BUNIF is the first consumer of **`peel_sides`** and BDOUBLE the second the same day; the count was 3 — the **FIRST external consumer of any `bpeel` device** was BSPREAD, recorded 2026-09-01; BONEONE the second the same day, which **trips the §2 rule-2 threshold** — recorded, no move made. `rnode_shaped` is the load-bearing one: it is the arc's only computable stand-in for *“`uv` is a virtual edge of a simple 3-connected SPQR skeleton”*, and BONEONE's (BE-79)(ii) turns on reading it as **per-side**) |
