...
| `R_BATTERY`, `nested_piece`, `d3`, `weld_d3`, `girth`, `hubs_and_branches`, `sample_by_branches`, `assemble`, `flag_assignment`, `draw_branch` | **`w4/bdecor`** | **`w4/bpeel`**, **`w4/bgenuine`** (2 — the **FIRST external consumer of any `bdecor` device** was BPEEL, recorded 2026-09-01, the same day `bdecor` itself landed; BGENUINE the second the same day (`girth` only), which **trips the §2 rule-2 threshold** — recorded, no move made. BGENUINE also records a **negative** about this module: `sample_by_branches` **cannot** draw the charts of a forced-coincidence witness, because its flag stage never discovers the forced coplanarity — see (BE-85)(i)) |
