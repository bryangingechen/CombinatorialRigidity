...
| `a_battery`, the flag-regime builders, `loss_of`, `reach_pred`, **`bad_predicate` / `reach_measured` / `pencil_vertex` / `sample_flags` (BRULE)** | **`w4/bimage`** | **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`**, **`w4/bwin`**, **`w4/brnode`** (6 — the **FIRST external consumer of any `bimage` device**, recorded 2026-08-27 at the BEARCASE landing. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it) |
...
| `battery`, `adj_battery`, `sample_piece_config_adj`, `guarded_draw`, `measure`, `simple_paths`, `usable_first_edges` | **`w4/bsharp`** | **`w4/brule`**, **`w4/bwin`**, **`w4/brnode`** (3 — the **FIRST external consumer of any `bsharp` device** was BRULE, recorded 2026-08-28; BWIN, 2026-08-29, is the second, also consuming the shape builders (`barbell`, `_relabel`, `K4`/`K33`) and `lines_of_path`; BRNODE, 2026-09-01, is the third, consuming `guarded_draw`, `sample_piece_config_adj` and `_relabel` only. `brule` consumes `sample_piece_config_adj` unchanged, so BSHARP's successor sampler now has a second measurer and must not move either) |
| `dehom` | **`w4/bwin`** | **`w4/brnode`** (1 — the **FIRST external consumer of any `bwin` device**, recorded 2026-09-01 at the BRNODE landing. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it) |
