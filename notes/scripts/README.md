...
| `def3_exact`, `flat_witness`, `two_cut_law` and the `bzavoid` deficiency oracles | **`w4/bzavoid`** | **`w4/binduc`**, **`w4/btwocut`**, **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`** **`w4/bsharp`** (6 — the FIRST external consumer of any `bzavoid` device, recorded 2026-08-26 at the BINDUC landing; the `battain → bzavoid → binduc → btwocut → bimage` chain is now **five** deep, which is the shape the §2 rule-2 threshold watches) |
| `def2_exact`, `sample_Y`, `solve_schedule`, `closed_star`, `span_dim`, `necklace`, `target_of`, `rows_from_W` | **`w4/battain`** | **`w4/bzavoid`**, **`w4/binduc`**, **`w4/btwocut`**, **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`** **`w4/bsharp`** (7 — the FIRST external consumer of any `battain` device, recorded 2026-08-26 at the BZAVOID landing. A one-consumer row is **not** yet debt by the §2 rule-2 threshold; it is recorded so the next consumer trips it rather than rediscovering it, and so that if the `kbare/` move-down happens `bzavoid.py` joins the acceptance test alongside `battain.py`) |
| `deltas_at`, `delta_all_pairs`, `extended_construction`, `g_exact`, `split_battery`, `two_cut_census` | **`w4/btwocut`** | **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`** **`w4/bsharp`** (4 — the **FIRST external consumer of any `btwocut` device**, recorded 2026-08-27 at the BIMAGE landing; a second arrived the same day at the BEARCASE landing, which **trips the §2 rule-2 threshold** — recorded, no move made) |
| `a_battery`, the flag-regime builders, `loss_of`, `reach_pred` | **`w4/bimage`** | **`w4/bearcase`**, **`w4/bearfull`** **`w4/bsharp`** (3 — the **FIRST external consumer of any `bimage` device**, recorded 2026-08-27 at the BEARCASE landing. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it) |
| `sample_piece_config`, `subdivide` | **`w4/bearcase`** | **`w4/bearfull`**, **`w4/bsharp`** (2 — the **FIRST external consumer of any `bearcase` device** was BEARFULL, recorded 2026-08-27; BSHARP is the second, 2026-08-28, which **trips the §2 rule-2 threshold** — recorded, no move made. `bsharp.sample_piece_config_adj` is a **successor**, not a replacement: the old sampler stays the one every landed figure was measured on) |
| `dist_in_graph`, `far_piece_battery` | **`w4/bearfull`** | **`w4/bsharp`** (1 — the **FIRST external consumer of any `bearfull` device**, recorded 2026-08-28 at the BSHARP landing) |
