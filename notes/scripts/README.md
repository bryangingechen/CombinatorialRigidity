...
| `deltas_at`, `delta_all_pairs`, `extended_construction`, `g_exact`, `split_battery`, `two_cut_census` | **`w4/btwocut`** | **`w4/bimage`**, **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`**, **`w4/bpeel`**, **`w4/bspread`**, **`w4/boneone`** (8 — the **FIRST external consumer of any `btwocut` device**, recorded 2026-08-27 at the BIMAGE landing; a second arrived the same day at the BEARCASE landing, which **trips the §2 rule-2 threshold** — recorded, no move made) |
| `a_battery`, the flag-regime builders, `loss_of`, `reach_pred`, **`bad_predicate` / `reach_measured` / `pencil_vertex` / `sample_flags` (BRULE)** | **`w4/bimage`** | **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`**, **`w4/bwin`**, **`w4/brnode`**, **`w4/bdecor`**, **`w4/bpeel`** (8 — the **FIRST external consumer of any `bimage` device**, recorded 2026-08-27 at the BEARCASE landing. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it) |
...
| `dist_in_graph`, `far_piece_battery`, **`step_shape` / `optimal_partitions` / `cycle_classes` / `def3_fast` / `forcing_derivation` / `short_cycles` / `tri_chain` (BSPREAD)** | **`w4/bearfull`** | **`w4/bsharp`**, **`w4/brule`**, **`w4/bspread`**, **`w4/boneone`** (`step_shape` only) (4 — the **FIRST external consumer of any `bearfull` device**, recorded 2026-08-28 at the BSHARP landing; BRULE the second the same day, which **trips the §2 rule-2 threshold** — recorded, no move made. BSPREAD, 2026-09-01, is the third and reaches the forcing/partition devices; BONEONE the fourth, 2026-09-01, taking only `step_shape` to classify the derivation at its own witness) |
...
| `dehom` | **`w4/bwin`** | **`w4/brnode`**, **`w4/bdecor`** (2 — the **FIRST external consumer of any `bwin` device** was BRNODE, recorded 2026-09-01; BDECOR the second the same day, which **trips the §2 rule-2 threshold** — recorded, no move made) |
...
| `R_BATTERY`, `nested_piece`, `d3`, `weld_d3`, `girth`, `hubs_and_branches`, `sample_by_branches`, `assemble`, `flag_assignment`, `draw_branch` | **`w4/bdecor`** | **`w4/bpeel`** (1 — the **FIRST external consumer of any `bdecor` device**, recorded 2026-09-01 at the BPEEL landing, the same day `bdecor` itself landed. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it) |
