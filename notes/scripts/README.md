...
| `sample_piece_config`, `subdivide` | **`w4/bearcase`** | **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`**, **`w4/bwin`** (`subdivide` only, 4 — the **FIRST external consumer of any `bearcase` device** was BEARFULL, recorded 2026-08-27; BSHARP is the second, 2026-08-28, which **trips the §2 rule-2 threshold** — recorded, no move made. `bsharp.sample_piece_config_adj` is a **successor**, not a replacement: the old sampler stays the one every landed figure was measured on) |
...
| `rnode_shaped`, `peel_scan`, `pair_forced`, `constructed_tier`, `subdivided`, `SKELETONS`, `hubs_of`, `closure_from`, `peel_sides` | **`w4/bpeel`** | **`w4/bspread`**, **`w4/boneone`**, **`w4/bgenuine`**, **`w4/bbase`**, **`w4/bunif`**, **`w4/bdouble`** (6 — BUNIF, 2026-09-02, is the first consumer of **`peel_sides`** and BDOUBLE the second the same day; the count was 3 — the **FIRST external consumer of any `bpeel` device** was BSPREAD, recorded 2026-09-01; BONEONE the second the same day, which **trips the §2 rule-2 threshold** — recorded, no move made. `rnode_shaped` is the load-bearing one: it is the arc's only computable stand-in for *“`uv` is a virtual edge of a simple 3-connected SPQR skeleton”*, and BONEONE's (BE-79)(ii) turns on reading it as **per-side**) |
| `battery_peels`, `battery_rows`, `tier_rows`, `measure_row`, `blocks_of`, `profile`, `blockcap`, `blockdeg`, `SUBS`, `CAP` | **`w4/bunif`** | **`w4/bdouble`** (1 — recorded 2026-09-02, the day `bunif` itself landed, the same shape as the `bdecor`→`bpeel` edge one day earlier. **Below the §2 rule-2 threshold** (one consumer), so nothing is owed yet; noted because the borrowed set is the arc's block frame and profile instrument — an **oracle**, not a helper — so a second consumer would trip the same interface decision (BE-83)(iii) parks, not a cheap move-down. `bdouble` adds no primitive of its own) |
