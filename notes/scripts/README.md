...
| `a_battery`, the flag-regime builders, `loss_of`, `reach_pred`, **`bad_predicate` / `reach_measured` / `pencil_vertex` / `sample_flags` (BRULE)** | **`w4/bimage`** | **`w4/bearcase`**, **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`** (4 — the **FIRST external consumer of any `bimage` device**, recorded 2026-08-27 at the BEARCASE landing. Recorded so the next consumer trips the §2 rule-2 threshold rather than rediscovering it) |
| `sample_piece_config`, `subdivide` | **`w4/bearcase`** | **`w4/bearfull`**, **`w4/bsharp`**, **`w4/brule`** (3 — the **FIRST external consumer of any `bearcase` device** was BEARFULL, recorded 2026-08-27; BSHARP is the second, 2026-08-28, which **trips the §2 rule-2 threshold** — recorded, no move made. `bsharp.sample_piece_config_adj` is a **successor**, not a replacement: the old sampler stays the one every landed figure was measured on) |
...
| `battery`, `adj_battery`, `sample_piece_config_adj`, `guarded_draw`, `measure`, `simple_paths`, `usable_first_edges` | **`w4/bsharp`** | **`w4/brule`** (1 — the **FIRST external consumer of any `bsharp` device**, recorded 2026-08-28 at the BRULE landing. `brule` consumes `sample_piece_config_adj` unchanged, so BSHARP's successor sampler now has a second measurer and must not move either) |
