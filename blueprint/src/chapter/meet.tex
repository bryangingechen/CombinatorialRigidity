...
\begin{proof}
  ...
  (\cref{lem:complement-iso-decomposable-wedge-perp}). Since $\dim \Phi = 3 + 3 - 1 = 5$ (the
  intersection of the two summands is the line $\mathrm{span}\{n_u \wedge n'\}$), its
  wedge-orthogonal complement $\Omega$ is $6 - 5 = 1$-dimensional
  (\cref{lem:complement-iso-line-one-dim}, in the $\bigwedge^{2}\R^4$-internal form via the
  injective pull-back of \cref{lem:complement-iso-exterior-map-injective}); the panel-meet is
  nonzero (the normals are independent), so it spans $\Omega$ and the point-join is its multiple,
  $\bar{p_i} \vee \bar{p_j} = \lambda\, C(L)$. The annihilation transfer is then immediate:
  $r(C(L)) = 0$ gives $r(\bar{p_i} \vee \bar{p_j}) = \lambda \cdot r(C(L)) = 0$.
\end{proof}
\begin{fmlnote}
