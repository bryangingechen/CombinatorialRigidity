...
iso (\cref{def:meet-complement-iso}) is in progress: the perfect wedge
pairing and its vanishing/non-vanishing on the standard exterior-power basis
are formalized --- the pairing of the basis vectors indexed by $S$ and $T$ is
$0$ off the diagonal ($T \ne S^{c}$) and nonzero ($\pm 1$, the sign of the
interleaving permutation) on the diagonal ($T = S^{c}$), so the pairing matrix
is a signed-permutation matrix. The remaining step is to package this
nondegeneracy as the iso itself (the $\mathrm{toDualEquiv}^{-1}$ composition).
The regressive product (\cref{def:meet}) remains to be formalized.
...
\begin{definition}[Complement iso $\bigwedge^{j} V \cong \bigwedge^{N-j} V$]
  ...
  so it is linearly independent and its extensor is nonzero --- in fact
  $\pm 1$, the sign of the interleaving permutation. Hence the pairing matrix
  on the standard basis is a signed-permutation matrix, which is invertible.
\end{definition}
