...
exterior product on $\bigwedge[K] K^{k+2}$) and a coordinatized Plücker
...
inner product, only the top-power volume form (an orientation) --- so, like
\cref{sec:molecular-homog}, it is stated over an arbitrary field $K$, no
characteristic hypothesis needed; Katoh--Tanigawa's original argument is
the $K = \R$ specialization the later, genuinely metric chapters
(\cref{sec:molecular-rigidity-matrix} onward) instantiate. Write $N = k+2$
and $V = K^{N}$ for the homogeneous-coordinate carrier; the top
graded piece $\bigwedge^{N} V$ is free of rank $\binom{N}{N} = 1$, hence
canonically $\cong K$. In order: the volume-form isomorphism
...
\begin{definition}[Top-power volume form $\bigwedge^{N} V \cong K$]
  ...
  the standard carrier $V = K^{N}$ this is the \emph{volume
  form}: the linear isomorphism
  \[
    \mathrm{topEquiv} : \bigwedge^{N} V \xrightarrow{\ \sim\ } K,
  \]
...
\begin{proof}
  ...
  $\mathrm{Module.Basis.exteriorPower}\,(\mathrm{Pi.basisFun}\ K\ (\mathrm{Fin}\ N))$,
...
\begin{definition}[Complement iso $\bigwedge^{j} V \cong \bigwedge^{N-j} V$]
  ...
  \cong K$ (the join of \cref{sec:molecular-homog} followed by the
...
\begin{lemma}[Complement of a decomposable lies in the wedge-orthogonal complement]
  ...
  be two independent vectors of $K^4$ --- in the application below, two
...
\begin{lemma}[The exterior square of a subspace includes into $\bigwedge^{2} K^4$ injectively]
  ...
  \cref{lem:complement-iso-line-one-dim}. For a subspace $W \subseteq K^4$,
  the exterior-power map $\bigwedge^{2}(W.\mathrm{subtype}) : \bigwedge^{2}
  W \to \bigwedge^{2} K^4$ induced by the inclusion $W \hookrightarrow K^4$
  is injective.
\end{lemma}
\begin{proof}
  \leanok
  Over a field, an injective linear map induces an injective exterior power
...
\begin{lemma}[Point-join $\leftrightarrow$ panel-meet duality; KT eq.~(6.45)]
  ...
  At $d = 3$ ($\bigwedge^{2}K^4$), let $n_u, n'$ be two independent panel normals of a panel
...
\begin{proof}
  ...
  $1$-dimensional (\cref{lem:complement-iso-line-one-dim}, in the $\bigwedge^{2}K^4$-internal form
