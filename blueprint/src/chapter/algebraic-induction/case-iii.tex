...
\cref{lem:cycle-realization}. KT state Lemma~5.4 at $3 \le |V| \le D$, so this
merge is faithful to the source, which never treats the triangle as a separate
...
upgrading by the genericity device) is retained as the accessible worked
...
\begin{proof}
  ...
  $G$ as the $3$-cycle $C_3$ then realizes it as the $m = 3$ instance of the cycle family
  (\cref{lem:cycle-realization}) --- the merge of the former separate triangle base into the KT
  Lemma-5.4 node, which KT state at $3 \le |V| \le D$. At $|V| \ge 4$, the
  ...
  remains consumed on the triangle floor, which now closes through
  ...
  cycle family stays --- as it always was --- load-bearing for the $k > 0$ Case~II
  (\cref{lem:case-II}).
\end{proof}
