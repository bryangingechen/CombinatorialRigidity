...
same span argument that closes the fixed three-candidate Claim~6.12
(\S\ref{sec:molecular-algebraic-induction-claim612}), now applied to $k+2$
panel normals instead of four points. What KT's remark compresses is the
...
\begin{lemma}[The shared redundancy and the chain discriminator; KT Lemma~6.13, eqs.\ (6.57)--(6.67)]
  ...
  algebraically-independent seed when it is simple, and a bare full-rank
  realization (\cref{def:rank-hypothesis}) in all cases. If the split-off
...
\begin{proof}
  ...
  The all-$k$ induction hypothesis gives the eq.-(6.22) rank bound at the
  vertex-removal $G - v_1$ (\cref{lem:case-III-nested-rank-lower}); Claim~6.11
  (\cref{lem:case-III-claim-6-11}), applied once at the base pair
  $(v_0, v_2)$, converts this bound into the shared redundancy $\rho_0 \ne
  0$ with $\rho_0(C(v_0 v_2)) = 0$. The $d + 1 = k + 2$ panel normals of the
  chain's candidate vertices are linearly independent at the
  algebraically-independent seed the induction hypothesis supplies, so
  ...
  (\cref{lem:case-III-claim612-orthseq-vanish}), so $\rho_0$ fails to
  annihilate some join, fixing the matched candidate index $i$ and a
  transversal direction for its line.
\end{proof}
\begin{fmlnote}
  The Lean statement returns further data used only by
  \cref{lem:case-III-chain-dispatch}'s two realization branches: an
  endpoint selector recording every link of $G - v_1$, the realizing seed
...
\begin{proof}
  ...
  a general-position, algebraically-independent seed.
\end{proof}
...
\begin{proof}
  ...
  realization of $G$ itself, establishing both conjuncts for $G$.
