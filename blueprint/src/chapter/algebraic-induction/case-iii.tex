...
q}(u)$). The argument is a contrapositive: if all three fail, a single nonzero $r \in \R^D$
...
\begin{lemma}[The $D$ panel-support extensors of $k+2$ linearly-independent homogeneous vectors span $\R^D$; KT eq.\ (6.45) via Lemma 2.1]
  ...
  \]
  At $d = 3$ this is the $k = 2$ case ($D = 6$, $\bigwedge^2 \R^4$), the form used in the $d = 3$
  case below.
\end{lemma}
\begin{proof}
  \leanok
  By Lemma~2.1 (\cref{lem:extensor-independence}), applied at grade $e = k$ to any $k+2$ linearly
  independent (not necessarily affinely independent) vectors in $\R^{k+2}$, the $D$ omit-two
  extensors $\bar p_i \vee \bar p_j$ are linearly independent in $\bigwedge^k \R^{k+2}$. The
  strictly-increasing index pairs number $\binom{k+2}{2} = D = \operatorname{finrank} \bigwedge^k
  \R^{k+2}$, so this linearly independent family is a basis, hence spans.
\end{proof}
...
\begin{lemma}[A functional annihilating a spanning set is zero]
  ...
  If a functional $r \in (\R^D)^* = (\bigwedge^k \R^{k+2})^*$ on the screw space $\bigwedge^k
  \R^{k+2}$ vanishes on every element of a set $S$ whose span is all of $\R^D$, then $r = 0$. (The
...
\begin{proof}
  ...
  $S$, and $\operatorname{span} S = \R^D$, so $r = 0$.
\end{proof}

The contrapositive needs the union of eq.~(6.45) to span $\R^D$, which rests on two facts: first,
...
\begin{lemma}[A nonzero determinant polynomial in the seed yields an affinely-independent realization; KT eq.\ (6.45), the closure step]
  ...
  equals a \emph{nonzero} multivariate polynomial $P \in \R[\sigma]$ evaluated at $q$. Then there is
  a seed $q$ at which the four points $p(q)$ are affinely independent.
\end{lemma}
\begin{proof}
  \leanok
  Over the infinite field $\R$ a nonzero polynomial does not vanish identically, so $P$ has a
...
\begin{lemma}[The affine-independence determinant of polynomial-valued candidate points is a polynomial in the seed; KT eq.\ (6.45), determinant-polynomial bridge]
  ...
  $P \in \R[\sigma]$: the determinant of the $(d+1) \times (d+1)$ polynomial matrix with rows
...
\begin{proof}
  ...
  three standard covectors of $\R^4$, hence linearly independent, satisfying the nonparallel
  hypothesis), and the standard affine $3$-simplex $p = (0, e_3, e_1, e_2)$ of $\R^3$ (the origin
...
\begin{proof}
  ...
  $\R^4$, hence nonparallel, and the standard affine $3$-simplex $p = (0, e_3, e_1, e_2)$ of $\R^3$
  ...
  (\cref{lem:case-III-claim612-points-affineIndep-producer,lem:case-III-claim612-detpoly}, together
  ...
  lem:case-III-claim612-p3-placement}) realize $G_v^{ab}$ at a panel-coordinate seed $q$ that
...
\begin{lemma}[The witness points, parameterized by the actual split-leg panel normals; KT eq.\ (6.45)]
  ...
  For any linearly independent panel normals $n \colon \mathrm{Fin}\,3 \to \R^4$, there exist four
  linearly independent homogeneous vectors $\bar p \colon \mathrm{Fin}\,4 \to \R^4$ realizing the
...
\begin{lemma}[Point-join $\leftrightarrow$ panel-meet duality: a line in $\Pi(u)$ is annihilated by $r$ once $r \perp C(L)$; KT eq.\ (6.45) via Grassmann--Cayley]
  ...
  At $d = 3$ ($D = 6$, $k = 2$), working in $\bigwedge^2 \R^4$, let $p_i, p_j$ be two of the points
  of \cref{lem:case-III-claim612-points-affineIndep} whose connecting line $L = p_i p_j$ lies in a
  panel $\Pi(u)$ ($u \in \{a, b, c\}$). Then the point-join $\bar{p_i} \vee \bar{p_j} \in
  \bigwedge^2 \R^4$ and the panel-meet supporting extensor $C(L) \in \bigwedge^2 \R^4$
  (\cref{def:panel-support-extensor}) of the same line are \emph{scalar multiples} of one another
  (both are the Pl\"ucker coordinate vector of the line $L$, up to the projective scale).
  Consequently a functional $r$ on $\bigwedge^2 \R^4$ that annihilates \emph{every} panel-meet
  extensor $C(L)$ of lines $L \subset \Pi(a) \cup \Pi(b) \cup \Pi(c)$ also annihilates each
  spanning join: $r(C(L)) = 0 \Rightarrow r(\bar{p_i} \vee \bar{p_j}) = 0$.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{lemma}[Fixed-seed realization transport across the $\rho = (a\,v)$ relabel; KT eq.\ (6.31)]
  ...
  some endpoint selector $\mathrm{ends}_0$ and seed $q_0$, satisfying four conjuncts: general
  position, rigidity on $V(G) \setminus \{v\}$, link-recording (every link of $G_v^{ab}$ is among
  the selector's pairs), and algebraic independence of $q_0$ over $\Q$. Under the hypotheses of
  \cref{lem:splitOff-isLink-relabel}, the $a$-split $G_a^{vc}$ satisfies the same four conjuncts for
  the free-normal panel framework with the transported selector $\mathrm{ends}_{\sigma\rho}(e) :=
...
\begin{proof}
  \leanok
  \uses{lem:splitOff-isLink-relabel}
  Each of the four conjuncts transports along the vertex-swap $\rho$ and edge-permutation $\sigma$
  ...
  records, transported back through $\rho$ to $\mathrm{ends}_{\sigma\rho}$. Algebraic independence:
  $q_\rho$ is an injective $\rho$-reindex of $q_0$.
\end{proof}
...
\begin{lemma}[Claim 6.12: for some line choice the candidate block has full rank; KT Claim 6.12, eqs.\ (6.30)--(6.45)]
  ...
  \bar p_4 \in \R^4$ be four linearly independent homogeneous vectors
  ...
  discharging the one conditional the candidate-completion
  (\cref{sec:molecular-algebraic-induction-candidate-completion}) leaves open.
\end{lemma}
\begin{proof}
  \leanok
  ...
  vectors span $\R^D$ (\cref{lem:case-III-claim612-extensor-span}, Lemma~2.1), so $r$ would
  ...
  $\bigwedge^2 \R^4$, so $r \perp C_1, C_2, C_3$ alone cannot force $r = 0$; only the full six-join
