...
$0$-dof-graph, and the realization must be produced \emph{directly}: pin the
vertex set and find the third edge by edge counting
(\cref{lem:triangle-third-edge}), realize three mutually transversal panels by
a cyclically consistent standard-basis seed (\cref{lem:triangle-normals}),
conclude rigidity from the three-extensor telescoping independence
(\cref{lem:theorem-55-triangle}, the three-body sibling of the two-body base
case \cref{lem:theorem-55-base}), and upgrade the attained rank to a
general-position, algebraically-independent seed by the genericity device.
This is the one floor of the $d = 3$ recursion, the counterpart of KT's
Lemma~5.4 short-cycle family, whose general-$m$ node
(\cref{lem:cycle-realization}) is exactly the dichotomy's cycle disjunct
above, vacuous at $d = 3$.
...
\begin{proof}
  ...
  general-position, algebraically-independent seed, giving the generic
