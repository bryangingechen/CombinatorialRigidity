...
induction's realizing seed with algebraically-independent coordinates, so that
the inductively-fixed seed --- not merely \emph{some} generic seed --- attains
the matroid-predicted rank of each subgraph. The formalization instead chooses
each composition's seed off the zero locus of the finitely many rank
polynomials that composition tests (\cref{fmlnote:generic-full-rank}); the
two lemmas below record the transfer in KT's original
algebraically-independent form, in which the argument was first formalized.
At the $0$-dof level this is KT eq.~(6.18): the split-off $G_v^{ab}$ is
$0$-dof, so its generic realization is rigid, and the following lemma
transfers that rigidity to the fixed seed.
...
general-position seed by the genericity device.
