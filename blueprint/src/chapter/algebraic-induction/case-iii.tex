...
\begin{lemma}[Eq.~(6.22) rank lower bound at the nested $k'$-dof subgraph; KT eq.\ (6.22), nested hypothesis (6.1)]
  ...
  \lean{CombinatorialRigidity.Molecular.PanelHingeFramework.case_III_nested_rank_lower_all_k,
        ...
        CombinatorialRigidity.Molecular.PanelHingeFramework.exists_nested_rankPolynomial_lower_all_k}
  \leanok
...
\begin{fmlnote}
  ...
  recorded as a separate specialization. A third form exposes the rank
  polynomial $P$ itself in place of the seed hypotheses --- for a fixed
  link-recording selector, a nonzero rational polynomial whose every
  non-root seed attains the bound --- and is the form the chain
  discriminator (\cref{lem:case-III-chain-discriminator}) consumes: the
  bound then transfers to a seed chosen off the zero locus of a product of
  such polynomials rather than to an algebraically-independent one.
\end{fmlnote}
...
span argument runs on the six joins of four points in $\bigwedge^2 \R^4$, a six-dimensional
...
\begin{fmlnote}
  ...
  three-candidate specialization.
\end{fmlnote}
