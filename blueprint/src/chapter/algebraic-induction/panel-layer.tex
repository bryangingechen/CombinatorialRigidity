...
\begin{proof}
  ...
  $\mathtt{hgen}$ at every deficiency is Theorem~5.6 (\cref{thm:theorem-55-6});
  the $d = 3$ specialization is \cref{thm:theorem-55-6-d3}. Applied to a spanning
  $G$ ($V(G) = V_\alpha$),
...
\begin{theorem}[Realization at the target rank; KT Theorem 5.5]
  ...
  fix this floor); a fresh-edge supply, available at each minimal-dof-graph
  the induction visits (not assumed for every graph on the edge-label type,
  which would be unsatisfiable --- an all-loops-at-one-vertex graph exhausts
  it), rides as ambient data. (The molecular
...
\begin{theorem}[Theorem 5.5 at $d = 3$: the zero-carry instance; KT Theorem 5.5, \S6.4.1]
  ...
  \lean{CombinatorialRigidity.Molecular.PanelHingeFramework.theorem_55_all_k,
        CombinatorialRigidity.Molecular.theorem_55_base_producer_gen,
  ...
  A fresh-edge supply, scoped as in Theorem~5.5, rides as ambient data.
...
\begin{theorem}[Theorem 5.6: every multigraph attains the deficiency rank; KT Theorem 5.6, \S5.2]
  ...
  Every simple spanning multigraph $G$ has a panel-hinge realization $(G,p)$
  attaining the deficiency rank
  \[
    \operatorname{rank} R(G,p) = D(|V|-1) - \operatorname{def}(\tilde G),
  \]
  ...
  to an arbitrary deficiency. Formalized at general $d$ for
  $D = \binom{n+1}{2} \ge 6$, i.e.\ $n \ge 3$ (the Phase-20 chain extractors fix
  this floor, inherited through Theorem~5.5); a fresh-edge supply, scoped as in
  Theorem~5.5, rides as ambient data. It is the genuine $\operatorname{def} > 0$ feed of the
  Proposition-1.1 reconciliation (\cref{prop:rigidity-matrix-prop11}); the
  $\operatorname{def} = 0$ stratum is Theorem~5.5 itself (\cref{thm:theorem-55}),
  and the $d = 3$ specialization is \cref{thm:theorem-55-6-d3}.
\end{theorem}
\begin{proof}
  \leanok
  \uses{thm:theorem-55, lem:motions-mono-of-graph-le,
  ...
  deficiency $c$ by Theorem~5.5 (\cref{thm:theorem-55}, the grade-general spine:
  ...
  D + c = D + \operatorname{def}(\tilde G)$. This is exactly the upper bound
  $\mathtt{hgen}$, which together with the genericity-free lower bound discharges
  the rank hypothesis via the Proposition-1.1 rank bridge
...
\begin{theorem}[Theorem 5.6 at $d = 3$: the specialization; KT Theorem 5.6, \S5.2]
  ...
  arbitrary deficiency, routed through the $d = 3$ Theorem~5.5 spine rather than
  the general one. It is the genuine $\operatorname{def} > 0$ feed of the
  Proposition-1.1 reconciliation (\cref{prop:rigidity-matrix-prop11}) at $d = 3$;
  the $\operatorname{def} = 0$ stratum is \cref{thm:theorem-55-d3-instance}.
\end{theorem}
\begin{proof}
  \leanok
  \uses{thm:theorem-55-d3-instance, lem:motions-mono-of-graph-le,
  ...
  Theorem~5.5 (\cref{thm:theorem-55-d3-instance}, the $d = 3$ spine: $G'$ is
