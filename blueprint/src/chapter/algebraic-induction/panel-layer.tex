...
$\operatorname{def}(\tilde G) = \operatorname{corank} M(\tilde G)$ --- is
\cref{thm:def-eq-corank}. The analytic half below connects the
rigidity-matrix rank $R(G,p)$ of \cref{sec:molecular-rigidity-matrix} to
the matroid corank, and depends on the generic-rank argument (Claim~6.4)
that drives the whole induction.
...
\begin{proof}
  ...
  \cref{lem:trivial-motions-rank-bound} together with a direct
  partition-counting argument: a partition $P$ attaining
  $\operatorname{def}(\tilde G)$ contributes $\operatorname{def}$ independent
  motions beyond the $D$ trivial ones, for \emph{every} realization, with no
  genericity needed. The reverse (generic max-rank) bound
  $\operatorname{rank} R(G,p) \ge D(|V|-1) - \operatorname{def}$ --- equivalently
  $\dim Z(G,p) \le D + \operatorname{def}$ --- holds at a generic realization:
  delete $G$ down to a minimal $k$-dof spanning subgraph, realize it at full
  generic rank by Theorem~5.5 (\cref{thm:theorem-55}, the Claim~6.4
  generic-rank argument selecting the attaining point), then re-add the
  deleted edges, which only grows the rank
  (\cref{lem:motions-mono-of-graph-le}). The proposition's only further inputs
  are the dimension identity $n = k+1$ and the genuine-hinge condition
  $C(e) \neq 0$ the partition cut needs. This reverse bound at \emph{every}
  deficiency, not just the one attained by a minimal spanning subgraph, is
  Theorem~5.6 (\cref{thm:theorem-55-6}); the $d = 3$ specialization is
  \cref{thm:theorem-55-6-d3}. Applied to a spanning $G$ ($V(G) = V_\alpha$),
  the $V(G)$-relative rank form of \cref{def:rank-hypothesis} coincides with
...
\begin{definition}[Generic-rank hypothesis (6.1), $V(G)$-relative --- rank-form substrate]
  ...
  $\R^{D|V_\alpha|}$ of the ambient body type: the rank form above depends
  only on $G$ itself, not on the ambient body type, which is what lets it
  compose through an induction that shrinks $G$ one vertex at a time.

  \emph{Formalization note.} Two predicates carry this hypothesis in Lean.
  $\mathtt{HasFullRankRealization}\,k\,G$ is the bare existential
  $\exists\,Q,\; Q.\mathrm{graph} = G \;\wedge\;
  Q.\mathrm{toBodyHinge}.\mathrm{IsInfinitesimallyRigidOn}\,V(G)$; it is
  equivalent to the rank equality above (B1,
  $\mathtt{isInfinitesimallyRigidOn\_vertexSet\_iff\_finrank\_span\_rigidityRows}$)
  when $V(G)$ is nonempty, and is the realization predicate that
  Theorem~5.5 establishes by induction on $|V(G)|$. It remains
  \emph{weaker} than KT's (6.1)
  --- a degenerate welded framework satisfies it on every connected $0$-dof
  graph --- pending the induction's restatement onto the honest
  genuine-hinge form (\cref{def:genuine-hinge-realization}).

  $\mathtt{HasGenericFullRankRealization}\,k\,n\,G$ is the strengthening
  that carries the rank equality as an explicit conjunct (at deficiency
  $n$, alongside general position and algebraic independence of the panel
  normals); it is the realization predicate for the generic realizations
  Theorem~5.5 produces when $G$ is simple. The rank conjunct is the
  $\mathbb{Z}$-cast form
  $(\dim \operatorname{span}(\text{rigidity rows}) : \mathbb{Z}) =
    D\,(|V(G)| - 1) - \operatorname{def}(\tilde{G})$,
  matching the $\mathbb{Z}$ arithmetic of the genuine-hinge form
...
\begin{proof}
  ...
  Induction on $|V(G)|$, driven by the reduction theorem (Theorem~4.9,
  \cref{thm:minimal-kdof-reduction}): every minimal $k'$-dof-graph on at
  least three vertices has a cut edge, a proper rigid subgraph, or ---
  when it is $2$-edge-connected with no rigid subgraph --- a vertex whose
  splitting-off is again minimal. The two-vertex base is
  \cref{lem:theorem-55-base}. Each reduction move is then realized at full
  rank: contraction of a proper rigid subgraph by Case~I
  ...
  the heart of the proof --- by Case~III (\cref{lem:case-III}). Throughout,
  the induction hypothesis is quantified over \emph{all} deficiencies $k'$,
  not only $k' = 0$ (KT hypothesis~(6.1)): Case~III applies the statement to
  an auxiliary minimal $k''$-dof graph with $0 \le k'' \le D - 2$
  (Claim~6.11, eq.~(6.22)), consumed through its nested rank bound
  (\cref{lem:case-III-nested-rank-lower}). At general dimension, Case~III is
  run by a chain-length dispatch together with a chain-realization argument
  valid for cycles of any length (\cref{lem:chain-data-extract}); the
  $d = 3$ specialization of the whole induction is
  \cref{thm:theorem-55-d3-instance}.
\end{proof}

\emph{Formalization note.} The Lean statement carries an ambient hypothesis
supplying, at each minimal-dof-graph the induction visits, an edge label not
already used by that graph. The fully unconditioned form of this supply ---
one fresh label for \emph{every} graph on the ambient edge-label type ---
is not satisfiable (an all-loops-at-one-vertex graph already uses every
label), so it is threaded through the induction rather than assumed once
and for all. The molecular conjecture needs only the $0$-dof case stated
above; formalized as \texttt{theorem\_55\_gen} at $k' = 0$, with the
all-deficiency form the induction actually runs as
\texttt{theorem\_55\_minimalKDof\_gen}.
...
\begin{proof}
  ...
  \uses{thm:minimal-kdof-reduction, lem:theorem-55-base-producer,
        lem:case-I-realization, lem:case-I-dispatch,
        lem:case-II-realization, lem:case-III, lem:case-III-nested-rank-lower}
  Induction on $|V|$, driven by the reduction theorem (Theorem~4.9,
  \cref{thm:minimal-kdof-reduction}): every minimal $0$-dof-graph on at
  ...
  base --- the double edge --- is realized directly
  (\cref{lem:theorem-55-base-producer}). Each reduction move is then
  realized at full rank: contraction of a proper rigid subgraph by Case~I
  (\cref{lem:case-I-dispatch}, \cref{lem:case-I-realization}); the cut edge
  and a $k > 0$ splitting-off by Case~II (\cref{lem:case-II-realization});
  and a $k = 0$ splitting-off --- the heart of the proof --- by Case~III
  (\cref{lem:case-III}). Throughout, the induction hypothesis is quantified
  over \emph{all} deficiencies $k'$, not only $k' = 0$ (KT
  hypothesis~(6.1)); Case~III consumes this stronger hypothesis through its
  nested rank bound (\cref{lem:case-III-nested-rank-lower}). This is the
  $d = 3$ specialization of the general-dimension induction
  \cref{thm:theorem-55}.
\end{proof}
...
an edge admit no ``nonparallel'' realization); the Lean statement is the
conjunction of the two claims above, and a fresh-edge-supply hypothesis
rides as ambient data, per \cref{thm:theorem-55}'s Formalization note.
Formalized as \texttt{theorem\_55\_all\_k}, the $d = 3$ specialization of
\cref{thm:theorem-55}; \texttt{theorem\_55\_d3} is a thin restatement for
callers using the $n$-parameter form.
...
\begin{corollary}[Theorem 5.5 feeds Proposition 1.1 at deficiency zero]
  ...
  of the strip-and-readd assembly that proves Theorem~5.6 at $d = 3$
...
\begin{proof}
  ...
  transversal hinge; then apply the Proposition~1.1 rank count
  (\cref{prop:rigidity-matrix-prop11}).
\end{proof}

\begin{theorem}[Theorem 5.6: every multigraph attains the deficiency rank; KT Theorem 5.6, \S5.2]
  ...
  at $k' = \operatorname{def}(\tilde G)$. This is Katoh--Tanigawa's Theorem~5.6,
  lifting the minimal-$0$-dof realization of Theorem~5.5 (\cref{thm:theorem-55})
  to an arbitrary deficiency: it is the $\operatorname{def} > 0$ feed of the
  Proposition-1.1 reconciliation (\cref{prop:rigidity-matrix-prop11}), of which
  the $\operatorname{def} = 0$ case is Theorem~5.5 itself. The $d = 3$
  specialization is \cref{thm:theorem-55-6-d3}.
\end{theorem}
\begin{proof}
  \leanok
  \uses{thm:theorem-55, lem:motions-mono-of-graph-le,
        lem:subgraph-minimality, prop:rigidity-matrix-prop11}
  Following Katoh--Tanigawa (p.~670), at general $d$. When $|V| = 1$ the
  framework is trivially rigid and $\operatorname{def}(\tilde G) = 0$, settled
  directly. Otherwise: delete edges of $G$ down to a minimal $k$-dof spanning
  subgraph $G' \le G$ with $V(G') = V(G)$ and $\operatorname{def}(\tilde{G'}) =
  \operatorname{def}(\tilde G) =: c$ (the deficiency-preserving strip; the
  matroid engine behind \cref{lem:subgraph-minimality}). Realize $G'$ at its own
  deficiency $c$ by Theorem~5.5 (\cref{thm:theorem-55}, at general dimension:
  $G'$ is simple as a subgraph of a simple $G$), yielding a generic full-rank
  realization $q$ on $G'$ with $\dim Z(G',q) = D + c$. Re-aim $q$ to the full
  graph $G$, keeping the supporting extensors on every $G'$-link unchanged and
  placing a genuine in-panel hinge on each re-added edge --- in the homogeneous
  model two distinct hyperplanes through the origin always meet, so KT's
  projective-invariance move is free at \emph{every} dimension (no new geometry
  beyond the $d = 3$ argument). Re-adding edges only shrinks the motion space
  (\cref{lem:motions-mono-of-graph-le}), so $\dim Z(G,p) \le \dim Z(G',q) =
  D + c = D + \operatorname{def}(\tilde G)$ --- exactly the upper bound the rank
  hypothesis needs, which together with the genericity-free lower bound
  discharges it via the Proposition-1.1 rank bridge
  (\cref{prop:rigidity-matrix-prop11}); the bridge's $n = k+1$ premise is the
  dimension identity $\operatorname{bodyBarDim} n = \operatorname{screwDim} k
  \Rightarrow n = k+1$.
\end{proof}

\emph{Formalization note.} As in Theorem~5.5, a fresh-edge-supply hypothesis
rides as ambient data (\cref{thm:theorem-55}'s Formalization note).

\begin{theorem}[Theorem 5.6 at $d = 3$: the specialization; KT Theorem 5.6, \S5.2]
  ...
  (\cref{def:rank-hypothesis}) at $k' = \operatorname{def}(\tilde G)$. It lifts the
  minimal-$0$-dof realization of \cref{thm:theorem-55-d3-instance} to an
  arbitrary deficiency, via the $d = 3$ instance of Theorem~5.5
  rather than the general one. It is the genuine $\operatorname{def} > 0$ feed of the
  Proposition-1.1 reconciliation (\cref{prop:rigidity-matrix-prop11}) at $d = 3$;
  the $\operatorname{def} = 0$ case is the spanning corollary
...
\begin{proof}
  ...
  \uses{thm:theorem-55-d3-instance, lem:motions-mono-of-graph-le,
        lem:subgraph-minimality, prop:rigidity-matrix-prop11}
  The general-$d$ argument (\cref{thm:theorem-55-6}) specialized to $D = 6$.
  When $|V| = 1$ the framework is trivially rigid and
  $\operatorname{def}(\tilde G) = 0$, settled directly. Otherwise: delete edges of
  $G$ down to a minimal $k$-dof spanning subgraph $G' \le G$ with $V(G') = V(G)$
  and $\operatorname{def}(\tilde{G'}) = \operatorname{def}(\tilde G) =: k$ (the
  deficiency-preserving strip; the matroid engine behind
  \cref{lem:subgraph-minimality}). Realize $G'$ at its own deficiency $k$ by
  Theorem~5.5 (\cref{thm:theorem-55-d3-instance}, at $d = 3$: $G'$ is
  simple as a subgraph of a simple $G$), yielding a generic full-rank realization
  $q$ on $G'$ with $\dim Z(G',q) = D + k$. Re-aim $q$ to the full graph $G$,
  keeping the supporting extensors on every $G'$-link unchanged and placing a
  genuine in-panel hinge on each re-added edge --- in the homogeneous model two
  distinct hyperplanes through the origin always meet, so KT's
  projective-invariance move is free. Re-adding edges only shrinks the motion
  space (\cref{lem:motions-mono-of-graph-le}), so
  $\dim Z(G,p) \le \dim Z(G',q) = D + k = D + \operatorname{def}(\tilde G)$. This
  is exactly the upper bound the rank hypothesis needs, which together with the
  genericity-free lower bound discharges it via the
  Proposition-1.1 rank bridge (\cref{prop:rigidity-matrix-prop11}).
\end{proof}

\begin{theorem}[The Molecular Conjecture (Conjecture 1.2): body-hinge $\Leftrightarrow$ panel-hinge realizability; conjectured Tay--Whiteley 1984, proved KT 2011]
  ...
  Let $G$ be a simple spanning graph on $\ge 2$ bodies (at general dimension,
  $6 \le D = \binom{n+1}{2} = \binom{k+2}{2}$, i.e.\ $n \ge 3$). Then $G$ can be
  realized as an infinitesimally rigid \emph{body-hinge} framework in $\R^{n}$ if
  and only if $G$ can be realized as an infinitesimally rigid \emph{panel-hinge}
  framework in $\R^{n}$. This is Katoh--Tanigawa's Conjecture~1.2, the
  \emph{Molecular Conjecture}, conjectured by Tay and
  Whiteley~\cite{tayWhiteley1984} and proved in~\cite{katohTanigawa2011}; it is
  the headline statement of the molecular-conjecture program.

  ``Realized as an infinitesimally rigid $\cdots$ framework'' means: there is a
  framework $(G,p)$ on $G$ whose every hinge is \emph{genuine} --- a nondegenerate
  supporting extensor $C(p(e)) \ne 0$, matching KT's $p(e)$ being a genuine
  $(d-2)$-flat --- and all of whose infinitesimal motions are trivial. The
  genuine-hinge condition is \emph{essential}: a degenerate hinge $C(p(e)) = 0$
  welds two bodies ($S(u) = S(v)$), \emph{over}-constraining, so dropping it makes
  the statement false --- a welded framework can be infinitesimally rigid on a
  combinatorially deficient graph ($\operatorname{def}(\tilde G) > 0$). The
  $\ge 2$-body hypothesis is essential, but not for a vacuous reason: on a
  single body $E(G) = \emptyset$, yet the genuine-hinge condition is quantified
  over the framework's \emph{whole} edge-label type, not just $E(G)$ --- so
  pick any label, which is then trivially outside the empty $E(G)$. At that
  spare label the body-hinge side remains trivially realizable (its
  supporting extensor is unconstrained data, and rigidity is automatic with
  a single unconstrained body), but the
  panel-hinge side is not: a panel-hinge's hinge at any label is the meet of
  the two panel normals at its endpoints, and with one body every label's
  endpoints coincide, forcing a degenerate (repeated-argument) meet regardless
  of the normals chosen. So without the hypothesis the iff would be
  \emph{false} --- the body-hinge side realizable, the panel-hinge side not
...
\begin{proof}
  ...
  \uses{thm:theorem-55-6, lem:trivial-motions-rank-bound, def:rank-hypothesis,
        def:D-deficiency}
  ($\Leftarrow$, the elementary direction) A panel-hinge framework \emph{is} a
  body-hinge framework ($\mathtt{toBodyHinge}$) with the same supporting extensors,
  so a rigid genuine panel realization is at once a rigid genuine body-hinge
  realization. ($\Rightarrow$, KT's content) A genuine body-hinge realization
  forces $\operatorname{def}(\tilde G) = 0$: the genericity-free lower bound
  $D + \operatorname{def}(\tilde G) \le \dim Z(G,p)$
  (\cref{lem:trivial-motions-rank-bound}, which needs the genuine hinges) meets
  $\dim Z(G,p) = D$ from rigidity (\cref{def:rank-hypothesis} at $k' = 0$), forcing
  $\operatorname{def}(\tilde G) \le 0$, hence $= 0$. Then Theorem~5.6
  (\cref{thm:theorem-55-6}, in its genuine-hinge witness form) produces a genuine
  panel-hinge realization at $\operatorname{def}(\tilde G) = 0$, i.e.\
  infinitesimally rigid. Combined with the Tay--Whiteley body-hinge theorem
  (Proposition~1.1: body-hinge rigid iff $\tilde G = (D-1)G$ has $D$ edge-disjoint
  spanning trees, i.e.\ $\operatorname{def}(\tilde G) = 0$), this is the full
  molecular characterization.
\end{proof}

\begin{remark}[Non-vacuity witness]
  \label{rem:molecular-conjecture-nonvacuous}
  The fresh-edge-label hypothesis threaded through the Theorem~5.5 induction and
  the theorem above was originally quantified as a single unconditioned
  supply, $\forall G',\ \exists e_0,\ e_0 \notin E(G')$ --- unsatisfiable for
  every nonempty vertex type (the all-loops-at-one-vertex graph has
  $E(G') = \mathrm{univ}$), making the statement vacuous as formalized. The
  supply is now conditioned on minimality: a minimal $k$-dof-graph's edge
  count is bounded by $D(|V|-1) - \operatorname{def}(\tilde G)$
  (\texttt{Graph.edgeSet\_ncard\_add\_deficiency\_le\_of\_isMinimalKDof}), so a
  large-enough edge-label type always has an unused label
  (\texttt{Graph.freshEdgeSupply\_of\_card\_lt}). Non-vacuity is checked
  directly: \texttt{CombinatorialRigidity.Molecular.molecular\_conjecture\_witness}
  fully applies the theorem above at a concrete $d = 3$ instance ($n = 3$,
  $k = 2$, two bodies, seven edge labels, the one-edge graph), discharging
  every hypothesis --- including the fresh-edge supply --- with no
  \texttt{sorry}.
\end{remark}
...
\begin{lemma}[Theorem 5.5 base region ($|V(G)| \le 2$): the empty graph]
...
\begin{lemma}[Theorem 5.5 base region ($|V(G)| \le 2$): a single edge]
...
\begin{lemma}[Theorem 5.5 base region ($|V(G)| \le 2$): a parallel edge pair]
...
\begin{lemma}[Theorem 5.5 base region ($|V(G)| \le 2$): the trichotomy dispatch]
  ...
  instance, at $|V(G)| \le 2$, of the induction hypothesis the all-$k$ form of Theorem~5.5
  is proved against.
\end{lemma}
\begin{proof}
  \leanok
