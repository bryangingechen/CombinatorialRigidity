...
\begin{theorem}[Theorem 5.6: every multigraph attains the deficiency rank; KT Theorem 5.6, \S5.2]
  ...
  this floor, inherited through Theorem~5.5); a fresh-edge supply rides as
  ambient data. It is the genuine $\operatorname{def} > 0$ feed of the
...
\begin{theorem}[The Molecular Conjecture (Conjecture 1.2): body-hinge $\Leftrightarrow$ panel-hinge realizability; conjectured Tay--Whiteley 1984, proved KT 2011]
  ...
  over the framework's \emph{whole} edge-label type, not just $E(G)$, and a
  spare label outside $E(G)$ always exists. At that spare label the body-hinge
  side remains trivially realizable (its supporting extensor is unconstrained
  data, and rigidity is automatic with a single unconstrained body), but the
