...
\begin{theorem}[Theorem 5.5 at $d = 3$: the formalized instance; KT Theorem 5.5, \S6.4.1]
  ...
  $D(|V|-1)$ (\cref{def:genuine-hinge-realization}). The recursion is
  \cref{thm:minimal-kdof-reduction} against the conditioned motive; the
  two-vertex base is \cref{lem:theorem-55-base-producer} (the all-$k$ base
  dispatch; its general-position half is vacuous: only the parallel pair is
  $0$-dof, and parallel-pair graphs cannot be simple); the contraction
  branch is \cref{lem:case-I-realization} under the 6.3-vs-6.5 dispatch
  (\cref{lem:case-I-dispatch}); and the splitting-off branch is the
  Case-III producer (\cref{lem:case-II-realization}, \cref{lem:case-III}).
  A fresh-edge supply (every graph leaves some edge name unused, the
  splitting-off's short-circuit edge) rides as ambient data.

  \emph{The carried family.} The instance holds modulo four named
  hypotheses, each with a tracked artifact: $\mathtt{h622}$, the nested
  eq.-(6.22) rank bound (\cref{lem:case-III-nested-rank-lower}); $\mathtt{h65}$,
  the Lemma-6.5 vertex-removal arm of the Case-I dispatch
  (\cref{lem:case-I-dispatch}); and the two genuine-hinge slots
  $\mathtt{hsplit}$/$\mathtt{hcontract}$ (the no-rigid-subgraph and
  contraction realization producers, typed at \cref{def:genuine-hinge-realization}).
  The $\mathtt{hbase}$ slot is now discharged by \cref{lem:theorem-55-base-producer}.
  The delivered content is the general-position conjunct --- it is all the
...
\begin{proof}
  ...
  Instantiate the conditioned-motive reduction at $D = 6$. The two-vertex
  base is \cref{lem:theorem-55-base-producer}; its simple conjunct is
  vacuous (a simple two-vertex minimal $0$-dof-graph does not exist). The
  no-rigid-subgraph branch is \cref{lem:case-III}; the contraction branch
  cases classically on whether some proper rigid subgraph has a simple
  contraction --- positive to \cref{lem:case-I-realization}, negative to
  the carried $\mathtt{h65}$ (\cref{lem:case-I-dispatch}). For the
  spanning stratum corollary, re-aim the realization's endpoint selector
  at a fixed pair of distinct bodies on non-edges (the motion space reads
  only links, so rigidity and general position survive, and every
  supporting extensor is nonzero); the spanning condition kills the
  motion-space complement, and the Proposition-1.1 rank form
  (\cref{prop:rigidity-matrix-prop11}) reads the deficiency-$0$ rank
  hypothesis off the count.
\end{proof}
...
\begin{lemma}[Theorem 5.5 base producer, empty arm]
  ...
  \leanok
  \uses{def:genuine-hinge-realization, lem:two-vertex-trichotomy}
  ...
  (\cref{def:genuine-hinge-realization}) at the target rank $D(|V(G)|-1) - \mathrm{def} = 0$.
\end{lemma}
\begin{proof}
  \leanok
  The construction is geometry-free: take the all-zero framework (every edge carries the zero
  supporting extensor) at a fixed nonzero panel normal. With no edges no hinge constraint fires,
  so the rigidity-row family is empty, its span is $\{0\}$, and the rank is $0$; the per-link
  conjunct of the motive is vacuous. The target rank vanishes because the empty-graph deficiency
  is exactly $D(|V(G)|-1)$ (\cref{lem:two-vertex-trichotomy}).
\end{proof}
...
\begin{lemma}[Theorem 5.5 base producer, single-edge arm]
  ...
  \leanok
  \uses{def:genuine-hinge-realization, lem:two-vertex-trichotomy, lem:extensor-pair-in-panel,
        def:rigidity-matrix}
  ...
  $D(|V(G)|-1) - \mathrm{def} = D - 1 = 5$ (at $d = 3$, $D = 6$).
\end{lemma}
\begin{proof}
  \leanok
  \uses{lem:extensor-pair-in-panel, def:rigidity-matrix}
  Place coincident panels $\Pi(x) = \Pi(y) = n^{\perp}$ at a fixed nonzero normal $n$ and assign
  the single hinge one nonzero supporting extensor inside the common panel (the first of the pair
  of \cref{lem:extensor-pair-in-panel}; nonzero by its linear independence). The lone hinge's row
  block already \emph{is} the entire rigidity-row family (there is only one edge), and that block
  has rank exactly $D - 1$ (\cref{def:rigidity-matrix}); so the rigidity-row span has rank
  $D - 1 = D(|V(G)|-1) - \mathrm{def}$, the rank equality of the motive. Unlike the parallel-pair
  arm the framework is not rigid ($\mathrm{def} = 1$): the rank is the single hinge-row count, not
  the full two-vertex base rank.
\end{proof}
...
\begin{lemma}[Theorem 5.5 base producer, trichotomy dispatch]
  ...
  $(G\text{ simple} \Rightarrow \text{has genuine-hinge panel realization at target rank})$
  and $(\text{has genuine-hinge panel realization at target rank})$ both hold
  (\cref{def:genuine-hinge-realization}).
\end{lemma}
\begin{proof}
  \leanok
  \uses{lem:theorem-55-base-producer-empty, lem:theorem-55-base-producer-single,
        lem:theorem-55-base-producer-parallel}
  Dispatch on the base trichotomy (\cref{lem:two-vertex-trichotomy}). In the empty and
  single-edge arms the bare genuine-hinge panel realization already holds, so the
  $G$-simple conjunct is trivially implied. In the parallel-pair arm ($k = 0$) a
  two-vertex minimal-$0$-dof-graph cannot be simple (the single-vertex cut supplies
  two distinct parallel edges, both forced to link the same pair, which simplicity
  collapses --- contradiction); the $G$-simple antecedent is therefore vacuous.
\end{proof}
