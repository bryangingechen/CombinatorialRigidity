...
\cref{lem:rows-polynomial-in-normals} (B0); and (ii) the device needs a
...
the glue \cref{lem:realization-of-independent-rows}, shared by both cases;
what remains in each is the \emph{placement}: the seed realization to feed it. For
Case~I the producer (\cref{lem:case-I-realization}) is KT
eq.~(6.3)'s block-triangular rank-addition over one common framework, supplying the
$D(|V(G)|-1)$ independent rows directly with KT Claim~6.4
(\cref{lem:claim-6-4}) as the surviving-block input; for Case~II
the placement is the $1$-extension normal
(\cref{lem:case-II-realization-placement}). Nor
does the device supply \cref{prop:rigidity-matrix-prop11}'s genericity-free
upper bound $\mathtt{hub}$, a distinct partition-machinery obligation
(\cref{lem:trivial-motions-rank-bound}); the device underwrites only the
$\mathtt{hgen}$ half.
...
the induction --- those are the producer nodes
(\cref{lem:case-I-realization}, \cref{lem:case-II-realization},
\cref{lem:case-III}), each of which feeds a seed realization through the
device. The device is also consumed by the Case~III candidate-framework
genericity (\cref{sec:molecular-algebraic-induction-claim612}).
...
\begin{lemma}[Rank-input rank polynomial; Phase 22i L4b-1]
  ...
  The deficiency-aware sibling of the rank-polynomial device
  \cref{lem:rows-polynomial-in-normals}: a framework $\mathrm{ofNormals}\,G\,
  \mathrm{ends}\,q_0$ with a rank lower bound $N \le
  \mathrm{rank}\,R(G,q_0)$ (no full rigidity required) yields a nonzero
  rational polynomial $Q$ whose non-vanishing at any $q$ forces the same
  rank bound $N \le \mathrm{rank}\,R(G,q)$.
\end{lemma}
\begin{proof}
  \leanok
  \uses{lem:rows-polynomial-in-normals}
  The rigid sibling \cref{lem:rows-polynomial-in-normals} uses the
  rigidity of the seed to extract $D(|V(G)|-1)$ independent linking
  panel rows; here the rank bound $N$ feeds the rank-input W6e extractor
  directly, yielding exactly $N$ such rows. The same Gram-determinant
  $g/c/\varphi$ coordinatization and
  $\mathtt{exists\_polynomial\_ne\_zero\_of\_linearIndependent}$ device
  produce $Q$; at a non-root $q$ the extracted subfamily is linearly
  independent, so $N \le \mathrm{finrank}(\mathrm{span}\,
  \mathrm{rigidityRows}(G,q))$ via \texttt{finrank\_span\_eq\_card}
  and \texttt{Submodule.finrank\_mono}.
\end{proof}

\begin{lemma}[Case II placement: the $1$-extension normal, one row short; KT (6.12)]
  ...
  The eq.-(6.12) placement brick of \cref{lem:case-II-realization}: from the
  inductive realization of the splitting-off $G_v^{ab}$ at a seed $q$ (rigid on
  $V(G_v^{ab})$, transversal hinges), construct a free normal assignment $q_0$
  for the parent graph $G$ --- placing the re-inserted body $v$'s panel normal at
  the \emph{shear} $n_a + t\,n_b$, $t \ne 0$ --- together with a linearly
  independent family of $D(|V(G)|-1) - 1$ rigidity rows of
  $\mathrm{ofNormals}\,G\,\mathrm{ends}\,q_0$, \emph{one row short} of the
  $k = 0$ full target $D(|V(G)|-1)$; the missing $+1$ is the Case-III
  redundant-row content (\cref{lem:case-III-claim-6-11},
  \cref{lem:case-III-candidate-row}). Geometrically this is the
  \emph{degenerate} placement of KT eq.~(6.12): the shear identity makes $v$'s
  $b$-hinge \emph{reproduce} the $e_0 = ab$ hinge of the inductive realization
  (the $vb$-row equals the $e_0$-row of $R(G_v^{ab}, q)$), while $t \ne 0$
  keeps $v$'s $a$-hinge a nondegenerate line $L \subset \Pi_q(a)$. So the
  inductive rows transport onto $G$ through the common subgraph $G - v$ without
  an $e_0$-free old block --- the deleted $e_0$ constraint is recovered as the
  reproduced $vb$-row, not as a free transport. This is the shallower parallel
  of the Case~I splice placement (one re-inserted body, $+(D-1)$ rows). The two
  earlier dead-ends N7b-4 (\cref{lem:case-II-placement-e0-recovery}) and M1--M3
  (\cref{lem:case-II-placement-motion-side-assembly}) are \emph{off} the live
  route, retained only for the audit trail; see ``The superseded row-side and
  motion-side dead-ends'' below.
\end{lemma}
\begin{proof}
  \leanok
  ...
  inductive realization's normals on $V(G)\setminus\{v\} = V(G_v^{ab})$ and
  assign $v$ the sheared normal $n_a + t\,n_b$ ($t \ne 0$); overriding only the
  fresh body $v$ leaves the inductive block untouched, so the IH rigidity
  transports to $q_0$. The family is the union of the \emph{new-edge block} ---
  the $D-1$ panel rows of $v$'s $b$-edge, independent through $v$'s screw
  column (\cref{lem:case-II-placement-new-rows}) --- and the \emph{old block}
  --- $D(|V(G)|-2)$ independent panel rows extracted from the inductive
  realization's rigidity (\cref{lem:case-II-placement-old-rows-extract}) and
  transported onto $G$ through the common subgraph $G - v$
  (\cref{lem:case-II-placement-old-rows}; the per-row match is exactly the
  $vb$-row reproduction, so the full old count transports with no $e_0$-free
  sub-selection). The pin-a-body column split
  (\cref{lem:case-II-placement-block-independent}, the Lean form of KT's
  eq.~(6.16) column operations) makes the union independent: the old rows
  vanish on $v$'s column, the new rows read it. The count is
  $(D-1) + D(|V(G)|-2) = D(|V(G)|-1) - 1$. See
