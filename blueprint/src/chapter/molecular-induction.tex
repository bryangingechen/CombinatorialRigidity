...
\begin{lemma}[Rigid-subgraph contraction preserves minimality; KT Lemma 3.5]
  ...
  \uses{def:rigid-subgraph, lem:subgraph-minimality, def:k-dof, lem:rigid-full-rank}
...
\end{lemma}
\begin{proof}
  \uses{lem:rigid-full-rank}
  Contracting a rigid subgraph removes $D(|V(H)| - 1)$ from both the
  rank of $M(\tilde G)$ and the ambient $D(|V| - 1)$
  (\cref{lem:rigid-full-rank}), so the corank --- hence the deficiency
  (\cref{thm:def-eq-corank}) --- is unchanged.
