...
This chapter develops the \emph{Grassmann--Cayley / extensor algebra}
layer of Katoh--Tanigawa~\cite[\S2.1]{katohTanigawa2011}, the algebraic
foundation the molecular conjecture of Tay and Whiteley, proved by
Katoh--Tanigawa~\cite{katohTanigawa2011}, is built on. The development
is purely algebraic --- no metric, order, or characteristic hypothesis
is used --- so it is stated over an arbitrary field $K$; Katoh--Tanigawa's
original argument is the $K = \R$ specialization, which is what the
later, genuinely metric chapters
(\cref{sec:molecular-rigidity-matrix} onward) instantiate. It is new
linear algebra: mathlib carries the exterior algebra $\bigwedge[K] M$
but not the Plücker/extensor coordinatization, the join, or the
affine-subspace-to-extensor map. \Cref{sec:molecular-homog} homogenizes
points of $K^{d}$ into $K^{d+1}$ and characterizes affine independence
of a family of points as linear independence of the homogenized family
(\cref{lem:affine-indep-iff}). \Cref{sec:molecular-extensor} builds the
symbolic extensor and join on the exterior algebra $\bigwedge[K]
K^{d+1}$, the coordinatized Plücker bridge, and the extensor
$C(\cdot)$ of an affine subspace. \Cref{sec:molecular-lemma21} proves
the chapter's target, \textbf{Lemma~2.1}
(\cref{lem:extensor-independence}): the $D = \binom{d+1}{2}$ many
$(d-1)$-extensors associated to $d+1$ affinely independent points are
linearly independent --- the linear-algebraic fact the hardest case of
the conjecture's algebraic induction
(\cref{sec:molecular-algebraic-induction}, Case~III) bottoms out on. The
panel-hinge rigidity matrix built from this algebra follows in
\cref{sec:molecular-rigidity-matrix}.

\subsection{Homogeneous coordinates and affine independence}
\label{sec:molecular-homog}

We work in $K^{d}$ with points homogenized into $K^{d+1}$. Throughout,
...
(\cref{sec:body-bar} and \cref{sec:body-hinge}); the screw-motion reading
of $D$ is specific to the $K = \R$ specialization, but the dimension
count itself is the field-general fact this chapter proves.

\begin{definition}[Homogeneous coordinatization]
  \label{def:homogeneous-coords}
  \lean{CombinatorialRigidity.Molecular.homogenize}
  \leanok
  For a point $p \in K^{d}$ write $\bar p = (p, 1) \in K^{d+1}$ for its
  homogeneous coordinate vector. A finite family $p_1, \dots, p_k$ of
  points of $K^{d}$ is \emph{affinely independent} when the homogenized
...
\begin{lemma}[Affine independence via homogenization and the top extensor]
  ...
  Points $p_1, \dots, p_k \in K^{d}$ are affinely independent if and
  only if the homogenized family $\bar p_1, \dots, \bar p_k$ is linearly
  independent in $K^{d+1}$ --- equivalently, once the join is in hand
...
\begin{proof}
  ...
  available it is the image in $\bigwedge^{k} K^{d+1}$, nonzero exactly
...
\begin{definition}[Extensor]
  ...
  \leanok
  A \emph{$j$-extensor} in $K^{d+1}$ is a decomposable element of the
  $j$-th exterior power $\bigwedge^{j} K^{d+1}$, i.e.\ one of the form
  $v_1 \wedge \cdots \wedge v_j$ for vectors $v_i \in K^{d+1}$
  (Barnabei--Brini--Rota~\cite{barnabeiBriniRota1985}). We build the
  symbolic layer on mathlib's $\bigwedge[K] K^{d+1}$ --- the extensor is
  the value of mathlib's alternating multilinear map into the exterior
  algebra, landing in the graded piece $\bigwedge^{j} K^{d+1}$ --- and
...
\begin{definition}[Plücker coordinates]
  ...
  Assemble a family $v_1, \dots, v_j \in K^{d+1}$ into the
...
\begin{definition}[Extensor of an affine subspace]
  ...
  $p_1, \dots, p_k \in K^{d}$, its \emph{extensor} is
  $C(\cdot) = \bar p_1 \vee \cdots \vee \bar p_k \in \bigwedge^{k} K^{d+1}$,
...
\begin{lemma}[Extensor independence; Katoh--Tanigawa Lemma 2.1]
  ...
  Let $p_1, \dots, p_{d+1} \in K^{d}$ be affinely independent. The
  ...
  \]
  are linearly independent in $\bigwedge^{d-1} K^{d+1}$.
\end{lemma}
\begin{proof}
  \leanok
  ...
  the $e + 2$ points are in $K^{e+1}$ and each omit-two extensor is an
...
the conjecture's algebraic induction
(\cref{sec:molecular-algebraic-induction}) bottoms out on: at the
$K = \R$ specialization the later chapters instantiate, a nonzero screw
$r \in \R^{D}$ orthogonal to the extensors of $d+1$ distinct generic
panels would, by \cref{lem:extensor-independence}, be orthogonal to a
spanning set of $\R^{D}$ and hence zero. The panel-hinge rigidity
matrix that uses this fact is built next, in
\cref{sec:molecular-rigidity-matrix}.
