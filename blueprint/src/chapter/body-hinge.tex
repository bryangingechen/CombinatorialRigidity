...
index for \textbf{Phase~16}: the body-hinge (equivalently
panel-hinge) Tay--Whiteley theorem in $n$-space, in the
existence-of-realization form. It is the natural follow-on to the
body-bar Tay theorem of \cref{sec:body-bar} (Phase~15) and reduces to
it through a single combinatorial device --- replacing each hinge by a
bundle of $\delta - 1$ coincident bars --- so the chapter adds no new
linear algebra, only the hinge-to-bar reduction and its bookkeeping.

The nodes below are mostly \emph{red} (no \leanok) on first build:
this dep-graph is the to-do list, formalized leaf-first in subsequent
sessions per the forward-mode discipline. As each Lean declaration
lands, its node gains a \lean{} pointer and \leanok in the same commit.
...
\begin{definition}[Edge multiplication $(\delta-1)\cdot G$]
  ...
  and $|E(m \cdot G)| = m\,|E(G)|$, and a subset spans the same
  vertices in $m \cdot G$ as its image does in $G$.
\end{definition}
...
\begin{proof}
  ...
  A generic hinge two-extensor assignment realizes the
  $\delta - 1$ coincident bars of each hinge as an independent bundle
  spanning the hinge's dual space, so the induced body-bar framework on
  $(\delta - 1)\cdot G$ is independent iff the body-hinge framework is;
  rigidity and independence then transport node-for-node through the
  body-bar predicates of \cref{sec:body-bar-framework}.
\end{proof}
...
(Proposition~6 in~\cite{whiteley1988}), deferred out of Phase~15,
is re-assessed here and the standard-basis specialization on the
multiplied graph is again sufficient for the witness form.
