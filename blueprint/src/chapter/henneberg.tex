...
\begin{proof}
  ...
  degree-$\le 2$ branch collapses to $\deg_G(v) = 2$. In each branch,
  the corresponding move's edge-count identity
  (\cref{lem:typeI-edgeSet-ncard} or \cref{lem:typeII-edgeSet-ncard})
  combined with $G$'s tight global count
  $|E(G)| + 3 = 2 |V|$ forces $G'$'s edge count to be $2 |\{w \ne v\}|
  - 3$, upgrading $G'$'s sparsity from
  \cref{thm:isSparse-exists-typeI-or-typeII-reverse} to tightness,
  hence Laman.
\end{proof}
