...
\begin{proof}
  ...
  pendant branch ($\deg_G(v) = 1$) is impossible and we dispatch it
  by contradiction. In the Type~I branch (where the sparse
  decomposition already gives $\deg_G(v) = 2$ and the two neighbors),
  \cref{thm:typeI-isLaman-iff} (combined with the canonical typeI
  iso) bumps $G'$'s sparsity to Laman. In the Type~II branch, the
  typeII iso transports $G$'s Laman to $\mathrm{typeII}\,G'\,x\,y\,c$,
  and \cref{lem:typeII-edgeSet-ncard} combined with $G$'s tight global
