...
\end{lemma}
\begin{proof}
  \uses{def:g-cut-edges, lem:normal-cross-count-rooted,
    lem:normal-cross-count-fiber, lem:tight-partition-cross-pair-nbr}
  The normal cross edges rooted at $A$ partition, by their common
  neighbor $v \in A$ and their endpoint $w \notin A$, into the fibers
  over the $d_G(A)$ crossing edges $vw$ of $A$ (\cref{def:g-cut-edges});
  each fiber has exactly two elements
  (\cref{lem:normal-cross-count-fiber}), and distinct crossing edges
  give disjoint fibers because $v$ and $w$ are recovered from the edge
  $uw$ (\cref{lem:tight-partition-cross-pair-nbr},
  \cref{lem:normal-cross-count-rooted}). Summing over the crossing edges
  gives $2\,d_G(A)$.
\end{proof}
