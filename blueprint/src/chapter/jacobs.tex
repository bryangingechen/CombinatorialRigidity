...
\begin{lemma}[Normal cross edges rooted at a part; JJ eq.\ (6)]
  ...
  number exactly $2\,d_G(A)$, where $d_G(A)$ is the number of edges of
  $G$ with exactly one endpoint in $A$.
\end{lemma}
\begin{proof}
  \uses{lem:normal-cross-count-rooted, lem:normal-cross-count-local,
    lem:normal-cross-count-producer, lem:tight-partition-cross-pair-nbr}
  A normal cross edge rooted at $A$ is determined by a crossing edge
  $vw$ of $G$ ($v \in A$, $w \notin A$) together with a neighbor
  $u \in N_G(v) \cap A$: the cross edge is $uw$. Each crossing edge at
  $A$ contributes exactly two such pairs
  (\cref{lem:normal-cross-count-local}), and distinct pairs yield
  distinct normal cross edges by the uniqueness of the common neighbor
  (\cref{lem:tight-partition-cross-pair-nbr}); every normal cross edge
...
\begin{fmlnote}
  ...
  As anticipated, this node decomposed during formalization. Three
  bricks are separated out: the well-definedness of the root
  ...
  (\cref{lem:normal-cross-count-local}); and the construction of a
  rooted normal cross edge from such a pair
  (\cref{lem:normal-cross-count-producer}), the map $(vw, u) \mapsto uw$.
  What remains in \cref{lem:normal-cross-count} is the double count
  assembling these across all of $A$'s $d_G(A)$ crossing edges, the
  pair-to-edge assignment being injective by uniqueness of the common
  neighbor (\cref{lem:tight-partition-cross-pair-nbr}).
\end{fmlnote}
