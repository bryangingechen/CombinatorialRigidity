...
\begin{fmlnote}
  ...
  As anticipated, this node decomposed during formalization. The
  well-definedness of the root (\cref{lem:normal-cross-count-rooted})
  and the local count --- each crossing edge at $A$ contributes exactly
  two pairs, i.e.\ its endpoint in $A$ has exactly two in-part neighbors
  (\cref{lem:normal-cross-count-local}) --- are separated out; what
  remains in \cref{lem:normal-cross-count} is the global injectivity of
  the pair-to-edge assignment across all of $A$'s crossing edges,
  yielding the count $2\,d_G(A)$.
\end{fmlnote}
