...
\begin{lemma}[Cross pairs of a tight partition]
  \label{lem:tight-partition-cross-pair}
  \uses{def:D-deficiency}
  Let $G$ be a simple graph and $P$ a tight partition of $V$. If
  $D \ge 3$, then any two distinct parts of $P$ are joined by at most
  one edge of $G$. If $D \ge 5$, then any two nonadjacent vertices
  lying in distinct parts have at most one common neighbor.
\end{lemma}
\begin{proof}
  \uses{lem:tight-partition-subfamily}
  Both are instances of \cref{lem:tight-partition-subfamily}. Two edges
  between parts $A \ne B$ give $e_G(Q) \ge 2$ for $Q = \{A, B\}$,
  contradicting $(D-1)\cdot 2 \le D$ for $D \ge 3$. For the second
  claim, let $u, w$ be nonadjacent in distinct parts with common
  ...
  $Q$ with either two edges between two of its members (excluded
  above) or $e_G(Q) \ge |Q|$ with $|Q| \le 4$, contradicting
  \cref{lem:tight-partition-subfamily} for $D \ge 5$.
\end{proof}
...
(\cref{lem:tight-partition-cross-pair}), and the cross edge is
...
\end{lemma}
\begin{proof}
  \uses{lem:tight-partition-cross-pair, lem:isLaman3-degree-le-three,
  ...
  is unique (\cref{lem:tight-partition-cross-pair}). For a special
...
\begin{lemma}[Special cross edges at a singleton part; JJ eq.\ (5), (7)]
  ...
  \]
  Every special cross edge arises this way from exactly one singleton
  part.
\end{lemma}
\begin{proof}
  \uses{lem:square-cliques, lem:clique-edgesIn-count,
    lem:isLaman3-degree-le-three, lem:tight-partition-cross-pair,
    lem:tight-partition-subfamily, lem:square-cross-classification}
  ...
  between $A$ and $\{v\}$ (\cref{lem:tight-partition-cross-pair}) ---
  ...
  by \cref{lem:tight-partition-cross-pair}), special because $v$'s
  ...
  cases. Conversely a special cross edge's common neighbor is a
  singleton part (\cref{lem:square-cross-classification}), and it is
  unique, so the assignment is a partition of the special cross edges.
\end{proof}
...
\begin{lemma}[Normal cross edges rooted at a part; JJ eq.\ (6)]
  ...
  rooted at exactly one part, that part has at least three vertices,
  and for each part $A$ with $|A| \ge 3$ the normal cross edges rooted
  at $A$ number exactly $2\,d_G(A)$, where $d_G(A)$ is the number of
  edges of $G$ with exactly one endpoint in $A$.
\end{lemma}
\begin{proof}
  \uses{lem:square-cross-classification, lem:isLaman3-degree-le-three,
    lem:tight-partition-parts, lem:tight-partition-cross-pair}
  The root is well defined and non-singleton by
  \cref{lem:square-cross-classification}. Fix $A$ with $|A| \ge 3$. A
  normal cross edge rooted at $A$ is determined by an edge $vw$ of $G$
  with $v \in A$, $w \notin A$, together with a neighbor
  $u \in N_G(v) \cap A$: the cross edge is $uw$. For such $v$: its
  in-part degree is at least two (\cref{lem:tight-partition-parts})
  and its total degree at most three
  (\cref{lem:isLaman3-degree-le-three}), so $v$ lies on exactly one
  crossing edge and has exactly two neighbors inside $A$; each
  crossing edge at $A$ therefore contributes exactly two pairs
  $(vw, u)$. Distinct pairs yield distinct normal cross edges by the
  uniqueness of the common neighbor
  (\cref{lem:tight-partition-cross-pair}), and every normal cross edge
  rooted at $A$ arises from the pair given by its common neighbor and
  its endpoint outside $A$. Hence the count is $2\,d_G(A)$.
\end{proof}

\begin{fmlnote}
  \label{fmlnote:normal-cross-split}
  This is the one node of the chapter expected to decompose further
  during formalization --- into the local count (each crossing edge at
  $A$ contributes exactly two pairs) and the global injectivity of the
  pair-to-edge assignment across all of $A$'s crossing edges. The split
  is recorded when it stabilizes rather than fixed in advance.
\end{fmlnote}
...
\begin{proof}
  \uses{lem:exists-tight-partition, lem:square-cross-classification,
    lem:singleton-part-neighborhood, lem:normal-cross-count,
    lem:tight-partition-parts}
  If $\operatorname{def}(\tilde G) = 0$ the bound is the Laman
  condition at $X = V$ (minimum degree two forces $|V| \ge 3$). Let
  $P = \{P_1, \dots, P_t\}$ be a tight partition
  (\cref{lem:exists-tight-partition}). Summing the classification of
  \cref{lem:square-cross-classification} with the counts of
  \cref{lem:singleton-part-neighborhood} and \cref{lem:normal-cross-count}, and
  bounding the edges of $G^2$ inside a part $P_i$ with $|P_i| \ge 3$ by
  the Laman condition (a singleton part spans none):
  \[
  ...
  special-edge count of \cref{lem:singleton-part-neighborhood}. Since
  $\sum_i |P_i| = |V|$ and $\sum_i d_G(P_i) = 2\,d_G(P)$ (each crossing
  edge is counted from both sides), the right-hand side is
...
lemma (Whiteley~\cite[Lemma~9.1.3]{whiteley1996}; Lemma~3.3
of~\cite{jacksonJordan2008}): attaching a new vertex to $s \le 3$
distinct old vertices adds exactly $s$ to the generic rank. On a fixed
vertex type the operation is stated by comparing a graph $H$ with the
graph $H - E_H(v)$ obtained by deleting the edges at a vertex $v$.
...
\begin{lemma}[Row-independence lift for a vertex of degree at most three]
  ...
  $s \le 3$, and let $H' = H - E_H(v)$. If $E(H')$ is row-independent
  at a placement $p' : V \to \R^3$, then there is a placement $p$,
  agreeing with $p'$ away from $v$, at which $E(H)$ is
  row-independent.
\end{lemma}
\begin{proof}
  Move only $v$: the rows of $E(H')$ do not involve $v$'s coordinates
  and stay independent. The $s$ new rows have their nonzero blocks in
  $v$'s three columns, given by the difference vectors $p(v) - p'(u_i)$
  to the $s \le 3$ distinct neighbors $u_i$; a position $p(v)$ making
  these vectors linearly independent (generic choice; for $s = 3$, not
  coplanar with the three neighbors) makes the new rows independent
  from each other and from the old ones, as in the planar lifts
  (\cref{lem:typeI-rowIndependent-lift}, \cref{lem:pendant-rowIndependent-lift}).
\end{proof}

\begin{corollary}[$0$-extension: rank and independence]
  \label{cor:zero-extension-genericRank}
  \uses{def:genericRank, def:genericRigidityMatroid}
  Let $H$ be a graph on $V$ and $v$ a vertex of $H$. Then
  \[
    r(H) \;=\; r\bigl(H - E_H(v)\bigr) + \min\bigl(3,\, d_H(v)\bigr),
  \]
  and if $d_H(v) \le 3$, then $E(H)$ is independent in
  $\mathcal{R}_3(V)$ if and only if $E(H - E_H(v))$ is.
\end{corollary}
\begin{proof}
  ...
  $E(H - E_H(v))$ together with any $\min(3, d_H(v))$ of the edges at
  $v$ is independent, by \cref{lem:zero-extension-rowIndependent}
  applied to the subgraph carrying just those edges at $v$
  (\cref{lem:genericRigidityMatroid-indep-iff}). For the upper bound,
  the rows of the edges at $v$ lie, modulo the rows not involving $v$,
  in the three-dimensional space of $v$'s coordinate columns, so
  adjoining them raises the rank by at most three; and trivially by at
  most $d_H(v)$. The independence equivalence at $d_H(v) \le 3$
  follows since $|E(H)| = |E(H - E_H(v))| + d_H(v)$ and independence
  of a finite edge set is the equality of its rank with its
  cardinality.
\end{proof}

\begin{lemma}[Independence under vertex-set restriction]
  \label{lem:genericMatroid-induce-transport}
  \uses{def:genericRigidityMatroid}
  Let $S \subseteq V$ and let $H$ be a graph on $S$. Then $E(H)$ is
  independent in $\mathcal{R}_3(S)$ if and only if its image under the
  inclusion $S \hookrightarrow V$ is independent in $\mathcal{R}_3(V)$.
\end{lemma}
\begin{proof}
  \uses{lem:genericRigidityMatroid-indep-iff}
  ...
  row transport of \cref{lem:isSparse-of-rowIndependent-two}.
\end{proof}
...
\begin{proof}
  \uses{cor:genericMatroid-indep-isLaman3, thm:jacobs-min-degree-two,
    ...
    cor:zero-extension-genericRank, lem:genericMatroid-induce-transport,
    lem:square-delete-degree-le-one}
  ...
  let $G' = G - E_G(v)$; then $G'^2 = G^2 - E_{G^2}(v)$
  (\cref{lem:square-delete-degree-le-one}), $G'^2$ is Laman
  (\cref{lem:isLaman3-mono}), and $v$ has degree
  $d_G(u) \le 3$ in $G^2$ ($u$ the neighbor of $v$;
  \cref{lem:isLaman3-degree-le-three}), so
  \cref{cor:zero-extension-genericRank} reduces the independence of
  $E(G^2)$ to that of $E(G'^2)$, which holds by induction. Otherwise
  every vertex has degree zero or at least two. If $G$ has no edges,
  $E(G^2)$ is empty. Else restrict to the support $S$ of $G$
  (\cref{lem:genericMatroid-induce-transport}; the Laman condition
  restricts, and squaring commutes with dropping isolated vertices):
  the restricted graph has minimum degree at least two on $S$, and
  \cref{thm:jacobs-min-degree-two} applies. This makes Jackson--Jord\'an's
...
\end{theorem}
\begin{proof}
  \uses{cor:zero-extension-genericRank, lem:square-delete-degree-le-one,
    lem:genericMatroid-induce-transport}
  ...
  and since $v$ has $G^2$-degree $d_G(u)$, the rank drops by
  $\min(3, d_G(u))$ (\cref{cor:zero-extension-genericRank}). If
  ...
  tree $G - v$ on $|V| - 1$ vertices
  (\cref{lem:genericMatroid-induce-transport} for the isolated
...
\begin{proof}
  ...
  $G^2$-degree $d_G(u)$, so the rank drops by $\min(3, d_G(u))$
  (\cref{cor:zero-extension-genericRank};
