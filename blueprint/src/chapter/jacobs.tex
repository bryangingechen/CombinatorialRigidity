...
\begin{fmlnote}
  ...
  The formalization separates this count into four
  pieces: the well-definedness of the root
...
\begin{corollary}[$0$-extension at degree at most three]
  ...
  \lean{SimpleGraph.zero_extension_genericRank_add_degree,
    SimpleGraph.zero_extension_indep_iff_of_degree_le_three}
  \leanok
...
\begin{corollary}[Rank of a $0$-extension at a clique neighborhood]
  ...
  \lean{SimpleGraph.zero_extension_genericRank_add_min_le,
    SimpleGraph.zero_extension_genericRank_add_min_of_isClique}
  \leanok
...
\begin{proof}
  ...
  The lower bound holds with no hypothesis on $N_H(v)$: for
  $d_H(v) \le 3$ it is \cref{cor:zero-extension-degree-le-three}, and
  for $d_H(v) \ge 4$, restricting the star at $v$ to three of its
  edges gives a degree-three $0$-extension $H_3 \le H$ with
  $r(H_3) = r(H - E_H(v)) + 3$ by
  \cref{cor:zero-extension-degree-le-three}, so
  $r(H - E_H(v)) + 3 \le r(H_3) \le r(H)$ by monotonicity of the rank.
  For the matching upper bound, $d_H(v) \le 3$ is again
  \cref{cor:zero-extension-degree-le-three}. For $d_H(v) \ge 4$, fix
  three neighbors $u_1, u_2, u_3$ and let $H_3$ be $H - E_H(v)$
  together with the edges $vu_1, vu_2, vu_3$, so
  $r(H_3) = r(H - E_H(v)) + 3$ as above; it suffices that every
  further edge $vw$, $w \in N_H(v) \setminus \{u_1, u_2, u_3\}$, lies
  in the closure of $E(H_3)$. The five vertices $v, u_1, u_2, u_3, w$
  span a $K_5$ in $H$ whose edges other than $vw$ all lie in
  $E(H_3)$: the six edges among $u_1, u_2, u_3, w$ are edges of the
  clique $N_H(v)$, none of them incident to $v$. That $K_5$ minus
  $vw$ is independent — starting from the empty graph, add its nine
  edges vertex by vertex, each step a $0$-extension of degree at
  most three (\cref{cor:zero-extension-degree-le-three}) — while the
  full $K_5$ is dependent, since ten edges on five vertices violate
  the Laman count (\cref{cor:genericMatroid-indep-isLaman3}). Hence
  $vw$ lies in the closure of the other nine edges, and
  $r(H) \le r(H_3) = r(H - E_H(v)) + 3$.
\end{proof}

\begin{lemma}[Independence and rank under vertex-set restriction]
  \label{lem:genericMatroid-induce-transport}
  \lean{SimpleGraph.genericRigidityMatroid_indep_image_iff,
    SimpleGraph.genericRank_eq_rk_image}
  \leanok
...
\begin{definition}[The core]
  \label{def:two-core}
  \lean{SimpleGraph.twoCore, SimpleGraph.twoCore_le, SimpleGraph.le_twoCore,
    SimpleGraph.twoCore_minDegree}
  \leanok
  ...
  $H$-degree at least two.
\end{definition}

For a connected graph on at least two vertices the core is empty
exactly when the graph is a tree~\cite{jacksonJordan2008}; the
development below does not use this fact.

\begin{theorem}[The degree-one rank formula for trees; JJ Lemma 4.2(a)]
  \label{thm:degree-one-rank-tree}
  \lean{SimpleGraph.degree_one_rank_tree}
  \leanok
...
\begin{proof}
  \leanok
  \uses{cor:zero-extension-clique-rank, cor:zero-extension-degree-le-three,
    lem:square-delete-degree-le-one, lem:square-leaf-neighborhood}
  Induction on $|V|$, with base case a single edge:
  $r(K_2^2) = r(K_2) = 1 = 2\cdot 2 - 5 + 2$
  (\cref{cor:zero-extension-degree-le-three} at either endpoint). For
  $|V| \ge 3$, pick a leaf $v$ with neighbor $u$; here $d_G(u) \ge 2$,
  since $d_G(u) = 1$ would make $\{u, v\}$ a connected component of
  the tree $G$. Remove the edges at $v$: squaring commutes with the
  removal (\cref{lem:square-delete-degree-le-one}), and $v$ has
  ...
  tree $G - v$ on $|V| - 1$ vertices, drops by the same amount.
\end{proof}

\begin{theorem}[The degree-one rank formula; JJ Lemma 4.2(b)]
  \label{thm:degree-one-rank}
  \lean{SimpleGraph.degree_one_rank}
  \leanok
...
\begin{fmlnote}
  \label{fmlnote:degree-one-fixed-carrier}
  Both inductions are formalized over a fixed vertex set: deleting the
  edges at the leaf keeps the vertex in $V$ as an isolated vertex, and
  the induction runs on strengthened statements in which $|V|$ is read
  as the size of the support of $G$, connectivity as reachability
  between support vertices, and, for \cref{thm:degree-one-rank}, the
  hypothesis that $G$ is not a tree as the edge count
  $|E(G)| \ge |V(G)|$. The core is likewise carried as a graph on the
  same vertex set, with $V(G^{\mathrm{core}})$ its support. No
  restriction of the ambient vertex set --- and hence no transport of
  ranks along an inclusion
  (\cref{lem:genericMatroid-induce-transport}) --- is needed. The tree
  formula is formalized in the subtraction-free form
  $r(G^2) + 5 = 2|V| + |V_1(G)|$.
\end{fmlnote}
