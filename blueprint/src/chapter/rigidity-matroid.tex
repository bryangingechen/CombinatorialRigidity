...
\begin{proof}
  ...
  family $p_t := (1 - t)\,p_0 + t\,q$ for $t \in \R$. The rigidity
  row $\mathrm{row}_{p_t}(e) \in \mathrm{Hom}_\R(\mathrm{Framework}\,V\,2, \R)$
  is \emph{affine in $t$} at each edge $e$ (since $p_t u - p_t v$ is
  affine in $t$). Hence for each $S \subseteq E(K_V)$, ``$S$ is
  row-LI at $p_t$'' is the non-vanishing of a Gramian-style
  polynomial in $t$ of degree at most $2 |S|$. For $S \in
  \mathcal{S}_n$ this polynomial is nonzero at $t = 0$ (the IH); for
  $S = I_0$ it is nonzero at $t = 1$. Each nonzero polynomial has
  finitely many roots, so the union of bad-$t$ values across
  $\mathcal{S}_n \cup \{I_0\}$ is finite; pick $t$ outside this
  union and $p_t$ is row-LI on the enlarged subfamily.
\end{proof}
