...
\emph{Local mirror of the matroid-union subsystem}).
...
\begin{lemma}[Uniformly generic placement, dim $2$]
  ...
  $\mathrm{fromEdgeSet}\,I$ $(2, 3)$-sparse, $I$ is row-LI at $p$.
\end{lemma}
\begin{proof}
  \leanok
  ...
  \]
  The base case $\mathcal{S} = \emptyset$ picks any placement.
  Inductive step: given $p_0$ row-LI on every set in an $n$-element
  subfamily $\mathcal{S}_n \subseteq \mathcal{S}$, and a new set
  $I_0 \in \mathcal{S} \setminus \mathcal{S}_n$,
  \cref{thm:isSparse-exists-rowIndependent-placement} yields a
  placement $q$ row-LI on $I_0$. Form the linear-interpolation
  family $p_t := (1 - t)\,p_0 + t\,q = p_0 + t\,(q - p_0)$ for $t \in \R$.
  The rigidity row $\mathrm{row}_{p_t}(e) \in
  \mathrm{Hom}_\R(\mathrm{Framework}\,V\,2, \R)$ is \emph{affine in $t$}
  at each edge $e$ (\texttt{rigidityRow\_add\_smul}: linearity of
  $\mathrm{row}$ in $p$). Hence for each $S \subseteq E(K_V)$,
  ``$S$ is row-LI at $p_t$'' is the non-vanishing of a one-variable
  ...
  is nonzero at $t = 0$ (the IH); for $S = I_0$ it is nonzero at
  $t = 1$. Each nonzero polynomial in $\R[t]$ has finitely many roots,
  so the union of bad-$t$ values across $\mathcal{S}_n \cup \{I_0\}$ is
  finite; pick $t$ outside this union and $p_t$ is row-LI on the
  enlarged subfamily. The rectangular-LI / Gram-det iff and the
  cofiniteness-along-an-affine-line step are packaged as
  \texttt{Matrix.linearIndependent\_rows\_iff\_det\_mul\_transpose\_ne\_zero}
  and
  \texttt{LinearIndependent.finite\_setOf\_not\_along\_affine\_path}
  in the \texttt{Mathlib/LinearAlgebra/Matrix/Rank.lean} mirror.
\end{proof}
...
\begin{proof}
  ...
  $(2, 3)$-sparse edge subset of $K_V$ is row-LI at $p$. The two
  matroids share the ground set
  ...
  in $\mathcal{L}_p(V)$ iff $I$ is row-LI at $p$
  (\cref{lem:linearRigidityMatroid-indep-iff}); it is independent in
  $\mathcal{R}(V)$ iff $I$ is row-LI at \emph{some} placement
  ...
  genericity, which carries any $(2, 3)$-sparse $I$ to row-LI at $p$.
\end{proof}
