...
\begin{lemma}[The generic rank dominates the rank at any placement]
  ...
  \uses{def:genericRank, def:generic-placement,
    lem:genericRigidityMatroid-indep-iff, lem:genericRank-eq-finrank-span}
...
\begin{lemma}[A simple graph has a shadowing multigraph carrier]
  ...
  $6(|V| - 1)$ edge labels available. The deficiency
  $\operatorname{def}(\tilde{G'})$ depends only on the adjacency structure
  of $G$, so it is a well-defined invariant $\operatorname{def}(\tilde G)$
  of the simple graph.
\end{lemma}
\begin{proof}
  Take the label type large enough (any type of cardinality exceeding
  $6(|V|-1)$ carrying the adjacency edges) and let $G'$ be the multigraph
  whose links realize exactly the adjacencies of $G$. Because $G$ is simple,
  $G'$ is simple with the same adjacency relation as $G$; the crossing-edge
  and partition counts entering $\operatorname{def}$
  (\cref{def:D-deficiency}) read only that relation, so any two such
  carriers have equal deficiency.
\end{proof}
