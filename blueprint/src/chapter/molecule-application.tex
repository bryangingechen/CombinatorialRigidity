...
\begin{lemma}[Lower bound: attainment leg]
  ...
  \lean{SimpleGraph.molecule_rank_lower_bound}
  \leanok
  ...
  nonempty vertex set $V$. Then
  $3|V| - 6 - \operatorname{def}(\tilde G) \le r(G^2)$.
\end{lemma}
\begin{proof}
  \leanok
...
\begin{lemma}[Upper bound]
  ...
  \lean{SimpleGraph.molecule_rank_upper_bound}
  \leanok
  ...
  Let $G$ be a simple graph of minimum degree at least two on a finite,
  nonempty vertex set $V$. Then
  $r(G^2) \le 3|V| - 6 - \operatorname{def}(\tilde G)$.
\end{lemma}
\begin{proof}
  \leanok
  ...
  Unlike the lower bound, no earlier result supplies an endpoint selector
  for the shadowing carrier here, so the proof builds one directly: a
  linked label gets its (classically chosen) link pair, and every
  never-linked label --- in particular every padding label --- gets a
  fixed default pair of distinct vertices, which exists because $G$ has
  minimum degree at least two. The dictionary
  (\cref{thm:molecular-iff-square-bar-joint}) identifies
  $\dim \ker R(G^2, p)$ with the motion-space dimension $\dim Z$ of the
  resulting molecular framework of $G$ on the centres $p$, whose hinges
  are genuine because general position makes the centres distinct. The
  genericity-free lower bound $D + \operatorname{def}(\tilde G) \le \dim Z$
...
\begin{corollary}[The molecule rank formula; Jackson--Jord\'an,
  ...
  \lean{SimpleGraph.molecule_rank_formula}
  \leanok
...
\begin{proof}
  ...
  \cref{lem:molecule-rank-upper-bound} are complementary.
\end{proof}

\begin{remark}
  The rigidity reading of the formula: $G^2$ is generically rigid in
  $\R^3$ exactly when $r(G^2)$ attains the full generic rank $3|V| - 6$,
  i.e.\ when $\operatorname{def}(\tilde G) = 0$, i.e.\ when $\tilde G$
  contains six edge-disjoint spanning trees
  (\cref{thm:def-eq-corank}) --- the combinatorial flexibility count for a
  molecule modelled on $G$. This specialization, quantified over
  every placement generic for row independence, is the subject of
  \cref{sec:generic-lift-molecule}.
\end{remark}
