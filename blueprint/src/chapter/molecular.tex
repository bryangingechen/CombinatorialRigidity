...
Phases~6 onward, this chapter was authored as the phase's authoritative
dep-graph: each node carries its intended \texttt{\textbackslash uses\{\}}
dependency structure, and turned green as its Lean landed. Phase~17 is
now complete, so every node below is formalized
(\texttt{\textbackslash lean\{\}} / \texttt{\textbackslash leanok}); the
dep-graph records the realized proof structure of
