...
\begin{definition}[Hinge-row block]
  ...
  $(S(u) - S(v)) \cdot r_i(p(e)) = 0$ for all $1 \le i \le D-1$. The
  formalization keeps the screw space as the full exterior-algebra
  element (\cref{def:hinge-constraint}) and so carries the orthogonal
  complement basis-free, as the dual annihilator
  $(\operatorname{span} C(p(e)))^{\circ} \subseteq (\R^{D})^{*}$; the
  dot products become functional applications $r_i(S(u) - S(v))$ and the
  equivalence is the field-level double-annihilator identity
  $(\operatorname{span} C)^{\circ\circ} = \operatorname{span} C$. The
  explicit $\bigwedge^{k}\R^{k+2} \cong \R^{D}$ coordinatization is
  therefore still deferred to \cref{def:rigidity-matrix}.
\end{definition}
...
\begin{definition}[Panel-hinge rigidity matrix]
  ...
  The formalization carries the screw center in the degree-$k$ graded
  piece $\bigwedge^{k}\R^{k+2}$ of the exterior algebra
  (\texttt{ScrewSpace}, the subspace where the supporting extensors
  $C(p(e))$ live) and carries $R(G,p)$ as the family of per-edge hinge
  constraints on the screw-assignment space, not as an explicit real
  coordinate matrix: a screw assignment
  $S \colon V \to \bigwedge^{k}\R^{k+2}$ is an \emph{infinitesimal
  motion} (\texttt{IsInfinitesimalMotion}) when it meets the hinge
  constraint (\cref{def:hinge-constraint}) at every edge, i.e.
  $S(u) - S(v) \in \operatorname{span} C(p(e))$ for every edge $e$
  linking $u, v$. The constraint is orientation-independent (the span
  is closed under negation), so this is well-defined on the undirected
  multigraph, and the null space $Z(G,p) = \ker R(G,p)$ is the
  submodule of all infinitesimal motions
  (\texttt{infinitesimalMotions}). The
  $\bigwedge^{k}\R^{k+2} \cong \R^{D}$ coordinatization is realized as
  the dimension count
  $\operatorname{finrank} \bigwedge^{k}\R^{k+2} = D = \binom{k+2}{2}$
  (\texttt{screwSpace\_finrank}, \texttt{screwDim}) rather than an
  explicit basis --- the numeric gate for the rank bounds of
  \cref{lem:trivial-motions-rank-bound} and the degree of freedom of
  \cref{def:dof-generic}.
\end{definition}
...
\begin{definition}[Degree of freedom; generic realization]
  ...
  attain their maximum ranks over realizations of $G$.

  The formalization lands the rigidity predicate
  (\texttt{IsInfinitesimallyRigid}) as the inclusion
  $Z(G,p) \subseteq \{\text{trivial motions}\}$ (the reverse of the always-true
  $\{\text{trivial}\} \subseteq Z(G,p)$ of
  \cref{lem:trivial-motions-rank-bound}), basis-free; the numeric degree of
  freedom and the maximum-rank \emph{generic} condition wait on the explicit
  $\bigwedge^{k}\R^{k+2} \cong \R^{D}$ rank count (\cref{def:rigidity-matrix}).
\end{definition}
...
\begin{proof}
  ...
  hinge constraint, so $S^*_i \in Z(G,p)$ and is trivial
  (\texttt{isInfinitesimalMotion\_of\_isTrivialMotion}, giving
  ...
  the trivial motions. The formalization carries the trivial-motion space
  basis-free as the constant assignments
  (\texttt{trivialMotions}), identifies it with the diagonal of
  $V \to \bigwedge^{k}\R^{k+2}$
  (\texttt{trivialMotions\_eq\_range\_const}, the range of $s \mapsto (\_\mapsto s)$,
  the basis-free stand-in for ``determined by a single common value''), and
  records infinitesimal rigidity as the submodule equality
  $Z(G,p) = \{\text{trivial}\}$
  (\texttt{infinitesimalMotions\_eq\_trivialMotions\_iff}). The diagonal
  identification makes the constant-assignment map a linear isomorphism
  $\bigwedge^{k}\R^{k+2} \cong \{\text{trivial}\}$ for nonempty $V$, so with
  the screw-dimension count $\operatorname{finrank} \bigwedge^{k}\R^{k+2} = D$
  (\cref{def:rigidity-matrix}) the trivial kernel has dimension exactly
  $D$ (\texttt{finrank\_trivialMotions}) inside the $D|V|$-dimensional
  screw-assignment space (\texttt{finrank\_screwAssignment}, from
  $\operatorname{finrank}(V \to \bigwedge^{k}\R^{k+2}) = D|V|$). The
  $D$-dimensional trivial kernel therefore forces
  $\operatorname{rank} R(G,p) \le D|V| - D = D(|V|-1)$.
\end{proof}
...
\end{proposition}
\begin{proof}
  The rank form is the generic-rank specialization of Proposition~2.3 of
  Jackson--Jordán~\cite{jacksonJordan2009}, whose deficiency $\operatorname{def}(\tilde G)$
  ...
  existence form of \cref{thm:body-hinge-tay}. The rank lemmas
