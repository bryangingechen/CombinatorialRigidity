...
\begin{definition}[Panel-hinge rigidity matrix]
  ...
  The formalization keeps the screw space the full exterior-algebra
  element (\cref{def:hinge-constraint}) and carries $R(G,p)$ as the
  family of per-edge hinge constraints on the screw-assignment space,
  not as an explicit real coordinate matrix: a screw assignment
  ...
  (\texttt{infinitesimalMotions}). The explicit
  $\bigwedge^{k}\R^{k+2} \cong \R^{D}$ coordinatization stays deferred,
  as it does in \cref{def:hinge-row-block}.
\end{definition}
