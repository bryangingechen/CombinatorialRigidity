...
\begin{lemma}[Affine independence via the top extensor]
  ...
  only if the join $\bar p_1 \vee \cdots \vee \bar p_k$ is nonzero. In
  particular $d+1$ points are affinely independent iff their top extensor
  $\bar p_1 \vee \cdots \vee \bar p_{d+1}$ --- the determinant of the
  $(d+1)\times(d+1)$ matrix of their homogeneous coordinates --- is
  nonzero.
\end{lemma}
\begin{proof}
  The join of vectors in $\R^{d+1}$ is their image in
  $\bigwedge^{k} \R^{d+1}$, which is nonzero exactly when the vectors are
  linearly independent. For $k = d+1$ the top exterior power is
  one-dimensional and the coordinate of the join is the determinant of
  the homogeneous coordinate matrix; nonvanishing of that determinant is
  affine independence.
\end{proof}
