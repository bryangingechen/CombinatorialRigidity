...
\begin{proof}
  ...
  space'' fact of White--Whiteley~\cite{whiteWhiteley1987}.
\end{proof}
...
\end{lemma}
\begin{proof}
  Each hinge constrains the relative screw to the orthogonal complement
  of its supporting extensor; two extensors in general position span,
  by the extensor-independence engine
  (\cref{lem:extensor-independence}, specialized to two hinges), so
  their complements meet only at $0$ and the combined block has the full
  rank $D$.
\end{proof}
...
\begin{lemma}[Lower-semicontinuity of rank under a panel rotation; KT Lemma 5.2]
  ...
  attains rank $\rho$, so does the generic one.
\end{lemma}
\begin{proof}
  ...
  formalization is expected to run this genericity argument rather than
  an analytic perturbation (cf.\ \texttt{notes/MolecularConjecture.md}
  risk register).
\end{proof}
