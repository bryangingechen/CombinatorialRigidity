...
\medskip
\noindent\emph{Forward mode --- to-do list.} Every node in this section
is red: stated with its intended \texttt{\textbackslash uses\{\}}
dependency chain and a prose sketch of the intended proof, awaiting its
Lean. The leaf-most node is the hinge constraint
(\cref{def:hinge-constraint}); the load-bearing targets are the three
rank lemmas (\cref{lem:rank-delete-vertex,lem:rank-rotation-semicont,lem:rank-parallel-full})
and the reconciliation with Phase~16 (\cref{prop:rigidity-matrix-prop11}).
