...
\begin{proof}
  ...
  $Z(G,p) = \{\text{trivial}\}$
  (\texttt{infinitesimalMotions\_eq\_trivialMotions\_iff}). A $D$-dimensional
  trivial kernel forces $\operatorname{rank} R(G,p) \le D|V| - D$; the
  numeric $D = (k+2 \text{ choose } 2)$ count of $\bigwedge^{k}\R^{k+2}$ is
  the only piece that waits on the explicit
  $\bigwedge^{k}\R^{k+2} \cong \R^{D}$ coordinatization
