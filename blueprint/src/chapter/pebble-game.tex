...
\begin{definition}[Try-add-edge]
  ...
  at the wrapper layer via \cref{lem:workhorseWitness-certifies}; the
  user-facing $\mathrm{runPebbleGame}$ wrapper (\cref{def:runPebbleGame})
  consumes that recovery to populate $\mathrm{PebbleGameResult.reject}$'s
  proof fields (Phase~11 Layer~4b — maximal reshape: the wrapper layer
  is where the $\mathrm{Sum}$ shape collapses into the
  $\mathrm{PebbleGameResult}$ verdict).
\end{definition}
...
\begin{definition}[Run-pebble-game]
  ...
  math-layer verdict-bearing wrapper $\mathtt{runPebbleGame}\,G\,k\,\ell
  \,h_{\mathrm{mat}}$ (Phase~11 Layer~4b — maximal reshape;
  $\mathtt{noncomputable}$) enumerating
  ...
  $\mathtt{empty}$ with $\mathtt{Reachable.empty}$, and finally packaging
  the $\mathrm{Sum}$ output as a $\mathrm{PebbleGameResult}$ verdict via
  an internal aux helper (avoiding the $\mathtt{match}\,h:$ substitution
  quirk). The workhorse stays fully computable as long as $\mathtt{toSucc}$
  and the input edge list do; IO callers invoke
  $\mathtt{runPebbleGameWith}$ directly with their own $\mathrm{List}$-shaped
  data. The workhorse's output is
  $\mathrm{Sum}\,(\mathrm{WorkhorseWitness}\,k\,\ell\,V)\,
  (\mathrm{PartialOrientation}\,V)$ (Phase~11 Layer~3 shape preserved at
  the workhorse layer because $G$ is not in scope there); the wrapper
  consumes it to build the verdict.
...
\begin{theorem}[Soundness of the pebble game]
  ...
  $G$ is $(k, \ell)$-sparse. (Phase~11 Layer~4b maximal-reshape
  retains the workhorse-level statement as the single source of
  truth; the math-layer specialization
  $\mathrm{runPebbleGame\_sound}$ retired alongside
  $\mathrm{runPebbleGame\_correct}$ since the math-layer
  $\mathrm{runPebbleGame}$ now returns the verdict directly.)
\end{theorem}
\begin{proof}
  \leanok
...
matroidal regime, stated at the workhorse layer (where the
$\mathrm{Sum}$-shape lives): $\mathrm{runPebbleGameWith}$ from
$\mathrm{empty}$ against an edge list whose $\mathrm{Sym}_2$-image
is $E(G)$ returns $\mathrm{.inr}\,D$ exactly when $G$ is
$(k, \ell)$-sparse, and the $\mathrm{.inl}\,w$ branch carries the
blocking-witness data inline (Phase~11 Layer~3 reshape; the
workhorse-level return type is
$\mathrm{Sum}\,(\mathrm{WorkhorseWitness}\,k\,\ell\,V)\,
(\mathrm{PartialOrientation}\,V)$, with the $G$-shaped
sparsity-violation pair recovered by
\cref{lem:workhorseWitness-certifies}).

Phase~11 Layer~4b — maximal reshape — retires the math-layer
specialization $\mathrm{runPebbleGame\_correct}$ that previously lifted
this workhorse-level iff to a math-layer statement on
$\mathrm{runPebbleGame}\,G\,k\,\ell$. The math-layer wrapper
(\cref{def:runPebbleGame}) now returns the verdict
$\mathrm{PebbleGameResult}\,G\,k\,\ell$ directly, so the iff statement
collapses into the verdict's \emph{type}. The verdict-form correctness
restatement lives at \cref{thm:pebbleGameResult-isAccept-iff}; the
workhorse-level statement below is the single source of truth that
both wrappers' verdict constructions consume.
...
\begin{theorem}[Pebble game correctness, workhorse certificate form]
  ...
  is $E(G)$. Concretely (Phase~11 Layer~3 reshape):
  \begin{enumerate}
...
\begin{proof}
  ...
  $w.V'$. The Phase~11 Layer~3 reshape replaced the Phase~10-era
  ...
  $\mathrm{certifies\_against}$). Phase~11 Layer~4b additionally
  retired the math-layer specialization $\mathrm{runPebbleGame\_correct}$
  in favour of the verdict-form correctness statement
  \cref{thm:pebbleGameResult-isAccept-iff}.
\end{proof}
...
the algorithm's $\mathrm{Sum}$-shaped output. The Phase~11 Layer~4b
(maximal-reshape) \emph{verdict} re-packages that iff structurally:
the user-facing math- and exec-layer wrappers (\cref{def:runPebbleGame}
and \cref{def:runPebbleGameExec}) return a two-constructor inductive
$\mathrm{PebbleGameResult}\,G\,k\,\ell$ whose constructor IS the
certificate.
...
\begin{definition}[Pebble-game verdict]
  ...
  \emph{Lean placement note (Phase 11 Layer 4 / Layer 4b
  maximal reshape):} the verdict lives in
  \texttt{PebbleGame/Correctness.lean}; the math-layer wrapper
  \texttt{runPebbleGame} (\cref{def:runPebbleGame}) is in the same file,
  and the exec-layer wrapper \texttt{runPebbleGameExec} lives in
  \texttt{PebbleGame/Exec.lean}. Phase~11 Layer~4 had wrapped the
  Layer~3 $\mathrm{Sum}$-shaped raw functions additively into
  $\mathrm{runPebbleGame\_result}$ / $\mathrm{runPebbleGameExec\_result}$;
  Phase~11 Layer~4b retires that additive form by collapsing the raw
  $\mathrm{Sum}$-returning wrappers (the workhorse-level
  $\mathrm{runPebbleGameWith}$ still returns $\mathrm{Sum}$, since
  $G$ is not in scope there) into the verdict-returning wrappers. Both
  wrappers re-package the workhorse-level $\mathrm{Sum}$ output's
  constructor selection as a verdict inductive while populating the
  verdict's proof fields from $\mathrm{WorkhorseWitness.certifies\_against}$
  (\cref{lem:workhorseWitness-certifies}) on the $\mathrm{.reject}$
  side and from the Phase~9 soundness chain on the $\mathrm{.accept}$
  side. The matroidal-regime hypothesis $\ell < 2k$ is consumed by
...
\begin{corollary}[Pebble-game certificate of matroidal independence]
  ...
  = \mathrm{true}$ (Phase~11 Layer~4b — maximal reshape; the
  Phase~11 Layer~4 additive $\mathrm{runPebbleGame\_result}$-form
  bullet retired alongside the parallel
  $\mathrm{countMatroid\_indep\_iff\_runPebbleGame\_result}$).
\end{corollary}
\begin{proof}
  \leanok
