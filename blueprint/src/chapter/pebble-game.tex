...
\begin{lemma}[Failure produces a blocking witness]
  ...
  \uses{def:tryAddEdge, def:blockingWitness,
    lem:pebble-game-invariants}
  Let $D$ be a partial orientation reachable as above and $\{u, v\}$
  a candidate edge fresh w.r.t.\ $\underline{D}$ in the matroidal
  regime $\ell < 2k$, and write $G$ for $\mathrm{fromEdgeSet}\,
  (\underline{D} \cup \{\{u, v\}\})$. If $\mathrm{tryAddEdge}\,k\,
  \ell\,D\,\{u, v\}$ rejects (returns $\mathrm{none}$), then there
  exists $V' \subseteq V$ with $\ell \le k|V'|$ and
  ...
  through the algorithm's recursive path-reversal steps.
\end{lemma}
\begin{proof}
  \leanok
  \uses{lem:pebble-game-invariants,
  ...
  \texttt{span\_succ\_le\_edgesIn\_ncard\_of\_insert}) yields
