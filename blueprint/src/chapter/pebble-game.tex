...
\begin{definition}[Path-reversal move]
  ...
  and the per-vertex \texttt{out}/\texttt{peb} effects are
  formalised. The subset-level $\mathrm{out}_D(V')$ shift and the
  $\mathrm{peb}_{D, k}(V') + \mathrm{out}_D(V')$ invariant on
  subsets follow in subsequent commits feeding
  \cref{lem:pebble-game-invariants}.
\end{definition}
