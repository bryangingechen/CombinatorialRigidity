...
\begin{definition}[Arc-insertion move]
  ...
  valid). The invariant-preservation half and per-vertex
  \texttt{out}/\texttt{peb} effects are formalised; the subset-level
  effects on $\mathrm{span}_D(V')$ and $\mathrm{outOn}_D(V')$ split
  three ways on the position of $\{u, v\}$ relative to $V'$ and are
  the content of \cref{lem:pebble-game-invariants}~(3) at the
  insertion step.
\end{definition}
...
\begin{lemma}[Pebble-game invariants]
  ...
  \uses{def:partial-orientation, def:pebble-counts,
    def:arc-insertion, def:path-reversal}
