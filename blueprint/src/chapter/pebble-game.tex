...
\begin{definition}[Try-add-edge]
  ...
  transitive closure of $D$'s arc relation, routed through
  $\mathrm{reachClosureComputable}$ of \texttt{Search/DFS.lean}
  (cf.\ \cref{def:reachClosureComputable}) --- is materialised at the
  failure site inside \cref{lem:pebble-game-failure-witness}, keeping
  the algorithm's signature aligned with the $\mathrm{Option}$-shape
  of \texttt{tryReachPebbleWith}. (Phase~11 Layer~3 absorbs this
  separate extraction into \texttt{tryAddEdgeWith}'s return.)
\end{definition}
