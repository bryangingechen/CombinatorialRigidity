...
The per-edge correctness iff of L-S Lemma~14 is structurally encoded
in the Phase~11 Layer~3 reshape of \cref{def:tryAddEdge}: the
algorithm's $\mathrm{Sum}$-return shape gives one constructor per
direction --- $\mathrm{.inr}\,D'$ for accept (proved
$(k, \ell)$-sparse by \cref{lem:pebble-game-tryAddEdgeWith-isSparse})
and $\mathrm{.inl}\,w$ for reject (proved non-sparse by
\cref{lem:workhorseWitness-certifies}). The full input-graph
...
\begin{definition}[Try-add-edge]
  ...
  $\mathtt{noncomputable}$ wrapper plugging in
  $\mathtt{toSucc}\,D' := D'.\mathtt{outList}$. The function also
  takes the reachability witness $\mathtt{hD} :
  \mathrm{Reachable}\,k\,\ell\,D$ as a hypothesis, which subsumes the
  per-vertex out-degree bound previously carried as a separate
  $\mathtt{h\_outle}$ argument (Phase~11 Layer~3).

  The output is $\mathrm{Sum}\,(\mathrm{WorkhorseWitness}\,k\,\ell\,V)\,
  (\mathrm{PartialOrientation}\,V)$ (Phase~11 Layer~3): $\mathrm{.inr}\,D'$
  on accept, $\mathrm{.inl}\,w$ on simultaneous DFS failure carrying the
  workhorse-level failure witness $w$ inline. The witness's $V'$ field
  is the computable reach-union
  $\mathrm{reachClosureComputable}\,(\mathtt{toSucc}\,D)\,u \cup
  \mathrm{reachClosureComputable}\,(\mathtt{toSucc}\,D)\,v$
  (cf.\ \cref{def:reachClosureComputable}), with the algebraic invariants
  $\mathrm{outOn}_D(V') = 0$ and $\mathrm{pebOn}_{D, k}(V') \le \ell$
  established at case-5 construction time from the closure-of-reach
  property and the DFS-failure "no free pebble outside $\{u, v\}$"
  assertion respectively. The $G$-shaped sparsity-violation certificate
  $\ell \le k|V'| \wedge k|V'| < |\edgesIn[G]{V'}| + \ell$ is recovered
  at the wrapper layer via \cref{lem:workhorseWitness-certifies}.
\end{definition}
...
\begin{definition}[Run-pebble-game]
  ...
  partial orientation. On success it returns
  $\mathrm{.inr}\,D$ with $\underline{D} = E(G)$ satisfying the four
  invariants of \cref{lem:pebble-game-invariants}; on failure it
  returns $\mathrm{.inl}\,w$ carrying the workhorse-level failure
  witness $w$ (\cref{def:workhorseWitness}) inline (Phase~11 Layer~3
  reshape; the $G$-shaped blocking-witness subset $w.V'$ is recovered
  via \cref{lem:workhorseWitness-certifies}). Termination of the
  fold: the number of unprocessed edges strictly decreases per
  iteration.

  The Lean encoding splits into a computable workhorse
  $\mathtt{runPebbleGameWith}\,D\,k\,\ell\,\mathtt{hD}\,\mathtt{toSucc}\,\mathtt{h\_toSucc}\,\mathtt{edges}$
  taking the orientation state, the reachability witness
  $\mathtt{hD} : \mathrm{Reachable}\,k\,\ell\,D$, a caller-supplied
  $\mathtt{toSucc} : \mathrm{PartialOrientation}\,V \to V \to
  ...
  $\mathtt{toSucc}\,D' := D'.\mathtt{outList}$, starting from
  $\mathtt{empty}$ with $\mathtt{Reachable.empty}$. The workhorse stays
  fully computable as long as $\mathtt{toSucc}$ and the input edge list
  do; IO callers invoke $\mathtt{runPebbleGameWith}$ directly with their
  own $\mathrm{List}$-shaped data. The output is
  $\mathrm{Sum}\,(\mathrm{WorkhorseWitness}\,k\,\ell\,V)\,
  (\mathrm{PartialOrientation}\,V)$ (Phase~11 Layer~3):
  $\mathrm{.inr}\,D$ on accept (with $\underline{D} = E(G)$ once the
  fold completes successfully), $\mathrm{.inl}\,w$ on rejection
  carrying the workhorse-level failure witness $w$ produced by the
  first $\mathtt{tryAddEdgeWith}$ call that rejected.

  Each fold step performs three runtime checks before invoking
  $\mathtt{tryAddEdgeWith}$ — the candidate is not a loop, and neither
  orientation $(u, v)$ nor $(v, u)$ already sits in $D.\mathtt{arcs}$.
  All three fail only on malformed input (loops or duplicate edges);
  the algorithmic out-degree invariant $\forall x.\ \mathrm{out}_D(x)
  \le k$ is no longer checked at runtime because it follows from
  $\mathtt{hD} : \mathrm{Reachable}\,k\,\ell\,D$ (Phase~11 Layer~3
  signature absorption). A failing check is a no-op (skip and recurse
  on the unchanged $D$). On accept-branch hits, the recursive call
  receives $\mathtt{tryAddEdgeWith\_reachable}\,\dots\,\mathtt{hD}$ as
  its reachability witness for the updated orientation; on reject-branch
  hits, the witness propagates unchanged through the first $\mathrm{.inl}$.
\end{definition}
...
% Phase 11 Layer 3: `lem:pebble-game-tryAddEdgeWith-isSome`,
% `lem:pebble-game-tryAddEdge-iff-independent`, and
% `lem:pebble-game-failure-witness` retired. The "tryAddEdge accepts when
% edge is independent" / "tryAddEdge rejects produces blocking witness"
% halves are now structurally encoded by the new return type of
% `def:tryAddEdge`: the algorithm structurally has only two outcomes
% (`.inr D'` and `.inl w`), with the `.inl w` branch carrying a
% `WorkhorseWitness` whose `lem:workhorseWitness-certifies` discharge IS
% the failure-witness extraction. Only the "accept implies independent"
% half (below) remains as a named lemma.
...
\begin{lemma}[Pebble game accept implies independent]
  ...
  (Phase~11 Layer~3 reshape; was $\mathrm{some}\,D'$), then $G$ is
  $(k, \ell)$-sparse.
\end{lemma}
\begin{proof}
  \leanok
...
% Phase 11 Layer 3: `lem:pebble-game-tryAddEdge-iff-independent` and
% `lem:pebble-game-failure-witness` retired (their content is now
% structurally encoded in `def:tryAddEdge`'s reshape: the algorithm's
% `.inl w` branch IS the failure-witness extraction, with
% `lem:workhorseWitness-certifies` doing the $G$-shaped discharge).
...
$\mathrm{.inr}\,D$ exactly when $G$ is $(k, \ell)$-sparse, and the
$\mathrm{.inl}\,w$ branch carries the blocking-witness data inline
(Phase~11 Layer~3 reshape; the Lean return type is now
$\mathrm{Sum}\,(\mathrm{WorkhorseWitness}\,k\,\ell\,V)\,
(\mathrm{PartialOrientation}\,V)$, with the $G$-shaped sparsity-violation
pair recovered by \cref{lem:workhorseWitness-certifies}).
...
\begin{theorem}[Pebble game correctness, certificate form]
  ...
  $\mathrm{runPebbleGame}\,k\,\ell\,G = \mathrm{.inr}\,D$.
  Concretely (Phase~11 Layer~3 reshape):
  \begin{enumerate}
    \item If $\mathrm{runPebbleGame}\,k\,\ell\,G = \mathrm{.inr}\,D$,
      then $G$ is $(k, \ell)$-sparse and $\underline{D} = E(G)$.
    \item If $\mathrm{runPebbleGame}\,k\,\ell\,G = \mathrm{.inl}\,w$
      for some $w : \mathrm{WorkhorseWitness}\,k\,\ell\,V$, then
      $w.V'$ certifies $\ell \le k|w.V'| \wedge k|w.V'| <
...
\begin{proof}
  ...
  The biconditional comes from the dichotomy on the $\mathrm{Sum}$
  output: $\mathrm{.inr}$ closes via~(1); $\mathrm{.inl}$ closes
  via~(2) by contradicting the sparsity hypothesis at the witness
  $w.V'$. The Phase~11 Layer~3 reshape replaces the Phase~10-era
  $\mathrm{Option}$-shaped existence-form failure-witness extraction
  with this structural alternative; the case-5 inline construction
  inside \cref{def:tryAddEdge} produces the witness data directly,
  and the previously-named lemmas
  $\mathrm{tryAddEdge\_isSome\_iff\_sparse}$,
  $\mathrm{tryAddEdgeWith\_eq\_none\_imp\_exists\_witness}$,
  $\mathrm{runPebbleGameWith\_eq\_none\_imp\_exists\_witness}$, and
  $\mathrm{runPebbleGame\_eq\_none\_imp\_exists\_witness}$ are
  retired (their content absorbed into the new return type and
  $\mathrm{certifies\_against}$).
\end{proof}

\subsection{User-facing verdict}
\label{sec:pebble-game-verdict}

The certificate-form correctness theorem
(\cref{thm:pebble-game-correct}) states an iff between sparsity and
the algorithm's $\mathrm{Sum}$-shaped output. The Phase~11 Layer~4
\emph{verdict} re-packages that iff structurally: instead of
exposing the raw $\mathrm{Sum}$ result and an external iff theorem,
the user-facing wrapper returns a two-constructor inductive whose
constructor IS the certificate.
...
\begin{definition}[Pebble-game verdict]
  ...
  $\mathrm{.reject}$. The verdict's constructor IS the certificate:
  the Phase~9-era certificate-form iff
  $G\text{ is }(k,\ell)\text{-sparse} \iff \exists D,\, \mathrm{algorithm
  returns } \mathrm{.inr}\,D$ collapses into the verdict's TYPE.

  \emph{Lean placement note (Phase 11 Layer 4):} the verdict and
  its math-/exec-layer wrappers live in
  \texttt{PebbleGame/Correctness.lean} (math layer:
  \texttt{runPebbleGame\_result}) and \texttt{PebbleGame/Exec.lean}
  (exec layer: \texttt{runPebbleGameExec\_result}). Both stay
  additive on top of Phase~11 Layer~3's $\mathrm{Sum}$-shaped raw
  functions $\mathrm{runPebbleGame}$ / $\mathrm{runPebbleGameExec}$,
  which remain available; the Phase~11 Layer~4 wrappers re-package
  the $\mathrm{Sum}$ output's constructor selection as a verdict
  inductive while populating the verdict's proof fields from
  $\mathrm{WorkhorseWitness.certifies\_against}$
  (\cref{lem:workhorseWitness-certifies}) on the
  $\mathrm{.reject}$ side and from the Phase~9 soundness chain on
  the $\mathrm{.accept}$ side. The matroidal-regime hypothesis
  $\ell < 2k$ is consumed by $\mathrm{certifies\_against}$.
\end{definition}

\begin{theorem}[Pebble game correctness, verdict form]
  \label{thm:pebbleGameResult-isAccept-iff}
  \lean{CombinatorialRigidity.PebbleGame.PartialOrientation.runPebbleGame_result_isAccept_iff,
    CombinatorialRigidity.PebbleGame.PartialOrientation.runPebbleGameExec_result_isAccept_iff}
  \leanok
  \uses{def:pebbleGameResult, thm:pebble-game-correct, def:isSparse}
  Let $V$ be finite, $G : \mathrm{SimpleGraph}\,V$, and $k, \ell$
  integers with $\ell < 2k$. Then $G$ is $(k, \ell)$-sparse if and
  only if the verdict-bearing wrapper
  $\mathrm{runPebbleGame\_result}\,G\,k\,\ell$ (resp.\ its exec-layer
  sibling) returns an $\mathrm{.accept}$ verdict — equivalently, its
  $\mathrm{.isAccept}$ Boolean projection is $\mathrm{true}$.
\end{theorem}
\begin{proof}
  \leanok
  \uses{thm:pebble-game-correct}
  The verdict's $\mathrm{.isAccept}$ projection is $\mathrm{true}$
  exactly when the underlying $\mathrm{Sum}$-shaped wrapper returned
...
\begin{corollary}[Pebble-game certificate of matroidal independence]
  ...
  \lean{SimpleGraph.countMatroid_indep_iff_runPebbleGame,
    SimpleGraph.countMatroid_indep_iff_runPebbleGame_result}
  \leanok
  \uses{thm:pebble-game-correct, thm:pebbleGameResult-isAccept-iff,
    def:countMatroid, thm:countMatroid-indep-iff}
  ...
  (\cref{def:countMatroid}) if and only if any of the equivalent
  pebble-game outputs accepts:
  \begin{itemize}
    \item certificate form: $\mathrm{runPebbleGame}\,k\,\ell\,G =
      \mathrm{.inr}\,D$ for some partial orientation $D$ (Phase~11
      Layer~3 reshape; was $\mathrm{some}\,D$);
    \item verdict form: $\mathrm{runPebbleGame\_result}\,k\,\ell\,G
      = \mathrm{.accept}\,(D, \dots)$ — equivalently,
      $(\mathrm{runPebbleGame\_result}\,k\,\ell\,G).\mathrm{isAccept}
      = \mathrm{true}$ (Phase~11 Layer~4).
  \end{itemize}
\end{corollary}
\begin{proof}
  \leanok
  ...
  \cref{thm:pebble-game-correct} (resp.\
  \cref{thm:pebbleGameResult-isAccept-iff}), the latter holds iff
  the pebble game's certificate-form output (resp.\ verdict-form
  output) is on the accept branch.
\end{proof}
