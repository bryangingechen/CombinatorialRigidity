...
\begin{lemma}[Underlying-edge tracking through the fold]
  ...
  ordered pairs. If
  $\mathtt{runPebbleGameWith}\,D\,k\,\ell\,\mathtt{hD}\,\mathtt{toSucc}\,\mathtt{h\_toSucc}\,\mathit{edges}
   = \mathrm{.inr}\,D'$ (Phase~11 Layer~3 reshape; was $\mathrm{some}\,D'$),
  then
  \begin{enumerate}
...
\begin{lemma}[No skip fires; underlying-edge identity on the wrapper]
  ...
  $\mathtt{runPebbleGameWith}\,D\,k\,\ell\,\mathtt{hD}\,\mathtt{toSucc}\,\mathtt{h\_toSucc}
  \,\mathit{edges} = \mathrm{.inr}\,D'$ (Phase~11 Layer~3 reshape; was
  $\mathrm{some}\,D'$), then every $p \in \mathit{edges}$ satisfies
...
\begin{proof}
  ...
  \le k$ from \cref{lem:pebble-game-invariants}~(1) is no longer a
  per-step runtime check after the Phase~11 Layer~3 absorption into
  $\mathtt{hD} : \mathrm{Reachable}\,k\,\ell\,D$); on success
