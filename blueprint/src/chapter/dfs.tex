...
Mathlib has no verified search algorithms in its
\texttt{Combinatorics.SimpleGraph} hierarchy: all of its
$\mathrm{Walk}$ / $\mathrm{Reachable}$ infrastructure is declarative
(``a walk exists'', ``a reverse exists''), not algorithmic. The only
ecosystem precedent for a verified iterative graph-state algorithm
is \texttt{Batteries.UnionFind}, whose termination uses a
strictly-decreasing numeric measure on the data structure. The DFS
here follows the same pattern, with the strictly-decreasing measure
specialised to a $\mathrm{Finset}$-cardinality on unvisited vertices.
