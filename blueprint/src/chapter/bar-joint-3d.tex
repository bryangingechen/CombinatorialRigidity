...
molecule application reads at $d = 3$; the restatement at the
rigidity matrix of a single graph
(\cref{lem:rigidityMap-rank-at-generic}) is the form the generic
realization statements of \cref{sec:generic-lift} use.
...
\begin{lemma}[Dimension two: the planar rigidity matroid]
  ...
  \lean{SimpleGraph.genericRigidityMatroid_two_eq_rigidityMatroid}
  \leanok
...
\begin{lemma}[Row-space form of the generic rank]
  ...
  \lean{SimpleGraph.genericRank_eq_finrank_span}
  \leanok
...
\begin{lemma}[Realized rank at a generic placement]
  \label{lem:rigidityMap-rank-at-generic}
  \lean{SimpleGraph.finrank_range_rigidityMap_eq_genericRank}
  \leanok
  \uses{def:generic-placement, def:genericRank, def:rigidityMap}
  Let $p : V \to \R^d$ be a placement generic for row independence.
  For every graph $H$ on $V$ the rank of the rigidity matrix of
  $(H, p)$ equals the generic rank:
  $\operatorname{rank} R(H, p) = r_d(H)$.
\end{lemma}
\begin{proof}
  \leanok
  \uses{lem:genericRank-eq-finrank-span}
  The rank of $R(H, p)$ equals the dimension of the span of the
  rigidity rows of $H$'s edges at $p$ (row rank equals column rank),
  which is $r_d(H)$ by \cref{lem:genericRank-eq-finrank-span}.
\end{proof}
