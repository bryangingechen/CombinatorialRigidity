...
\begin{proof}
  ...
  \Cref{thm:isLaman-exists-typeI-or-typeII-reverse} provides a
  vertex $v$ and a Laman graph $G'$ on $\{w \ne v\}$: either
  $\deg_G(v) = 2$ with $G' = G[\{w \ne v\}]$, or $\deg_G(v) = 3$ with
  $G' = G[\{w \ne v\}] \cup \{\{a, b\}\}$ for a non-adjacent neighbor
  pair $\{a, b\}$. The vertex type $\{w \ne v\}$ has cardinality
  $|V| - 1$, so the induction hypothesis applies to $G'$: it is
  injectively generically rigid in dimension $2$. Reconstruct the
  isomorphism $G \simeq \mathrm{typeI}\,G'\,a\,b$ or
  ...
  (\cref{thm:typeI-isGenericallyRigidInj-two} or
  \cref{thm:typeII-isGenericallyRigidInj-two}) to
  $\mathrm{typeI}\,G'\,a\,b$ or $\mathrm{typeII}\,G'\,a\,b\,c$, and
  transport the conclusion back to $G$ along that isomorphism
  (\cref{lem:isGenericallyRigidInj-iso}).
\end{proof}
...
size $2|V| - 3$ exists whenever $G$ is generically rigid (a rank lower
bound together with a linear-algebra extension). Combining (a) and (b)
gives a $(2, 3)$-tight spanning subgraph, i.e.\ a Laman subgraph.
...
row-independent --- is not needed for the iff itself; it is proved
separately in \cref{sec:rigidity-matroid}, where, combined with the
easy direction, it identifies the rigidity matroid with the
$(2, 3)$-count matroid.
...
\begin{definition}[Row-independent edge set at a placement]
  ...
  Let $G$ be a graph on $V$, let $p : V \to \R^{d}$ be a
  ...
  Equivalently, the composite of $\mathrm{RigidityMap}\,G\,p$ with
  the restriction map $(E(G) \to \R) \to (I \to \R)$ has rank $|I|$.
\end{definition}

\begin{remark}
  The row-independent edge sets form the independent sets of a
  matroid on $E(G)$, the \emph{rigidity matroid} of $(G, p)$. The
  $\Rightarrow$ direction of Laman's theorem uses only the
  independence relation and the two facts about it below; the matroid
  itself is assembled in \cref{sec:rigidity-matroid}.
\end{remark}

\subsubsection{The rigidity rows as a family in the algebraic dual}

The row-basis extraction (\cref{lem:exists-rowIndependent-edge-basis})
works throughout in the algebraic dual $(\mathrm{Framework}\,V\,d)^{*}$,
where the row-rank-equals-column-rank identity
...
\begin{definition}[Rigidity row at a placement]
  ...
  evaluation-at-$e$ map $(E(G) \to \R) \to \R$. As a
...
\begin{lemma}[Rank lower bound at a generically rigid placement, general dimension]
  ...
  Let $V$ be finite and let $G$ be a graph on $V$. If $G$ is
...
\begin{proof}
  ...
  $d(d+1)/2$, which rearranges to the stated bound. This is the rank-half
  of the edge-count bound \cref{thm:isGenericallyRigid-card-mul-le},
  keeping the rank itself in place of its upper bound by the edge count
  (\cref{lem:rigidityMap-finrank-range-le}).
\end{proof}
...
\begin{proof}
  ...
  row family (\texttt{exists\_linearIndepOn\_extension}); its
  cardinality equals the dimension and so is at least $2|V| - 3$.
  Truncate to size $2|V| - 3$ (\texttt{Set.exists\_subset\_card\_eq})
  and read $I$ as the edge labels indexing the chosen rows;
...
trivial Euclidean motions of the framework (translations plus
...
\begin{lemma}[Rank upper bound from affine spanning, general dimension]
