...
\begin{definition}[Edges induced on a vertex set]
  ...
  \]
  In Lean, $\edgesIn{S}$ is implemented as
  \texttt{G.edgeSet $\cap$ s.sym2}, where \texttt{s.sym2} is the set of
  unordered pairs supported on $s$.
\end{definition}
...
\begin{proof}
  ...
  $v = \varphi(q)$. The Lean proof unfolds the
  \texttt{Sym2.map}-image membership and uses
  \texttt{Iso.map\_adj\_iff} to transport adjacency. Both directions
  are by case analysis on the symmetric pair's two representatives.
\end{proof}
...
edge-counting input used in Phase~5 to extract a tight set from a
sparsity violation on a candidate graph.
