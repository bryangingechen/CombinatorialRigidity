...
edge-count identity used in the minimum-degree argument for Laman
graphs in later chapters.
...
The map $\edgesIn{\cdot}$ distributes exactly over intersection, but
only sub-distributes over union: a crossing edge with one endpoint in
$S \setminus T$ and the other in $T \setminus S$ lies in
$\edgesIn{S \cup T}$ but in neither $\edgesIn{S}$ nor $\edgesIn{T}$. The
cardinality consequence is the supermodular bound below, used for the
tight-subset lattice in \cref{thm:isTightOn-union-inter}.
...
\end{definition}
\begin{fmlnote}
  The bound is stated additively, as
  $\bigl|\edgesIn{s}\bigr| + \ell \le k\,|s|$ under the hypothesis
  $\ell \le k\,|s|$, rather than as
  $\bigl|\edgesIn{s}\bigr| \le k\,|s| - \ell$. The two forms agree
  exactly when $\ell \le k\,|s|$, and the additive one avoids truncated
  subtraction over $\mathbb{N}$.
\end{fmlnote}
...
Tightness of a subset is the local counterpart of $(k,\ell)$-tightness
of the whole graph (\cref{def:isTight}): the graph is tight when
equality holds on the full vertex set, and $s$ is tight when it holds on
$s$. In a $(2, 3)$-sparse graph, a $(2, 3)$-tight subset is what
Jord\'an~\cite{jordan2016}~\S 2.1 calls a \emph{critical set}; the two
terms are interchangeable, and we use ``tight'' throughout.
...
are used in the Henneberg constructions for Laman graphs and for
arbitrary $(2,3)$-sparse graphs in later chapters. The first uses the
handshake identity to show that some vertex has degree at most $3$; the
second applies the sparsity bound at a $4$-vertex subset to show that
any three neighbors of a vertex contain a non-adjacent pair.
