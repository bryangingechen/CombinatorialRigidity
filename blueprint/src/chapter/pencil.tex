...
\begin{theorem}[Pencil realization from the contraction and split cases, conditioned pair]
  ...
  Suppose that for every loopless multigraph $G$ with $|V(G)| \ge 3$: (i) if
  every strictly smaller such graph satisfies the conditioned pair
  (\cref{def:pencil-conditioned-pair}) at $n = 3$, then so does $G$
  whenever it has a proper rigid subgraph, and so does $G$ whenever it is
  two-edge-connected with no proper rigid subgraph; and (ii) whenever $G$
  is simple and nondegeneracy-feasible (\cref{def:pencil-nondegenerate})
  and is obtained from a smaller multigraph by attaching one new vertex
  to a single vertex $u$ by one edge, with $u$ of degree exactly three in
  $G$, $G$ has a generic pencil realization
  (\cref{def:pencil-generic-motive}). Then every multigraph on the whole
  ambient body set satisfies the conditioned pair at $n = 3$.
\end{theorem}
\begin{proof}
  \leanok
  ...
  one further vertex to the rest, provided that vertex's unique
  neighbour has degree other than three in $G$. The one remaining
  configuration, a single further vertex attached at degree exactly
  three, is hypothesis (ii) above.
\end{proof}
