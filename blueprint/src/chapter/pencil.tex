...
does not work here: at $K_4$ every vertex's closed hub-neighbourhood
(below) is all four vertices, and the necessity direction of
\cref{lem:two-pencil-extension-iff} forces every hub's panel normal into
the same vertex's orthogonal complement, so no simple realization of
$K_4$ has independent hub normals. The conditioning below asks instead
for the realization's own nondegenerate-satisfiability.
...
\begin{definition}[The conditioned pair]
  ...
  (\cref{def:pencil-rank-hypothesis}) and, whenever it is
...
\begin{fmlnote}
  ...
  statement (\cref{def:rank-hypothesis}), with nondegeneracy-feasibility
  in place of simplicity as the conditioning, for the reason given above.
  Dropping the first conjunct recovers the bare realization of
  \cref{def:pencil-rank-hypothesis} directly.
\end{fmlnote}
...
\begin{proof}
  ...
  (\cref{def:pencil-nondegenerate}); the base and cut-edge cases'
  generic conjuncts remain to be established.
\end{proof}
