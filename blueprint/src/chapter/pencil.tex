...
\begin{lemma}[The base case, $|V(G)| \le 2$]
  ...
  \lean{CombinatorialRigidity.Molecular.hasPencilRealization_of_ncard_le_two}
  \leanok
...
\begin{proof}
  \leanok
  The one-vertex and edgeless cases contribute no rank (a single fixed
  hinge with no incidence to discharge); a single edge contributes rank
  $D - 1$ from one genuine hinge; a class of parallel edges reduces to
  \cref{lem:pencil-base-parallel-pair}'s two independent hinges (any
  further edges reuse the second hinge and cost nothing), whose
  realization is infinitesimally rigid on both bodies.
\end{proof}
...
\begin{theorem}[Pencil realization from the contraction and split cases]
  ...
  \lean{CombinatorialRigidity.Molecular.pencil_conjecture_of_arms}
  \leanok
...
\subsection{Nondegenerate realizations and the conditioned pair}
\label{sec:pencil-nondegenerate}

Discharging the contraction and split hypotheses of
\cref{thm:pencil-conditional-realization} is expected to need the
induction statement strengthened by a genericity conjunct, the way
Theorem~5.5's induction statement carries a generic full-rank
realization conditioned on simplicity alongside the bare panel
realization (\cref{def:rank-hypothesis}). Conditioning on simplicity
alone does not work here: at $K_4$ every vertex's closed
hub-neighbourhood (below) is all four vertices, and the necessity
direction of \cref{lem:two-pencil-extension-iff} forces every hub's
panel normal into the same vertex's orthogonal complement, so no
simple realization of $K_4$ has independent hub normals. Conditioning
on the realization's own nondegenerate-satisfiability alone does not
work either: a multigraph with a parallel class of hinges can carry a
nondegenerate realization without any hub, yet no such realization
attains the deficiency-rank target, since the class's two hinges are
then forced onto a single common supporting line. The conditioning
below asks for both together: simplicity, as in Theorem~5.5, and the
realization's own nondegenerate-satisfiability.

\begin{definition}[Nondegenerate pencil realization, feasibility]
  \label{def:pencil-nondegenerate}
  \lean{Graph.PencilHub, Graph.closedHubNbhd, Graph.closedNbhd,
        CombinatorialRigidity.Molecular.IsNondegPencilRealization,
        CombinatorialRigidity.Molecular.PencilNondegFeasible}
  \leanok
  \uses{def:pencil-panel-realization}
  A vertex of a multigraph is a \emph{pencil hub} when it has degree at
  least three, the degree at which the point conjunct of
  \cref{def:pencil-panel-realization} first bites. The \emph{closed
  hub-neighbourhood} of a vertex $v$ is the set of pencil hubs among $v$
  itself and its neighbours, and the \emph{closed neighbourhood} of $v$
  is $v$ together with all of its neighbours, hub or not. A pencil
  realization of $G$ (\cref{def:pencil-panel-realization}) is
  \emph{nondegenerate} when the two concurrency points of every link are
  projectively distinct; at every vertex, the panel normals of its
  closed hub-neighbourhood are linearly independent; and at every
  non-hub vertex, the concurrency points of its closed neighbourhood are
  linearly independent. A multigraph is \emph{nondegeneracy-feasible}
  when it has a nondegenerate pencil realization.
\end{definition}
\begin{fmlnote}
  \label{fmlnote:pencil-nondegenerate-hub}
  The closed hub-neighbourhood of $v$ is exactly the set of vertices
  whose panel normal a link's concurrency point at $v$ is forced
  orthogonal to: $v$'s own normal by the panel--point incidence of
  \cref{def:pencil-panel-realization}, and a neighbouring hub's normal by
  the necessity direction of \cref{lem:two-pencil-extension-iff} applied
  along the connecting link.
\end{fmlnote}
\begin{fmlnote}
  \label{fmlnote:pencil-nondegenerate-nonhub}
  The third conjunct is not automatic from the first two: at a non-hub
  vertex (degree at most two) two coplanar lines are automatically
  concurrent, so nothing about projectively distinct link endpoints or
  independent hub normals prevents the two hinges meeting at such a
  vertex from coinciding at the same point. The conjunct rules out
  exactly that coincidence, and so bites only where the closed
  neighbourhood has all three of its members (an ordinary degree-two
  vertex with two distinct neighbours); a lower-degree vertex is already
  covered by nonzero-ness together with the link conjunct above. It is
  not imposed at hubs, whose closed neighbourhood can have arbitrarily
  many members.
\end{fmlnote}

\begin{definition}[Generic pencil realization]
  \label{def:pencil-generic-motive}
  \lean{CombinatorialRigidity.Molecular.HasGenericPencilRealization}
  \leanok
  \uses{def:pencil-nondegenerate, def:pencil-rank-hypothesis}
  A multigraph $G$ \emph{has a generic pencil realization} when it has a
  nondegenerate pencil realization (\cref{def:pencil-nondegenerate})
  whose rigidity rows attain the deficiency-rank target of
  \cref{def:pencil-rank-hypothesis}.
\end{definition}

\begin{definition}[The conditioned pair]
  \label{def:pencil-conditioned-pair}
  \lean{CombinatorialRigidity.Molecular.PencilPair}
  \leanok
  \uses{def:pencil-nondegenerate, def:pencil-generic-motive,
        def:pencil-rank-hypothesis}
  A multigraph $G$ satisfies the \emph{conditioned pair} at $n$ when it
  has a pencil realization at the deficiency rank
  (\cref{def:pencil-rank-hypothesis}) and, whenever it is simple and
  nondegeneracy-feasible (\cref{def:pencil-nondegenerate}), a generic one
  (\cref{def:pencil-generic-motive}).
\end{definition}
\begin{fmlnote}
  \label{fmlnote:pencil-conditioned-pair}
  This is the pencil analogue of Theorem~5.5's own conditioned induction
  statement (\cref{def:rank-hypothesis}), now conditioned on both
  simplicity, exactly as Theorem~5.5, and nondegeneracy-feasibility, for
  the $K_4$ reason above. Neither conjunct alone suffices: a multigraph
  with a parallel class of hinges can carry a nondegenerate realization
  (the two hinges can point to independent concurrency points and panel
  normals, with neither body a hub), yet no nondegenerate realization of
  such a multigraph attains the deficiency-rank target, since two hinges
  through the same pair of bodies are forced onto a single common
  supporting line whenever their concurrency points are projectively
  distinct, contributing one row block where the target needs two
  independent ones. Dropping the first conjunct recovers the bare
  realization of \cref{def:pencil-rank-hypothesis} directly.
\end{fmlnote}

\begin{theorem}[Pencil realization from the contraction case and two open kernels, conditioned pair]
  \label{thm:pencil-conditional-realization-pair}
  \lean{CombinatorialRigidity.Molecular.pencil_conjecture_of_arms_pair,
        CombinatorialRigidity.Molecular.pencil_conjecture_of_hcontract_hK_hbareSplit,
        CombinatorialRigidity.Molecular.pencil_conjecture_of_hcontract_hK_hbareSplit_of_card}
  \leanok
  \uses{thm:pencil-reduction, def:pencil-conditioned-pair, def:rigid-subgraph,
        def:pencil-nondegenerate, def:pencil-generic-motive, def:pencil-rank-hypothesis}
  Let $K$ be an infinite field. Suppose that for every loopless
  multigraph $G$ with $|V(G)| \ge 3$, if every strictly smaller such graph
  satisfies the conditioned pair (\cref{def:pencil-conditioned-pair}) at
  $n = 3$, then so does $G$ whenever it has a proper rigid subgraph. For the
  two-edge-connected case with no proper rigid subgraph, the conditioned
  pair holds unconditionally on three or four vertices (such a multigraph is
  forced to be the spanning $3$-cycle, resp.\ $4$-cycle). On five or more
  vertices it holds given the following, at a degree-two vertex $v$ one of
  whose two neighbours is not a pencil hub, with split-off multigraph $G'$
  already satisfying the conditioned pair: whenever $G$ is
  nondegeneracy-feasible, a generic pencil realization of $G'$ extends to one
  of $G$ at the deficiency rank (kernel (K)); whenever it is not, a pencil
  realization of $G'$ at the deficiency rank extends to one of $G$ at the
  deficiency rank (kernel (K-bare)); and some edge outside $E(G)$ is always
  available. Then every multigraph on the whole ambient body set satisfies
  the conditioned pair at $n = 3$.
\end{theorem}
\begin{fmlnote}
  \label{fmlnote:pencil-conditional-realization-pair-kernels}
  Kernels (K) and (K-bare) are the two hypotheses this formalization
  currently carries rather than proves. Both are supported by exact-rational
  computation across many habitat graphs with zero counterexamples found.
  Kernel (K) is the pencil analogue of Katoh--Tanigawa's
  Claim~6.12~\cite{katohTanigawa2011}, but is not implied by it: their proof
  of that claim — a span argument showing one of the three candidate
  certificates $M_1, M_2, M_3$ is always nonsingular — uses a six-dimensional
  span that the pencil condition cuts to five, so the pencil case is a
  genuinely open statement rather than an instance of their result.
\end{fmlnote}
\begin{proof}
  \leanok
  \uses{thm:pencil-reduction, def:pencil-nondegenerate}
  As \cref{thm:pencil-conditional-realization}, replacing the loop, base,
  and cut-edge cases of \cref{thm:pencil-reduction} by their
  conditioned-pair analogues: the loop case's generic conjunct is
  vacuous, since a loop already breaks nondegeneracy-feasibility
  (\cref{def:pencil-nondegenerate}); the base case's parallel-class
  sub-case is vacuous too, since a parallel class of edges is never
  simple, and its remaining sub-cases (no edge, one edge) each carry a
  direct nondegenerate construction. The cut-edge case's generic
  conjunct assembles the two sides' induction hypotheses directly when
  no edge crosses the cut; when a single crossing edge joins two sides
  each with at least two vertices, by repositioning one side's frame as
  in the bare cut-edge case; and when a single crossing edge attaches
  one further vertex to the rest, from the induction hypothesis on the
  larger side. The one remaining configuration, a single further vertex
  attached at a neighbour of degree exactly three, is where infiniteness
  of $K$ enters: the neighbour is a hub of $G$ but not of the larger
  side, so its promoted concurrency family is re-selected by steering the
  larger side's chart realization to a common generic seed, and the
  attached vertex's fresh data avoids the resulting exceptional locus.

  The split case's own hypothesis is built directly from the case split
  above: on three or four vertices, from a direct construction placing a
  point at each vertex; on five or more vertices, from a degree-two vertex
  with a non-hub neighbour (existence needs no minimality — a graph with
  every vertex of degree at least three already has a proper rigid
  subgraph, \cref{lem:pencil-min-degree-rigid}), a fresh edge, and whichever
  of kernel (K) or kernel (K-bare) applies to $G$'s own
  nondegeneracy-feasibility.
\end{proof}
