...
\begin{theorem}[Pencil realization from the contraction and split cases, conditioned pair]
  ...
  $n = 3$, then so does $G$ whenever it has a proper rigid subgraph, and so
  does $G$ whenever it is two-edge-connected with no proper rigid subgraph.
  Then every multigraph on the whole ambient body set satisfies the
  conditioned pair at $n = 3$.
\end{theorem}
\begin{proof}
  \leanok
