...
The two cross-incidences are not only sufficient but necessary: a
nonzero hinge lying in the panel of $n_v$ and passing through $q_u$
puts $q_u$ in the span of points orthogonal to $n_v$, forcing
$q_u \cdot n_v = 0$, and symmetrically for $q_v \cdot n_u = 0$. So the
extension hinge exists precisely when the two bodies' concurrency points
are compatible in this sense. The necessity direction rests on the
span of a nonzero decomposable $2$-extensor being determined by the
extensor, which is developed separately.
