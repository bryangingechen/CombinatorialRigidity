...
\begin{lemma}[Case II placement: the $1$-extension normal; KT (6.12)]
  ...
  \begin{enumerate}
    ...
    \item \cref{lem:case-II-placement-old-rows-extract} (N7b-0): \emph{produce} the
      old block --- extract $D(|V(G)|-2)$ independent panel rows from the inductive
    ...
    \item \cref{lem:case-II-placement-old-rows} (N7b-2): transport that family onto
      $G$ through the common subgraph $G - v$.
...
\begin{lemma}[N7b-0: a rigid realization carries a full-rank independent row family]
  ...
  \cref{lem:case-II-placement-new-rows}'s honest form). \emph{Red} (Phase~21b
  realization layer): this rank-equals-codimension forward direction is the
  genuine deficit between ``rigid on a set'' and ``carries that many independent
  rows'' --- it is \emph{not} discharged by the transport
  \cref{lem:case-II-placement-old-rows} (which only carries an
  \emph{already-witnessed} independent family across the graph swap) nor by the
  graph-forest existence \cref{def:rigidity-matrix}
