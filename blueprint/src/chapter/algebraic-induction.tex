...
now green. The device greening does \emph{not} build the placements: it lifts an
attained rank to a generic point, but in its \emph{absolute} screw-count, so each
producer must (i) convert that count to the $V(G)$-relative motive --- the green
$V(G)$-relative count bridge (\cref{lem:relative-screw-split},
\cref{lem:relative-device-count}, \cref{lem:isInfRigidOn-of-relative-count}) does
this --- and (ii) construct a seed realization.

\emph{Realization producers re-scoped to Phases 22--23 (Phase~21b close,
2026-06-04).} A math-first feasibility pass against
Katoh--Tanigawa~\cite[\S6.2--6.3]{katohTanigawa2011} found that the two
\emph{producers} — the only red realization-layer nodes — belong with the
realization layer of Phases~22--23, not Phase~21b (whose named deliverable, the
device, is green). Two findings (full record: \texttt{notes/Phase21b.md}; this
re-scope is honesty-gate-driven, \cref{lem:case-II-realization} below):
\begin{itemize}
  \item \emph{The Case~II producer (\cref{lem:case-II-realization}) is KT
        ...
        redundant-edge row (Lemma~6.10/6.13). KT's Lemma~6.8 (where $+(D-1)$
        suffices) is $k>0$ \emph{only}; the prior $k>0$ framing of
        \cref{lem:case-II} below was a conflation. The motion-side M1/M2/M3 nodes
        (\cref{lem:case-II-placement-disjoint-line-meet,%
        lem:case-II-placement-pin-vertex,lem:case-II-placement-motion-side-assembly})
        are \emph{not} KT's argument (they assume "a motion constant on
        $V(G)\setminus\{v\}$", which a $G$-motion need not be) and are superseded.
  \item \emph{The Case~I producer (\cref{lem:case-I-realization}) is full-rank
        and tractable (KT~\S6.2), but research-shaped geometry.} It splices the
        two inductive legs along boundary hinges at panel intersections (eq.~6.6),
        reaching full $D(|V|-1)$ with \emph{no} shortfall; its genuine obligation
        is the splice placement \cref{lem:case-I-splice-placement} (a
        panel-transversality lemma + block-triangular independence, three KT
        sub-cases 6.2/6.3/6.5) plus the graph$\leftrightarrow$matroid bridge
        \cref{lem:rigidContract-isMinimalKDof} (Phase-20 carry-forward).
\end{itemize}
The reusable green infra (the row sub-nodes N7b-0/1/2/3, the device-closure glue
\cref{lem:realization-of-independent-rows}, the count bridge) feeds those
producers. The capstone \cref{thm:theorem-55} and the reconciliation
\cref{prop:rigidity-matrix-prop11} therefore remain green-modulo-22--23 (awaiting
the producers and the genericity-free $\mathtt{hub}$ partition brick); Case~III
(\cref{lem:case-III}) is also Phases~22--23, where all three flip.
...
\begin{lemma}[Case II realization: the $1$-extension produces a full-rank realization; KT Lemma 6.8]
  ...
  \uses{def:panel-hinge-framework, lem:case-II, lem:reduction-step,
        lem:case-II-placement-motion-side-assembly}
  The realization motive of \cref{thm:theorem-55}
  ($\mathrm{HasFullRankRealization}$, the $V(G)$-relative rank
  ...
  framework realizes its rank. \emph{Red --- re-scoped to Phases~22--23 as KT Case~III (Phase~21b close,
  2026-06-04).} The project's $\mathtt{hsplit}$ is the $k=0$ reducible-vertex
  ...
  the sub-nodes N7b-0/1/2/3) feeds it there. The earlier motion-side path
  (\cref{lem:case-II-placement-motion-side-assembly}, M3) was \emph{not} KT's
  argument --- it assumes a $G$-motion is constant on $V(G)\setminus\{v\}$, which
  the non-rigid $G-v$ does not force --- and is superseded; the row-side
  $e_0$-recovery is supplied by the eq.~(6.12) placement, not by an
  ($e_0$-free) block. Full record: \texttt{notes/Phase21b.md} \emph{Finding A/B}.
  ...
  a full splice. \emph{Premise audit (Phase 21b, 2026-06-04):} the producer needs
...
\end{lemma}
\begin{proof}
  \uses{lem:case-II, lem:case-II-placement-motion-side-assembly}
  The motion-side assembly \cref{lem:case-II-placement-motion-side-assembly} (M3)
  supplies, from the inductive realization of $G_v^{ab}$ (rigid on
  $V(G)\setminus\{v\}$), a realization of $G$ rigid on $V(G)$: re-insert $v$ joined
  to $a, b$ by hinges whose support extensors are linearly independent
  (\cref{lem:exists-independent-panel-extensor}, $m = 2$), so a motion constant on
  $V(G)\setminus\{v\}$ is pinned at $v$ (\cref{lem:case-II-placement-pin-vertex}),
  and re-adding $G$'s remaining edges keeps it rigid
  (\cref{lem:motions-mono-of-graph-le}). That is the full target rank
  $D(|V(G)|-1)$.
\end{proof}
