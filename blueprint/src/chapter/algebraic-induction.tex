...
A math-first feasibility pass against
Katoh--Tanigawa~\cite[\S6.2--6.3]{katohTanigawa2011} placed the two
realization \emph{producers} with the realization layer of Phases~22--23:
\begin{itemize}
  ...
  \item \emph{The Case~I producer (\cref{lem:case-I-realization}) is full-rank
        ...
        row-count closure. It is green-modulo its single residual analytic
        obligation, KT Claim~6.4 (\cref{lem:claim-6-4}, the surviving block's
        exterior-projected rank, eq.~(6.9)), deferred to Phase~22b; the
        graph$\leftrightarrow$matroid bridge
        \cref{lem:rigidContract-isMinimalKDof} (a Phase-20 carry-forward) is
        green.
\end{itemize}
...
\begin{proof}
  ...
  argument selects the point attaining it. \emph{Red:} it depends on
  \cref{thm:theorem-55}, whose $V(G)$-relative motive
  ...
  the absolute $\dim Z = D + \operatorname{def}$). The genericity-free upper
  bound $\mathtt{hub}$ remains a distinct Phase-19-partition obligation, carried
