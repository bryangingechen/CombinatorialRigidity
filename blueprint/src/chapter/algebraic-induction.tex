...
\begin{lemma}[Case II placement: the $1$-extension normal; KT (6.12)]
  ...
  \begin{enumerate}
    ...
    \item \cref{lem:case-II-placement-e0-recovery} (N7b-4, red): \emph{the missing
      producer.} A scrutiny pass found N7b-0/1/2/3 do not compose: N7b-0 extracts an
      old block with no control over $e_0$-rows, but N7b-2 transports only the
      $e_0$-free rows, so the count can fail. Recovering the $e_0$-row's constraint
      from $v$'s two new edges (the path $a$--$v$--$b$, KT~(6.12)) --- equivalently
      producing an $e_0$-free old block of the full count --- is the genuine rank-lift
      content and is not discharged by the four green sub-nodes.
  \end{enumerate}
  The assembly below packages these into the witness $(q_0, s)$ the glue
  \cref{lem:realization-of-independent-rows} consumes, once N7b-4 is green.
\end{lemma}
\begin{proof}
...
\begin{lemma}[N7b-4: recover the short-circuit edge $e_0$'s constraint from $v$'s two new edges; KT eq.~(6.12)]
  ...
  The genuine $1$-extension rank-lift content, surfaced by a scrutiny pass on the
  N7b assembly (Phase 21b, 2026-06-04): the four green N7b sub-nodes do
  \emph{not} compose into the assembly without it. The inductive realization's old
  block (\cref{lem:case-II-placement-old-rows-extract}) is a family of
  $D(|V(G)|-2)$ \emph{actual} $\mathrm{panelRow}$ rows of the rigid splitting-off
  $G_v^{ab}$, but those rows are extracted from a spanning family with \emph{no
  control over whether they use the short-circuit edge} $e_0$ --- whereas the
  transport \cref{lem:case-II-placement-old-rows} can carry only the
  $e_0$-\emph{free} rows onto $G$ (the edge substitution deletes $e_0$ in $G$). If
  the extracted block uses $e_0$-rows, dropping them leaves fewer than
  $D(|V(G)|-2)$ rows and the count $(D-1) + D(|V(G)|-2) = D(|V(G)|-1)$ fails. This
  node is the missing producer: in $G$, the deleted edge $e_0$ (linking $a,b$ in
  $G_v^{ab}$) is replaced by the path $a$--$v$--$b$ of $v$'s two new edges, and the
  span-membership constraint $e_0$ imposed on a base-$v$-pinned motion
  ($S a - S b \in \operatorname{span} C(e_0)$) is \emph{recovered} from the two new
  hinge constraints at $v$ (\cref{lem:case-II-placement-new-rows}); equivalently,
  an $e_0$-free independent old block of the full count $D(|V(G)|-2)$ is exhibited
  by replacing each $e_0$-row of the extracted block with the corresponding row in
  $v$'s column. This is the actual geometric heart of why splitting-off preserves
  rank (KT~(6.12)); it is not a mechanical indexing reconciliation and is
  \emph{not} discharged by N7b-0/1/2/3. \emph{Red} (Phase~21b realization layer).
\end{lemma}
\begin{proof}
  \uses{lem:case-II-placement-old-rows-extract, lem:case-II-placement-new-rows}
  Either extract the old block directly in an $e_0$-free form (an independent
  $D(|V(G)|-2)$-family of $G_v^{ab}$-rows avoiding $e_0$, available because $v$'s
  re-attachment column carries the residual rank the $e_0$-row would have), or
  recover $e_0$'s row as a combination of $v$'s two new-edge rows through the
  $a$--$v$--$b$ path. See \cite[\S6.3]{katohTanigawa2011} (Lemma~6.8, eq.~(6.12)).
\end{proof}
