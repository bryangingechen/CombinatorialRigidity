...
\begin{itemize}
  ...
  \item \emph{The Case~I producer (\cref{lem:case-I-realization}) is full-rank
        and tractable (KT~\S6.2), reaching full $D(|V|-1)$ with \emph{no}
        shortfall.} \emph{Landed (Phase 22a) green-modulo:} it is KT eq.~(6.3)'s
        block-triangular rank-addition over a single common framework --- the
        rigid-block rows $E(H)$ $\sqcup$ the surviving-edge rows
        $E(G) \setminus E(H)$, made jointly independent by the exterior-column
        projection (KT's top-right $0$), fed to the device's row-count closure.
        Its single residual analytic obligation is KT Claim~6.4
        (\cref{lem:claim-6-4}, the surviving block's exterior-projected rank,
        eq.~(6.9)), carried as a green-modulo hypothesis and deferred to
        Phase~22b, together with the graph$\leftrightarrow$matroid bridge
        \cref{lem:rigidContract-isMinimalKDof} (Phase-20 carry-forward, green).
\end{itemize}
...
\begin{proof}
  ...
  inductive subgraphs --- chapter status). The node stays \emph{red} because not
  all three case-producers it depends on are built: Case~I
  (\cref{lem:case-I-realization}) is now green-modulo KT Claim~6.4
...
\begin{lemma}[KT Claim 6.4: the contraction block attains its rank at the generic placement (eq.\ (6.9))]
  ...
  inductive rank (\cref{lem:contraction-minimality}, on the relabelled
  collapsed graph $G/E(H)$) is transported to the surviving-edge leg
  $G \setminus E(H) \le G$ at the parent placement, the joint panel
  coefficients being algebraically independent over $\Q$ (KT \S5.1 / Claim~6.4,
  the deliberate genericity simplification). \emph{Red, deferred to Phase~22b:}
  it is carried as the green-modulo hypothesis $\mathtt{hclaim64}$ of
  \cref{lem:case-I-realization}, the single remaining analytic obligation of
  Case~I.
\end{lemma}

\begin{lemma}[Case I realization: the contraction producer; KT eqs.\ (6.3), (6.9)]
  ...
  Let $G$ be a simple minimal $0$-dof-graph with a proper rigid subgraph $H$ on
  $s = V(H)$ ($2 \le |V(H)|$, KT Lemma~6.3's case object) and representative
  body $r \in V(H)$; the (conditioned, generic) induction hypothesis supplies
  generic realizations of both $H$ and the contraction $G/E(H)$ at their target
  ranks. Then $G$ carries a generic realization at rank $D(|V(G)|-1)$,
  discharging the Case~I premise ($\mathtt{hcontract}$) of
  \cref{thm:theorem-55}.
...
\begin{proof}
  ...
  (\cref{lem:claim-6-4}), the single green-modulo hypothesis, supplied by the
  contraction's generic IH (\cref{lem:rigidContract-isMinimalKDof}) at the
  Zariski-open locus of a contraction rank polynomial's non-roots. The
