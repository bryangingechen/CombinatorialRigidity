...
enters here as an input. The remaining red nodes are the Case~I
contraction splice (\cref{lem:case-I-realization}, the geometric wiring
that feeds the device, Phase~21b) and the crux Case~III
(\cref{lem:case-III}, deferred to Phases~22--23).
...
\begin{lemma}[Case I realization: contraction splice attains full rank; KT eqs.\ (6.2), (6.6)]
  ...
  of \cref{thm:theorem-55}. As with \cref{lem:case-I}/\cref{lem:case-II}, the
  formalized carrier takes the splice's two rigidity outputs --- the rigid
  block subgraph $G_H \le G$ and the block-internal contraction subgraph
  $G_c \le G$ (the parent with $E(H)$ deleted) each realizing rigidly on the
  shared $\mathtt{ofParam}$ witness --- as explicit hypotheses, so the node is
  green modulo the splice's geometric simultaneous-rigid placement of $p$
  (the construction of a single $p$ making both subgraphs rigid).
\end{lemma}
\begin{proof}
  \leanok
  \uses{lem:case-I, lem:genericity-device}
  The splice places the rigid block $(H, p_1)$ as a single rigid body at the
  contracted vertex of $(G/E(H), p_2)$. The rigid block makes every parent
  motion constant on $s = V(H)$ (the $\mathtt{hblock}$ brick of
  \cref{lem:case-I}), and the contraction's inductive rigidity makes the
  residual block pin $Z_s(G,p)$ vanish; the block-triangular glue of
  \cref{lem:case-I} then turns the pair into rigidity of the parent, which is
  $\mathrm{RankHypothesis}\,0$ (full rank $D(|V|-1)$). The genericity device
  (\cref{lem:genericity-device}, green) underwrites the no-rank-loss at the
  generic splice point through the block-pin bounds of \cref{lem:case-I}. The
  splice's simultaneous-rigid placement of $p$ (the geometric construction of
  the single witness from $p_1$, $p_2$, and the boundary panel intersection)
  is the remaining geometric content, carried as the two rigidity hypotheses.
\end{proof}
...
analytic crux, scoped out into the focused sub-phase
\textbf{Phase~21b} (the analytic sibling of the Phase-21a meet). The
device is the node \cref{lem:genericity-device} that those consumers
reference.
