...
\begin{lemma}[Case I realization: contraction splice attains full rank; KT eqs.\ (6.2), (6.6)]
  ...
  full rank $D(|V|-1)$. This discharges the Case~I premise ($\mathtt{hcontract}$)
  of \cref{thm:theorem-55}.
\end{lemma}
\begin{proof}
  \uses{lem:case-I, lem:genericity-device}
  The splice places the rigid block $(H, p_1)$ as a single rigid body at the
  contracted vertex of $(G/E(H), p_2)$. By \cref{lem:case-I} the rank of the
  spliced realization is $D + \dim Z_s$, full at $k = 0$ (the contraction's
  block pin vanishes). The block-triangular coupling between the block rows and
  the contraction rows incurs no rank loss at a generic choice of the
  algebraically independent panel coefficients
  (\cref{lem:genericity-device}, Claim~6.4). The splice construction and its
  generic full rank are the remaining geometric content of Phase~21b.
\end{proof}
...
\begin{proof}
  ...
  \emph{Status (Phase~21b).} The formalization currently lands only the
  univariate/affine special case --- a finite root set of a one-variable minor
  along an affine line. Because the panel entries are bilinear, the consumers'
  realizations are not reached along an affine line, so the genuine multivariate
  form --- a nonzero multivariate minor polynomial has a non-root, via
  \texttt{MvPolynomial.funext} and the determinant machinery --- is the
