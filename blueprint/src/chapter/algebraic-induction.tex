...
\begin{lemma}[Cycle realizations; KT Lemma 5.4]
  ...
  to general $d$ without modification. The two-body base is
  \cref{lem:cycle-realization-base} and the upper bound $|V| \le D$ on
  the hypothesis is \cref{lem:cycle-realization-dim-bound}; the general
  cycle ($|V| \ge 3$) and the generic-panel independence argument
  supplying the independent supporting extensors (bottoming on the
  extensor-independence Lemma~2.1, \cref{lem:extensor-independence})
  remain --- the genericity device (Claim~6.4/6.9) shared with
  Cases~I and~II.
\end{lemma}
\begin{proof}
  \uses{lem:cycle-realization-base}
  The two-body base case is \cref{lem:cycle-realization-base}. For the
  general cycle, a single cycle of $k$ panels and $k$ hinges is infinitesimally rigid
  iff the $k$ hinge lines are independent in the Grassmann line
  geometry --- the dimension-$D$ extensor space supports at most $D$
  independent lines, so a cycle of length $3 \le |V| \le D$ admits a
  nonparallel realization whose supporting extensors are independent,
  hence rigid (\cite[Prop.~3.4]{crapoWhiteley1982},
  \cite[Prop.~3]{whiteley1999}). An external geometric input, cited
  rather than reproved (KT~\cite[Lemma~5.4]{katohTanigawa2011}
  likewise defers the detailed projective calculation to these
  sources).
\end{proof}
