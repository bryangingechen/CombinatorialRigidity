...
\begin{lemma}[Graph-level minimality of the rigid-subgraph contraction (the graph/matroid bridge)]
  ...
  prove its minimality to invoke the hypothesis. \emph{Red (Phase~22 target,
  gating \cref{lem:case-I-realization}); a several-node Whitney-style build, not
  the single-commit node earlier prose implied} --- see the proof.
\end{lemma}
\begin{proof}
  Unfold $\mathrm{IsMinimalKDof}$ (deficiency $0$ and every base meeting every
  surviving edge-fiber) through the matroid $\mathrm{matroidMG}$ of $G/E(H)$, and
  identify that matroid with $M(\tilde G)/E(\tilde H)$. The identification is the
  deferred graph$\leftrightarrow$matroid map, and it is the genuine content: the
  \emph{graph} $G/E(H) = (G \setminus E(H))$ with $V(H)$ collapsed to $r$
  \emph{deletes} the edges $E(H)$ and \emph{relabels} vertices (a pure
  $\mathrm{map}$, since $E(G\setminus E(H))$ is disjoint from $E(H)$), whereas the
  \emph{matroid} $M(\tilde G)/E(\tilde H)$ \emph{contracts} the fibers
  $E(\tilde H)$. Reconciling them is the classical Whitney identity
  ``contracting a connected edge set in the cycle matroid $=$ collapsing its
  vertex set in the graph'', which (i) needs $\tilde H$ connected on $V(H)$ to
  license the collapse ($0$-dof $\Rightarrow$ connected, by the cut-partition
  argument of \cref{lem:two-edge-conn}), and (ii) does \emph{not} follow from the
  per-cycle-matroid contraction lemma alone, since $\mathrm{Union}$ does not
  commute with contraction. The realizable route is independence-level
  ($\mathrm{ext\_indep}$), routing both sides through the $(D,D)$-sparsity
  characterization $\mathrm{matroidMG\_indep\_iff}$ and the contraction-independence
  characterization (a $(D,D)$-boundary rank-of-contraction count). \emph{Deferred}
  (Phase~22 realization layer).
\end{proof}
