...
\begin{lemma}[Case II realization: the $1$-extension produces a full-rank realization; KT Lemma 6.8]
  ...
  framework realizes its rank. \emph{Red:} the producer routes through the
  \emph{motion-side} rank-lift \cref{lem:case-II-placement-motion-side-assembly}
  (M3) --- re-attaching $v$ to the rigid $G_v^{ab}$ by two transversal hinges with
  independent support extensors pins $v$, so $G$ is rigid on $V(G)$ directly. This
  supersedes the original row-side path (the witnessed-independent-row placement
  \cref{lem:case-II-realization-placement} fed to the glue
  \cref{lem:realization-of-independent-rows}), which was found geometrically
  unbuildable at its missing producer N7b-4 (\cref{lem:case-II-placement-e0-recovery});
  M3 is still red, so the producer is.
