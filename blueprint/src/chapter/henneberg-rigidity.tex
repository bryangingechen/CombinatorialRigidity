...
\begin{proof}
  ...
  by $\rho(x) = x \circ \mathrm{some}$. We must show $\rho(x)$ kills
  every $G$-edge. For $\{u, v\} \ne \{a, b\}$, the edge lifts to a
  $\mathrm{typeII}$-edge with the same row formula, so the constraint
  transports.

  The deleted edge $\{a, b\}$ is recovered from the two new-edge
  constraints at $\{\mathrm{none}, \mathrm{some}\,a\}$ and
  $\{\mathrm{none}, \mathrm{some}\,b\}$. Hypothesis (i) gives
  ...
  $\langle p_b - p_a,\, x(\mathrm{some}\,a) - x(\mathrm{some}\,b)
   \rangle = 0$, which is the $\{a, b\}$-row of $G.\mathrm{RigidityMap}$
  applied to $\rho(x) = x \circ \mathrm{some}$.

  Injectivity of $\rho$ uses hypothesis (ii): if $\rho(x) = \rho(y)$
  ...
  kernel-dim bound as in \cref{thm:typeI-isInfinitesimallyRigid-extend}.
\end{proof}
