...
$\ker (G.\mathrm{RigidityMap}\,p)$, and shows it is \emph{injective}
...
\begin{itemize}
  ...
  \item Type~II: place $q$ \emph{on} the line through $p_a$ and $p_b$
    ...
    non-collinear; the unconditional Type~II wrapper produces such a
...
\begin{theorem}[Type~I rank-nullity core]
  ...
  Let $V$ be finite, $G : \mathrm{SimpleGraph}\,V$, and
...
\begin{proof}
  \leanok
  Define $\rho : \ker ((\mathrm{typeI}\,G\,a\,b).\mathrm{RigidityMap}\,\hat p)
  \to_{\R} \ker (G.\mathrm{RigidityMap}\,p)$ by
  $\rho(x) = x \circ \mathrm{some}$. The map lands in the right kernel
  because every $G$-edge $\{u, v\}$ lifts to
  ...
  \ker((\mathrm{typeI}\,G\,a\,b).\mathrm{RigidityMap}\,\hat p) \le
  \dim \ker(G.\mathrm{RigidityMap}\,p) \le 3$.
\end{proof}

\begin{theorem}[Type~I preserves injective generic rigidity in dim 2]
  ...
  Let $V$ be finite, $G : \mathrm{SimpleGraph}\,V$ injectively
...
\begin{proof}
  ...
  union of a $1$-parameter family and a finite set, hence has empty
  interior; pick $q$ off both. The pair $(q - p_a, q - p_b)$ is
  linearly independent (because $q$ is off the line), so
  \cref{thm:typeI-isInfinitesimallyRigid-extend} applies and gives
  rigidity of the extended placement. Injectivity of the extension
  follows from injectivity of $p$ and $q \notin p[V]$.
\end{proof}

\subsection{Type~II rigidity preservation}

The Type~II proof has the same shape as Type~I, but the rigidity-map
extension uses the new vertex placement $q$ in a more delicate way.
Place $q$ on the line through $p_a$ and $p_b$, distinct from both.
...
\begin{proof}
  ...
  $\rho : \ker ((\mathrm{typeII}\,G\,a\,b\,c).\mathrm{RigidityMap}\,\hat p)
   \to_{\R} \ker (G.\mathrm{RigidityMap}\,p)$
  by $\rho(x) = x \circ \mathrm{some}$. The map lands in the right
  kernel: every $G$-edge $\{u, v\}$ either lifts to a
  $\mathrm{typeII}$-edge (when $\{u, v\} \ne \{a, b\}$) with the same
  formula, or is the deleted edge $\{a, b\}$, whose constraint is
  recovered from the two new edges
  $\{\mathrm{none}, \mathrm{some}\,a\}$ and
  $\{\mathrm{none}, \mathrm{some}\,b\}$: both rigidity-rows lie along
  the direction $p_b - p_a$ thanks to hypothesis (i), and a suitable
  combination reproduces the $\{a, b\}$-row in $G$.

  Injectivity of $\rho$ uses hypothesis (ii): the third new edge at
  $\{\mathrm{none}, \mathrm{some}\,c\}$ together with one of the
  $a, b$ edges pins $x(\mathrm{none}) - y(\mathrm{none})$ against an
  LI pair, forcing it to zero. Rank--nullity then transports the
  bound as in \cref{thm:typeI-isInfinitesimallyRigid-extend}.
\end{proof}

\begin{theorem}[Type~II preserves rigidity, non-collinear case]
...
\begin{proof}
  ...
  $p[V]$ (a cofinite $\alpha$-set of points works). The
...
of $(p_a, p_b, p_c)$ admits a free 1-parameter family of perpendicular
infinitesimal motions of the new vertex, and rigidity genuinely fails.
...
\begin{theorem}[Type~II preserves injective generic rigidity in dim 2]
...
\begin{proof}
  ...
  for some non-zero $\gamma$. Pick a direction $w \in \R^{2}$ with
  $w$ not parallel to $p_0(b) - p_0(a)$, and consider the
