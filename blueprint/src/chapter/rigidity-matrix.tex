...
\begin{lemma}[Block-rank addition for splice decompositions; L5a-i shared-body brick]
  ...
  \leanok
  \uses{def:rigidity-matrix, def:hinge-row-block}
  Let $(G,p)$ be a body-hinge framework, let $V_H \subseteq V(G)$ be a vertex
  subset, and let $r \in V_H$ be a representative body.  Write
  $V_c = (V(G) \setminus V_H) \cup \{r\}$ for the complement side (sharing $r$
  with $V_H$).  Then
  \[
    \operatorname{rank} R(G[V_H],p)
    + \operatorname{rank} R(G[V_c],p)
    \;\le\;
    \operatorname{rank} R(G,p).
  \]
  This is the shared-body sibling of Lemma~\ref{lem:block-rank-cut}
  (equations~(6.3)--(6.5) of Katoh--Tanigawa~\cite[Lemma~6.2]{katohTanigawa2011}):
  the two sides overlap at $r$ rather than being vertex-disjoint, so the
  $(D-1)|C_{\mathrm{cut}}|$ correction term is absent.
\end{lemma}
\begin{proof}
  \leanok
  \uses{def:rigidity-matrix, def:hinge-row-block}
  Let $S_H = \operatorname{span} R(G[V_H],p)$, $S_c = \operatorname{span} R(G[V_c],p)$,
  and $S = \operatorname{span} R(G,p)$.  Both $S_H$ and $S_c$ inject into $S$.
  The key claim is $S_H \cap S_c = 0$.  For $\varphi \in S_H \cap S_c$ and any
  screw assignment $T : V(G) \to \mathrm{Screw}_k$, define $T' v = T r$ for
  $v \in V_H$ and $T' v = T v$ otherwise.  Since $T'$ is constant on $V_H$, every
  hinge-row generator of $S_H$ reads the difference $T' u - T' v = 0$, so
  $\varphi(T') = 0$.  Since $T - T'$ vanishes on $V_c$ (outside $V_H$ the two
  agree; at $r \in V_H$ we have $T' r = T r$), every hinge-row generator of $S_c$
  evaluates to zero on $T - T'$, giving $\varphi(T - T') = 0$.  By linearity
  $\varphi(T) = 0$, establishing $S_H \cap S_c = 0$.
  Then
  $\operatorname{finrank}(S_H) + \operatorname{finrank}(S_c)
  = \operatorname{finrank}(S_H \sqcup S_c) \le \operatorname{finrank}(S)$
  by \texttt{Submodule.finrank\_sup\_add\_finrank\_inf\_eq} and
  \texttt{Submodule.finrank\_mono}.
\end{proof}
