...
its rank directly. The infinitesimal-motion vectors are $D$-dimensional
\emph{screw centers} $S(v) \in \R^{D}$ with $D = \binom{d+1}{2}$,
realized as the degree-$(d-1)$ graded piece $\bigwedge^{d-1}\R^{d+1}$ of
...
\begin{definition}[Panel-hinge rigidity matrix]
  ...
  elsewhere. A screw assignment $S \in \R^{D|V|}$ is an infinitesimal
...
$\bigwedge^{d-1}\R^{d+1}$ rather than fixing a coordinate matrix: a screw
assignment $S \colon V \to \R^{D}$ is an infinitesimal motion when it
...
\begin{proof}
  ...
  $Q$ is nonzero (its evaluation at $q_0$ is nonzero) and rational, and
