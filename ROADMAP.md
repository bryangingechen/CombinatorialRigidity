...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm interior cert): three kernel-checked refutations this session (override §(4.91); (D-substitution) §(4.90)/row 598; route-(a) corner §(4.93)) converged on the TRUE obstruction = the CERT INTERFACE. The §(4.92) KT-faithful architecture (union-count + a genuine corner whose `±r` row IS the redundancy `ρ₀`) has the right corner core, but the cert's `hr : rRow ∈ span F.rigidityRows` slot (an UPPER-bound certification) forces the `±r` row onto a framework edge — colliding with the corner gate — whereas KT sources `±r` via the eq.-(6.63) row-op of a bottom row. The honest path is a CERT-INTERFACE RESHAPE (reopens 23e's cert); the WORKING `d=3` `_matrix`/M₃ cert is the template. User authorized redoing old work + NO shortcuts. Next = a `d=3`-anchored design-pass to scope the reshape. `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.93). |
