...
| 21a. Grassmann–Cayley meet / projective-duality foundations | `Molecular/Meet.lean` (KT §2.1 dual half) + mirror lemmas | ◷ In progress — deliverables 1–2 (`topEquiv`, `pairingDualEquiv`) green; deliverable 3 `complementIso` started (`wedgeProd`, `wedgePairing`, and the off-diagonal vanishing of the basis-vector pairing green; diagonal sign + `toDualEquiv` composition remain); `meet` remains. Mirrors under `Mathlib/LinearAlgebra/ExteriorPower/Basis.lean` + `Mathlib/Data/Finset/Card.lean`. See `notes/Phase21a.md`; dep-graph `meet.tex` `sec:molecular-meet`. |
