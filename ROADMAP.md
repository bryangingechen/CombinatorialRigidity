...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress, **BLOCKED on a user decision** — 23a–23e closed; **23f** = the geometry arm (interior cert via **(D-canonical)**, design §(4.71)). The bottom block + cert spine are BUILT (D-CAN-1/2/3a/3b + the `hsupp`/`hgp`/`Gab`-bottom/`hfr₂` feeders); the corner `hA`'s last genuinely-new source **route α is BLOCKED** — its `_escape` side-condition `q b ∉ ker(of p)` is provably FALSE for reachable matched joins (§(4.77)). **Next = a user pick** among three faithful corner re-routes: (D) the `ρ₀`-route cert re-shape (most KT-faithful — KT's `Mᵢ = [r(Lᵢ); ±r=ρ₀]`), (α′) candidate-aware discriminator, (β) KT's disjunction-over-all-`Mᵢ` count. Then `chainData_dispatch` (re/hre + router) + CHAIN-5 + the C.3 `hIH` add. See `notes/Phase23f.md` + design §(4.77). `d=3` fully green. |
