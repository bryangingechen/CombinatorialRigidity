...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a/CARRIER + 23b/CHAIN + 23c closed (23c built option (A), the general-`d` `±r`-block rank cert, but hit the member-mapping wall — all three escape routes refuted, a phase STOP; the interior-`hρe₀` conjecture-crux closed + all leaves in tree); **23d open** on the rank-certification reconsideration — route B (KT (6.64) inversion: redundant row → corner, genuine rows → base block `W`) landed all leaves (LEAF-B1/B2/B3 + LEAF-4 + the arm-corner-assembly call), but a dispatch recon found they do NOT compose: the interior dispatch's `hS` premise is UNSATISFIABLE (the wrap-edge rows relabel to the dead `(a,b)`-block tag, re-introducing the member-mapping wall) — **BLOCKED on a phase-direction decision** (option-A group transport vs. seed-framework base block re-architecture; design §(4.26), `notes/Phase23d.md`) |
