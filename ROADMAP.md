...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm interior cert) faithful architecture SCOPED (both built arms kernel-refuted; route (a) = KT's union-count + genuine `ρ₀`-corner, dropping the override device); next = a flagged corner-composition spike then the dispatch build. `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.92). |
