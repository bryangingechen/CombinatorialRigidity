...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress (three carried items — `hcontract` (W4, parked) and the kernels `hK` / `hbareSplit`; the research arc continues, **68 directions landed** (65 on kernel (K), three on W4), **WGROW (61) in flight** at (PAIR-5) (WTRI (58) proved route 3's cost (T) — a feasible residual is triangle-free — from two landed feasibility transfers a step's inventory had missed; WELOC (59) then **refuted (E-loc)**, the reduction (E) had, with an exhibited 32-vertex residual, leaving (E) itself standing and now known tight; WPAIR (60) **reduced the split arm's actual obligation** — two adjacent degree-2 vertices — to one condition on the hub graph, proved it in the case where no two hubs are adjacent, and made route 3's remaining local-choice cost a theorem, leaving W4 with that single condition — WGROW's target — plus the user-call kernel), `hK` / (GR-15) / class uniformity untouched by all of them; see `notes/Phase39.md`) |
