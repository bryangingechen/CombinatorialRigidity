...
| 22d. Claim 6.11's first green-machinery prerequisite (the matroid-base 4.3(ii) leaf) | `Molecular/Induction/ForestSurgery.lean` (extends `molecular-induction.tex` + `algebraic-induction.tex`) | ◷ In progress (opened 2026-06-05 design-pass-first; re-scoped 2026-06-05 per a fresh user direction; **Gap-2 leaf landed green 2026-06-06**; **Gap-3 + footnote-6 kernel recons 2026-06-06**; **kernel-route decision (user): build the alg-independence route directly**; see `notes/Phase22d.md`) — attacks the conjecture's hardest node (KT **Claim 6.11**, the *missing `+1` row* lifting 22c's `D(|V|−1)−1` brick to full `D(|V|−1)`) **bottom-up**, building the leaf-most missing-green prerequisite rather than axiomatizing it (user override of the opening axiomatize-as-hypothesis verdict). Claim 6.11's discharge factors into three gaps in dependency order — (2) the **matroid-base 4.3(ii) form** → (3) the nested IH-at-restriction → (1) the `M(G̃)`↔row-dependence bridge. **Gap 2 landed:** `Graph.splitOff_exists_base_inter_fiber_lt` (green + axiom-clean, node `lem:case-III-claim-6-11-base`) — the *existence* of a base `B'` of `M(G̃_v^{ab})` with `\|ãb∩B'\| < D−1` (KT 4.3(ii) at `k=0`, the existence form, not "every base"), via the strengthened `forest_surgery_count` (KT Lemma 4.1's two conclusions: `\|I'\|=\|base\|−D` and `\|ãb∩I'\|<D−1`) + def=corank making the full-rank independent set a base (`Indep.isBase_of_ncard`). **Gap-3 recon verdict (docs-only): Gap 3 splits** — a green combinatorial shell (`splitOff_removeVertex_minimalKDof`: `G_v=removeVertex v` minimal `k'`-dof, `0≤k'≤D−2`, from the Gap-2 base + `subgraph_minimality`/`removeVertex_deficiency_ge`/def=corank, all green) plus an **irreducible research-shaped kernel** (KT eq. (6.22): the rank of the *specific* restricted realization `R(G_v,q\|_{E_v})` at the inductively-fixed seed, KT footnote 6 — NOT green-buildable, since the IH motive is existence-only and the device runs the opposite direction — the footnote-6 recon named the missing brick (`non-root-from-algebraic-independence`); **decided: build the alg-independence route directly** to discharge it, with the product-route relaxation tracked in `notes/AlgebraicIndependence.md`). **Next: the Gap-3 leaf** `splitOff_removeVertex_minimalKDof` (green shell, `ForestSurgery.lean`). The kernel + Gap 1 (`M(G̃)`↔row bridge, likely one merged "seed attains matroid-predicted rank" node), the candidate-completion + **Claim 6.12** disjunction (de-risked on green Lemma 2.1), and the `d=3` assembly are **named, deferred, unlettered** further sub-phases. `lem:case-III` / `lem:case-II-realization` stay red. |
