...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress (three carried items — `hcontract` (W4, parked) and the kernels `hK` / `hbareSplit`; the kernel-(K) research arc continues — **38 directions landed** — the eighth fan-out settles ledger attack (c)'s AA-glue case at an exact `n_hub` boundary (GTMPL), gives (b′) its first proven `n`-free constant (GFLOW), returns NO HIT on the authorized disproof hunt while killing the counting route to a disproof (SIGZ), reduces (a₁) to one determinant (OSCHU), and refutes (GR-64)(R2) (GCOLL). **Both architecture-testing probes have landed: KBARE-FALSIFY (2026-08-20) refutes (K-bare-ext) as stated — a *route* finding, `hbareSplit` itself untouched and still carried; C3-AVOID (2026-08-24) prices option C3's gate at threshold `\|S\| ≤ 2`, so C3 is NO-GO as a crux-avoidance route. Direction GFLIP (2026-08-25) PROVES (GR-R1), so (b′)'s `n`-free `≤ 12` bound is now a theorem** — see `notes/Phase39.md`) |
