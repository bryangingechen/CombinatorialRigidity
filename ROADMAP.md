...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry arm, **BLOCKED at the interior corner; STOP for a FOUNDATIONAL-DEF decision** — all three escapes (α1)/(α2)/(C) blocked/relocate (the §(4.69.6) spike found (C) RELOCATES the wall, design §(4.70)); only forward path = a foundational-def change (support-extensor-keyed `blockBasisOn`, recommended). See `notes/Phase23f.md` + design §(4.70). `d=3` fully green. |
