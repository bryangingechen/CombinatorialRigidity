...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a/CARRIER + 23b/CHAIN closed; 23c building option (A) (the forked general-`d` `±r`-block engine, no `hρGv`): cert + arm + corner-data assembly ✓, the `±r` seam closed end-to-end; `chainData_dispatch` DECOMPOSED (5 leaves) + the `Relabel/` split + LEAF-1/2 + the full discriminator-index plumbing + LEAF-3 + the LEAF-4 interior-`hρe₀` conjecture-crux leaf ✓; LEAF-4 step (ii)'s rank-cert half is BLOCKED — BOTH `±r`-corner wall-escape routes REFUTED (option (A) static-`W` + option (B′) operated-frame block-rank both INFEASIBLE; the `±r`-corner escapes `hρGv` at the row level but NOT in the general-`d` rank cert — the wall is unavoidable; design §I.8.24(4.17)–(4.19)); at a USER-DECISION POINT (the remaining route (A′) confronts the wall via a general-`d` W9a-analogue, may itself be blocked) (see `notes/Phase23c.md`) |
