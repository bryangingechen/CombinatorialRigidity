...
│   └── ToolchainBumps.md  toolchain/mathlib/Matroid bump log + playbook
...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress (three carried items — `hcontract` (W4, build parked) and the kernels `hK` / `hbareSplit`; the research arc continues, **79 directions landed** (75 on kernel (K), four on W4). **BBASE (62)** settled the last-but-one of them: the *flag base*, the choice of a point and a plane at each branch endpoint that every other piece of the argument is built over. It was recorded as "the phase's own problem one level down"; it is not, because the relevant graph — the one whose edges are the branches of length 1 — carries a plane at *every* vertex rather than only at the branching ones, which makes it a strictly easier object. It is always solvable, and its solutions form one irreducible rational family with no extra hypothesis at all in the shapes that actually occur; the only obstruction it finds — a short cycle among branch endpoints — is ruled out by the girth the class already has, with one denser shape left open and ranked low. That left **one** item standing between the 2-cut composition lemma's general-piece half and a class statement, and **BUNIF (63)** turned it into a question about one side at a time, and **BDOUBLE (64)** settled the single place that question is tight — by showing the condition that had been named there is **false**, and harmlessly so, and that the block it guarded is redundant given one measured clause; **BSATUR (65)** then settled that clause: it is **false**, and the repair costs nothing. **GPACK (66)** took the arc's first direction in 22 dispatches at the hardest of the three carried items and **split its target in two**: the question about dividing a graph's edges into six spanning trees is settled — a legal division always exists — leaving only a local colouring condition. **BSIGMA (67)**, run in parallel, showed that the clause the previous landing rested on fails too, harmlessly, and proved a floor for it. **GLIST (68)** then took the remaining colouring condition apart. Two things about it turn out to be free. The choice of which end of each short branch to mark can be dropped from the statement entirely, leaving a single label-pair to choose at each branch point; and the demand that no branch point end up marked all one way follows from the rest, because each of the six trees reaches every branch point. At a branch point whose branches are all as short as they can be, what is left is one sharp test: the point must be a *leaf* of at least one of the six trees — equivalently, the three trees absent from its branches must not be three different ones. That is a second way the argument can fail, unrelated to the parity failure the previous direction found, and it does happen. It is not the whole difficulty, though: across every case small enough to enumerate exhaustively, 82% of the failures are invisible at each branch point taken singly, so no purely local repair can finish the argument. A forced audit of the previous direction's own evidence found its headline count had been taken over only the cases its test accepted — restoring the denominator turns 229 320 out of 229 320 into 229 320 out of 472 680 — while overturning the expectation that the hard configuration never appeared there: it appears in half of those shapes. Where the question can be settled outright — every shape with four branch points and a critical edge count — the argument does go through. The question was whether such a six-way split can always be regrouped into two halves of three so that every branch of the graph is shared between the halves as evenly as it can be. Half of that is now a theorem: the regrouping always exists, with no hypothesis beyond the one the chapter already assumes, and the proof applies three classical results about packing trees into a graph a second time, to a halved copy of it. Two things follow. The regrouping is never what obstructs the argument, so the object the previous direction had named as the thing to attack next is free. And the freedom to choose *which* six-way split to start from is doing real work rather than being a formality — of 266 700 splits enumerated by hand, 6 840 admit no regrouping at all, every one of them for the same parity reason, which also means the existing measurement of 907 out of 907 was evidence for a statement that was already a theorem and none for this one. What is left is one thing, and it is now small and local: a colouring of the graph's branch points from a three-element palette, in which each branch point's choices are restricted by the split, and the only tight spot is at branches of length two — independently where an earlier result says the argument's own circuit law binds. The kernel is not closer and the two statements it waits on are unchanged; what changed is that the remaining work is no longer a packing problem. A second, forced question priced what a success there would buy: it would settle the kernel only for the graphs whose edge count is exactly critical, and the rest of the family — a bare odd cycle is the standing example — is owned by another chapter, which routes it to the older route rather than this one, so it is accounted for but not free. The named condition forbade one half of a split from being maximally floppy while the other half is even slightly floppy at the same one of the four canonical pieces. But a half whose shortest route between the two split vertices is long enough is *already* maximally floppy in every direction, by a lemma the argument had landed two months earlier and had itself labelled a degenerate corner — so the two halves of the condition are not in tension at all; one lemma produces both, and an example sits inside the very family of splits the previous direction had measured. It is **not** a counterexample to anything that matters: when one half is that floppy the inequality being checked has slack to spare, and the split still attains. What replaces the condition is smaller and about **one half at a time** — *if a half's motion space contains the whole two-dimensional piece, it is everything* — and under it the two two-dimensional pieces stop being checkable cases altogether, because any failure there would already show up in the crudest of the fourteen inequalities. That leaves twelve pieces, none of which any measurement has ever seen bite on both halves at once. The clause all of that rested on is **false**, and the reason is worth stating plainly. When a half meets a split vertex along a single edge, the plane recorded at that vertex is not determined by that half at all: it is free to swing about the edge, and the rest of the graph is what pins it down. Among that one-parameter family of legal planes there is — exactly when the half is floppy enough, and never otherwise — precisely **one** that makes the clause fail, and every sampler the argument had ever run drew that plane at random, so in twenty-four pieces times fourteen draws it could not once have been seen. An explicit split exhibits the failure, in the intended setting and through both of the argument's own consistency checks. Nothing that matters breaks: at each of 260 exhibited failures the split still attains and no inequality is violated. What is lost is the *proof* that the two two-dimensional pieces are redundant — and that is recovered by reading the clause at a *generic* plane rather than at every legal one, which costs nothing, because the whole argument ever has to do is exhibit one good configuration and good configurations are dense. The shape the previous direction had named as the one to hunt for next was measured, and found empty. The quantity that item turns on measures how much freedom the two halves of a split have *relative to each other*, which sounds irreducibly two-sided. It is not: the only data the two halves share is a point and a plane at each of the two split vertices, and that data is preserved by a five-parameter family of projective motions which can be applied to one half while the other is held still (checked, including that the two halves reglue afterwards). Under that family the six-dimensional space of infinitesimal motions breaks into four canonical pieces of sizes 2, 1, 1, 2, and the quantity is trapped between two bounds — one a standard dimension inequality, one obtained by letting the family stretch each half independently — that were found to agree at 121 of 122 measured splits, so it is *computed* rather than estimated, from each half separately. The condition to be checked is then a short list of numerical inequalities, one per canonical piece; measurement over two constructed families of splits found it satisfied everywhere, with **no** failure, and narrowed the only place it could fail to a single one of the four pieces — where it holds with exactly no margin to spare. That also answers, for the generic case, a question the previous direction had explicitly left open: whether the list of ways this can go wrong was complete. It is; the two ways already known are two entries on it. The four W4 directions between them **closed W4's informal side**: a feasible residual is triangle-free (WTRI 58, from two landed transfers a step's inventory had missed); the local reduction (E-loc) is **refuted** by an exhibited 32-vertex residual, leaving the plain edge count standing and now known tight (WELOC 59); and the condition the split arm actually consumes — two adjacent degree-2 vertices — was reduced to one condition on the hub graph (WPAIR 60) and then **proved** (WGROW 61), by moving the whole question onto the multigraph of hubs, where rigidity becomes a weighted partition count; the same move refutes the reduced condition *as it had been stated*, with a five-vertex graph that is not a residual, so the edge count is untouched. That makes the split arm's remaining local choice a theorem too, and leaves W4 with **no dispatchable item** — only the user-call kernel and the held build. `hK` / (GR-15) / class uniformity untouched by all of them; see `notes/Phase39.md`) |
| ⋮ Doc-split / harness move-down / phase-note compression rounds (Phase 39, PENCIL) | `notes/Pencil-{informal,informal-grid,fanout,fanout-archive,adjudications}.md` doc set + `notes/Phase39.md` + `notes/scripts/` layering | ✓ Complete — rounds 1–2 (slices 1–2, 4–5: the two doc splits, the ordinals-1–19 adjudication bullets, the five §2-rule-2 harness move-downs paid with re-exports, no recorded figure moved) and round 3's slice 6, the phase-note compression pass (`Phase39.md` 1 500 → 554; ordinals 20–44's adjudication bullets → `Pencil-adjudications.md` verbatim), all LANDED (see `notes/Pencil-structure.md`) |
...
(`isoTypeIOfTwoNeighbors`, `isoTypeIIOfThreeNeighbors`)
...
**✓ Complete** (opened 2026-07-21 recon-first, closed 2026-07-22; work
log `notes/Phase36.md`). A proof-engineering phase, **internals-only**:
no new mathematics, every headline statement and its axiom profile
unchanged (all 17 re-verified at the three standard axioms at close).
Inserted ahead of the queued PIN phase at the user's initiative. A
top-rung recon settled the `grind`-adoption policy: the global
`@[grind]`/`@[grind =]` annotation set resolves to **∅** — a measured
no-op under the project's `grind only` idiom, which ignores ambient tags
(`TACTICS-GOLF.md` §1; `DESIGN.md` *Predicates are `def`s*), so the
explicit `grind only [Def]` unfold-hint stays the idiom — and no custom
tactic cleared the built-ins-first bar. The remaining work was a strict
build-neutral-gated *deterministic-smell* sweep across the non-fragile
files (combinatorial-core, Jacobs, BodyBar, matroid/pebble): **66
`rw→simp only` / `unfold→grind only` collapses** landed, each per-file
4-run-median build-neutral and warning-clean, with three defeq-fragility
...
**✓ Complete** (opened and closed 2026-07-22; work log `notes/Phase37.md`).
The deferred slice 6 of Phase 36, spun off as its own phase at the user's
...
mathematics, every headline statement and its axiom profile unchanged (all
17 re-verified at the three standard axioms at close). It swept the
defeq-fragile zone the AUTOMATE build-neutral sweep deliberately left
untouched — `Molecular/`'s `AlgebraicInduction/` (incl. `CaseIII/` +
`Theorem55.lean`), `RigidityMatrix/`, and the ScrewSpace-carrier-touching
files (~381 four-plus-arg `rw` chains + ~114 `change`/`show` sites across 28
files) — as a six-slice, file-cluster sweep with no opening recon (the
automation policy was already settled in AUTOMATE). The zone opened **Default
NO-GO** under strict build-neutrality (the carrier is an opaque `def`, Phase
22l), but that prior was **overturned to GO** at the Z1–Z2 pivot and held
through Z6: the complete sweep landed **103 build-neutral `rw→simp only`
collapses (17 reverted)**, the defeq wall confined to chains touching a *raw*
carrier-coercion site — not the carrier-cluster files, and not
carrier-adjacent chains routed through **packaged** API (which collapse). The
deliverable is that measured per-slice verdict, plus six collapse predictors /
...
**✓ Complete** (opened and closed 2026-07-23; work log `notes/Phase38.md`).
Opened ahead of the queued PIN phase at the user's initiative (PIN now
next-after-38), following how AUTOMATE / AUTOMATE-Z (§§36–37) were spun off.
A proof-engineering phase, **internals-only** (no new mathematics; all 17
headline theorems' `#print axioms` re-verified unchanged at the standard three
at close) but — unlike the build-neutral AUTOMATE sweeps — one that *adds*
shared helper lemmas and unifies duplicate proofs. Motivated by a 4-way
...
accessors, and a top-level `Framework`/`rigidityRow` glue pair — for a **net
−365 Lean lines** (the 931-line `case_II_realization_all_k` fell to ~803),
every `\lean{...}` pin preserved via the thin-wrapper discipline. Two disciplined
non-actions were themselves findings: named `simp` sets were declined as
against the project's pin-the-set idiom (recorded in `DESIGN.md`), and the
