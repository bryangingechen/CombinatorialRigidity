...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress (three carried items — `hcontract` (W4, parked) and the kernels `hK` / `hbareSplit`; the kernel-(K) research arc continues — **36 directions landed; the eighth fan-out settles ledger attack (c)'s AA-glue case at an exact `n_hub` boundary (GTMPL), gives (b′) its first proven `n`-free constant (GFLOW), returns NO HIT on the authorized disproof hunt while killing the counting route to a disproof (SIGZ) and reduces (a₁) to one determinant (OSCHU); one direction still in flight** — see `notes/Phase39.md`) |
