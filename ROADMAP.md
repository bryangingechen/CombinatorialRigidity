...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry arm. Interior corner unblocked via **(D-canonical)** (user-picked; FEASIBLE + kernel-de-risked, design §(4.71)): support-extensor-keyed `blockBasisOn` ⟹ the literal-IH-bottom (C) cert builds. D-CAN-1 (def swap) + D-CAN-2 (the literal-`Matrix` (C) bottom bridge) + D-CAN-3a (the (C) `hD` leaf) + D-CAN-3b (the interior-arm spine `chainData_arm_realization_zero₁₂`, firing the `_zero₁₂` cert) landed; D-CAN-4 in progress — all feeder leaves landed (`hsupp`/`hgp`/`Gab`-bottom/`hfr₂` producers), next = wire the `chainData_dispatch` router + CHAIN-5 + the C.3 `hIH` add. See `notes/Phase23f.md` + design §(4.71)/(4.72). `d=3` fully green. |
