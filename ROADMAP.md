...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry arm (interior cert via **(D-canonical)**, design §(4.71)). The bottom block + cert spine are BUILT (D-CAN-1/2/3a/3b + the `hsupp`/`hgp`/`Gab`-bottom/`hfr₂` feeders); the **corner `hA` was re-routed `ρ₀`-free** after the dispatch composition spike found the `Sum.elim blockBasisOn ρ₀` corner unsatisfiable (opaque-basis obstruction §(4.74)) and re-routed it through block incomparability (§(4.75)), reducing it to ONE genuinely-new input — the 3-normal LI `![q a, n', q b]` (route α, user-chosen). **Next = the route-α producer** (supply the side-condition `q b ∉ ker(of p)` + thread the LANDED LA core `exists_independent_perp_family_escape` through the discriminator), then the `chainData_dispatch` wiring (re/hre + router) + CHAIN-5 + the C.3 `hIH` add. See `notes/Phase23f.md` + design §(4.71)–(4.76). `d=3` fully green. |
