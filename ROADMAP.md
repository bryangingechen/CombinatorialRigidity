...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry-arm interior cert, BLOCKED at the corner `±r`-row `hr`. Routes (D)/(α)/(γ)/(β-as-scoped) all block on ONE obstruction — a short-circuit-panel perp provably false for the generic seed, an artifact of the `caseIIICandidate` extensor-override (not KT's; §(4.69.2)). The (β) decisive recon (design §(4.82)): the union-dimension half (Q1) is FULLY LANDED, but (β) only re-selects the candidate — it does NOT remove the false `hr`. **Next = a DECISION (not a build):** the route to close is a cert re-architecture (literal-IH bottom + literal-chain-edge `±r` row, (D-substitution) §(4.70.4)); recommended FIRST step is a narrow chain-edge-re-key recon. See `notes/Phase23f.md` *Hand-off* + design §(4.82). `d=3` fully green. |
