...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm interior cert) AT A FOUNDATIONAL DECISION POINT (session paused, surfaced to user). The (D-substitution) re-architecture built out (S1–S5 + spine, LANDED) but a final dispatch found it OFF-BY-ONE at the corner; the §(4.90) recon recommends REVERTING to the LANDED `caseIIICandidate` OVERRIDE route — but that verdict is UNVERIFIED (prose, no composition spike) + contradicts the §(4.82)/(4.83) refutations, so it was NOT acted on. Next = re-present the reversal to the user + run a decisive override-composition spike before deciding. `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.90). |
