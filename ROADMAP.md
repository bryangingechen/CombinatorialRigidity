...
| 21b. Genericity device (Claim 6.4/6.9) | `Molecular/AlgebraicInduction.lean` + `Mathlib/{Algebra/MvPolynomial,LinearAlgebra/Matrix}/` (+ `algebraic-induction.tex` `lem:genericity-device`) | ◷ In progress (opened 2026-06-03; see `notes/Phase21b.md`) — the analytic sibling of 21a, scoped out of Phase 21 (risk #4/#7). The panel-coordinate parametrization of `R(G,p)` + generic-max-rank argument; the shared analytic crux of Cases I/II, Thm 5.5, Prop 1.1, cycle assembly; discharges the Phase-21 black-box leaves (`hglue`/`hspan`/`hub`/`hgen`). **State (2026-06-04):** **`lem:genericity-device` GREEN** — the genuine *multivariate* Claim 6.4 (panel rows bilinear ⇒ no affine line works) landed via route (a) (`exists_good_realization` + `exists_good_realization_const`, on the `exists_…_polynomial` mirror engine); the affine wrapper chain was consolidated away in the same commit. **Realization layer honestly RED (2026-06-04 audit):** the device greening does *not* discharge the geometric placements the induction consumes. `lem:case-I-realization` is **red** (`\leanok` dropped) — its Lean carrier `hasFullRankRealization_ofParam_of_contraction` proves the *closure* but takes the two splice-rigidity outputs (`hHrig`/`hcrig`, both subgraphs rigid on the **same** `ofParam` witness) as hypotheses; the construction of that `p` is the new red node `lem:case-I-splice-placement`. A parallel hole in Case II — no `HasFullRankRealization` *producer* discharges `theorem_55`'s `hsplit` (only the accounting iff exists) — is now tracked by the new red node `lem:case-II-realization`. `prop:rigidity-matrix-prop11`'s `hub` (genericity-free upper bound) remains an untracked Phase-19-partition obligation; the device underwrites only `hgen`. The accounting iffs `lem:case-I`/`lem:case-II` stay green. Remaining red: Case-I splice placement, Case-II 1-extension realization, Prop 1.1 `hub`, Case III. Hand-off in `notes/Phase21b.md`. Also consumed by Phases 22–23. See `notes/MolecularConjecture.md` *Phase 21b*, `DESIGN.md` *Genericity device …* + *Forward-mode reduction chains*. |
