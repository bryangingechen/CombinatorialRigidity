...
| 22b. KT Claim 6.4 (discharge the Case-I green-modulo obligation) | `Molecular/AlgebraicInduction/CaseI.lean` (extends `algebraic-induction.tex`) | ◷ In progress — **paused at the reduction checkpoint** (opened 2026-06-05; reduction N-22b-1/2/3 landed 2026-06-05; see `notes/Phase22b.md`) — KT **Claim 6.4** (`lem:claim-6-4`) is reduced to its single analytic core. The surviving block of `G ＼ E(H)` projected to the surviving columns `V(G)∖V(H)` (via `(extProj V(H)).dualMap`) attaining independent rank `≥ D(|sc|−1)` at the generic locus (KT eqs. (6.5)/(6.9), §5.1) is delivered by two landed bricks — N-22b-2 (bounded `D∘panelRow` packaging `exists_rankPolynomial_of_rigidOn_linking_set_proj`) + N-22b-1 (rank-transport `rigidContract_exterior_rank_transport`, carrying the irreducible algebraic-independence content as the explicit hypothesis `htransport`) — composed by N-22b-3 (the wire-up: `case_I_realization`'s third `hbundle` conjunct reshaped from the packaged `hclaim64` to the narrower `htransport`, the `Qc`-non-root packaging reconstructed in-proof; graph-arg mismatch closes by `panelRow`'s graph-independence). **The reduction does NOT close the phase:** the honesty gate keeps `lem:claim-6-4` red (the `\lean{…}`-pinned but undischarged `htransport` has no node) and `lem:case-I-realization` green-modulo `htransport` (case-(b) pattern). A 2026-06-05 validation pass (design doc §1.18) found discharging `htransport` is tractable and **stays Phase 22b**; the follow-up **T2b math-first re-recon** (design doc §1.19) designed the layer — finding §1.18's planned lower-semicontinuity crux is already green inside N-22b-2, shrinking the cut to a **4-node cut (U1→U2→U3→U4)** with the one research-shaped node re-localized to **U2** (the collapse-relabel projected-row reproduction, KT eq. (6.7)/§1.7). **U1 + U2 are now landed** (the degenerate placement `degeneratePlacement` + the per-edge *row* reconciliation `panelRow_collapseTo_comp_extProj_dualMap`, building on the U2-opening column core `hingeRow_collapseTo_comp_extProj_eq`) — **sound, but a 2026-06-05 course-correction (design doc §1.20, after the U2-opening session forked under backgrounding) found the research-shaped crux was NOT retired by U2, only relocated to U3**: KT Claim 6.4 proper — that the exterior-column projection (dropping the `r`-column) preserves rank `D(|sc|−1)` — is a pin-a-body fact needing a MISSING brick (**U3b**). Corrected cut: **U3a** (alignment transport via the `ends`-swap brick; bricked) + **U3b** (the pin-the-`r`-column projected-rank brick; the genuine Claim 6.4 crux, research-shaped) + **U4** (assemble + flip + phase-close). The **U3b build-recon is done + corrected** (design doc §1.21 → §1.22): §1.21's "bounded one-line Lemma 5.1 corollary" was wrong (it assumed `Qcf'` rigid ⟹ `finrank(Z)=D`; in fact `Qcf'` is rigid on its vertex set `sc ⊊ α`, so `finrank(Z)=D(|scᶜ|+1)` and `finrank(pinnedMotions r)≠0`). §1.22 gives the verified-closing layer: U3b reduces to `Z ⊔ range(extProj V(H)) = ⊤` (mathlib dual API), one real-content fact = the rigid-block pin-count `finrank(F.pinnedMotionsOn V(H)) = D(|scᶜ|−|V(H)|+1)`. **Both U3b §1.22 bricks are now landed**: the pin-count walling node (`finrank_pinnedMotionsOn_of_isInfinitesimallyRigidOn_vertexSet_inter_eq_singleton` + two `s ⊇ V(G)` pin bricks, `Pinning.lean`) and the `Z ⊔ range(extProj V(H)) = ⊤` dual-annihilator assembly (`infinitesimalMotions_sup_range_extProj_eq_top` + two supporting bricks `extProj_range_eq_iInf_ker_proj`, `infinitesimalMotions_inf_range_extProj_eq_pinnedMotionsOn`, `CaseI.lean`), all axiom-clean. **ALL of U3b is now landed** (the projected-subfamily extraction `exists_independent_panelRow_subfamily_of_rigidOn_linking_set_proj` + the injective-form core, `CaseI.lean`) — the genuine KT Claim 6.4 crux is fully bricked. §1.23 then found **U3a is not a leaf** (the IH motive `HasGenericFullRankRealization` carries a *free* `ends` with no link-recording, so `Q`'s rigidity does not transport to the relabel selector; same gap = the undischarged `H`-leg `hswap`); the user decided **route (i)** (strengthen the motive), scope-verified buildable as a 5-commit sequence (design doc §1.24). **Commits 1+2+3 + Commit 4a + Commit 4b of 5 landed** (edge-restrict `hne_ends` + `endsOf_fst_ne_snd`; the link-recording bridge `ofNormals_endsOf_recordsLinks`; the motive re-type — the link-recording conjunct threaded through the `def` + all 6 producers + the forgetful map + the composer's IH consumers via the new helper `ofNormals_recordsLinks_of_hends`; **Commit 4a** spends that conjunct to discharge the `H`-leg alignment — `case_I_realization`'s `hbundle` reshaped **3→1 conjunct**, only `htransport` survives, via the new brick `recordsLinks_swap_endsOf` + `endsOf_fst_ne_snd`; **Commit 4b** builds U3a — the standalone contraction-leg rigidity transport `hasGenericRealization_transport_relabel` producing the `Qcf'` (rigid on `V(Gc.map f)` at the relabel selector `endsᵐ`) that the landed U3b extraction consumes, plus the infra bricks `recordsLinks_map_of_records` (risk (c), off mathlib `Graph.map_isLink`) + `recordsLinks_agree_swap`); next is **Commit 5** (U4: assemble `htransport` from U3a+U2+U3b, delete it from `hbundle`, flip `lem:claim-6-4` green, phase-close). See `notes/Phase22b.md` for the live commit pointer. |
