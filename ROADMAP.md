...
| 21b. Genericity device (Claim 6.4/6.9) | `Molecular/AlgebraicInduction.lean` + `Mathlib/{Algebra/MvPolynomial,LinearAlgebra/Matrix}/` (+ `algebraic-induction.tex` `lem:genericity-device`) | ◷ In progress (opened 2026-06-03; see `notes/Phase21b.md`) — the analytic sibling of 21a, scoped out of Phase 21 (risk #4/#7). The panel-coordinate parametrization of `R(G,p)` + generic-max-rank argument; the shared analytic crux of Cases I/II, Thm 5.5, Prop 1.1, cycle assembly; discharges the Phase-21 black-box leaves (`hglue`/`hspan`/`hub`/`hgen`). **State (2026-06-04):** **`lem:genericity-device` GREEN** — the genuine *multivariate* Claim 6.4 (panel rows bilinear ⇒ no affine line works) landed via route (a) (`exists_good_realization` + `exists_good_realization_const`, on the `exists_…_polynomial` mirror engine); the affine wrapper chain was consolidated away in the same commit. **Realization layer RED, re-planned (2026-06-04).** A spike found the realization *motive* was mis-defined: `HasFullRankRealization`/`RankHypothesis 0` was the *absolute* null-space equality `dim Z = D` over the whole ambient body type `α`, which equals rigidity only when `G` spans `α` — but the induction reduces to graphs with fewer vertices on the same `α`, so `theorem_55`'s case premises are unsatisfiable for `card α ≥ 3` (verified in Lean: an isolated body is a free non-trivial motion; `theorem_55_base`'s `hcover` is the symptom). The retired Case-I carrier `hasFullRankRealization_ofParam_of_contraction` likewise took unsatisfiable `withGraph`-subgraph-rigid hypotheses. **Fix (user decision):** carry the motive in the `V(G)`-relative **rank form** `rank R(G,p) = D(|V(G)|−1)` (α-independent; resurrects the block-constancy machinery). The realization spine (`def:rank-hypothesis`, `lem:theorem-55-base`, `thm:theorem-55`, `lem:case-I`/`lem:case-II` iffs, `prop:rigidity-matrix-prop11`) is **re-stated red** against it; the producers (`lem:rows-polynomial-in-normals` (B0, keystone — coordinate bilinearity spike-verified, mathlib-supported), `lem:case-I-splice-seed`, `lem:case-I-realization`, `lem:case-II-realization`) are the genuine build, device-direct (not the vacuous closure). The device `lem:genericity-device` is GREEN and motive-independent; the accounting/cycle/panel green inventory is retained. **State (2026-06-05):** hand-off item 1 (relativize the motive + base case) **LANDED** — `BodyHingeFramework.IsInfinitesimallyRigidOn` + API in `RigidityMatrix.lean`, `HasFullRankRealization` re-pinned to `IsInfinitesimallyRigidOn V(G)`, `theorem_55_base` relative (`hcover` dropped), `theorem_55` recursion unchanged; the four absolute-motive Case-I producers + the orphaned vacuous block-internal chain retired; `def:rank-hypothesis` + `lem:theorem-55-base` flipped GREEN, `thm:theorem-55` stays red (producers red). **Item 2 (accounting re-stated rank-side) LANDED** — two genericity-free, α-independent rigidity bridges `isInfinitesimallyRigidOn_iff_pinnedMotionsOn_le` (Case I) + `isInfinitesimallyRigidOn_insert_iff` (Case II); nullity iffs kept as α-dependent siblings; `lem:case-I`/`lem:case-II` flipped GREEN-modulo-21b (device leg enters only in the producers). **Item 3 (B0 keystone `lem:rows-polynomial-in-normals`) LANDED (2026-06-06, GREEN)** — `PanelHingeFramework.exists_good_realization_ofParam`, the device closure on the *varying* panel family `ofNormals G ends q`. Sub-commits 1–3 (the polynomial bricks `normalsJoinPoly`, `panelSupportPoly`, `annihRow`/`span_annihRow_eq_dualAnnihilator`/`annihRowPoly`) landed 2026-06-04…06; sub-commit 4 (the closure) on 2026-06-06 added `ofNormals` (free-normal panel framework), `exists_good_realization_reindex` (basis-flexible device wrapper), `panelRow` + `span_panelRow_eq_rigidityRows`, and generalized the device's `hcoord` to a `≤`-containment (it must hold at degenerate output normals, where the row family under-spans). **Item 4 (Case-I splice glue `lem:case-I-splice-seed`) LANDED (2026-06-06, GREEN)** — `isInfinitesimallyRigidOn_of_splice`; the placement-construction half split out as the red node `lem:case-I-splice-placement` per the producer-scrutiny gate. **Re-plan (2026-06-06, planning commit):** the realization layer is decomposed into a cold-start-ready node list (spike + recons). The device's output is an *absolute* codimension bound, so the producers need a **`V(G)`-relative count bridge** (`lem:relative-screw-split`/`lem:relative-device-count`/`lem:isInfRigidOn-of-relative-count`, N1–N3 — **LANDED GREEN 2026-06-04** via mathlib `LinearMap.iInfKerProjEquiv` + the green block-pin bricks). The rigid-subgraph contraction is **mostly built, not fully**: `rigidContract` + vertex-drop + matroid-side `contraction_isMinimalKDof` are green, but the graph↔matroid minimality bridge `(rigidContract).IsMinimalKDof` (new red node `lem:rigidContract-isMinimalKDof`, N4 — a deliberate Phase-20 carry-forward) gates Case I. The Case-I splice placement (`lem:case-I-splice-placement`, N5) is the one genuinely hard node (research-shaped geometry, KT 6.2/6.6). **State (2026-06-04):** N7's first Case-II placement sub-node **N7b-1 `lem:case-II-placement-new-rows` LANDED GREEN** — `BodyHingeFramework.exists_independent_panelRow_of_edge` (a transversal hinge gives `D−1` independent rigidity rows in that edge's panel-row span, via the new per-edge span identity `span_panelRow_edge_eq` + `linearIndependent_hingeRow` + `finrank_hingeRowBlock`); the panel-row form of `exists_independent_rigidityRows_of_edge`. **`thm:theorem-55` does NOT flip in 21b** — Case III is Phases 22–23; **Phase 21b closes when the two producers `lem:case-I-realization` (N6) + `lem:case-II-realization` (N7) are green.** **Re-plan (2026-06-04, recon/decomposition):** a scrutiny pass before the N7b assembly found it blocked on a missing *producer* for the Case-II old block — N7b-2 (`exists_independent_panelRow_transport`) only *transports* an already-witnessed independent family, and the inductive-rigidity → independent-rows extraction (the forward converse of N3) is absent from the codebase. Added the new RED node **N7b-0 `lem:case-II-placement-old-rows-extract`** (rigid-on-`s` ⇒ `D(|s|−1)` independent `panelRow`s, build-shaped from N1 + `finrank_pinnedMotions_add_screwDim` + `Submodule.exists_fun_fin_finrank_span_eq`); the prior hand-off mislabelled the assembly "next" (process lesson (d)). **State (2026-06-04):** **N7b-0 LANDED GREEN** — `BodyHingeFramework.exists_independent_panelRow_subfamily_of_rigidOn` + the dimension half `finrank_infinitesimalMotions_of_isInfinitesimallyRigidOn_vertexSet` (forward converse of N3: rigid-on-`V` ⇒ `dim Z = D(|Vᶜ|+1)` ⇒ `rank R = D(|V|−1)` ⇒ that many independent panel rows; stated on the `G_v^{ab}` framework directly so the `s ≠ V(F.graph)` watch dissolved). All four N7b sub-nodes green. Build order: N1–N3 + N7b-0/1/2/3 (done) → **N7b-assembly (next)** → N7 → N4 → N5 → N6. Ordered cold-start hand-off in `notes/Phase21b.md`. Also consumed by Phases 22–23. See `notes/MolecularConjecture.md` *Phase 21b*; `DESIGN.md` *Realization motive must be V(G)-relative* + *Forward-mode reduction chains* + *Genericity device …*. |
