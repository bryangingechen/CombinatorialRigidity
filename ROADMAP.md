...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm reshape, §(4.94)–(4.100)): honest interior arm + LEAF-1 + orientation-override shim LANDED; `chainData_dispatch` interior branch built head-on (10/13 slots sorry-free), BLOCKED on the interior `hρGv`. The §(4.100) route spike SETTLED the fix (kernel-checked sorry-free): NOT a fold re-statement — RE-TARGET `chainData_relabel_arm_hρGv`'s selector `candidateEnds → ends₀` (the existing fold at `ends₀` lands at the genuine framework; the sparse d=3-`ends₃`-pattern override bridges via `congr_ends`); (B′) discriminator exposure stands. `d=3` fully green. See `notes/Phase23f.md` *Hand-off*. |
