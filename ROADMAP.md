...
| 22a. Case I realization | `Molecular/{Deficiency,Induction,AlgebraicInduction}.lean` (extends `algebraic-induction.tex`) | ◑ In progress (opened 2026-06-04; see `notes/Phase22a.md`) — Track A only: the Theorem-5.5 **Case I** realization producer the Phase-21b genericity device feeds. Target: `lem:case-I-realization` (the N6 composer) green, discharging `theorem_55.hcontract`'s Case-I branch. **All Case-I bricks GREEN except the N6 composer:** N4 `rigidContract_isMinimalKDof` (graph↔matroid contraction-minimality, incl. N4a/b/c), the N5 splice/seed/rank-polynomial bricks (all-edges + leg-restricted), N6a (non-simple Case I), the two-motive split, the (G2) general-position factor, the N6b/N6c simple Case-I coupling `hasFullRankRealization_of_couple_ofNormals`, and the N6 leg-transport `hasGenericRealization_transport_ends`. **The remaining red work is the generic-motive induction** (the N6 composer's IH-shape gap): `minimal_kdof_reduction` supplies the *bare* `HasFullRankRealization`, but the simple Case-I coupling needs each leg `HasGenericFullRankRealization` (general position). The generic-motive recon (2026-06-04) settled this as a **hybrid** and cut the composer into N6-G1/G2/G3 (KT Lemma 6.7(ii) verified vs. source — it covers only the Case-II/III split-off branch, absent in repo). **N6-G1 (the generic *producer* `hasGenericFullRankRealization_of_splice_ofNormals`) is GREEN** — the N6-G1a spike found the recon's device-GP re-expose route was a false premise (the genericity device's output point is not general position) and replaced it with a direct realize-at-the-GP-seed proof that bypasses the device, collapsing N6-G1 to one commit. N6-G2 has now been **re-reconned** (2026-06-04, design doc §1.6): the generic-motive reduction is `minimal_kdof_reduction` at the *conditioned* motive `(G.Simple → GP) ∧ bare`, and — verified vs. KT §6.2 — only the Lemma-6.3 Case-I legs need the generic IH (hard kernel: `G/E(H)` simplicity, KT 6.3-vs-6.5). Cut into **G2a** (conditioned-motive reduction skeleton, Phase-20-touch), **G2b** (`map`-simplicity, the new fact, math-first), **G2c** (wire-up). **G2a + G2b are now GREEN.** G2a (`theorem_55_generic`, 2026-06-04): `minimal_kdof_reduction` at the conditioned motive `Pc G := (G.Simple → GP G) ∧ bare G`; the flagged `hsplit`-vs-`splitOff` routing sub-question is settled by *scope* (the splitting-off branch's `Simple → GP` conjunct is KT Case III / Track B, carried as `hsplitGP` green-modulo), not routing. G2b (`rigidContract_simple` + the abstract kernel `map_simple`, 2026-06-04): the `map`/`collapseTo` simplicity fact — a *positive* `map`-simplicity criterion (no-self-collapse + no-pair-collapse) specialized to `rigidContract`, the faithful statement of KT Lemma 6.3's `G/E′`-simple *case hypothesis* (`map` is the one op that breaks `Simple`, which is *why* KT trifurcates). The expected "research-shaped dichotomy" collapsed to this clean primitive (the dichotomy is downstream). **G2c is now GREEN too** (2026-06-05): the *generic coupling producer* `hasGenericFullRankRealization_of_couple_ofNormals` — the generic sibling of the bare coupling, identical up to the final splice, routing the shared GP seed through the device-free N6-G1 instead of the device-routing bare splice; the deliverable was the missing producer *brick* (the connector between N6-G1, the transport brick, and G2a/G2b), not a full wire-up. **All Case-I producer bricks are green, and N6-G3-G3a (the Claim-6.4 collapse transport `rigidContract_rigidity_transport`) is now GREEN-MODULO** (2026-06-05): the §1.7 recon settled the binding `Gc ≤ G` obstruction (verified vs. KT §6.2 eqs. 6.3–6.9) — the splice's contraction leg is **`G.deleteEdges E(H)`** (`≤ G`, KT eq. 6.3's `R(G,p; E∖E′, V∖V′)`), **not** the relabelled `G.rigidContract H r` — so the mismatch dissolves at the graph level, and the contraction IH's rigidity is *transported across the collapse map* to that `G ＼ E(H)` leg by **KT's Claim 6.4** (eq. 6.9). The G3a math-first pass found that transport irreducibly research-shaped (the collapse redirects surviving-edge endpoints, so the green linking-edge brick does not apply — recovering the un-collapsed-endpoint rank *is* Claim 6.4), so per the design-doc escalation it carries Claim 6.4 as the narrow explicit hypothesis `htransport` (axiom-clean, no `sorry`); `lem:case-III`/`thm:theorem-55` stay red. **N6-G3-G3b (cover/shared-body/selector geometry) is now GREEN** (`couple_geometry_of_isProperRigidSubgraph`, both legs `≤ G`). **N6-G3-G3c was re-reconned 2026-06-05 (design doc §1.8) as NOT pure green-brick assembly** — a **body-set mismatch**: the G2c coupling + its rank-polynomial brick hardcode each leg rigid on its *full* `V(·)`, but the contraction leg `G.deleteEdges E(H)` (`V = V(G)`) is rigid *only* on `(V(G)∖V(H)) ∪ {r}` (G3a's output set; KT eq. 6.3's surviving bodies `V∖V′`, collapsed away in the earlier all-of-`V`-leg nodes). Cut into **G3c-i** (body-set-relative rank polynomial + N7b-0), **G3c-ii** (body-set-relative coupling), **G3c-iii** (the original assembly + flip). **G3c-i + G3c-ii are now GREEN** (2026-06-05). G3c-i: the four body-set *producer* bricks lifted from `V(G)`/count `D(|V(G)|−1)` to an arbitrary nonempty body set `s`/count `D(|s|−1)` — the body-set N1 **upper bound** `finrank_pinnedMotionsOn_le` (the N1 *equality* `= D·|sᶜ|` is *false* for `s ⊊ V(G)`, but the upper bound is all the producer needs), the N7b-0 helper `finrank_infinitesimalMotions_le_of_isInfinitesimallyRigidOn`, the body-set N7b-0 `exists_independent_panelRow_subfamily_of_rigidOn_linking_set`, and the body-set rank polynomial `exists_rankPolynomial_of_rigidOn_linking_set`. G3c-i surfaced that the body-set N3 *consumer* (re-derive rigid-on-`s` from the row count) does NOT close from the count alone off `V(G)`; the **G3c-ii re-recon (design doc §1.9)** settled **route (a)**: route (b) ("feed rigidity straight to the glue") cannot avoid the body-set N3 because the *shared seed* `q₀` is reachable only through the rank-polynomial round-trip (independence transfers across non-roots, rigidity-on-`s` does not). G3c-ii (GREEN) lifts the consumer + coupling off `V(·)` carrying the complement-isolation **equality** `hpin : finrank (pinnedMotionsOn s) = D·|sᶜ|` (the green sibling of `finrank_pinnedMotionsOn_vertexSet`, discharged per-leg by G3c-iii — *not* a new analytic hypothesis like G3a's `htransport`): the body-set N3 `isInfinitesimallyRigidOn_of_finrank_le_set`, the body-set consumer `…_rankPolynomial_ne_zero_linking_set`, the body-set splice `hasFullRankRealization_of_splice_set_of_supportExtensor`, and the body-set coupling `hasFullRankRealization_of_couple_ofNormals_set`. **G3c-iii was re-cut into G3c-iii-a/b (design doc §1.10): its two GP-conjunct *producer* bricks (body-set generic splice + coupling) are GREEN, and N6-G3-G3c-iii-a (the parent-`ends` impedance) is now RESOLVED** (recon-level, design doc §1.11) — the impedance *dissolves*: the producers need only an *edge-restricted* `hends`, constructible from `G` alone (option (iii); option (ii) `β=E(G)` ruled out by `hfresh`'s fresh-label supply, option (i) layer-wide motive re-type unnecessary). A first **N6-G3-G3c-iii-b** composer (`PanelHingeFramework.case_I_realization`) landed in commit c1ef55a, but a **coordinator verification pass (2026-06-05, design doc §1.12) found it valid-but-VACUOUS**: its "Claim-6.4 bundle" carries a FALSE combinatorial equality `hpinc` (the contraction leg's complement-isolation `finrank (pinnedMotionsOn ((V(G)∖V(H))∪{r})) = D·|(…)ᶜ|`, a placement-independent graph fact — NOT Claim 6.4 — generically unsatisfiable because the interior bodies `V(H)∖{r}` are not isolated in `G ＼ E(H)`; the surviving boundary edges constrain them, and the project's own `finrank_pinnedMotionsOn_le` proves only the *upper* bound). Root cause: KT eq. (6.3)'s contraction block lives on the *collapsed* `G/E(H)` (no interior bodies; eq. (6.5)/(6.9) "delete the v∗ columns, rank unchanged"), so §1.9's rank-polynomial round-trip for the `G ＼ E(H)` leg forced the (false) pin equality — the pin-count does not bridge the collapse the way the rigidity does. **G3c-iii-b is RE-OPENED; the next concrete task is the FIX** (route (b)-corrected, design doc §1.12; stays in 22a): an **asymmetric coupling** — H-leg keeps its green round-trip (rigid on its full `V(H)`, where the pin equality is the true `finrank_pinnedMotionsOn_vertexSet`) and produces the shared seed; the contraction leg's rigidity at that seed on `sc` is fed to the splice glue *directly* from the Claim-6.4 bundle (mirrors KT eq. (6.6)'s single-placement construction), so `hpinc` is deleted and the honest bundle is genuinely Claim-6.4-only. The fix commit also corrects two false Lean doc-comments (`case_I_realization` narration; the body-set consumer's "surviving edges isolate the interior bodies"). Do NOT flip the blueprint / run phase-close / open 22b until the fix lands. See `notes/Phase22a.md` *Current state* / *Blockers* / *Hand-off* + `notes/Phase22-realization-design.md` §1.12 (and the corrected §1.9). |
...
proof, in a new file `Molecular/AlgebraicInduction.lean`. Where Phase 20
...
contraction-minimality bridge incl. N4a/b/c; the N5 splice/seed/rank-polynomial
bricks; N6a non-simple Case I; the two-motive split; the (G2) general-position
factor; the N6b/N6c simple Case-I coupling; the N6 leg-transport `ends`-swap
brick) **except the N6 composer**: its one remaining red node is the
**generic-motive induction** (IH-shape gap — `minimal_kdof_reduction` supplies the
*bare* `HasFullRankRealization`, the simple coupling needs
`HasGenericFullRankRealization`, re-opening the parked `Simple`-threading / KT
