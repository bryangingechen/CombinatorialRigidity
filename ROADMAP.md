...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress — a docs+scripts-only research arc (standing 2026-08-05 Lean hold) against the three carried items `hcontract` (W4), `hK` (kernel (K)) and `hbareSplit` (kernel (K-bare)); **84 directions landed** (79 on kernel (K), four on W4, one on (K-res)). Standing result unchanged: **`hK` is not closer**. See `notes/Phase39.md` (current state + the next concrete task); per-direction specs and verdicts `notes/Pencil-fanout.md`, every status word `notes/Pencil-informal.md`'s *State of (K)* gap map. |
...
Since 2026-08-02 the phase has run a **docs+scripts-only research arc**
(no `.lean`, under the standing 2026-08-05 Lean hold) — overwhelmingly on
