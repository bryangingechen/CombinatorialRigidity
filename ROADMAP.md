...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a/CARRIER + 23b/CHAIN closed; 23c building option (A) (the forked general-`d` `±r`-block engine, no `hρGv`): cert + arm + corner-data assembly ✓, the `±r` seam closed end-to-end; `chainData_dispatch` DECOMPOSED (5 leaves) + the `Relabel/` split + LEAF-1/2 + the full discriminator-index plumbing + LEAF-3 + the LEAF-4 interior-`hρe₀` conjecture-crux leaf ✓; LEAF-4 step (ii)'s rank-cert half is BLOCKED and ALL THREE routes (A static-`W`, B′ operated-frame block-rank, A′ general-`d` W9a) are REFUTED — the `±r`-corner escapes `hρGv` at the row level but NOT in the general-`d` rank cert; the member-mapping wall (§I.8.18–20, intrinsic to KT) is unavoidable (design §I.8.24(4.17)–(4.21), four spikes + a primary-source KT-§6.4.2 recon confirming NO missed KT route — KT's moving-member rank-count is realized in Lean as the relabel-image submatrix containment = the wall). **At a phase STOP** (user-surfaced 2026-06-24); next (user-decided, a fresh session) = the **A1 §I.8.21(α) feasibility recon** (a read-only spike: does a matrix-level block-rank-additivity lemma compose with the `±r` corner without the fixed-member membership? — gap localized, CHAIN-3/4 supply KT's 6.67 finish; dispatch kit in `notes/Phase23c.md` Hand-off), build only if FEASIBLE; ENTRY (23d) is the parallel-safe alternative; all landed leaves stay in tree (see `notes/Phase23c.md`) |
