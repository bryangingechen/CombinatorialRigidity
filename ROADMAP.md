...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed (23e landed the KT-faithful A3-transposed rank certificate axiom-clean); **23f** = the geometry arm; route (α) chosen (the KT eq.-(6.66) `±r`-row re-shape via an augmented matrix); αE1 + the augmented engine αE2 landed; the augmented matrix STILL needs the row op `Lrow` (design §(4.66.F/G)) — next commit is αE3, the augmented cert (see `notes/Phase23f.md` + design §(4.66.G)). |
