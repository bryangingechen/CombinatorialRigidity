...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry-arm interior cert. §(4.90) recon VERDICT = GO via the LANDED `caseIIICandidate` OVERRIDE route (KT's `+1` is the full-rank corner `Mᵢ`, free panel `Lᵢ` + separate redundant `±r`; the (D-substitution) `ofNormals` detour was the wrong turn — it collapsed both corner conditions onto one chain edge → off-by-one). Next = build the override `chainData_dispatch` `Fin cd.d` router on `chainData_arm_realization_aug_zero₁₂`, discard the `_ofNormals` siblings (~2–4 commits); then 23f ✓ (CHAIN-5 + `cd` producer → 23g per option A). Reverses the (D-substitution) authorization. `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.90). |
