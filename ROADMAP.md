...
| 22d. Claim 6.11's green-machinery prerequisites (matroid-base 4.3(ii) + nested-IH shell + seed-rank bridge + `hub` codimension bound) | `Molecular/Induction/ForestSurgery.lean` + `AlgebraicInduction/{CaseI,PanelLayer,PanelHinge}.lean` (extends `algebraic-induction.tex`) | ◷ In progress — the full Gap-1 analytic chain (incl. `hub`) + both combinatorial factors green; remaining is the Gap-1 pigeonhole (eq. (6.23)) (see `notes/Phase22d.md`) |
