...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry arm, route (α) (the KT eq.-(6.66) `±r`-row re-shape via an augmented matrix); αE1–αE5 landed (the augmented matrix + engine + cert + wrapper, then the dead `(e_b,j₀)`-collision machinery deleted, the row op `Lrow` retained), next commit αE6 (see `notes/Phase23f.md` + design §(4.66.F/G)). |
