...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress (three carried items — `hcontract` (W4, parked) and the kernels `hK` / `hbareSplit`; the kernel-(K) research arc continues — **43 directions landed** — the eighth fan-out settles ledger attack (c)'s AA-glue case at an exact `n_hub` boundary (GTMPL), gives (b′) its first proven `n`-free constant (GFLOW), returns NO HIT on the authorized disproof hunt while killing the counting route to a disproof (SIGZ), reduces (a₁) to one determinant (OSCHU), and refutes (GR-64)(R2) (GCOLL). **Both architecture-testing probes have landed: KBARE-FALSIFY (2026-08-20) refutes (K-bare-ext) as stated — a *route* finding, `hbareSplit` itself untouched and still carried; C3-AVOID (2026-08-24) prices option C3's gate at threshold `\|S\| ≤ 2`, so C3 is NO-GO as a crux-avoidance route. Direction GFLIP (2026-08-25) PROVES (GR-R1), so (b′)'s `n`-free `≤ 12` bound is now a theorem; direction GCHEAP (2026-08-25) settles (GR-C2) per-configuration in both directions — (b′) at the constant 2 is a theorem on the whole `n_hub ≤ 6` stratum, refuted-from-`n_hub = 12` per-configuration, residual reshaped to the price form (GR-104)(i); direction OQRANK (2026-08-25) delivers input (a) at all 174 certified classes by the completed ⋆-eigen-block mechanism, with a combinatorial wall found and the naive form refuted as a class statement; direction GPRICE (2026-08-25) settles (GR-104)(i) at `2k = 2`, every `n`, modulo the minted balance law (GR-108); direction GBLAW (2026-08-26) proves the exchange calculus and reduces the law to existential escape; direction GXESC (2026-08-26) then REFUTES both — (GR-108) and existential escape FALSE from `n = 16` by four verified witnesses — while the price form (GR-104)(i) SURVIVES everywhere (⟺ the gap-2 law), the reshaped residual the half-witness clause (GR-117)(iii)** — see `notes/Phase39.md`) |
