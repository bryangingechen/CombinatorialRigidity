...
| 21a. Grassmann–Cayley meet / projective-duality foundations | `Molecular/Meet.lean` (KT §2.1 dual half) + mirror lemmas | ◷ In progress — deliverables 1–3 (`topEquiv`, `pairingDualEquiv`, `complementIso`) green; the complement iso closed via the signed-permutation basis matrix ⇒ `wedgePairing_injective` + equal finrank ⇒ `LinearMap.linearEquivOfInjective` ≪≫ `toDualEquiv.symm`; deliverable 4 `meet` (regressive product + `meet_ne_zero_iff` + geometric reading) remains. Mirrors under `Mathlib/LinearAlgebra/ExteriorPower/Basis.lean` + `Mathlib/Data/Finset/Card.lean`. See `notes/Phase21a.md`; dep-graph `meet.tex` `sec:molecular-meet`. |
