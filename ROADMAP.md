...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry arm, **BLOCKED at the interior corner; STOP for user decision** — route α both-faces-blocked (design §(4.68)); the KT-faithfulness recon RECOMMENDS escape (C)/fresh (literal-IH-matrix-bottom cert, the only structural wall-escape; design §(4.69)). See `notes/Phase23f.md` + design §(4.69). `d=3` fully green. |
