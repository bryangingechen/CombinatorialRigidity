...
| 22d. Claim 6.11's green-machinery prerequisites (matroid-base 4.3(ii) + nested-IH shell + seed-rank bridge) | `Molecular/Induction/ForestSurgery.lean` + `AlgebraicInduction/CaseI.lean` (extends `algebraic-induction.tex`) | ◷ In progress — Gap-2/Gap-3 + the seed-rank bridge + the eq. (6.22) upper bound + rank-attainment packaging green; next is the `hub` brick + Gap-1 pigeonhole (see `notes/Phase22d.md`) |
