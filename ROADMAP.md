...
| ⋮ Structure pass (pre-Phase-22b) | `Molecular/{AlgebraicInduction,Induction}/` splits + `algebraic-induction.tex` split | In progress (see `notes/Phase22-structure.md`) — split the two over-cap Molecular giants into subdirectories (no-hub; the aggregator imports the terminal leaf) and split the large algebraic-induction blueprint chapter, before opening 22b. Pure semantics-preserving moves (`\lean{…}` pins are by-name, so `checkdecls` is unaffected). Done: `AlgebraicInduction.lean` → 5 files (`PanelLayer`/`Pinning`/`PanelHinge`/`GenericityDevice`/`CaseI`); `algebraic-induction.tex` → thin parent + 5 `\input`s; `Induction.lean` → 5 files (`Operations`/`SplitOffDeficiency`/`ReducibleVertex`/`Contraction`/`ForestSurgery`). Pass-close (PERFORMANCE.md executed-split record) pending. |
