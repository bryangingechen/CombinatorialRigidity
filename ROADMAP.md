...
| 21b. Genericity device (Claim 6.4/6.9) | `Molecular/AlgebraicInduction.lean` (+ `algebraic-induction.tex` `lem:genericity-device`) | ◷ In progress (opened 2026-06-03; see `notes/Phase21b.md`) — the analytic sibling of 21a, scoped out of Phase 21 on 2026-06-03 (risk #4/#7). The panel-coordinate parametrization of `R(G,p)` + generic-max-rank argument: the shared analytic crux of Cases I/II, Thm 5.5, Prop 1.1, and the cycle assembly; discharges the cited black-box Phase 21 leaves (`hglue`/`hspan`/`hub`/`hgen`). Also consumed by Phases 22–23. See `notes/MolecularConjecture.md` *Phase 21b* + `DESIGN.md` *Genericity device …*. |
