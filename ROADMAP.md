...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a/CARRIER + 23b/CHAIN closed; 23c building option (A) (the forked general-`d` `±r`-block engine, no `hρGv`): cert + arm + corner-data assembly ✓, the `±r` seam closed end-to-end; `chainData_dispatch` DECOMPOSED (5 leaves) + the `Relabel/` split + LEAF-1/2 + the full discriminator-index plumbing + LEAF-3 + the LEAF-4 interior-`hρe₀` conjecture-crux leaf ✓; LEAF-4 step (ii)'s `hWS` base block is BLOCKED (option (A) static-`W` re-shape proved INFEASIBLE — the `hρGv` wall was relocated to the interior-arm `W`-production; design §I.8.24(4.18)); next = the (B′) feasibility spike (a column-op-aware block-rank lemma that may sidestep the wall), then LEAF-5 + CHAIN-5, then ENTRY/ASSEMBLY (see `notes/Phase23c.md`) |
