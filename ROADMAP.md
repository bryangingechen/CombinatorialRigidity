...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm reshape, §(4.94)/(4.95)): the honest interior arm `chainData_interior_realization_hρGv` + its LEAF-1 selector supplier `candidateEnds_records_splitOff_isLink` are LANDED (the interior routes through the general-`k` engine `case_III_rank_certification`); next = the rest of the `chainData_dispatch` router wiring, then discard the `_aug` fork. `d=3` fully green. See `notes/Phase23f.md` *Hand-off*. |
