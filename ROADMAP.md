...
| 21a. Grassmann–Cayley meet / projective-duality foundations | `Molecular/Meet.lean` (KT §2.1 dual half) + mirror lemmas | ◷ In progress — deliverable 1 `topEquiv` green (top-power volume-form iso `⋀ᴺ V ≃ ℝ`, mirrored under `Mathlib/LinearAlgebra/ExteriorPower/Basis.lean`); `pairingDual`-iso / `complementIso` / `meet` remain. See `notes/Phase21a.md`; dep-graph `meet.tex` `sec:molecular-meet`. |
