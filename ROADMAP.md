...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress (three carried items — `hcontract` (W4, parked) and the kernels `hK` / `hbareSplit`; the kernel-(K) research arc continues — **37 directions landed, the EIGHTH FAN-OUT COMPLETE** — it settles ledger attack (c)'s AA-glue case at an exact `n_hub` boundary (GTMPL), gives (b′) its first proven `n`-free constant (GFLOW), returns NO HIT on the authorized disproof hunt while killing the counting route to a disproof (SIGZ), reduces (a₁) to one determinant (OSCHU), and refutes (GR-64)(R2) (GCOLL). **Probe KBARE-FALSIFY landed 2026-08-20: (K-bare-ext), route A's arbitrary-seed insertion lemma, is REFUTED as stated** — a *route* finding, `hbareSplit` itself untouched and still carried — see `notes/Phase39.md`) |
| ⋮ Doc-split / harness move-down rounds (Phase 39, PENCIL) | `notes/Pencil-{informal,informal-grid,fanout,fanout-archive,adjudications}.md` doc set + `notes/scripts/` layering | ✓ Complete — round 1's slices 1–2 (§(K-grid) → `Pencil-informal-grid.md`; `Pencil-fanout.md` ordinals 1–19 → `Pencil-fanout-archive.md`) and round 2's slices 4–5 (`Phase39.md`'s adjudication bullets → `Pencil-adjudications.md`; the five §2-rule-2 harness move-downs paid with re-exports, no recorded figure moved) all LANDED (see `notes/Pencil-structure.md`) |
...
| ⋮ Toolchain bump to Lean v4.34.0-rc1 (maintenance, cross-phase) | `lean-toolchain`, `lake-manifest.json`, `lakefile.toml` + ~36 proof fixups across 26 files | ✓ Complete — v4.30.0-rc2 → v4.34.0-rc1, mathlib 2026-05-13 → 2026-08-14, and the `bryangingechen/Matroid` fork **retired** in favour of a plain upstream `apnelson1` pin. No mathematical change — `lake build` green, all 17 headline declarations verified at `[propext, Classical.choice, Quot.sound]`, all 11 `PebbleGame/Examples.lean` `#eval`s match their documented values. Post-bump cleanup is under way in `notes/ToolchainBumps.md` *Scheduled cleanup*: the **deprecation-rename sweep is done** (846 script-applied renames + 8 hand sites, 920 → 0 deprecation warnings) and the bump scripts have landed. **`lake lint` is now GREEN** (64 errors → 0 over three cascade rounds; 63 declarations strictly generalized, 7 `Fintype.ofFinite`-bridge false positives kept under `@[nolint]`). The style sweeps are done too, and so are the last five warnings — **1441 → 0** (the 1 `flexible` site took one extra simp argument, `Fin.castLT`; the 4 `overlappingInstances` sites took `section`-scoping moves, generalized as TACTICS-QUIRKS §§ 107–108). **Hopscotch (item 5) is UNSTUCK**, and it never needed the workflow-vs-cadence decision it was queued for: the three-month *"would break the build"* phantom was our own `require` order — Lake reads root requires in **reverse** and adopts a dep's transitive pins first-writer-wins, so `require mathlib` sitting *first* let `Matroid`'s frozen manifest plant `batteries`/`proofwidgets` and mathlib's own pins were dropped. Moving that require **last** makes `lake update mathlib` adopt mathlib's pins (measured both ways; manifest byte-identical, no Lean source touched, both gates re-verified). Left there: close issue #2 / PR #1, plus an optional upstream report. Merged into **local `master`** (fast-forward) and **still unpushed** — `origin/master` is at Phase-38 close, so **CI has never validated this stack**, and hopscotch's nightly still tests the pre-fix lakefile until master is pushed |
