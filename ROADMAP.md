...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm interior cert): **THE PATH IS FOUND (§(4.95)).** After three kernel-checked refutations converged on the cert-interface wall, the `d=3`-anchored design-pass found the honest engine `case_III_rank_certification` (already general-`k`) sources `±r` via the eq.-(6.27) row-op of a bottom `G−v`-row (decoupling the gate from the membership), and the interior-`hρGv` spike returned GO: the one genuinely-new leaf (the interior `hρGv` row membership) is TRUE and ALREADY LANDED sorry-free (`chainData_relabel_arm_hρGv`, propagating the single base `ρ₀` to the interior per KT eq. (6.59)). No new linear-algebra leaf remains; the reshape is pure ASSEMBLY — route the interior through the honest engine, then the `chainData_dispatch` router, discarding the `_aug` fork. ASSEMBLY underway: the interior-arm seed reads (`seedShift_succ_/pred_castSucc`) + structural accessors landed; next = the arm `chainData_interior_realization_hρGv` then the dispatch. `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.95). |
