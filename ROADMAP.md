...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed (23e landed the KT-faithful A3-transposed rank certificate axiom-clean); **23f** = the geometry arm; route (α) chosen (the KT eq.-(6.66) `±r`-row re-shape; user-adjudicated 2026-06-27) after D4's `hred` was refuted for the opaque-basis `(e_b, j₀)` row — next session starts the route-(α) decomposition (see `notes/Phase23f.md` + design §(4.65)). |
