...
| 21a. Grassmann–Cayley meet / projective-duality foundations | `Molecular/Meet.lean` (KT §2.1 dual half) + mirror lemmas | ◷ In progress — deliverables 1–2 (`topEquiv`, `pairingDualEquiv`) green; deliverable 3 `complementIso` started (`wedgeProd`, `wedgePairing`, and **both halves of the basis-vector pairing** green — off-diagonal `0`, diagonal `≠ 0`, so the standard-basis pairing matrix is a signed-permutation matrix; the `toDualEquiv` composition into the iso remains); `meet` remains. Mirrors under `Mathlib/LinearAlgebra/ExteriorPower/Basis.lean` + `Mathlib/Data/Finset/Card.lean`. See `notes/Phase21a.md`; dep-graph `meet.tex` `sec:molecular-meet`. |
