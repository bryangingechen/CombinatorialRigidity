...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm interior cert): cert-interface reshape SCOPED (§(4.94)). The working `d=3` cert `case_III_rank_certification` (already general-`k`) sources `±r` via the eq.-(6.27) row-op of a bottom `G−v`-row — the honest engine the diverged `_aug`/`rigidityMatrixEdgeAug` interior fork (the §(4.93) wall) was forked from. Reshape = route the interior through it; the one genuinely-new leaf (the interior `hρGv` ROW membership) is spike-flagged, truth OPEN. Next = the interior-`hρGv` spike. `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.94). |
