...
| ⋮ Toolchain bump to Lean v4.34.0-rc1 (maintenance, cross-phase) | `lean-toolchain`, `lake-manifest.json`, `lakefile.toml` + ~36 proof fixups across 26 files | ✓ Complete — v4.30.0-rc2 → v4.34.0-rc1, mathlib 2026-05-13 → 2026-08-14, and the `bryangingechen/Matroid` fork **retired** in favour of a plain upstream `apnelson1` pin. No mathematical change — `lake build` green, all 17 headline declarations verified at `[propext, Classical.choice, Quot.sound]`, all 11 `PebbleGame/Examples.lean` `#eval`s match their documented values. ⚠️ **`lake lint` is RED** (64 batteries `unusedArguments` errors in 32 files, disjoint from the files this bump edited — pre-existing code meeting a changed linter), and the deprecation-rename / style sweeps are deferred: both are scheduled cleanup, see `notes/ToolchainBumps.md` |
