...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a/CARRIER closed; 23b/CHAIN open (CHAIN-1/3/4 + OD-7 + CHAIN-2a closed, CHAIN-2c-i/2c-ii foundation + the H.10 de-risk gate + the genuine-row `hwmem` leaf `chainData_bottom_relabel` + the `hρGv` G1 bridges/LEAF-ρ2/LEAF-ρ1 telescope + P1 finite-range core landed; the arm wiring `chainData_relabel_arm` (gated on P2/P3) + 2c-iii + CHAIN-5 remain); ENTRY/ASSEMBLY remain (see `notes/Phase23b.md`) |
