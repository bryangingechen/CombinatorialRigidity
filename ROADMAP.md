...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** (geometry-arm interior cert) AT A FOUNDATIONAL RE-ARCHITECTURE DECISION POINT (surfaced to user). The §(4.91) decisive spike (kernel-checked) REFUTED the §(4.90) override-route GO, so BOTH built arms are dead — the override (its `±r` slot reads the short-circuit panel, the discriminator emits a chain-edge perp) and the (D-substitution) one (off-by-one corner collapse). Shared root: the project's `splitOff`+`caseIIICandidate` OVERRIDE device (a §(4.69.2) KT-divergence) deletes the body `v`, so KT's redundant-row edge `vᵢ₋₁vᵢ` can't be reproduced faithfully. The only un-refuted direction = KT's disjunction-over-all-`Mᵢ` union-count, a CHAIN-2c dispatch/spine re-architecture (drops the override device) — user-adjudication call. `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.91). |
