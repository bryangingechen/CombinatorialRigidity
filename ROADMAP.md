...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry-arm interior cert. All six narrow corner routes (b/α/D/γ/β/re-key) are REFUTED — one root: the project's `caseIIICandidate` extensor-override demands a short-circuit-panel perp provably false for the generic seed (not KT's; §§(4.77)–(4.83)). The route to close is the foundational **(D-substitution)** cert re-architecture — KT's literal-framework construction (eq. 6.59/6.61), faithful (the union-dimension half is LANDED general-`k`), scoped at design §(4.84). **USER-AUTHORIZED (2026-06-28); both make-or-break spikes GO (S2 §(4.85), S3 §(4.86)); S1 + S2 + S3 + S4 LANDED (the genuine-`±r` membership leaf + cert wrapper + realization tail + the cert→tail arm assembly); S5 dispatch-wiring interface KERNEL-SETTLED (§(4.88), a clean build); S5-(5c)+(5e) the augmented `hB`/`L₀` factoring + the bottom-block `re`/`hre`/`hD` assembly LANDED; PHASE-BOUNDARY DECIDED (option A: 23f closes at the `cd`-taking dispatch); Gap B underway — the `cd`-taking genuine corner-`hA` leaf LANDED; the `L₀`/`hφ` make-or-break SETTLED = GO and the `hφ`/`L₀` collapse NOT NEEDED (§(4.89): the genuine `ofNormals` arm is the `C = 0` pin-zero route — the `v`-incident `±r` row sits in the augmented corner not the bottom, so `A − L₀·C = A` and the corner `hA` closes via the LANDED `corner_hA_aug_zero₁₂_of_gate` with `L₀` unused); next = the Gap-B `cd`-taking `_ofNormals` spine + dispatch router, pure assembly of landed bricks (no genuinely-new leaf).** See `notes/Phase23f.md` *Hand-off* + design §(4.89). `d=3` fully green. |
