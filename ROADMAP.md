...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a/CARRIER + 23b/CHAIN closed; 23c building option (A) (the forked general-`d` `±r`-block engine, no `hρGv`): cert + arm + corner-data assembly ✓, the `±r` seam closed end-to-end; `chainData_dispatch` DECOMPOSED (5 leaves) + the `Relabel/` split + LEAF-1/2 + the full discriminator-index plumbing + LEAF-3 + the LEAF-4 interior-`hρe₀` conjecture-crux leaf ✓; LEAF-4 step (ii)'s rank-cert half is BLOCKED and ALL THREE routes (A static-`W`, B′ operated-frame block-rank, A′ general-`d` W9a) are REFUTED — the `±r`-corner escapes `hρGv` at the row level but NOT in the general-`d` rank cert; the member-mapping wall (§I.8.18–20, intrinsic to KT) is unavoidable (design §I.8.24(4.17)–(4.20), four spikes). **At a phase STOP / fundamental reconsideration** (user-surfaced 2026-06-24); all landed leaves stay in tree (see `notes/Phase23c.md`) |
