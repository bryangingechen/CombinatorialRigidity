...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a/CARRIER + 23b/CHAIN + 23c closed; **23d** reconsidered the general-`d` rank cert: the genuine-row base-block family (route B + route 4-bare/-splitOff) all walled on one obstruction (the wrap edge can't enter the corner-overridden candidate span — the discriminator gate, intrinsic to the slot-override, 4 wall appearances §(4.18)–(4.29)), so the user chose **route A** — the honest unconditional concrete-`Matrix` Theorem 5.5. Landed: A1+A2 (the literal `Matrix R(G,p)` + the `Matrix.rank`↔`finrank(span)` bridge; opacity CLEAN) + A3 (the (6.64) block-additivity kernel as a pure-`Matrix` inequality — no span membership, so it dissolves the wall) + A4 (the (6.61) column-op bridge) + A4.5 (the PRODUCT-column `rigidityMatrixProd` + the generalized `Matrix.rank_of_coordEquiv` rank bridge — the re-coordinatization the A5 spike found `hblock`'s `D×D` split needs, since the flat columns don't factor `α × Fin D`) + A5a (the (6.61) column-op-as-right-multiply on `rigidityMatrixProd`) + A5b (the gate re-wrap — the `LinearIndependent (·.row)` form the A3/A4 `hA`/`hD` consume) + the A5c-keystone + the A5c operated-entry facts (`rigidityMatrixProd_mul_columnOp_apply` + `..._apply_eq_zero_of_ne`, the operated (6.61) zero block, entrywise); + A4.5e (the edge-restricted matrix `rigidityMatrixEdge` — the real-arm row index, since the all-`β`-row `rigidityMatrixProd` honest-rank bridge can't fire once `β` has non-edges) + the A5c composition core (`finrank_span_rigidityRows_ge_of_edge_fromBlocks`, the A4 + A4.5e block-additivity composition) + the `en`/`em` splits + the ROUTE-A CERT `case_III_rank_certification_matrix` + the EDGE-restricted operated-entry facts + the CORRECTED §(4.33) `hblock` index-map bricks (`..._apply_pin_zero`/`..._apply_corner`/`..._reindex_toBlocks₂₁_eq_zero`, corner pin `v`) + the (4b′) row-submatrix cert-kernel reshape (`rank_ge_of_isUnit_mul_submatrix_fromBlocks` + `…_of_edge_submatrix_fromBlocks` + the cert `case_III_rank_certification_matrix` reshaped to fire it — resolving the cert-SHAPE obstruction the total-`em`-bijection had for `D ≥ 3`); + the A6 ARM SPINE `case_III_arm_realization_matrix` + the `hD` bottom-block LI bridge `linearIndependent_toBlocks₂₂_row_of_off_pin`; NEXT = the `hA` corner LI bridge then the dispatch wiring (see `notes/Phase23d.md` Hand-off + design §(4.34)) |
