...
| 21b. Genericity device (Claim 6.4/6.9) | `Molecular/AlgebraicInduction.lean` + `Mathlib/{Algebra/MvPolynomial,LinearAlgebra/Matrix}/` (+ `algebraic-induction.tex` `lem:genericity-device`) | ◷ In progress (opened 2026-06-03; see `notes/Phase21b.md`) — the analytic sibling of 21a, scoped out of Phase 21 (risk #4/#7). The panel-coordinate parametrization of `R(G,p)` + generic-max-rank argument; the shared analytic crux of Cases I/II, Thm 5.5, Prop 1.1, cycle assembly; discharges the Phase-21 black-box leaves (`hglue`/`hspan`/`hub`/`hgen`). **State (2026-06-04):** **`lem:genericity-device` GREEN** — the genuine *multivariate* Claim 6.4 (panel rows bilinear ⇒ no affine line works) landed via route (a) (`exists_good_realization` + `exists_good_realization_const`, on the `exists_…_polynomial` mirror engine); the affine wrapper chain was consolidated away in the same commit. **`lem:case-I-realization` now GREEN too** — the `HasFullRankRealization`-closure-under-contraction producer (`hasFullRankRealization_ofParam_of_contraction` + the splice-rigidity core `isInfinitesimallyRigid_of_rigid_subgraph_of_block_internal`) composing the two landed block bricks into `theorem_55`'s `hcontract` shape, taking the splice's two rigidity outputs as hypotheses (green modulo the geometric splice placement of `p`). Remaining: the geometric splice construction of `p` and per-consumer Case II / Prop 1.1 wiring. Hand-off in `notes/Phase21b.md`. Also consumed by Phases 22–23. See `notes/MolecularConjecture.md` *Phase 21b*, `DESIGN.md` *Genericity device …* + *Forward-mode reduction chains*. |
