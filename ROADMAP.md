...
| 23. Case III general `d` (Lemma 6.13) → Thm 5.5/5.6 → Conjecture 1.2 | `Molecular/` (sub-lettered; codes-until-open) | ◐ In progress — 23a–23e closed; **23f** = the geometry-arm interior cert via the user-authorized **(D-substitution)** re-architecture (KT eq. 6.59/6.61; the six narrow corner routes were refuted, §§(4.77)–(4.83)). S1–S4 + the S5 block-data feeders (5c/5e) + the Gap-B `cd`-taking `_ofNormals` spine (`chainData_arm_realization_ofNormals`) all LANDED; next = the `chainData_dispatch` `Fin cd.d` router (pure assembly, no new leaf), then 23f ✓ (CHAIN-5 + the `cd` producer → 23g per option A). `d=3` fully green. See `notes/Phase23f.md` *Hand-off* + design §(4.89). |
