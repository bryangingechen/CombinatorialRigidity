...
| 39. Hinge-pencil molecular conjecture (post-program) — PENCIL | `Molecular/Molecule/Pencil/` (+ `Meet.lean` adders), `pencil.tex` | ◐ In progress (three carried items — `hcontract` (W4, parked) and the kernels `hK` / `hbareSplit`; the research arc continues, **67 directions landed** (65 on kernel (K), two on W4), **WPAIR (60) in flight** (WTRI (58) proved route 3's cost (T) — a feasible residual is triangle-free — from two landed feasibility transfers a step's inventory had missed; WELOC (59) then **refuted (E-loc)**, the reduction (E) had, with an exhibited 32-vertex residual, leaving (E) itself standing and now known tight), `hK` / (GR-15) / class uniformity untouched by all of them; see `notes/Phase39.md`) |
