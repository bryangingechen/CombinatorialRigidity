...
`runPebbleGame_underline_eq_edgeFinset`. Phase 11 Layer 3 reshape:
hypothesis matches `.inr D'` (was `some D'`). Lee–Streinu Theorem 8
(forward direction); blueprint `thm:pebble-game-soundness`. -/
theorem runPebbleGame_sound [Fintype V] {G : SimpleGraph V} [Fintype G.edgeSet]
    {k ℓ : ℕ} {D' : PartialOrientation V} (h : runPebbleGame G k ℓ = .inr D') :
    G.IsSparse k ℓ := by
...
game** (Phase 10 Layer 2; Phase 11 Layer 3 reshape). In the matroidal regime
`ℓ < 2k`, the finite simple graph `G` is `(k, ℓ)`-sparse iff
`runPebbleGameWith` from the empty orientation returns `.inr D'` for some
partial orientation `D'`. Phase 11 Layer 3 reshape: the iff's right-hand
side now matches the `Sum`-return shape (was `some D'`). Two-line assembly:
...
respective discharges. The matroidal hypothesis `ℓ < 2k` is needed only
for the contrapositive direction (it discharges `ℓ ≤ k * V'.card` inside
`certifies_against`); `runPebbleGameWith_sound` itself is regime-agnostic. -/
...
`thm:pebble-game-correct`). Phase 11 Layer 3 reshape: the iff's right-hand
side matches the `Sum`-return shape (was `some D`). One-line composition of
`runPebbleGame_sound` (⇐) and the `.inl w`-witness extraction (⇒). The
success branch additionally satisfies `D.underline = G.edgeFinset`
(`runPebbleGame_underline_eq_edgeFinset`) — together this is the full content
of the blueprint theorem. -/
theorem runPebbleGame_correct [Fintype V] {G : SimpleGraph V} [Fintype G.edgeSet]
    {k ℓ : ℕ} (h_matroidal : ℓ < 2 * k) :
    G.IsSparse k ℓ ↔ ∃ D : PartialOrientation V, runPebbleGame G k ℓ = .inr D := by
  refine ⟨?_, ?_⟩
  · intro hG
    ...
    | .inl w =>
      ...
      have hsub : (empty : PartialOrientation V).underline ⊆ G.edgeFinset := by
        rw [underline_empty]; exact Finset.empty_subset _
      obtain ⟨h_uv_in, h_sub⟩ :=
        runPebbleGameWith_witness_bridges _ _ edges
          Reachable.empty hmem hsub h_opt
      obtain ⟨h_size, h_lt⟩ := w.certifies_against h_matroidal h_uv_in h_sub
      have := hG w.V' h_size
      omega
...
theorem _root_.SimpleGraph.countMatroid_indep_iff_runPebbleGame
    ...
    (SimpleGraph.countMatroid V k ℓ hℓ).Indep G.edgeSet ↔
      ∃ D : PartialOrientation V, runPebbleGame G k ℓ = .inr D := by
