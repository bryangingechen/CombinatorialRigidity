...
/-- **Deficiency-aware Case II / Lemma 6.8 eq. (6.12) placement brick** (`case_II_placement_eq612`
at a deficient `(k−1)`-dof IH; no blueprint node — churn-prone `_kdof` infra, Phase 22i L6a).
Mirror of `case_II_placement_eq612` with two swaps (the V7 delta, §1.67(b)(3)):
(i) `hrig + _of_rigidOn_linking` → abstract rank input `hNrank` fed to
`exists_independent_panelRow_subfamily_of_le_finrank` (W6e, rigidity-free);
(ii) count bound `N + (screwDim k − 1) ≤ Nat.card s` instead of
`screwDim k * (V(G).ncard − 1) − 1 ≤ Nat.card s`.

The rank transport at `withNormal v (n_a + t·n_b)` (step (1)) is rigidity-free:
`toBodyHinge_withNormal_infinitesimalMotions_eq` gives equal motion spaces →
`span_rigidityRows_eq_of_infinitesimalMotions_eq` gives equal row spans → equal finrank,
carrying `hNrank` from seed `q` to the shared seed `q₀`. Everything else copies unchanged. -/
theorem PanelHingeFramework.case_II_placement_eq612_kdof [DecidableEq α] [Finite α] [Finite β]
    (G Gv : Graph α β) (hGv : Gv ≤ G) (ends : β → α × α)
    {v a b : α} {e_a e_b : β} (hvVc : v ∉ V(Gv)) (haVc : a ∈ V(Gv)) (hbVc : b ∈ V(Gv))
    (hG_eb : G.IsLink e_b v b) (hends_eb : ends e_b = (v, b))
    (_hG_ea : G.IsLink e_a v a) (hends_ea : ends e_a = (v, a))
    (_hVcard : V(Gv).ncard = V(G).ncard - 1)
    {q : α × Fin (k + 2) → ℝ}
    (hends_Gv : ∀ e u w, Gv.IsLink e u w → Gv.IsLink e (ends e).1 (ends e).2)
    (hne_Gv : ∀ e, Gv.IsLink e (ends e).1 (ends e).2 →
      (PanelHingeFramework.ofNormals Gv ends q).toBodyHinge.supportExtensor e ≠ 0)
    -- rank lower bound at the original seed `q` (the deficient IH's rank eq. (6.11)):
    {N : ℕ} (hNrank : N ≤ Module.finrank ℝ (Submodule.span ℝ
  ...
  set n_b : Fin (k + 2) → ℝ := fun i => q (b, i) with hn_b
  -- (1) The shared seed is the IH seed with `v`'s normal overridden by `n_a + t • n_b`, so the IH
  -- rank transports to `q₀` (overriding the fresh `v ∉ V(Gᵥ)` leaves the `Gᵥ`-block untouched).
  have hqeq : (fun p => if p.1 = v then ((n_a + t • n_b) : Fin (k + 2) → ℝ) p.2 else q p) = q₀ := by
    rw [hq₀]
  have hwN : PanelHingeFramework.ofNormals Gv ends q₀
      = (PanelHingeFramework.ofNormals Gv ends q).withNormal v (n_a + t • n_b) := by
  ...
  have hvedge : ∀ e u w, Gv.IsLink e u w → (ends e).1 ≠ v ∧ (ends e).2 ≠ v := by
    ...
    have hl := hends_Gv e u w he
    exact ⟨fun h => hvVc (h ▸ hl.left_mem), fun h => hvVc (h ▸ hl.right_mem)⟩
  -- The motion space is unchanged when overriding the unhinged `v`, so the row span and finrank
  -- transport: hNrank at `q` lifts to `hN₀` at `q₀`.
  have hZeq : (PanelHingeFramework.ofNormals Gv ends q₀).toBodyHinge.infinitesimalMotions
      = (PanelHingeFramework.ofNormals Gv ends q).toBodyHinge.infinitesimalMotions := by
    rw [hwN]
    exact (PanelHingeFramework.ofNormals Gv ends q).toBodyHinge_withNormal_infinitesimalMotions_eq
      v (n_a + t • n_b) hvedge
  have hspan_eq :
      Submodule.span ℝ (PanelHingeFramework.ofNormals Gv ends q₀).toBodyHinge.rigidityRows
      = Submodule.span ℝ (PanelHingeFramework.ofNormals Gv ends q).toBodyHinge.rigidityRows :=
  ...
  have hne₀ : ∀ e, Gv.IsLink e (ends e).1 (ends e).2 →
    ...
    rw [hwN, (PanelHingeFramework.ofNormals Gv ends q).toBodyHinge_withNormal_supportExtensor_of_ne
      v (n_a + t • n_b) e (by simpa using h₁) (by simpa using h₂)]
    exact hne_Gv e he
  -- (2) OLD block (W6e, `_le_finrank`): the rank input `hN₀` carries `N` independent linking
  -- panel rows of `ofNormals Gv ends q₀`.
  set FGv := (PanelHingeFramework.ofNormals Gv ends q₀).toBodyHinge with hFGv
  obtain ⟨so, hso_link, hso_card, hso_indep⟩ :=
    FGv.exists_independent_panelRow_subfamily_of_le_finrank (ends := ends)
      (by simpa [hFGv] using hends_Gv) (by simpa [hFGv] using hne₀) (by simpa [hFGv] using hN₀)
  -- (3) Transport the old block onto `G` (N7b-2; `panelRow` reads only `ends`/`q₀`, not the graph,
  -- so `hrow := rfl`).
  have hso_indep_G : LinearIndependent ℝ (fun i : so =>
      FG.panelRow ends (i : β × _ × _)) :=
  ...
  have hane : FG.supportExtensor e_a ≠ 0 := by
    ...
    simp only [hq₀v, hq₀a, panelSupportExtensor_add_smul_left]
    exact smul_ne_zero (neg_ne_zero.mpr ht) ((panelSupportExtensor_ne_zero_iff n_a n_b).mpr hgab)
  -- (4) NEW block (N7b-1): the reproduced `vb`-hinge `e_b` is transversal, giving `D − 1` new rows.
  have hnewne : FG.supportExtensor e_b ≠ 0 := by
    rw [PanelHingeFramework.toBodyHinge_supportExtensor, PanelHingeFramework.ofNormals_ends,
      PanelHingeFramework.ofNormals_normal, PanelHingeFramework.ofNormals_normal, hends_eb]
    simp only [hq₀v, hq₀b, panelSupportExtensor_add_smul_right]
  ...
  have hunion : LinearIndependent ℝ (Sum.elim
    ...
    have hpin : LinearIndependent ℝ (fun i : sn =>
      ...
      exact this
    exact BodyHingeFramework.linearIndependent_sum_pinned_block (v := v) hold hpin hso_indep_G
  -- (7) Package the `Sum.elim` family as a single `Set`-indexed panel-row subfamily.
  have hso_ne_eb : ∀ i ∈ so, (i : β × _ × _).1 ≠ e_b := by
    intro i hi heq
    have hl := hso_link i hi
    rw [heq, hends_eb] at hl
  ...
  have hjinj : Function.Injective j := by
    ...
    · exact congrArg Sum.inr (Subtype.ext hab)
  refine ⟨hane, Set.range j, ?_, ?_, ?_⟩
  · -- Count: `N + (D−1) ≤ Nat.card s` (equality; `sn` has `D−1` rows, `so` has `N` rows).
    rw [Nat.card_range_of_injective hjinj, Nat.card_sum, hsn_card, hso_card]
    omega
  · -- Membership: each row's edge links in `G` (new edge `e_b`, or a `Gᵥ`-edge ≤ `G`).
    rintro i ⟨(⟨ic, hic⟩ | ⟨ic, hic⟩), rfl⟩ <;>
      refine FG.panelRow_mem_rigidityRows ?_
...
The proof assembles the eq. (6.12) placement directly (inlining `case_II_placement_eq612_kdof`'s
steps), using the deficient IH at the degree-2 split-off `G_v^{ab}` plus the W-suite rank-polynomial
...
The inlining is necessary because `case_II_placement_eq612_kdof` requires `hGv : Gv ≤ G`, which
fails for `Gv = G.splitOff v a b e₀` (`e₀ ∉ E(G)` but `e₀ ∈ E(Gab)`). Instead, OLD `e₀`-rows
...
theorem PanelHingeFramework.case_II_realization_all_k [DecidableEq β] [Finite α] [Finite β]
  ...
  --  · If |V(G)| ≥ 4: the triangle G[{v,a,b}] is a proper rigid subgraph (triangle_isProperRigidSubgraph),
  --    contradicting hnoRigid.
  ...
  have hGab_ne : V(Gab).Nonempty := ⟨a, by rw [hGab_def, Graph.vertexSet_splitOff]; exact ⟨haG, hav⟩⟩
  -- ── Step 4: Apply IH at (k-1, Gab). ─────────────────────────────────────────────────────────
  obtain ⟨Q, hQg, hQgp, hQrank, hQrec, hQalg⟩ := (hIH (k - 1) Gab hGab hGab_ne hGab_lt).1 hGab_simple
  ...
  have hZeq : (PanelHingeFramework.ofNormals Gab Q.ends q₀).toBodyHinge.infinitesimalMotions
    ...
    rw [hwN]
    exact (PanelHingeFramework.ofNormals Gab Q.ends q).toBodyHinge_withNormal_infinitesimalMotions_eq
      v (n_a + n_b) hvedge
  have hspan_eq :
  ...
  have hne₀ : ∀ e, Gab.IsLink e (Q.ends e).1 (Q.ends e).2 →
    ...
    intro e he
    obtain ⟨h₁, h₂⟩ := hvedge e _ _ he
    rw [hwN, (PanelHingeFramework.ofNormals Gab Q.ends q).toBodyHinge_withNormal_supportExtensor_of_ne
  ...
  -- At q₀ = q[v ↦ n_a + n_b]:
  --   C(e_b) = panelSupportExtensor(n_a + n_b, n_b) = panelSupportExtensor(n_a, n_b)  [add_smul_right]
  --   C(e_a) = panelSupportExtensor(n_a + n_b, n_a) = (-1) • panelSupportExtensor(n_a, n_b)  [add_smul_left]
  ...
  --   (case (v,a): panelSupportExtensor (n_a+n_b) n_a = (-1) • panelSupportExtensor n_a n_b via add_smul_left)
  --   (case (a,v): panelSupportExtensor n_a (n_a+n_b); via panelSupportExtensor_swap then add_smul_left: = (1) • ...)
  ...
  have hFG_ea : FG.supportExtensor e_a = (-1 : ℝ) • panelSupportExtensor n_a n_b ∨
    ...
    · left
      rw [hFG_def, PanelHingeFramework.toBodyHinge_supportExtensor, PanelHingeFramework.ofNormals_ends,
    ...
    · right
      ...
      --   = -panelSupportExtensor (n_a + n_b) n_a  [panelSupportExtensor_swap with n₂=n_a, n₁=n_a+n_b]
  ...
  -- FGab.panelRow Q.ends (e₀, t₁, t₂) = hingeRow (Q.ends e₀).1 (Q.ends e₀).2 (annihRow C(e₀) t₁ t₂).
  -- Case Q.ends e₀ = (a, b): C(e₀) = panelSupportExtensor n_a n_b, hingeRow a b ρ = hingeRow v b ρ - hingeRow v a ρ.
  ...
  --   • (v, b): FG.panelRow = hingeRow v b (annihRow C_b t₁ t₂) with C_b = panelSupportExtensor n_a n_b, = ρ. ✓
  --   • (b, v): FG.panelRow = hingeRow b v (annihRow C_b t₁ t₂) with C_b = -panelSupportExtensor n_a n_b,
    ...
    · -- ends e_a = (v, a): FG.supportExtensor e_a = -1 • panelSupportExtensor n_a n_b
      ...
      have hCa : FG.supportExtensor e_a = (-1 : ℝ) • panelSupportExtensor n_a n_b := by
        rw [hFG_def, PanelHingeFramework.toBodyHinge_supportExtensor,
          ...
          show n_a + n_b = n_a + (1 : ℝ) • n_b from by rw [one_smul]]
        exact panelSupportExtensor_add_smul_left n_a n_b 1
      -- goal: -(hingeRow v a ρ) = hingeRow v a (annihRow ((-1:ℝ) • panelSupportExtensor n_a n_b) t₁ t₂)
  ...
  have he₀_rows_mem : ∀ (t₁ t₂ : Set.powersetCard (Fin (2 + 2)) 2),
    ...
    · -- Q.ends e₀ = (a, b): FGab.supportExtensor e₀ = panelSupportExtensor n_a n_b.
      ...
      -- goal: hingeRow a b ρ ∈ span(FG.rigidityRows), where ρ = annihRow (panelSupportExtensor n_a n_b) t₁ t₂
  ...
  have hso_span : ∀ i : so, FGab.panelRow Q.ends (i : β × _ × _)
    ...
    · -- e ≠ e₀: e is a G-link, and panelRow agrees at FGab and a suitable F.
      ...
      -- ρ_FGab ∈ FG.hingeRowBlock e: need annihRow(FGab.supportExtensor e)(FG.supportExtensor e) = 0
      ...
      · -- Swapped: ends e = ((Q.ends e).2, (Q.ends e).1) → FG.supportExtensor e = -FGab.supportExtensor e.
  ...
  have hnewpin_eb : LinearIndependent ℝ (fun i : sn =>
    ...
    · -- ends e_b = (b, v), so v = (ends e_b).2, not .1; need to adjust hnewpin.
      ...
      -- (hingeRow b v r).comp (single v) x = (hingeRow b v r) (update 0 v x) = r (update 0 v x).b - r (update 0 v x).v = -r x.
      ...
      -- So (FG.panelRow ends (e_b, t₁, t₂)).comp (single v) = -(FG.panelRow ends (e_b, t₁, t₂)).comp (single b).
  ...
  obtain ⟨Q_rk, hQ_rk0, hQ_rkrat, hQ_rk⟩ :=
    PanelHingeFramework.exists_rankPolynomial_of_le_finrank_linking G ends hends_G hne_G hrank_lb_nat
  ...
  have hB2_q' : (Module.finrank ℝ (Submodule.span ℝ
    ...
    have hVGne' : (PanelHingeFramework.ofNormals G ends q').toBodyHinge.graph.vertexSet.Nonempty := by
