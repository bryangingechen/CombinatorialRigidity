...
A `case_II_placement_eq612`-shaped brick cannot be reused here: such a brick requires `hGv : Gv ≤ G`,
which fails for `Gv = G.splitOff v a b e₀` (`e₀ ∉ E(G)` but `e₀ ∈ E(Gab)`). Instead, OLD `e₀`-rows
