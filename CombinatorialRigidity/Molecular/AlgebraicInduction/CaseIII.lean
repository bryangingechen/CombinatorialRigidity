...
-/
import CombinatorialRigidity.Molecular.AlgebraicInduction.CaseIIICandidate

/-!
# The algebraic induction — Case III realization (arms + dispatch)

Phase 22 (molecular-conjecture program; see `notes/MolecularConjecture.md`). The *realization*
half of the Case-III block: the arm realizations that consume the candidate built and certified in
`AlgebraicInduction/CaseIIICandidate` (split off in the post-Phase-22l molecular split round,
`notes/Phase22l-perf.md`; pure semantics-preserving file split, no decl renamed). The whole Case-III
block was itself carved off `AlgebraicInduction/CaseI.lean` in the post-Phase-22j perf pass. This
file carries:

* the **Case III arm realizations** (`case_III_arm_realization{,_M2,_M3}`), the triangle base
  (`hasGenericFullRankRealization_of_triangle`), and the producer dichotomy spine
  (`case_III_hsplit_producer`);
* the **relabel / split-off transport** (`ofNormals_relabel`, `rigidityRows_ofNormals_relabel`,
  `case_III_bottom_relabel`, the `acolumn`/`hingeRow` span bridges) — the M₃ machinery;
* the **candidate dispatch** (`case_III_candidate_dispatch`) discharging the M₁/M₂/M₃ obligation,
  the eq. (6.22) nested-IH rank bound (`case_III_nested_rank_lower`), and the Case-III composer
  `case_III_realization`.

See `ROADMAP.md` §22 / `notes/Phase22i.md` and the `sec:molecular-algebraic-induction-caseIII`
dep-graph in `blueprint/src/chapter/algebraic-induction/case-iii.tex`.
-/
