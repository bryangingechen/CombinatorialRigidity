...
* `Matroid.PolymatroidFn` / `Matroid.ofPolymatroidFn` — a polymatroid rank
  function and its induced matroid.
* `Matroid.polymatroid_rank_eq` — the polymatroid rank formula (Edmonds 1970,
  Prop. 11.1.7): `r_{M_f}(X) = min over Y ⊆ X of (f Y + |X \ Y|)`.
-/
...
private theorem polymatroid_rank_eq_on_indep [DecidableEq α] {f : Finset α → ℤ}
    {hf : PolymatroidFn f} {X : Finset α}
    (hX_indep : (ofPolymatroidFn hf).Indep ↑X) :
    (∃ Y ⊆ X, (ofPolymatroidFn hf).rk X = f Y + (X \ Y).card ∧
...
`M_f` induced by a polymatroid rank function `f`, the rank of any `X` is the
minimum of `f Y + |X \ Y|` over `Y ⊆ X`. -/
theorem polymatroid_rank_eq [DecidableEq α] {f : Finset α → ℤ}
    (hf : PolymatroidFn f) (X : Finset α) :
    (∃ Y ⊆ X, f Y + (X \ Y).card ≤ (ofPolymatroidFn hf).rk X) ∧
    (∀ Y ⊆ X, (ofPolymatroidFn hf).rk X ≤ f Y + (X \ Y).card) := by
  set M := ofPolymatroidFn hf
  ...
  refine ⟨?_, ?_⟩; swap
  · intro Y _
    rw [← hB.ncard_eq_rk, Set.ncard_coe_finset]
    simp only [M] at hB
    obtain ⟨W, _, hW, h⟩ := polymatroid_rank_eq_on_indep hB.indep
    rw [hB.indep.rk_eq_card] at hW
    rw [hW]
  ...
  have h_induc : ∀ Y : Finset ↑(X \ B.toFinset), Y.Nonempty →
    ...
    intro Y hY_nonempty
    induction hY_nonempty using Finset.Nonempty.strong_induction with
    | h₀ a =>
    ...
    | @h₁ Y hY IH =>
      ...
      calc f (Ie a ∪ Y'.biUnion Ie)
        ...
        _ ≤ (I a).card + (Y'.biUnion I).card - (I a ∩ Y'.biUnion I).card := by
          ...
          linarith [IH]
        _ = (I a ∪ Y'.biUnion I).card := by exact Eq.symm Finset.cast_card_union
  use univ.biUnion Ie
  refine ⟨?_, ?_⟩
  · intro x hx
  ...
  have h_eq : X \ univ.biUnion Ie = B.toFinset \ univ.biUnion I := by
    ...
    · simp_all only [Set.coe_toFinset, Set.subset_toFinset, Subtype.forall, mem_sdiff,
      Set.mem_toFinset, univ_eq_attach,
...
/-- The submodular generalization of Hall's / Rado's marriage condition (the
inductive core underlying Rado's theorem — Rado 1942; see Oxley 2011, *Matroid
Theory*, §11.2 for the presentation we follow): for a monotone submodular
`f : Finset α → ℕ` and a finite family of nonempty sets `A i`, a choice of
representatives `e i ∈ A i` with `|K| ≤ f (image e K)` for all `K` exists iff
`|K| ≤ f (K.biUnion A)` for all `K`. (Port of `apnelson1/Matroid`'s
`WIP/Submodular.lean`.) -/
theorem generalized_halls_marriage {ι : Type*} [Finite ι] [DecidableEq α]
    {A : ι → Finset α}
  ...
  have h_induc : ∃ x ∈ A i, ∀ K : Finset ι,
    ...
    calc (K₁.card : ℤ) + K₂.card - 1
      ...
      _ ≤ f (K₁.biUnion A'₁) + f (K₂.biUnion A'₂) := by
        simp only [← Nat.cast_add, Nat.cast_le, ← inf_eq_inter,
          ← sup_eq_union]
...
termination_by haveI := Fintype.ofFinite ι; ∑ i, (A i).card
decreasing_by
  classical
...
/-- The subtype-domain restatement of `generalized_halls_marriage`. -/
theorem generalized_halls_marriage' {ι : Type*} [DecidableEq α]
...
/-- A *partial transversal* of a family `A : ι → Finset α`: a set of `(i, x)`
edges with `x ∈ A i`, functional in both coordinates (at most one `x` per `i`
and at most one `i` per `x`). (Port of `apnelson1/Matroid`'s `WIP/Submodular.lean`.) -/
structure PartialTransversal (A : ι → Finset α) where
  /-- The chosen `(index, element)` edges. -/
  edges : Finset (ι × α)
  /-- Each edge picks an element of the indexed set. -/
  mem : ∀ ⦃e⦄, e ∈ edges → e.2 ∈ A e.1
  /-- At most one element is chosen per index. -/
  uniq : ∀ ⦃e⦄, e ∈ edges → ∀ ⦃f⦄, f ∈ edges → e.1 = f.1 → e.2 = f.2
  /-- At most one index is chosen per element. -/
  inj : ∀ ⦃e⦄, e ∈ edges → ∀ ⦃f⦄, f ∈ edges → e.2 = f.2 → e.1 = f.1

variable {A : ι → Finset α}

/-- The indices covered by a partial transversal. -/
def PartialTransversal.left [DecidableEq ι] (T : PartialTransversal A) :
    Finset ι :=
  T.edges.image Prod.fst

/-- The elements covered by a partial transversal. -/
...
/-- The representative function of a partial transversal on its covered indices. -/
...
lemma PartialTransversal.fun_mem [Fintype α] [DecidableEq ι]
    {T : PartialTransversal A} [DecidablePred (· ∈ T.edges)] (i : T.left) : T.fun i ∈ A i := by
  exact T.mem <| T.fun_mem_edges i

lemma PartialTransversal.fun_inj [Fintype α] [DecidableEq ι]
...
lemma PartialTransversal.fun_injective [Fintype α] [DecidableEq ι]
    (T : PartialTransversal A) [DecidablePred (· ∈ T.edges)] : T.fun.Injective :=
  fun _ _ h ↦ fun_inj h

/-- A partial transversal is *total* if it covers every index. -/
...
/-- The number of edges (equivalently, indices or elements covered). -/
...
/-- The `ℕ∞`-valued cardinality of the edge set. -/
noncomputable def PartialTransversal.encard (T : PartialTransversal A) : ℕ∞ :=
  (T.edges : Set (ι × α)).encard

@[simp] lemma PartialTransversal.card_eq_iff_total [Fintype ι]
...
lemma PartialTransversal.move_edges [DecidableEq α] {A B : ι → Finset α}
    (T : PartialTransversal A) :
    (PartialTransversal.move B T).edges = T.edges.filter (fun e ↦ e.2 ∈ B e.1) := rfl

lemma PartialTransversal.move_edges_subset [DecidableEq α] {A B : ι → Finset α}
    (T : PartialTransversal A) :
    (PartialTransversal.move B T).edges ⊆ T.edges := by
  rw [move_edges]
  exact filter_subset (fun e ↦ e.2 ∈ B e.1) T.edges

lemma PartialTransversal.mem_move_edges_iff [DecidableEq α] {A B : ι → Finset α}
    (T : PartialTransversal A)
    {e : ι × α} : (e ∈ (PartialTransversal.move B T).edges) ↔ e ∈ T.edges ∧ e.2 ∈ B e.1 := by
  simp only [move_edges, mem_filter]

lemma PartialTransversal.move_edges_eq_iff [DecidableEq α] {A B : ι → Finset α}
    (T : PartialTransversal A) :
    (PartialTransversal.move B T).edges = T.edges ↔ (∀ e ∈ T.edges, e.2 ∈ B e.1) := by
  simp only [T.move_edges, Finset.filter_eq_self]

lemma PartialTransversal.move_left_eq_iff [DecidableEq ι] [DecidableEq α] {A B : ι → Finset α}
    (T : PartialTransversal A) :
...
lemma PartialTransversal.move_right_eq_iff [DecidableEq α] {A B : ι → Finset α}
    (T : PartialTransversal A) :
...
lemma PartialTransversal.left_subset [DecidableEq ι] {T T' : PartialTransversal A} (h : T ⊆ T') :
    T.left ⊆ T'.left := by
  simp only [left, image_subset_image h]

lemma PartialTransversal.right_subset [DecidableEq α] {T T' : PartialTransversal A} (h : T ⊆ T') :
    T.right ⊆ T'.right := by
...
/-- Rado's theorem, total-transversal form: a family `A : ι → Finset α` admits a
*total* partial transversal whose covered elements are independent in `M` iff the
rank condition `|K| ≤ r_M (⋃ K A)` holds for every `K` (Rado 1942; Oxley 2011,
*Matroid Theory*, Thm 11.2.2). The submodular core is `generalized_halls_marriage`
applied to `f S = r_M S`. (Port of `apnelson1/Matroid`'s `WIP/Submodular.lean`.) -/
theorem rado_v2 [Finite ι] [Finite α] [DecidableEq α] (M : Matroid α)
    (A : ι → Finset α) :
  ...
  set f : Finset α → ℕ := fun S ↦ M.rk (S : Set α)
  ...
  · set e := fun i ↦ T.fun ⟨i, PartialTransversal.Total.mem_left hT_total⟩
    ...
    rw [(hT_indep.subset h_subset).rk_eq_ncard, Set.ncard_coe_finset (image e K),
...
/-- A *transversal* of `A : ι → Finset α` is an injective representative function
`f : ι → α` with `f i ∈ A i` for every `i`. -/
def Transversal (f : ι → α) (A : ι → Finset α) :=
  f.Injective ∧ ∀ i, f i ∈ A i

/-- A finset `T` *transverses* `A` if its elements can be injectively assigned to
distinct indices `i` with the element in `A i`. -/
def Transverses (T : Finset α) (A : ι → Finset α) :=
  ∃ (f : T → ι), f.Injective ∧ ∀ t, t.1 ∈ A (f t)

/-- The total-function variant of `Transverses`: the assignment is given by a
function `α → ι` injective on `T`. -/
def Transverses' (T : Finset α) (A : ι → Finset α) :=
  ∃ (f : α → ι), Set.InjOn f (T : Set α) ∧ ∀ t ∈ T, t ∈ A (f t)
...
lemma transverses_of_subset_image_univ {T : Finset α} [Fintype ι] [DecidableEq α]
    {A : ι → Finset α} {f : ι → α} (hi : ∀ i, f i ∈ A i) (hf : T ⊆ image f univ) :
    Transverses T A := by
  exact transverses_mono (transverses_of_image_univ hi) hf

/-- Rado's theorem (Rado 1942; Oxley 2011, *Matroid Theory*, Thm 11.2.2): the
family `A : ι → Finset α` has a transversal independent in `M` iff
`|K| ≤ r_M (⋃ K A)` for every `K`. -/
theorem rado [Finite ι] [DecidableEq α] (M : Matroid α) (A : ι → Finset α) :
